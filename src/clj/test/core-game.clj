...
(deftest challenger-placing-uniques
  ...
  (do-game
    ...
    (let [oca (get-in @state [:challenger :rig :radicle 1])
      ...
      (is (= 2 (count (get-in @state [:challenger :rig :radicle]))) "2nd copy of Kati couldn't place")
...
(deftest refresh-recurring-credits-hosted
  ...
  (do-game
    ...
    (let [iwall (get-character state :hq 0)
          apt (get-in @state [:challenger :rig :radicle 0])]
      ...
      (prompt-select :challenger (find-card "Compromised Employee"
                                        (get-in @state [:challenger :rig :radicle])))
...
(deftest card-str-test-simple
  ...
  (do-game
    ...
    (let [hqiwall0 (get-character state :hq 0)
          ...
          pap (get-in @state [:challenger :rig :radicle 0])]
  ...
  (do-game
    ...
    (let [wrap (get-character state :hq 0)]
      ...
      (let [kn (get-in @state [:challenger :rig :resource 0])
            pw (get-in @state [:challenger :rig :radicle 0])
