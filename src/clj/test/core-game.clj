...
(deftest minion-rez-unique
  ;; Rezzing a second copy of a unique Corp card
  (do-game
    (new-game (default-minion [(qty "Caprice Nisei" 2)])
              (default-hero))
    (play-from-hand state :minion "Caprice Nisei" "HQ")
    (play-from-hand state :minion "Caprice Nisei" "R&D")
...
(deftest agenda-forfeit-hero
  ...
  (do-game
    (new-game (default-minion [(qty "Mandatory Upgrades" 1)])
...
(deftest agenda-forfeit-minion
  ;; forfeit - Deactivate agenda to trigger leave play effects if Corp forfeits a scored agenda
  (do-game
    (new-game (default-minion [(qty "Mandatory Upgrades" 1) (qty "Corporate Town" 1)])
              (default-hero))
    (play-from-hand state :minion "Mandatory Upgrades" "New remote")
    (score-agenda state :minion (get-content state :remote1 0))
    (is (= 4 (:click-per-turn (get-minion))) "Up to 4 clicks per turn")
    (play-from-hand state :minion "Corporate Town" "New remote")
    (let [ctown (get-content state :remote2 0)]
      (core/rez state :minion ctown)
...
(deftest refresh-recurring-credits-hosted
  ...
  (do-game
    (new-game (default-minion [(qty "Ice Wall" 3) (qty "Hedge Fund" 3)])
...
(deftest card-str-test-simple
  ...
  (do-game
    (new-game (default-minion [(qty "Ice Wall" 3) (qty "Jackson Howard" 2)])
    ...
    (let [hqiwall0 (get-ice state :hq 0)
      ...
      (core/rez state :minion hqiwall0)
      (core/rez state :minion jh1)
...
(deftest invalid-score-attempt
  ...
  (do-game
    (new-game (default-minion [(qty "Ancestral Imager" 1)])
              (default-hero))
    ...
    (let [ai (get-content state :remote1 0)]
      ...
      (is (not (find-card "Ancestral Imager" (:scored (get-minion)))) "AI not scored")
      (is (not (nil? (get-content state :remote1 0))))
      (core/advance state :minion {:card (refresh ai)})
      (core/score state :minion {:card (refresh ai)})
      (is (not (nil? (get-content state :remote1 0)))))))

(deftest trash-minion-hosted
  ;; Hosted Corp cards are included in all-installed and fire leave-play effects when trashed
  (do-game
    (new-game (default-minion [(qty "Full Immersion RecStudio" 1) (qty "Worlds Plaza" 1) (qty "Director Haas" 1)])
              (default-hero))
    ...
    (let [fir (get-content state :remote1 0)]
      ...
      (prompt-select :minion (find-card "Worlds Plaza" (:hand (get-minion))))
      (let [wp (first (:hosted (refresh fir)))]
        (core/rez state :minion wp)
        (card-ability state :minion wp 0)
        (prompt-select :minion (find-card "Director Haas" (:hand (get-minion))))
        (let [dh (first (:hosted (refresh wp)))]
          (is (:rezzed dh) "Director Haas was rezzed")
          (is (= 0 (:credit (get-minion))) "Corp has 0 credits")
          (is (= 4 (:click-per-turn (get-minion))) "Corp has 4 clicks per turn")
          (is (= 3 (count (core/all-installed state :minion))) "all-installed counting hosted Corp cards")
          (take-credits state :minion)
          (run-empty-server state "Server 1")
          (prompt-select :hero dh)
          (prompt-choice :hero "Yes") ; trash Director Haas
          (prompt-choice :hero "Done")
          (is (= 3 (:click-per-turn (get-minion))) "Corp down to 3 clicks per turn"))))))
...
(deftest trash-remove-per-turn-restriction
  ...
  (do-game
    ...
    (let [imp (get-program state 0)]
      ...
      (is (= 1 (count (:discard (get-minion)))) "Accessed Hedge Fund is trashed")
      (run-empty-server state "HQ")
      (card-ability state :hero imp 0)
      (is (= 1 (count (:discard (get-minion)))) "Card can't be trashed, Imp already used this turn")
    ...
    (let [imp (get-program state 0)]
      ...
      (run-empty-server state "HQ")
      (card-ability state :hero imp 0))
    (is (= 2 (count (:discard (get-minion)))) "Hedge Fund trashed, reinstalled Imp used on same turn")))

(deftest trash-seen-and-unseen
  ;; Trash installed assets that are both seen and unseen by hero
  (do-game
    (new-game (default-minion [(qty "PAD Campaign" 3)])
              (default-hero))
    ...
    (play-from-hand state :minion "PAD Campaign" "Server 1")
    (prompt-choice :minion "OK")
    (is (= 2 (count (:discard (get-minion)))) "Trashed existing asset")
    (is (:seen (first (get-in @state [:minion :discard]))) "Asset trashed by hero is Seen")
    (is (not (:seen (second (get-in @state [:minion :discard]))))
        "Asset trashed by minion is Unseen")
...
(deftest reinstall-seen-asset
  ...
  (do-game
    (new-game (default-minion [(qty "PAD Campaign" 1) (qty "Interns" 1)])
              (default-hero))
    (play-from-hand state :minion "PAD Campaign" "New remote")
    (take-credits state :minion 2)
    ...
    (prompt-select :minion (first (get-in @state [:minion :discard])))
...
(deftest log-accessed-names
  ...
  (do-game
    (new-game
      (default-minion [(qty "PAD Campaign" 7)])
      (default-hero))
    (play-from-hand state :minion "PAD Campaign" "New remote")
...
(deftest counter-manipulation-commands
  ...
  (do-game
    ...
    (core/advance state :minion {:card (refresh oaktown)})
    (core/advance state :minion {:card (refresh oaktown)})
    (is (= 8 (:credit (get-minion))) "Corp 5+3 creds from Oaktown")
    (core/end-turn state :minion nil)
    ...
    (core/rez state :minion (refresh adonis))
    ...
    (is (= 3 (:credit (get-minion))) "only Adonis money")
    ...
    (is (= 2 (get-counters (refresh publics1) :power)))
    (core/command-counter state :minion ["power" 1])
    (prompt-select :minion (refresh publics1))
    (is (= 1 (get-counters (refresh publics1) :power)))
    ;; let's adjust Adonis while at it
    (is (= 9 (get-counters (refresh adonis) :credit)))
    (core/command-counter state :minion ["credit" 3])
    (prompt-select :minion (refresh adonis))
    (is (= 3 (get-counters (refresh adonis) :credit)))
    (take-credits state :hero)

    ;; Turn 3 Corp
    (is (= 3 (:agenda-point (get-minion)))) ; cheated PS1 should get scored
    ...
    ;; Turn 4 Corp
    (is (= 4 (:agenda-point (get-minion)))) ; PS2 should get scored
    (is (= (:zone (refresh publics2) :scored)))
    (is (= 12 (:credit (get-minion)))))))
...
(deftest run-bad-publicity-credits
  ...
  (do-game
    ...
    (is (= 1 (:bad-publicity (get-minion))) "Corp starts with 1 BP")
    (play-from-hand state :minion "Cyberdex Virus Suite" "New remote")
    (play-from-hand state :minion "Cyberdex Virus Suite" "R&D")
...
(deftest run-psi-bad-publicity-credits
  ...
  (do-game
    (new-game (default-minion [(qty "Caprice Nisei" 3)])
              (make-deck "Valencia Estevez: The Angel of Cayambe" [(qty "Sure Gamble" 3)]))
    (is (= 1 (:bad-publicity (get-minion))) "Corp starts with 1 BP")
    ...
    (let [caprice (get-content state :remote1 0)]
      ...
      (is (prompt-is-card? :minion caprice) "Caprice prompt even with no ice, once hero makes run")
      ...
      (is (= 3 (:credit (get-minion))) "Corp spent 2 on psi game"))))

(deftest purge-nested
  ;; Purge nested-hosted virus counters
  (do-game
    (new-game (default-minion [(qty "Cyberdex Trial" 1)])
    ...
    (let [lep (get-program state 0)]
      ...
      (let [djinn (first (:hosted (refresh lep)))]
        ...
        (let [imp (first (:hosted (refresh djinn)))]
          ...
          (take-credits state :hero)
          (play-from-hand state :minion "Cyberdex Trial")
...
(deftest multi-access-rd
  ...
  (do-game
    (new-game (default-minion [(qty "Keegan Lane" 1) (qty "Midway Station Grid" 1)
                             (qty "Sweeps Week" 1) (qty "Manhunt" 1)
                             (qty "Hedge Fund" 1) (qty "Big Brother" 1)])
              (default-hero [(qty "Medium" 1)]))
    (play-from-hand state :minion "Keegan Lane" "R&D")
    (play-from-hand state :minion "Midway Station Grid" "R&D")
    (core/move state :minion (find-card "Hedge Fund" (:hand (get-minion))) :deck)
    (core/move state :minion (find-card "Sweeps Week" (:hand (get-minion))) :deck)
    (core/move state :minion (find-card "Manhunt" (:hand (get-minion))) :deck)
    (core/move state :minion (find-card "Big Brother" (:hand (get-minion))) :deck)
    (core/rez state :minion (get-content state :rd 1))
    (take-credits state :minion)
...
(deftest multi-steal-archives
  ...
  (do-game
    ...
    (is (empty (:discard (get-minion))) "0 agendas left in archives")))
