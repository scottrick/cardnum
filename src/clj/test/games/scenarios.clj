...
(deftest minigame-prevent-netdmg-resourcetrash
  ...
  (do-game
    (new-game
      (default-minion [(qty "Neural EMP" 1) (qty "Hedge Fund" 3) (qty "SEA Source" 1)])
    ...
    (let [apt (get-resource state 0)]
      ...
      (let [ns (get-program state 0)
        ...
        (prompt-choice :minion 3)                             ; boost trace to 6
        ...
        (prompt-select :minion (find-card "Off-Campus Apartment" (:rig (get-hero))))
        (is (= 3 (:credit (get-minion))) "WNP increased cost to trash a resource by 2")
  ...
  (do-game
    (new-game (make-deck "Haas-Bioroid: Engineering the Future"
              ...
              (default-hero [(qty "Desperado" 1)
                               ...
                               (qty "Data Folding" 1)
                               (qty "Career Fair" 1)]))
    (core/draw state :minion 1)
    (core/gain state :minion :click 1)
    (play-from-hand state :minion "Hedge Fund")
    (play-from-hand state :minion "Adonis Campaign" "New remote")
    (is (= 10 (:credit (get-minion))) "HB:EtF ability paid 1 credit")
    (play-from-hand state :minion "Breaker Bay Grid" "Server 1")
    ...
    (let [adon (get-content state :remote1 0)
      ...
      (core/rez state :minion bbg)
      (core/rez state :minion adon)
      (is (= 10 (:credit (get-minion))) "Breaker Bay Grid allowed rez of Adonis for free")
      ...
      (prompt-choice :hero 0)
      (is (and (= 2 (:credit (get-hero))) (= 7 (:credit (get-minion))))
      ...
      (is (= 11 (:credit (get-minion))) "Gained 3 from Adonis and 1 from HB:EtF")
      (play-from-hand state :minion "Turing" "Server 1")
      (take-credits state :minion 1)
      ...
      (is (and (= 1 (:credit (get-hero))) (= 11 (:credit (get-minion)))))
