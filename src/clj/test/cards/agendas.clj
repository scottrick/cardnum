...
(deftest fifteen-minutes
  ...
  (do-game
    (new-game (default-minion [(qty "15 Minutes" 1)]) (default-hero))
    ...
    (let [fifm (first (:scored (get-hero)))]
      ...
      (card-ability state :minion (refresh fifm) 0)
    ...
    ;; use 15 minutes to take it away from minion (hey, maybe some obscure case happens where minion would want that)
    ...
    (let [fifm (first (:scored (get-minion)))]
      (is (= 1 (count (:abilities (refresh fifm)))))
      (card-ability state :minion (refresh fifm) 0)
      (is (= 0 (:agenda-point (get-minion))))
      (is (= 0 (count (:scored (get-minion))))))
    (is (= "15 Minutes" (:title (first (:deck (get-minion))))))))

(deftest ancestral-imager
  ;; Ancestral Imager - damage on jack out
  (do-game
    (new-game (default-minion [(qty "Ancestral Imager" 3)])
              (default-hero))
    (play-from-hand state :minion "Ancestral Imager" "New remote")
    (let [ai (get-content state :remote1 0)]
      (score-agenda state :minion ai)
      (take-credits state :minion)
...
(deftest astro-script-token
  ...
  (do-game
    (new-game (default-minion [(qty "AstroScript Pilot Program" 3) (qty "Ice Wall" 2)])
              (default-hero))
    (core/gain state :minion :click 3)
    (letfn [(try-place [from to]
              (card-ability state :minion (refresh from) 0)
              (prompt-select :minion (refresh to)))
      ...
      (play-from-hand state :minion "AstroScript Pilot Program" "New remote")
      (let [scored-astro (get-in @state [:minion :scored 0])
            installed-astro (get-content state :remote2 0)
            hand-astro (find-card "AstroScript Pilot Program" (:hand get-minion))]
        (should-not-place scored-astro hand-astro " in hand")
        (should-place scored-astro installed-astro " that is installed")
        (core/advance state :minion {:card (refresh installed-astro)})
        (core/advance state :minion {:card (refresh installed-astro)})
        (core/score   state :minion {:card (refresh installed-astro)}))
      (play-from-hand state :minion "Ice Wall" "HQ")
      (let [no-token-astro (get-in @state [:minion :scored 0])
            token-astro (get-in @state [:minion :scored 1])
            hand-ice-wall (find-card "Ice Wall" (:hand get-minion))
...
(deftest braintrust
  ...
  (do-game
    (new-game (default-minion [(qty "Braintrust" 1) (qty "Ichi 1.0" 1)])
              (default-hero))
    (play-from-hand state :minion "Braintrust" "New remote")
    (let [bt (get-content state :remote1 0)]
      (core/add-prop state :minion bt :advance-counter 7)
      (core/score state :minion {:card (refresh bt)})
      (let [scored-bt (get-in @state [:minion :scored 0])]
        (is (= 2 (get-counters (refresh scored-bt) :agenda))
            "Scored w/ 4 over-advancements; 2 agenda counters")
        (play-from-hand state :minion "Ichi 1.0" "HQ")
        (core/rez state :minion (get-ice state :hq 0))
        (is (= 2 (:credit (get-minion))) "2c discount to rez Ichi")))))

(deftest breaking-news
  ;; Test scoring breaking news
  (do-game
    (new-game (default-minion [(qty "Breaking News" 3)])
              (default-hero))
...
(deftest character-assassination
  ...
  (do-game
    (new-game (default-minion [(qty "Character Assassination" 1)])
              (default-hero [(qty "Fall Guy" 1) (qty "Kati Jones" 1)]))
    (take-credits state :minion)
    (play-from-hand state :hero "Kati Jones")
    (play-from-hand state :hero "Fall Guy")
    (take-credits state :hero)
    (play-from-hand state :minion "Character Assassination" "New remote")
    (score-agenda state :minion (get-content state :remote1 0))
    (let [kati (get-in @state [:hero :rig :resource 0])]
      (prompt-select :minion kati)
      (is (empty? (:prompt (get-hero))) "Fall Guy prevention didn't occur")
      (is (= 1 (count (:discard (get-hero)))) "Kati Jones trashed"))))
	  
(deftest minionorate-sales-team
  ;; Corporate Sales Team - Places 10c on card, minion takes 1c on each turn start
  (do-game
    (new-game (default-minion [(qty "Corporate Sales Team" 2)])
              (default-hero))
    (play-from-hand state :minion "Corporate Sales Team" "New remote")
    (is (= 5 (:credit (get-minion))))
    (score-agenda state :minion (get-content state :remote1 0))
	(let [scored-cst (get-in @state [:minion :scored 0])]
	  (core/end-turn state :minion nil)
	  (core/start-turn state :hero nil)
	  (is (= 6 (:credit (get-minion))) "Increments at hero's start of turn")
	  (is (= 9 (get-counters (refresh scored-cst) :credit)))
	  (core/end-turn state :hero nil)
	  (core/start-turn state :minion nil)
	  (is (= 7 (:credit (get-minion))) "Increments at minion's start of turn")
	  (is (= 8 (get-counters (refresh scored-cst) :credit)))
	)))

(deftest minionorate-war
  ;; Corporate War - Gain 7c if you have 7c or more when scoring, otherwise lose all credits
  (do-game
    (new-game (default-minion [(qty "Corporate War" 2)])
              (default-hero))
    (play-from-hand state :minion "Corporate War" "New remote")
    (is (= 5 (:credit (get-minion))))
    (score-agenda state :minion (get-content state :remote1 0))
    (is (= 0 (:credit (get-minion))) "Lost all credits")
    (core/gain state :minion :credit 7)
    (play-from-hand state :minion "Corporate War" "New remote")
    (score-agenda state :minion (get-content state :remote2 0))
    (is (= 14 (:credit (get-minion))) "Had 7 credits when scoring, gained another 7")))

(deftest crisis-management
  ;; Crisis Management - Do 1 meat damage at turn start if Runner is tagged
  (do-game
    (new-game (default-minion [(qty "Crisis Management" 1)])
              (default-hero))
    (play-from-hand state :minion "Crisis Management" "New remote")
    (score-agenda state :minion (get-content state :remote1 0))
    (take-credits state :minion)
    (take-credits state :hero)
...
(deftest dedicated-neural-net
  ...
  (do-game
    (new-game (default-minion [(qty "Dedicated Neural Net" 1) (qty "Scorched Earth" 2)
                             (qty "Hedge Fund" 1) "Caprice Nisei"])
              (default-hero [(qty "HQ Interface" 1)]))
    (play-from-hand state :minion "Dedicated Neural Net" "New remote")
    (play-from-hand state :minion "Caprice Nisei" "HQ")
    (score-agenda state :minion (get-content state :remote1 0))
    (take-credits state :minion)
    ...
    (prompt-choice :hero "0")
    (prompt-choice :minion "1")
    (is (= 2 (-> (get-minion) :selected first :max)) "Corp chooses 2 cards for Runner to access")))

(deftest eden-fragment
  ;; Test that Eden Fragment ignores the install cost of the first ice
  (do-game
    (new-game (default-minion [(qty "Eden Fragment" 3) (qty "Ice Wall" 3)])
              (default-hero))
    (play-from-hand state :minion "Ice Wall" "HQ")
    (play-from-hand state :minion "Eden Fragment" "New remote")
    (score-agenda state :minion (get-content state :remote1 0))
    (take-credits state :minion)
    (take-credits state :hero)
    (take-credits state :hero)
    ...
    (is (= 6 (get-in @state [:minion :credit])) "Corp does not pay for installing the first ICE of the turn")
    (play-from-hand state :minion "Ice Wall" "HQ")
    (is (not (nil? (get-ice state :hq 2))) "Corp has three ice installed on HQ")
    (is (= 4 (get-in @state [:minion :credit])) "Corp pays for installing the second ICE of the turn")))

(deftest efficiency-committee
  ;; Efficiency Committee - Cannot advance cards if agenda counter is used
  (do-game
    (new-game (default-minion [(qty "Efficiency Committee" 3) (qty "Shipment from SanSan" 2)
                             (qty "Ice Wall" 1)])
              (default-hero))
    (core/gain state :minion :click 4)
    (play-from-hand state :minion "Efficiency Committee" "New remote")
    (play-from-hand state :minion "Efficiency Committee" "New remote")
    (play-from-hand state :minion "Efficiency Committee" "New remote")
    (play-from-hand state :minion "Ice Wall" "HQ")
    (let [ec1 (get-content state :remote1 0)
      ...
      (score-agenda state :minion ec1)
      (let [ec1_scored (get-in @state [:minion :scored 0])]
        (is (= 3 (get-counters (refresh ec1_scored) :agenda)))
        (is (= 2 (:agenda-point (get-minion))))
        ;; use token
        (is (= 3 (:click (get-minion))))
        (card-ability state :minion ec1_scored 0)
        (is (= 4 (:click (get-minion))))
        ;; try to advance Ice Wall
        (core/advance state :minion {:card (refresh iw)})
        (is (= 4 (:click (get-minion))))
        (is (= nil (:advance-counter (refresh iw))))
        ;; try to advance Efficiency Committee
        (core/advance state :minion {:card (refresh ec2)})
        (is (= 4 (:click (get-minion))))
        ...
        (prompt-select :minion ec2)
        (is (= 2 (:advance-counter (refresh ec2))))
        (play-from-hand state :minion "Shipment from SanSan")
        (prompt-choice :minion "2")
        (prompt-select :minion ec2)
        (is (= 4 (:advance-counter (refresh ec2))))
        (core/score state :minion {:card (refresh ec2)})
        ...
        (core/advance state :minion {:card (refresh ec3)})
        (is (= 1 (:advance-counter (refresh ec3))))))))

(deftest explode-a-palooza
  ;; Explode-a-palooza - Gain 5 credits when Runner accesses it
  (do-game
    (new-game (default-minion [(qty "Explode-a-palooza" 1)])
              (default-hero))
    (play-from-hand state :minion "Explode-a-palooza" "New remote")
    (take-credits state :minion)
    (run-empty-server state :remote1)
    (prompt-choice :hero "Access")
    (prompt-choice :hero "Steal")
    (prompt-choice :minion "Yes")
    (is (= 12 (:credit (get-minion))) "Gained 5 credits")))

(deftest explode-ttw
  ;; Explode-a-palooza - Interaction with The Turning Wheel. Issue #1717.
  (do-game
    (new-game (default-minion [(qty "Explode-a-palooza" 3)])
              (default-hero [(qty "The Turning Wheel" 1)]))
    (starting-hand state :minion ["Explode-a-palooza" "Explode-a-palooza"])
    (play-from-hand state :minion "Explode-a-palooza" "New remote")
    (take-credits state :minion)
    ...
    (let [ttw (get-resource state 0)]
      ...
      (is (= 17 (:credit (get-minion))) "Gained 5 credits"))))
...
(deftest genetic-resequencing
  ...
  (do-game
    (new-game (default-minion [(qty "Genetic Resequencing" 1) (qty "Braintrust" 2)])
              (default-hero))
    (play-from-hand state :minion "Braintrust" "New remote")
    (play-from-hand state :minion "Braintrust" "New remote")
    (play-from-hand state :minion "Genetic Resequencing" "New remote")
    (let [bt1 (get-content state :remote1 0)
          bt2 (get-content state :remote2 0)
          gr (get-content state :remote3 0)]
      (score-agenda state :minion bt1)
      (let [btscored (get-in @state [:minion :scored 0])]
        (is (= 0 (get-counters (refresh btscored) :agenda)) "No agenda counters on scored Braintrust")
        (score-agenda state :minion gr)
        (prompt-select :minion bt2)
        (is (zero? (get-counters (refresh bt2) :agenda))
            "No agenda counters on installed Braintrust; not a valid target")
        (prompt-select :minion btscored)
...
(deftest government-contracts
  ...
  (do-game
    (new-game (default-minion [(qty "Government Contracts" 1)])
              (default-hero))
    (play-from-hand state :minion "Government Contracts" "New remote")
    (score-agenda state :minion (get-content state :remote1 0))
    (is (= 2 (:click (get-minion))))
    (card-ability state :minion (get-in @state [:minion :scored 0]) 0)
    (is (= 0 (:click (get-minion))) "Spent 2 clicks")
    (is (= 9 (:credit (get-minion))) "Gained 4 credits")))
...
(deftest high-risk-investment
  ...
  (do-game
    (new-game (default-minion [(qty "High-Risk Investment" 1)])
              (default-hero))
    (play-from-hand state :minion "High-Risk Investment" "New remote")
    (let [hri (get-content state :remote1 0)]
      (score-agenda state :minion hri)
      (let [hriscored (get-in @state [:minion :scored 0])]
        (is (= 1 (get-counters (refresh hriscored) :agenda)) "Has 1 agenda counter")
        (take-credits state :minion)
        (is (= 7 (:credit (get-minion))))
        (take-credits state :hero)
        (is (= 9 (:credit (get-hero))))
        (card-ability state :minion hriscored 0)
        (is (= 16 (:credit (get-minion))) "Gained 9 credits")
        (is (= 2 (:click (get-minion))) "Spent 1 click")
...
(deftest hostile-takeover
  ...
  (do-game
    ...
    (let [ht (get-content state :remote1 0)]
      ...
      (is (= 12 (:credit (get-minion))) "Gain 7 credits")
      (is (= 1 (:bad-publicity (get-minion))) "Take 1 bad publicity"))))
...
(deftest labyrinthine-servers
  ...
  (do-game
    (new-game (default-minion [(qty "Labyrinthine Servers" 2)])
              (default-hero))
    (play-from-hand state :minion "Labyrinthine Servers" "New remote")
    (play-from-hand state :minion "Labyrinthine Servers" "New remote")
    (score-agenda state :minion (get-content state :remote1 0))
    (score-agenda state :minion (get-content state :remote2 0))
    (take-credits state :minion)
    (let [ls1 (get-in @state [:minion :scored 0])
          ls2 (get-in @state [:minion :scored 1])]
      ...
      (run-on state "HQ")
      (run-jack-out state)
      (card-ability state :minion ls1 0)
      (card-ability state :minion ls2 0)
      (card-ability state :minion ls1 0)
      (prompt-choice :minion "Done")
...
(deftest medical-breakthrough
  ...
  (do-game
    (new-game (default-minion [(qty "Medical Breakthrough" 3) (qty "Hedge Fund" 3)])
              (default-hero))
    (play-from-hand state :minion "Medical Breakthrough" "New remote")
    (play-from-hand state :minion "Medical Breakthrough" "New remote")
    ...
    (let [mb2 (get-content state :remote2 0)]
      (core/advance state :minion {:card (refresh mb2)})
      (core/advance state :minion {:card (refresh mb2)})
      (core/advance state :minion {:card (refresh mb2)})
      (core/score state :minion {:card (refresh mb2)})
      (is (= 2 (:agenda-point (get-minion))) "Only needed 3 advancements to score")
      (take-credits state :minion)
      (take-credits state :hero)
      (play-from-hand state :minion "Medical Breakthrough" "New remote")
      (let [mb3 (get-content state :remote3 0)]
        (core/advance state :minion {:card (refresh mb3)})
        (core/advance state :minion {:card (refresh mb3)})
        (core/score state :minion {:card (refresh mb3)})
        (is (= 4 (:agenda-point (get-minion))) "Only needed 2 advancements to score")))))
  ...
  (do-game
    ...
    (play-from-hand state :minion "NAPD Contract" "New remote")
      (let [napd (get-content state :remote1 0)]
        ...
        (core/advance state :minion {:card (refresh napd)})
        (core/score state :minion {:card (refresh napd)})
        (is (= 2 (:agenda-point (get-minion))) "Scored NAPD for 2 points after 5 advancements"))))

(deftest napd-contract-minionorate-scandal
...
(deftest net-quarantine
  ...
  (do-game
    (new-game (default-minion [(qty "Net Quarantine" 1)])
              (default-hero))
    (core/gain state :hero :link 1)
    (core/gain state :minion :click 3)
    (play-from-hand state :minion "Net Quarantine" "New remote")
    (score-agenda state :minion (get-content state :remote1 0))
    (is (= 5 (:credit (get-minion))) "Corp has 5 credits")
    (is (= 1 (:link (get-hero))) "Runner has 1 link")
    (core/minion-trace-prompt state {:title "/trace command" :side :minion} {:base 1})
    (prompt-choice :minion 0)
    (is (= 0 (:link (get-hero))) "Runner has 0 link")
    (prompt-choice :hero 3)
    (is (= 1 (:link (get-hero))) "Runner has 1 link again")
    (is (= 6 (:credit (get-minion))) "Corp gained a credit from NQ")
    ; second trace of turn - no link reduction
    (core/minion-trace-prompt state {:title "/trace command" :side :minion} {:base 1})
    (prompt-choice :minion 0)
    (is (= 1 (:link (get-hero))) "Runner has 1 link")
    (prompt-choice :hero 2)
    (is (= 7 (:credit (get-minion))) "Corp gained a credit from NQ")))

(deftest nisei-mk-ii-step-43
  ;; Nisei MK II - Remove hosted counter to ETR, check this works in 4.3
  (do-game
   (new-game (default-minion [(qty "Nisei MK II" 1)])
             (default-hero))
   (play-from-hand state :minion "Nisei MK II" "New remote")
   (score-agenda state :minion (get-content state :remote1 0))
   (let [scored-nisei (get-in @state [:minion :scored 0])]
     (is (= 1 (get-counters (refresh scored-nisei) :agenda)) "Scored Nisei has one counter")
     (take-credits state :minion)

     (run-on state "HQ")
     (run-phase-43 state)
     (card-ability state :minion (refresh scored-nisei) 0)
     (prompt-choice :minion "Done") ; close 4.3 minion
...
(deftest oaktown-renovation
  ...
  (do-game
    (new-game (default-minion [(qty "Oaktown Renovation" 1) (qty "Shipment from SanSan" 1)])
              (default-hero))
    (core/gain state :minion :click 3)
    ...
    (let [oak (get-content state :remote1 0)]
      ...
      (is (= 6 (:credit (get-minion))) "Spent 1 credit to advance, gained 2 credits from Oaktown")
      (play-from-hand state :minion "Shipment from SanSan")
      (prompt-choice :minion "2")
      (prompt-select :minion oak)
      (is (= 3 (:advance-counter (refresh oak))))
      (is (= 6 (:credit (get-minion))) "No credits gained due to advancements being placed")
      (core/advance state :minion {:card (refresh oak)})
      (is (= 7 (:credit (get-minion))) "Spent 1 credit to advance, gained 2 credits from Oaktown")
      (core/advance state :minion {:card (refresh oak)})
...
(deftest obokata-protocol
  ...
  (do-game
    ...
    (play-from-hand state :minion "Obokata Protocol" "New remote")
    (take-credits state :minion)
...
(deftest personality-profiles
  ...
  (do-game
    (new-game (default-minion [(qty "Personality Profiles" 1)])
    ...
    (score-agenda state :minion (find-card "Personality Profiles" (:hand (get-minion))))
    (take-credits state :minion)
...
(deftest philotic-entanglement
  ...
  (do-game
    (new-game (default-minion [(qty "Philotic Entanglement" 1) (qty "House of Knives" 3)])
...
(deftest posted-bounty-yes
  ...
  (do-game
    (new-game (default-minion [(qty "Posted Bounty" 1)])
              (default-hero))
    (play-from-hand state :minion "Posted Bounty" "New remote")
    (let [pb (get-content state :remote1 0)]
      (score-agenda state :minion pb)
	  (prompt-choice :minion "Yes")
	  (is (= 0 (:agenda-point (get-minion))) "Forfeiting Posted Bounty nullifies agenda points")
      (is (= 1 (:bad-publicity (get-minion))) "Forfeiting takes 1 bad publicity"))
...
(deftest profiteering
  ...
  (do-game
    (new-game (default-minion [(qty "Profiteering" 1)])
              (default-hero))
    (play-from-hand state :minion "Profiteering" "New remote")
    (let [prof (get-content state :remote1 0)]
      (score-agenda state :minion prof)
      (prompt-choice :minion "3")
      (is (= 1 (:agenda-point (get-minion))))
      (is (= 3 (:bad-publicity (get-minion))) "Took 3 bad publicity")
      (is (= 20 (:credit (get-minion))) "Gained 15 credits"))))

(deftest project-ares
  ;; Project Ares - Full test
  (do-game
    (new-game (default-minion [(qty "Project Ares" 2)])
              (default-hero [(qty "Clone Chip" 1)]))
    (take-credits state :minion)
    (play-from-hand state :hero "Clone Chip")
    (take-credits state :hero)
    (score-agenda state :minion (find-card "Project Ares" (:hand (get-minion))))
    (is (empty? (get-in @state [:hero :prompt])) "No prompt for Runner if scored with 4 advancement tokens")
    (core/gain state :minion :click 4)
    (play-from-hand state :minion "Project Ares" "New remote")
    (let [ares (get-content state :remote1 0)]
      (core/advance state :minion {:card (refresh ares)})
      (core/advance state :minion {:card (refresh ares)})
      (core/advance state :minion {:card (refresh ares)})
      (core/advance state :minion {:card (refresh ares)})
      (core/advance state :minion {:card (refresh ares)})
      ...
      (core/score state :minion {:card (refresh ares)}))
...
(deftest project-beale
  ...
  (do-game
    (new-game (default-minion [(qty "Project Beale" 2)])
              (default-hero))
    (core/gain state :minion :click 8 :credit 8)
    (play-from-hand state :minion "Project Beale" "New remote")
    (let [pb1 (get-content state :remote1 0)]
      (core/advance state :minion {:card (refresh pb1)})
      (core/advance state :minion {:card (refresh pb1)})
      (core/advance state :minion {:card (refresh pb1)})
      (core/advance state :minion {:card (refresh pb1)})
      (core/score state :minion {:card (refresh pb1)})
      (is (= 2 (:agenda-point (get-minion))) "Only 4 advancements: scored for standard 2 points")
      (play-from-hand state :minion "Project Beale" "New remote")
        (let [pb2 (get-content state :remote2 0)]
          (core/advance state :minion {:card (refresh pb2)})
          (core/advance state :minion {:card (refresh pb2)})
          (core/advance state :minion {:card (refresh pb2)})
          (core/advance state :minion {:card (refresh pb2)})
          (core/advance state :minion {:card (refresh pb2)})
          (core/score state :minion {:card (refresh pb2)})
          (is (= 5 (:agenda-point (get-minion))) "5 advancements: scored for 3 points")))))

(deftest puppet-master
  ;; Puppet Master - game progresses if no valid targets. Issue #1661.
  (do-game
    (new-game (default-minion [(qty "Puppet Master" 1)])
              (default-hero))
    (play-from-hand state :minion "Puppet Master" "New remote")
    (score-agenda state :minion (get-content state :remote1 0))
    (take-credits state :minion)
...
(deftest rebranding-team
  ...
  (do-game
    (new-game (default-minion [(qty "Rebranding Team" 1) (qty "Launch Campaign" 1) (qty "City Surveillance" 1)
                             (qty "Jackson Howard" 1) (qty "Museum of History" 1)])
              (default-hero))
    (score-agenda state :minion (find-card "Rebranding Team" (:hand (get-minion))))
    (is (core/has-subtype? (find-card "Launch Campaign" (:hand (get-minion))) "Advertisement"))
    (is (core/has-subtype? (find-card "City Surveillance" (:hand (get-minion))) "Advertisement"))
    (is (core/has-subtype? (find-card "Jackson Howard" (:hand (get-minion))) "Advertisement"))
    (is (core/has-subtype? (find-card "Jackson Howard" (:hand (get-minion))) "Executive"))
    (is (core/has-subtype? (find-card "Museum of History" (:hand (get-minion))) "Advertisement"))
    (is (core/has-subtype? (find-card "Museum of History" (:hand (get-minion))) "Alliance"))
    (is (core/has-subtype? (find-card "Museum of History" (:hand (get-minion))) "Ritzy"))
    (core/move state :minion (find-card "Rebranding Team" (:scored (get-minion))) :deck)
    (is (core/has-subtype? (find-card "Launch Campaign" (:hand (get-minion))) "Advertisement"))
    (is (not (core/has-subtype? (find-card "City Surveillance" (:hand (get-minion))) "Advertisement")))
    (is (not (core/has-subtype? (find-card "Jackson Howard" (:hand (get-minion))) "Advertisement")))
    (is (core/has-subtype? (find-card "Jackson Howard" (:hand (get-minion))) "Executive"))
    (is (not (core/has-subtype? (find-card "Museum of History" (:hand (get-minion))) "Advertisement")))
    (is (core/has-subtype? (find-card "Museum of History" (:hand (get-minion))) "Alliance"))
    (is (core/has-subtype? (find-card "Museum of History" (:hand (get-minion))) "Ritzy"))))
...
(deftest reeducation
  ...
  (do-game
    (new-game (default-minion [(qty "Reeducation" 1) (qty "Sweeps Week" 1) (qty "Hedge Fund" 1)
                             (qty "Jackson Howard" 1) (qty "Gutenberg" 1)])
              (default-hero [(qty "Self-modifying Code" 1) (qty "Clone Chip" 1)
                               (qty "Corroder" 1) (qty "Sure Gamble" 1) (qty "Desperado" 1)]))
    (starting-hand state :minion ["Reeducation" "Sweeps Week"])
    (starting-hand state :hero ["Self-modifying Code"])
    (score-agenda state :minion (find-card "Reeducation" (:hand (get-minion))))
    (is (prompt-is-type? :hero :waiting) "Runner has wait prompt")
    (is (= 1 (count (get-in @state [:minion :hand]))))
    (is (= 1 (count (get-in @state [:hero :hand]))))
    (prompt-choice :minion (find-card "Sweeps Week" (:hand (get-minion)))) ; put Sweeps Week at bottom of R&D
    (prompt-choice :minion "Done") ; finished selecting cards
    (prompt-choice :minion "Done") ; minion prompt for Done/Start Over
    (is (= "Sweeps Week" (:title (last (:deck (get-minion))))))
...
(deftest reeducation-extra-cards
  ...
  (do-game
    (new-game (default-minion [(qty "Reeducation" 1) (qty "Sweeps Week" 1) (qty "Hedge Fund" 1)
              ...
              (default-hero [(qty "Self-modifying Code" 1) (qty "Clone Chip" 1)
                               (qty "Corroder" 1) (qty "Sure Gamble" 1) (qty "Desperado" 1)]))
    (starting-hand state :minion ["Reeducation" "Sweeps Week" "Hedge Fund"])
    (starting-hand state :hero ["Self-modifying Code"])
    (score-agenda state :minion (find-card "Reeducation" (:hand (get-minion))))
    (is (prompt-is-type? :hero :waiting) "Runner has wait prompt")
    (is (= 2 (count (get-in @state [:minion :hand]))))
    (is (= 1 (count (get-in @state [:hero :hand]))))
    (prompt-choice :minion (find-card "Sweeps Week" (:hand (get-minion))))
    (prompt-choice :minion (find-card "Hedge Fund" (:hand (get-minion)))) ; this is the bottom card of R&D
    (prompt-choice :minion "Done") ; finished selecting cards
    (prompt-choice :minion "Done") ; minion prompt for Done/Start Over
    (is (= "Hedge Fund" (:title (last (:deck (get-minion))))))
    (is (= "Sweeps Week" (:title (last (butlast (:deck (get-minion)))))))
    (is (= "Self-modifying Code" (:title (first (:hand (get-hero))))))
    (is (= 2 (count (get-in @state [:minion :hand]))))
    (is (= 1 (count (get-in @state [:hero :hand]))))))

(deftest ssl-endorsement-scored
  ;; SSL Endorsement - gain credits when in minion score area before turn begins
  (do-game
    (new-game (default-minion [(qty "SSL Endorsement" 1)])
              (default-hero))
    (score-agenda state :minion (find-card "SSL Endorsement" (:hand (get-minion))))
    (take-credits state :hero)

    (is (not-empty (:prompt (get-minion))) "Corp prompted to take credits")
    (is (= 5 (:credit (get-minion))) "Corp starts with 5 credits")
    (prompt-choice :minion "Yes")
    (is (= 8 (:credit (get-minion))) "Corp gains 3 credits")
    (take-credits state :hero)

    (is (= 8 (:credit (get-minion))) "Corp starts with 8 credits")
    (prompt-choice :minion "No")
    (is (= 8 (:credit (get-minion))) "Corp doesn't gain 3 credits")
    (take-credits state :hero)

    (is (= 8 (:credit (get-minion))) "Corp starts with 8 credits")
    (prompt-choice :minion "Yes")
    (is (= 11 (:credit (get-minion))) "Corp gains 3 credits")
    (take-credits state :hero)

    (is (= 11 (:credit (get-minion))) "Corp starts with 11 credits")
    (prompt-choice :minion "Yes")
    (is (= 14 (:credit (get-minion))) "Corp gains 3 credits")
    (take-credits state :hero)

    (is (empty? (:prompt (get-minion))) "Not prompted when out of money")))
...
(deftest ssl-endorsement-stolen
  ...
  (do-game
    (new-game (default-minion [(qty "SSL Endorsement" 1)])
              (default-hero))
    (play-from-hand state :minion "SSL Endorsement" "New remote")
    (take-credits state :minion)
    ...
    (is (not-empty (:prompt (get-minion))) "Corp prompted to take credits")
    (is (= 7 (:credit (get-minion))) "Corp starts with 7 credits")
    (prompt-choice :minion "Yes")
    (is (= 10 (:credit (get-minion))) "Corp gains 3 credits")
    (take-credits state :hero)

    (is (= 10 (:credit (get-minion))) "Corp starts with 10 credits")
    (prompt-choice :minion "No")
    (is (= 10 (:credit (get-minion))) "Corp doesn't gain 3 credits")
    (take-credits state :hero)

    (is (= 10 (:credit (get-minion))) "Corp starts with 10 credits")
    (prompt-choice :minion "Yes")
    (is (= 13 (:credit (get-minion))) "Corp gains 3 credits")
    (take-credits state :hero)

    (is (= 13 (:credit (get-minion))) "Corp starts with 13 credits")
    (prompt-choice :minion "Yes")
    (is (= 16 (:credit (get-minion))) "Corp gains 3 credits")
    (take-credits state :hero)
...
(deftest ssl-endorsement-stolen-swapped
  ...
  (do-game
    (new-game (default-minion [(qty "SSL Endorsement" 1) (qty "Breaking News" 1)
    ...
    (prompt-select :minion (find-card "SSL Endorsement" (:scored (get-hero))))
    (prompt-select :minion (find-card "Breaking News" (:scored (get-minion))))
    (take-credits state :hero)

    (is (= 10 (:credit (get-minion))) "Corp starts with 10 credits")
    (prompt-choice :minion "No")
    (is (empty? (:prompt (get-minion))) "Not double prompted for credits")
    (is (= 10 (:credit (get-minion))) "Corp doesn't gain 3 credits")
    (take-credits state :hero)
...
(deftest sentinel-defense-program
  ...
  (do-game
    (new-game (default-minion [(qty "Sentinel Defense Program" 1) (qty "Viktor 1.0" 1)])
              (default-hero [(qty "Feedback Filter" 1) (qty "Sure Gamble" 3)]))
    (score-agenda state :minion (find-card "Sentinel Defense Program" (:hand (get-minion))))
    ...
    (let [viktor (get-ice state :hq 0)
      ...
      (run-on state "HQ")
      (core/rez state :minion viktor)
      (card-subroutine state :minion viktor 0)
...
(deftest tgtbt
  ...
  (do-game
    (new-game (default-minion [(qty "TGTBT" 2) (qty "Old Hollywood Grid" 1)])
              (default-hero))
    (play-from-hand state :minion "TGTBT" "New remote")
    (play-from-hand state :minion "Old Hollywood Grid" "Server 1")
    (play-from-hand state :minion "TGTBT" "New remote")
    (take-credits state :minion)
...
(deftest the-cleaners
  ...
  (do-game
    (new-game (default-minion [(qty "The Cleaners" 1) (qty "Scorched Earth" 1)])
...
(deftest the-cleaners-cybernetics
  ...
  (do-game
    (new-game (default-minion [(qty "The Cleaners" 1)])
...
(deftest the-future-perfect
  ...
  (do-game
    (new-game (default-minion [(qty "The Future Perfect" 2)])
              (default-hero))
    (play-from-hand state :minion "The Future Perfect" "New remote")
    (take-credits state :minion)
...
(deftest underway-renovation
  ...
  (do-game
    (new-game (default-minion [(qty "Underway Renovation" 1) (qty "Shipment from SanSan" 1)])
              (default-hero))
    ...
    (let [ur (get-content state :remote1 0)]
      (core/advance state :minion {:card (refresh ur)})
...
(deftest vulcan-coverup
  ...
  (do-game
    (new-game (default-minion [(qty "Vulcan Coverup" 2)])
              (default-hero))
    (play-from-hand state :minion "Vulcan Coverup" "New remote")
    (take-credits state :minion)
    (run-empty-server state :remote1)
    (prompt-choice :hero "Steal")
    (is (= 1 (:bad-publicity (get-minion))) "Took 1 bad pub from stolen agenda")
    (take-credits state :hero)
    (play-from-hand state :minion "Vulcan Coverup" "New remote")
    (let [vc (get-content state :remote2 0)]
      (score-agenda state :minion vc)
