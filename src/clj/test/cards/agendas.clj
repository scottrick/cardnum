...
(deftest fifteen-minutes
  ...
  (do-game
    ...
    (let [fifm (first (:scored (get-runner)))]
      ...
      (card-ability state :resPlayer (refresh fifm) 0)
...
(deftest ancestral-imager
  ...
  (do-game
    ...
    (let [ai (get-content state :remote1 0)]
      (score-agenda state :resPlayer ai)
      (take-credits state :resPlayer)
      (is (= 3 (count(get-in @state [:hazPlayer :hand]))) "Runner has 3 cards in hand")
      (run-on state :hq)
      (run-jack-out state)
      (is (= 2 (count(get-in @state [:hazPlayer :hand]))) "Runner took 1 net damage"))))
...
(deftest astro-script-token
  ...
  (do-game
    ...
    (letfn [(try-place [from to]
              (card-ability state :resPlayer (refresh from) 0)
              (prompt-select :resPlayer (refresh to)))
      ...
      (play-from-hand state :resPlayer "AstroScript Pilot Program" "New remote")
      (let [scored-astro (get-in @state [:resPlayer :scored 0])
        ...
        (core/advance state :resPlayer {:card (refresh installed-astro)})
        (core/advance state :resPlayer {:card (refresh installed-astro)})
        (core/score   state :resPlayer {:card (refresh installed-astro)}))
      (play-from-hand state :resPlayer "Ice Wall" "HQ")
      (let [no-token-astro (get-in @state [:resPlayer :scored 0])
            token-astro (get-in @state [:resPlayer :scored 1])
...
(deftest braintrust
  ...
  (do-game
    ...
    (let [bt (get-content state :remote1 0)]
      (core/add-prop state :resPlayer bt :advance-counter 7)
      (core/score state :resPlayer {:card (refresh bt)})
      (let [scored-bt (get-in @state [:resPlayer :scored 0])]
        (is (= 2 (get-counters (refresh scored-bt) :agenda))
            "Scored w/ 4 over-advancements; 2 agenda counters")
        (play-from-hand state :resPlayer "Ichi 1.0" "HQ")
...
(deftest breaking-news
  ...
  (do-game
    ...
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (is (= 2 (get-in @state [:hazPlayer :tag])) "Runner receives 2 tags from Breaking News")
    (take-credits state :resPlayer)
    (is (= 0 (get-in @state [:hazPlayer :tag]))) "Two tags removed at the end of the turn"))
...
(deftest character-assassination
  ...
  (do-game
    ...
    (take-credits state :hazPlayer)
    (play-from-hand state :resPlayer "Character Assassination" "New remote")
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (let [kati (get-in @state [:hazPlayer :rig :resource 0])]
      (prompt-select :resPlayer kati)
...
(deftest corporate-sales-team
  ...
  (do-game
    ...
    (score-agenda state :resPlayer (get-content state :remote1 0))
	(let [scored-cst (get-in @state [:resPlayer :scored 0])]
...
(deftest corporate-war
  ...
  (do-game
    (new-game (default-corp [(qty "Corporate War" 2)])
              (default-runner))
    (play-from-hand state :resPlayer "Corporate War" "New remote")
    (is (= 5 (:credit (get-corp))))
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (is (= 0 (:credit (get-corp))) "Lost all credits")
    (core/gain state :resPlayer :credit 7)
    (play-from-hand state :resPlayer "Corporate War" "New remote")
    (score-agenda state :resPlayer (get-content state :remote2 0))
...
(deftest crisis-management
  ...
  (do-game
    (new-game (default-corp [(qty "Crisis Management" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Crisis Management" "New remote")
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (take-credits state :resPlayer)
    (take-credits state :hazPlayer)
...
(deftest dedicated-neural-net
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Dedicated Neural Net" "New remote")
    ...
    ;; test for #2376
    (prompt-choice :hazPlayer "Unrezzed upgrade in HQ")
...
(deftest eden-fragment
  ...
  (do-game
    (new-game (default-corp [(qty "Eden Fragment" 3) (qty "Ice Wall" 3)])
              (default-runner))
    (play-from-hand state :resPlayer "Ice Wall" "HQ")
    (play-from-hand state :resPlayer "Eden Fragment" "New remote")
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (take-credits state :resPlayer)
    (take-credits state :hazPlayer)
    (take-credits state :hazPlayer)
    ...
    (is (= 6 (get-in @state [:resPlayer :credit])) "Corp does not pay for installing the first ICE of the turn")
    (play-from-hand state :resPlayer "Ice Wall" "HQ")
    (is (not (nil? (get-ice state :hq 2))) "Corp has three ice installed on HQ")
    (is (= 4 (get-in @state [:resPlayer :credit])) "Corp pays for installing the second ICE of the turn")))
...
(deftest efficiency-committee
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Efficiency Committee" "New remote")
    (play-from-hand state :resPlayer "Efficiency Committee" "New remote")
    (play-from-hand state :resPlayer "Efficiency Committee" "New remote")
    (play-from-hand state :resPlayer "Ice Wall" "HQ")
    (let [ec1 (get-content state :remote1 0)
      ...
      (score-agenda state :resPlayer ec1)
      (let [ec1_scored (get-in @state [:resPlayer :scored 0])]
        (is (= 3 (get-counters (refresh ec1_scored) :agenda)))
        (is (= 2 (:agenda-point (get-corp))))
        ;; use token
        (is (= 3 (:click (get-corp))))
        (card-ability state :resPlayer ec1_scored 0)
        (is (= 4 (:click (get-corp))))
        ;; try to advance Ice Wall
        (core/advance state :resPlayer {:card (refresh iw)})
        (is (= 4 (:click (get-corp))))
        (is (= nil (:advance-counter (refresh iw))))
        ;; try to advance Efficiency Committee
        (core/advance state :resPlayer {:card (refresh ec2)})
        (is (= 4 (:click (get-corp))))
        ...
        (prompt-select :resPlayer ec2)
        (is (= 2 (:advance-counter (refresh ec2))))
        (play-from-hand state :resPlayer "Shipment from SanSan")
        (prompt-choice :resPlayer "2")
        (prompt-select :resPlayer ec2)
        (is (= 4 (:advance-counter (refresh ec2))))
        (core/score state :resPlayer {:card (refresh ec2)})
        ...
        (core/advance state :resPlayer {:card (refresh ec3)})
...
(deftest explode-a-palooza
  ...
  (do-game
    (new-game (default-corp [(qty "Explode-a-palooza" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Explode-a-palooza" "New remote")
    (take-credits state :resPlayer)
...
(deftest explode-ttw
  ...
  (do-game
    ...
    (starting-hand state :resPlayer ["Explode-a-palooza" "Explode-a-palooza"])
    (play-from-hand state :resPlayer "Explode-a-palooza" "New remote")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "The Turning Wheel")
    (run-empty-server state :remote1)
    (prompt-choice :hazPlayer "Access")
    (prompt-choice :resPlayer "Yes")
    (prompt-choice :hazPlayer "Steal")
...
(deftest fetal-ai-cant-afford
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Fetal AI" "New remote")
    (take-credits state :resPlayer 2)
    (core/lose state :hazPlayer :credit 5)
    (run-empty-server state "Server 1")
    (prompt-choice :hazPlayer "Yes")
    (is (= 3 (count (:hand (get-runner)))) "Runner took 2 net damage from Fetal AI")
...
(deftest government-contracts
  ...
  (do-game
    (new-game (default-corp [(qty "Government Contracts" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Government Contracts" "New remote")
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (is (= 2 (:click (get-corp))))
    (card-ability state :resPlayer (get-in @state [:resPlayer :scored 0]) 0)
...
(deftest high-risk-investment
  ...
  (do-game
    (new-game (default-corp [(qty "High-Risk Investment" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "High-Risk Investment" "New remote")
    (let [hri (get-content state :remote1 0)]
      (score-agenda state :resPlayer hri)
      (let [hriscored (get-in @state [:resPlayer :scored 0])]
        (is (= 1 (get-counters (refresh hriscored) :agenda)) "Has 1 agenda counter")
        (take-credits state :resPlayer)
        (is (= 7 (:credit (get-corp))))
        (take-credits state :hazPlayer)
        (is (= 9 (:credit (get-runner))))
        (card-ability state :resPlayer hriscored 0)
...
(deftest labyrinthine-servers
  ...
  (do-game
    (new-game (default-corp [(qty "Labyrinthine Servers" 2)])
              (default-runner))
    (play-from-hand state :resPlayer "Labyrinthine Servers" "New remote")
    (play-from-hand state :resPlayer "Labyrinthine Servers" "New remote")
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (score-agenda state :resPlayer (get-content state :remote2 0))
    (take-credits state :resPlayer)
    (let [ls1 (get-in @state [:resPlayer :scored 0])
          ls2 (get-in @state [:resPlayer :scored 1])]
      ...
      (run-on state "HQ")
      (run-jack-out state)
      (card-ability state :resPlayer ls1 0)
      (card-ability state :resPlayer ls2 0)
      (card-ability state :resPlayer ls1 0)
      (prompt-choice :resPlayer "Done")
...
(deftest medical-breakthrough
  ...
  (do-game
    (new-game (default-corp [(qty "Medical Breakthrough" 3) (qty "Hedge Fund" 3)])
              (default-runner))
    (play-from-hand state :resPlayer "Medical Breakthrough" "New remote")
    (play-from-hand state :resPlayer "Medical Breakthrough" "New remote")
    ...
    (let [mb2 (get-content state :remote2 0)]
      (core/advance state :resPlayer {:card (refresh mb2)})
      (core/advance state :resPlayer {:card (refresh mb2)})
      (core/advance state :resPlayer {:card (refresh mb2)})
      (core/score state :resPlayer {:card (refresh mb2)})
      (is (= 2 (:agenda-point (get-corp))) "Only needed 3 advancements to score")
      (take-credits state :resPlayer)
      (take-credits state :hazPlayer)
      (play-from-hand state :resPlayer "Medical Breakthrough" "New remote")
      (let [mb3 (get-content state :remote3 0)]
        (core/advance state :resPlayer {:card (refresh mb3)})
        (core/advance state :resPlayer {:card (refresh mb3)})
        (core/score state :resPlayer {:card (refresh mb3)})
...
(deftest net-quarantine
  ...
  (do-game
    (new-game (default-corp [(qty "Net Quarantine" 1)])
              (default-runner))
    (core/gain state :hazPlayer :link 1)
    (core/gain state :resPlayer :click 3)
    (play-from-hand state :resPlayer "Net Quarantine" "New remote")
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (is (= 5 (:credit (get-corp))) "Corp has 5 credits")
    (is (= 1 (:link (get-runner))) "Runner has 1 link")
    (core/corp-trace-prompt state {:title "/trace command" :side :resPlayer} {:base 1})
...
(deftest nisei-mk-ii-step-43
  ...
  (do-game
   (new-game (default-corp [(qty "Nisei MK II" 1)])
             (default-runner))
   (play-from-hand state :resPlayer "Nisei MK II" "New remote")
   (score-agenda state :resPlayer (get-content state :remote1 0))
   (let [scored-nisei (get-in @state [:resPlayer :scored 0])]
     (is (= 1 (get-counters (refresh scored-nisei) :agenda)) "Scored Nisei has one counter")
     (take-credits state :resPlayer)

     (run-on state "HQ")
     (run-phase-43 state)
     (card-ability state :resPlayer (refresh scored-nisei) 0)
     (prompt-choice :resPlayer "Done") ; close 4.3 corp
...
(deftest obokata-protocol
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Obokata Protocol" "New remote")
    (take-credits state :resPlayer)
    (core/gain state :hazPlayer :agenda-point 6)
    (run-empty-server state "Server 1")
    (prompt-choice :hazPlayer "Yes")
    (is (= 4 (count (:discard (get-runner)))) "Runner paid 4 net damage")
    (is (= :hazPlayer (:winner @state)) "Runner wins")
...
(deftest personality-profiles
  ...
  (do-game
    ...
    (starting-hand state :hazPlayer ["Self-modifying Code" "Clone Chip" "Patron" "Patron"])
    (score-agenda state :resPlayer (find-card "Personality Profiles" (:hand (get-corp))))
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Self-modifying Code")
    (play-from-hand state :hazPlayer "Clone Chip")
    (let [smc (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer smc 0)
    ...
    (let [chip (get-in @state [:hazPlayer :rig :hardware 0])]
      ...
      (prompt-select :hazPlayer (find-card "Self-modifying Code" (:discard (get-runner))))
...
(deftest personality-profiles-empty-hand
  ...
  (do-game
    (new-game (default-corp [(qty "Personality Profiles" 1)])
              (default-runner [(qty "Self-modifying Code" 1) (qty "Clone Chip" 1)
                               (qty "Corroder" 1)]))
    (starting-hand state :hazPlayer ["Self-modifying Code" "Clone Chip"])
    (score-agenda state :resPlayer (find-card "Personality Profiles" (:hand (get-corp))))
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Self-modifying Code")
    (play-from-hand state :hazPlayer "Clone Chip")
    (let [smc (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer smc 0)
      (prompt-choice :hazPlayer (find-card "Corroder" (:deck (get-runner))))
      (let [cor (get-in @state [:hazPlayer :rig :program 0])]
        (is (not (nil? cor)))
    ...
    (let [chip (get-in @state [:hazPlayer :rig :hardware 0])]
      (card-ability state :hazPlayer chip 0)
      (prompt-select :hazPlayer (find-card "Self-modifying Code" (:discard (get-runner))))
      (let [smc (get-in @state [:hazPlayer :rig :program 1])]
        (is (not (nil? smc)))
...
(deftest posted-bounty-yes
  ...
  (do-game
    ...
    (let [pb (get-content state :remote1 0)]
      (score-agenda state :resPlayer pb)
	  (prompt-choice :resPlayer "Yes")
	  ...
	  (is (= 1 (get-in @state [:hazPlayer :tag])) "Runner receives 1 tag forfeiting Posted Bounty")))
...
(deftest posted-bounty-no
  ...
  (do-game
    ...
    (let [pb (get-content state :remote1 0)]
	  ...
	  (is (= 0 (get-in @state [:hazPlayer :tag])))))
...
(deftest profiteering
  ...
  (do-game
    (new-game (default-corp [(qty "Profiteering" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Profiteering" "New remote")
    (let [prof (get-content state :remote1 0)]
      (score-agenda state :resPlayer prof)
...
(deftest project-ares
  ...
  (do-game
    ...
    (play-from-hand state :hazPlayer "Clone Chip")
    (take-credits state :hazPlayer)
    (score-agenda state :resPlayer (find-card "Project Ares" (:hand (get-corp))))
    (is (empty? (get-in @state [:hazPlayer :prompt])) "No prompt for Runner if scored with 4 advancement tokens")
    (core/gain state :resPlayer :click 4)
    (play-from-hand state :resPlayer "Project Ares" "New remote")
    (let [ares (get-content state :remote1 0)]
      (core/advance state :resPlayer {:card (refresh ares)})
      (core/advance state :resPlayer {:card (refresh ares)})
      (core/advance state :resPlayer {:card (refresh ares)})
      (core/advance state :resPlayer {:card (refresh ares)})
      (core/advance state :resPlayer {:card (refresh ares)})
      ...
      (core/score state :resPlayer {:card (refresh ares)}))
      (is (prompt-is-card? :hazPlayer ares) "Runner has Ares prompt to trash installed cards"))
    (prompt-select :hazPlayer (find-card "Clone Chip" (:hardware (:rig (get-runner)))))
    (is (empty? (get-in @state [:hazPlayer :prompt])) "Runner must trash 2 cards but only has 1 card in rig, prompt ended")
...
(deftest project-beale
  ...
  (do-game
    (new-game (default-corp [(qty "Project Beale" 2)])
              (default-runner))
    (core/gain state :resPlayer :click 8 :credit 8)
    (play-from-hand state :resPlayer "Project Beale" "New remote")
    (let [pb1 (get-content state :remote1 0)]
      (core/advance state :resPlayer {:card (refresh pb1)})
      (core/advance state :resPlayer {:card (refresh pb1)})
      (core/advance state :resPlayer {:card (refresh pb1)})
      (core/advance state :resPlayer {:card (refresh pb1)})
      (core/score state :resPlayer {:card (refresh pb1)})
      (is (= 2 (:agenda-point (get-corp))) "Only 4 advancements: scored for standard 2 points")
      (play-from-hand state :resPlayer "Project Beale" "New remote")
        (let [pb2 (get-content state :remote2 0)]
          (core/advance state :resPlayer {:card (refresh pb2)})
          (core/advance state :resPlayer {:card (refresh pb2)})
          (core/advance state :resPlayer {:card (refresh pb2)})
          (core/advance state :resPlayer {:card (refresh pb2)})
          (core/advance state :resPlayer {:card (refresh pb2)})
          (core/score state :resPlayer {:card (refresh pb2)})
...
(deftest puppet-master
  ...
  (do-game
    (new-game (default-corp [(qty "Puppet Master" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Puppet Master" "New remote")
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (take-credits state :resPlayer)
...
(deftest rebranding-team
  ...
  (do-game
    (new-game (default-corp [(qty "Rebranding Team" 1) (qty "Launch Campaign" 1) (qty "City Surveillance" 1)
              ...
              (default-runner))
    (score-agenda state :resPlayer (find-card "Rebranding Team" (:hand (get-corp))))
    ...
    (core/move state :resPlayer (find-card "Rebranding Team" (:scored (get-corp))) :deck)
    (is (core/has-subtype? (find-card "Launch Campaign" (:hand (get-corp))) "Advertisement"))
...
(deftest reeducation
  ...
  (do-game
    ...
    (starting-hand state :resPlayer ["Reeducation" "Sweeps Week"])
    (starting-hand state :hazPlayer ["Self-modifying Code"])
    (score-agenda state :resPlayer (find-card "Reeducation" (:hand (get-corp))))
    (is (prompt-is-type? :hazPlayer :waiting) "Runner has wait prompt")
    (is (= 1 (count (get-in @state [:resPlayer :hand]))))
    (is (= 1 (count (get-in @state [:hazPlayer :hand]))))
    (prompt-choice :resPlayer (find-card "Sweeps Week" (:hand (get-corp)))) ; put Sweeps Week at bottom of R&D
    (prompt-choice :resPlayer "Done") ; finished selecting cards
    (prompt-choice :resPlayer "Done") ; corp prompt for Done/Start Over
...
(deftest reeducation-extra-cards
  ...
  (do-game
    (new-game (default-corp [(qty "Reeducation" 1) (qty "Sweeps Week" 1) (qty "Hedge Fund" 1)
              ...
              (default-runner [(qty "Self-modifying Code" 1) (qty "Clone Chip" 1)
                               (qty "Corroder" 1) (qty "Sure Gamble" 1) (qty "Desperado" 1)]))
    (starting-hand state :resPlayer ["Reeducation" "Sweeps Week" "Hedge Fund"])
    (starting-hand state :hazPlayer ["Self-modifying Code"])
    (score-agenda state :resPlayer (find-card "Reeducation" (:hand (get-corp))))
    (is (prompt-is-type? :hazPlayer :waiting) "Runner has wait prompt")
    (is (= 2 (count (get-in @state [:resPlayer :hand]))))
    (is (= 1 (count (get-in @state [:hazPlayer :hand]))))
    (prompt-choice :resPlayer (find-card "Sweeps Week" (:hand (get-corp))))
    (prompt-choice :resPlayer (find-card "Hedge Fund" (:hand (get-corp)))) ; this is the bottom card of R&D
    (prompt-choice :resPlayer "Done") ; finished selecting cards
    (prompt-choice :resPlayer "Done") ; corp prompt for Done/Start Over
...
(deftest ssl-endorsement-scored
  ...
  (do-game
    (new-game (default-corp [(qty "SSL Endorsement" 1)])
              (default-runner))
    (score-agenda state :resPlayer (find-card "SSL Endorsement" (:hand (get-corp))))
    (take-credits state :hazPlayer)
...
(deftest ssl-endorsement-stolen
  ...
  (do-game
    (new-game (default-corp [(qty "SSL Endorsement" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "SSL Endorsement" "New remote")
    (take-credits state :resPlayer)
...
(deftest ssl-endorsement-stolen-swapped
  ...
  (do-game
    ...
    (prompt-select :resPlayer (find-card "SSL Endorsement" (:scored (get-runner))))
...
(deftest sentinel-defense-program
  ...
  (do-game
    ...
    (score-agenda state :resPlayer (find-card "Sentinel Defense Program" (:hand (get-corp))))
    (play-from-hand state :resPlayer "Viktor 1.0" "HQ")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Feedback Filter")
    (let [viktor (get-ice state :hq 0)
          ff (get-in @state [:hazPlayer :rig :hardware 0])]
      (run-on state "HQ")
      (core/rez state :resPlayer viktor)
      (card-subroutine state :resPlayer viktor 0)
      (prompt-choice :hazPlayer "Done") ;don't prevent the brain damage
      (is (= 1 (count (:discard (get-runner)))))
      (is (= 1 (:brain-damage (get-runner))))
      (prompt-choice :hazPlayer "Done") ;so we take the net, but don't prevent it either
      (is (= 2 (count (:discard (get-runner)))))
      (card-subroutine state :resPlayer viktor 0)
      (card-ability state :hazPlayer ff 1) ;prevent the brain damage this time
...
(deftest tgtbt
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Old Hollywood Grid" "Server 1")
    (play-from-hand state :resPlayer "TGTBT" "New remote")
    (take-credits state :resPlayer)
    ...
    (prompt-choice :hazPlayer "Yes") ; Trashes OHG
...
(deftest the-future-perfect
  ...
  (do-game
    (new-game (default-corp [(qty "The Future Perfect" 2)])
              (default-runner))
    (play-from-hand state :resPlayer "The Future Perfect" "New remote")
    (take-credits state :resPlayer)
...
(deftest underway-renovation
  ...
  (do-game
    ...
    (let [ur (get-content state :remote1 0)]
      (core/advance state :resPlayer {:card (refresh ur)})
...
(deftest vulcan-coverup
  ...
  (do-game
    (new-game (default-corp [(qty "Vulcan Coverup" 2)])
              (default-runner))
    (play-from-hand state :resPlayer "Vulcan Coverup" "New remote")
    (take-credits state :resPlayer)
    (run-empty-server state :remote1)
    (prompt-choice :hazPlayer "Steal")
    ...
    (let [vc (get-content state :remote2 0)]
      (score-agenda state :resPlayer vc)
