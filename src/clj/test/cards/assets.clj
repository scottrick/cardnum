(ns test.cards.assets
  (:require [game.core :as core]
            [test.core :refer :all]
            [test.utils :refer :all]
            [test.macros :refer :all]
            [clojure.test :refer :all]))

(deftest adonis-campaign
  (do-game
    (new-game (default-minion [(qty "Adonis Campaign" 1)])
              (default-hero))
    (play-from-hand state :minion "Adonis Campaign" "New remote")
    (let [ac (get-content state :remote1 0)]
      (core/rez state :minion ac)
      (is (= 1 (get-in @state [:minion :credit])))
      (is (= 12 (get-counters (refresh ac) :credit)) "12 counters on Adonis")
      (take-credits state :minion 2)
      (take-credits state :hero)
      (is (= 6 (get-in @state [:minion :credit])) "Gain 3 from Adonis")
      (is (= 9 (get-counters (refresh ac) :credit))) "9 counter remaining on Adonis")))

(deftest aggressive-secretary
  (do-game
    (new-game
      (default-minion [(qty "Aggressive Secretary" 1)])
      (default-hero [(qty "Cache" 3)]))
    (play-from-hand state :minion "Aggressive Secretary" "New remote")
    (let [as (get-content state :remote1 0)]
      ;; Single advance AggSec
      (core/advance state :minion {:card (refresh as)})
      (take-credits state :minion)
      ;; Run on AggSec with 3 programs
      (play-from-hand state :hero "Cache")
      (play-from-hand state :hero "Cache")
      (play-from-hand state :hero "Cache")
      (run-empty-server state "Server 1")
      (prompt-choice :minion "Yes")
      (is (= 3 (get-in @state [:minion :credit])))
      ;; Corp can trash one program
      (prompt-select :minion (get-in @state [:hero :rig :program 1]))
      ;; There should be two Caches left
      (is (= 3 (get-in @state [:minion :credit])))
      (is (= 2 (count (get-in @state [:hero :rig :program])))))))

(deftest alexa-belsky
  (do-game
    (new-game
      (default-minion [(qty "Alexa Belsky" 1) (qty "Hedge Fund" 1) (qty "Breaking News" 1)
                     (qty "Gutenberg" 1) (qty "Product Placement" 1) (qty "Jackson Howard" 1)])
      (default-hero))
    (play-from-hand state :minion "Alexa Belsky" "New remote")
    (let [alexa (get-content state :remote1 0)]
      (core/rez state :minion alexa)
      (card-ability state :minion alexa 0)
      (is (= 1 (count (:discard (get-minion)))) "Alexa Belsky trashed")
      (is (= 5 (count (:hand (get-minion)))))
      (is (= 0 (count (:deck (get-minion)))))
      (prompt-choice :hero 5) ;Runner chooses to pay 5 credits so 2 cards are prevented from being shuffled
      (is (= 2 (count (:hand (get-minion)))))
      (is (= 3 (count (:deck (get-minion)))))
      (is (= 0 (:credit (get-hero)))))))

(deftest alix-t4lb07
  (do-game
    (new-game
      (default-minion [(qty "Alix T4LB07" 1) (qty "PAD Campaign" 3)])
      (default-hero))
    (play-from-hand state :minion "Alix T4LB07" "New remote")
    (let [alix (get-content state :remote1 0)]
      (core/rez state :minion alix)
      (play-from-hand state :minion "PAD Campaign" "New remote")
      (play-from-hand state :minion "PAD Campaign" "New remote")
      (take-credits state :minion)
      (take-credits state :hero)
      (is (= 2 (get-counters (refresh alix) :power)) "Two counters on Alix")
      (is (= 4 (get-in @state [:minion :credit])))
      (card-ability state :minion alix 0)
      (is (= 8 (get-in @state [:minion :credit]))))) "Gain 4 credits from Alix")

(deftest blacklist-steal
  ;; Blacklist - #2426.  Need to allow steal.
  (do-game
    (new-game (default-minion [(qty "Fetal AI" 3) (qty "Blacklist" 1)])
              (default-hero))
    (trash-from-hand state :minion "Fetal AI")
    (play-from-hand state :minion "Blacklist" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (= 1 (count (get-in @state [:minion :discard])))
    (take-credits state :minion)
    (run-empty-server state :archives)
    (prompt-choice :hero "Yes")
    (is (= 2 (:agenda-point (get-hero))) "Runner has 2 agenda points")
    (= 1 (count (get-in @state [:hero :scored])))))

(deftest bio-ethics-multiple
  ;; Bio-Ethics Association: preventing damage from multiple copies
  (do-game
    (new-game
      (default-minion [(qty "Bio-Ethics Association" 2)])
      (default-hero [(qty "Feedback Filter" 1) (qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Bio-Ethics Association" "New remote")
    (play-from-hand state :minion "Bio-Ethics Association" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (core/rez state :minion (get-content state :remote2 0))
    (take-credits state :minion)
    (play-from-hand state :hero "Feedback Filter")
    (take-credits state :hero)
    (let [filter (get-hardware state 0)]
      (is (= 1 (count (:prompt (get-hero)))) "Runner has a single damage prevention prompt")
      (card-ability state :hero filter 0)
      (prompt-choice :hero "Done")
      (is (= 0 (count (:discard (get-hero)))) "Runner prevented damage")
      (is (= 1 (count (:prompt (get-hero)))) "Runner has a next damage prevention prompt")
      (prompt-choice :hero "Done")
      (is (= 1 (count (:discard (get-hero)))) "Runner took 1 net damage"))))

(deftest brain-taping-warehouse
  ;; Brain-Taping Warehouse - Lower rez cost of Bioroid ICE by 1 for each unspent Runner click
  (do-game
    (new-game (default-minion [(qty "Brain-Taping Warehouse" 1) (qty "Ichi 1.0" 1)
                             (qty "Eli 1.0" 1)])
              (default-hero))
    (play-from-hand state :minion "Brain-Taping Warehouse" "New remote")
    (play-from-hand state :minion "Ichi 1.0" "Server 1")
    (play-from-hand state :minion "Eli 1.0" "HQ")
    (let [ichi (get-ice state :remote1 0)
          eli (get-ice state :hq 0)]
      (take-credits state :minion)
      (run-on state :remote1)
      (core/rez state :minion (get-content state :remote1 0))
      (is (= 3 (:click (get-hero))))
      (core/rez state :minion ichi)
      (is (= 2 (:credit (get-minion))) "Paid only 2c to rez Ichi; reduction of 3c")
      (run-jack-out state)
      (run-on state :hq)
      (is (= 2 (:click (get-hero))))
      (core/rez state :minion eli)
      (is (= 1 (:credit (get-minion))) "Paid only 1c to rez Eli; reduction of 2c"))))

(deftest capital-investors
  ;; Capital Investors - Click for 2 credits
  (do-game
    (new-game (default-minion [(qty "Capital Investors" 1)])
              (default-hero))
    (play-from-hand state :minion "Capital Investors" "New remote")
    (let [cap (get-content state :remote1 0)]
      (core/rez state :minion cap)
      (card-ability state :minion cap 0)
      (card-ability state :minion cap 0)
      (is (= 0 (:click (get-minion))) "Used twice, spent 2 clicks")
      (is (= 7 (:credit (get-minion))) "Used twice, gained 4 credits"))))

(deftest chairman-hiro
  ;; Chairman Hiro - Reduce Runner max hand size; add as 2 agenda points if Runner trashes him
  (do-game
    (new-game (default-minion [(qty "Chairman Hiro" 2)])
              (default-hero))
    (play-from-hand state :minion "Chairman Hiro" "New remote")
    (play-from-hand state :minion "Chairman Hiro" "Server 1")
    (prompt-choice :minion "OK")
    (is (= 1 (count (:discard (get-minion)))) "First Hiro trashed")
    (is (= 0 (:agenda-point (get-hero))) "No points for Runner if trashed by Corp")
    (let [hiro (get-content state :remote1 0)]
      (core/rez state :minion hiro)
      (is (= 3 (core/hand-size state :hero)) "Runner max hand size reduced by 2")
      (take-credits state :minion)
      (take-credits state :hero 3)
      (run-empty-server state "Server 1")
      (prompt-choice :hero "Yes") ; trash Hiro
      (is (= 2 (:credit (get-hero))) "Runner paid 6 credits to trash")
      (is (= 5 (core/hand-size state :hero)) "Runner max hand size restored to 5")
      (is (= 1 (count (get-in @state [:hero :scored])))
          "Chairman Hiro added to Runner score area")
      (is (= 2 (:agenda-point (get-hero))) "Runner gained 2 agenda points"))))

(deftest city-surveillance
  ;; City Surveillance - Runner chooses to pay 1 credit or take 1 tag at start of their turn
  (do-game
    (new-game (default-minion [(qty "City Surveillance" 1)])
              (default-hero))
    (play-from-hand state :minion "City Surveillance" "New remote")
    (let [surv (get-content state :remote1 0)]
      (core/rez state :minion surv)
      (take-credits state :minion)
      (prompt-choice :hero "Pay 1 [Credits]")
      (is (= 4 (:credit (get-hero))) "Runner paid 1 credit")
      (is (= 0 (:tag (get-hero))) "Runner didn't take a tag")
      (is (empty? (:prompt (get-hero))) "City Surveillance only fired once")
      (take-credits state :hero)
      (take-credits state :minion)
      (prompt-choice :hero "Take 1 tag")
      (is (= 8 (:credit (get-hero))) "Runner paid no credits")
      (is (= 1 (:tag (get-hero))) "Runner took 1 tag"))
      (is (empty? (:prompt (get-hero))) "City Surveillance only fired once")))

(deftest clyde-van-rite
  ;; Clyde Van Rite - Multiple scenarios involving Runner not having credits/cards to trash
  (do-game
    (new-game (default-minion [(qty "Clyde Van Rite" 1)])
              (default-hero [(qty "Sure Gamble" 3) (qty "Restructure" 2) (qty "John Masanori" 2)]))
    (play-from-hand state :minion "Clyde Van Rite" "New remote")
    (let [clyde (get-content state :remote1 0)]
      (core/rez state :minion clyde)
      (take-credits state :minion)
      (take-credits state :hero)
      (is (:minion-phase-12 @state) "Corp in Step 1.2")
      ;; Runner chooses to pay - has 1+ credit so pays 1 credit
      (card-ability state :minion clyde 0)
      (is (= 9 (:credit (get-hero))))
      (is (= 2 (count (:deck (get-hero)))))
      (prompt-choice :hero "Pay 1 [Credits]")
      (is (= 8 (:credit (get-hero))))
      (is (= 2 (count (:deck (get-hero)))))
      (core/end-phase-12 state :minion nil)
      (take-credits state :minion)
      (take-credits state :hero)
      ;; Runner chooses to pay - can't pay 1 credit so trash top card
      (core/lose state :hero :credit 12)
      (card-ability state :minion clyde 0)
      (is (= 0 (:credit (get-hero))))
      (is (= 2 (count (:deck (get-hero)))))
      (prompt-choice :hero "Pay 1 [Credits]")
      (is (= 0 (:credit (get-hero))))
      (is (= 1 (count (:deck (get-hero)))))
      (core/end-phase-12 state :minion nil)
      (take-credits state :minion)
      (take-credits state :hero)
      ;; Runner chooses to trash - has 1+ card in Stack so trash 1 card
      (card-ability state :minion clyde 0)
      (is (= 4 (:credit (get-hero))))
      (is (= 1 (count (:deck (get-hero)))))
      (prompt-choice :hero "Trash top card")
      (is (= 4 (:credit (get-hero))))
      (is (= 0 (count (:deck (get-hero)))))
      (core/end-phase-12 state :minion nil)
      (take-credits state :minion)
      (take-credits state :hero)
      ;; Runner chooses to trash - no cards in Stack so pays 1 credit
      (card-ability state :minion clyde 0)
      (is (= 8 (:credit (get-hero))))
      (is (= 0 (count (:deck (get-hero)))))
      (prompt-choice :hero "Trash top card")
      (is (= 7 (:credit (get-hero))))
      (is (= 0 (count (:deck (get-hero))))))))

(deftest daily-business-show
  ;; Daily Business Show - Full test
  (do-game
    (new-game (default-minion [(qty "Daily Business Show" 3) (qty "Hedge Fund" 1) (qty "Jackson Howard" 1)
                             (qty "Resistor" 1) (qty "Product Placement" 1) (qty "Breaking News" 1)])
              (default-hero))
    (starting-hand state :minion ["Daily Business Show" "Daily Business Show" "Daily Business Show" "Hedge Fund"])
    (core/gain state :minion :credit 1)
    (play-from-hand state :minion "Daily Business Show" "New remote")
    (play-from-hand state :minion "Daily Business Show" "New remote")
    (play-from-hand state :minion "Daily Business Show" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (core/rez state :minion (get-content state :remote2 0))
    (core/rez state :minion (get-content state :remote3 0))
    (take-credits state :minion)
    (is (= 1 (count (:hand (get-minion)))))
    (take-credits state :hero)
    (is (= 5 (count (:hand (get-minion)))) "Drew an additional 3 cards with 3 DBS")
    (is (not-empty (:prompt (get-hero))) "Runner is waiting for Corp to use DBS")
    (prompt-select :minion (find-card "Hedge Fund" (:hand (get-minion)))) ;invalid target
    (prompt-select :minion (find-card "Resistor" (:hand (get-minion))))
    (prompt-select :minion (find-card "Product Placement" (:hand (get-minion))))
    (prompt-select :minion (find-card "Breaking News" (:hand (get-minion))))
    (is (= 2 (count (:hand (get-minion)))))
    (is (= "Hedge Fund" (:title (first (:hand (get-minion))))))
    (is (= "Jackson Howard" (:title (second (:hand (get-minion))))))
    (is (= "Resistor" (:title (last (:deck (get-minion))))) "Resistor last card in deck")
    (is (= "Product Placement" (:title (last (butlast (:deck (get-minion))))))
        "Product Placement second last card in deck")
    (is (= "Breaking News" (:title (last (butlast (butlast (:deck (get-minion)))))))
        "Breaking News third last card in deck")))

(deftest daily-business-show-sensie-actors-union
  ;; Daily Business Show - Sensie Actors Union interaction
  (do-game
    (new-game (default-minion [(qty "Daily Business Show" 1) (qty "Sensie Actors Union" 2)
                             (qty "Hedge Fund" 1) (qty "Jackson Howard" 1)
                             (qty "Resistor" 1) (qty "Product Placement" 1) (qty "Breaking News" 1)])
              (default-hero))
    (starting-hand state :minion ["Daily Business Show" "Sensie Actors Union" "Sensie Actors Union" "Hedge Fund"])
    (play-from-hand state :minion "Daily Business Show" "New remote")
    (play-from-hand state :minion "Sensie Actors Union" "New remote")
    (play-from-hand state :minion "Sensie Actors Union" "New remote")
    (let [sensie1 (get-content state :remote2 0)
          sensie2 (get-content state :remote3 0)]
      (core/rez state :minion (get-content state :remote1 0))
      (core/rez state :minion sensie1)
      (core/rez state :minion sensie2)
      (take-credits state :minion)
      (take-credits state :hero)
      ;; Use first Sensie
      (is (= 1 (count (:hand (get-minion)))))
      (card-ability state :minion sensie1 0)
      (is (= 5 (count (:hand (get-minion)))) "Drew 3 cards with Sensie, +1 with DBS")
      (prompt-select :minion (find-card "Resistor" (:hand (get-minion)))) ; DBS target
      (prompt-select :minion (find-card "Hedge Fund" (:hand (get-minion)))) ; Sensie target
      (is (= 3 (count (:hand (get-minion)))))
      (is (= "Hedge Fund" (:title (last (:deck (get-minion))))) "Hedge Fund last card in deck")
      (is (= "Resistor" (:title (last (butlast (:deck (get-minion))))))
          "Resistor second last card in deck")
      ;; Try to use first Sensie again
      (card-ability state :minion sensie1 0)
      (is (empty? (get-in @state [:minion :prompt])) "Sensie didn't activate")
      (is (= 3 (count (:hand (get-minion)))))
      ;; Use second Sensie
      (starting-hand state :minion ["Hedge Fund" "Jackson Howard"])
      (is (= 2 (count (:hand (get-minion)))))
      (card-ability state :minion sensie2 0)
      (is (= 5 (count (:hand (get-minion)))) "Drew 3 cards with Sensie, DBS didn't activate")
      (prompt-select :minion (find-card "Breaking News" (:hand (get-minion)))) ; Sensie target
      (is (= "Breaking News" (:title (last (:deck (get-minion))))) "Breaking News last card in deck"))))

(deftest daily-business-show-manual-draw
  ;; Daily Business Show - Should not trigger if rezzed after mandatory draw
  (do-game
    (new-game (default-minion [(qty "Daily Business Show" 3) (qty "Hedge Fund" 1) (qty "Jackson Howard" 1)
                             (qty "Resistor" 1) (qty "Product Placement" 1) (qty "Breaking News" 1)])
              (default-hero))
    (starting-hand state :minion ["Daily Business Show"])
    (play-from-hand state :minion "Daily Business Show" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (core/draw state :minion)
    (is (= 1 (count (:hand (get-minion)))) "DBS did not fire on manual draw")
    (is (empty? (:prompt (get-minion))) "Corp is not being asked to bury a card with DBS")    ))

(deftest dedicated-response-team
  ;; Dedicated Response Team - Do 2 meat damage when successful run ends if Runner is tagged
  (do-game
    (new-game (default-minion [(qty "Dedicated Response Team" 1)])
              (default-hero))
    (play-from-hand state :minion "Dedicated Response Team" "New remote")
    (let [drt (get-content state :remote1 0)]
      (core/rez state :minion drt)
      (take-credits state :minion)
      (run-empty-server state :rd)
      (is (empty? (:discard (get-hero))) "Not tagged, no damage done")
      (core/gain state :hero :tag 1)
      (run-on state :rd)
      (run-jack-out state)
      (is (empty? (:discard (get-hero))) "Tagged but run unsuccessful, no damage done")
      (run-empty-server state :rd)
      (is (= 2 (count (:discard (get-hero)))) "Suffered 2 damage for successful run w/ tag"))))

(deftest early-premiere
  ;; Early Premiere - Pay 1c at start of turn to place an advancement on a card in a server
  (do-game
    (new-game (default-minion [(qty "Early Premiere" 1) (qty "Ice Wall" 1)
                             (qty "Ghost Branch" 1) (qty "Blacklist" 1)])
              (default-hero))
    (core/gain state :minion :click 1)
    (play-from-hand state :minion "Early Premiere" "New remote")
    (play-from-hand state :minion "Blacklist" "New remote")
    (play-from-hand state :minion "Ghost Branch" "New remote")
    (play-from-hand state :minion "Ice Wall" "HQ")
    (let [ep (get-content state :remote1 0)
          bl (get-content state :remote2 0)
          gb (get-content state :remote3 0)
          iw (get-ice state :hq 0)]
      (core/rez state :minion ep)
      (take-credits state :minion)
      (take-credits state :hero)
      (card-ability state :minion ep 0)
      (prompt-select :minion iw)
      (is (nil? (:advance-counter (refresh iw))) "Ice Wall can't targeted, not in server")
      (prompt-select :minion bl)
      (is (nil? (:advance-counter (refresh bl))) "Blacklist can't targeted, can't be advanced")
      (prompt-select :minion gb)
      (is (= 1 (:advance-counter (refresh gb))) "1 advancement on Ghost Branch")
      (is (= 4 (:credit (get-minion)))))))

(deftest echochamber
  ;; Echo Chamber - 3 clicks to become 1 point agenda
  (do-game
    (new-game (default-minion [(qty "Echo Chamber" 1)])
              (default-hero))
    (core/gain state :minion :click 1)
    (play-from-hand state :minion "Echo Chamber" "New remote")
    (let [ec (get-content state :remote1 0)]
      (core/rez state :minion ec)
      (card-ability state :minion ec 0))
    (is (= 1 (:agendapoints (get-in @state [:minion :scored 0]))) "Echo Chamber added to Corp score area")))

(deftest edge-of-world
  ;; Edge of World - ability
  (do-game
    (new-game (default-minion [(qty "Edge of World" 3) (qty "Ice Wall" 3)])
              (default-hero))
    (core/gain state :minion :credit 6 :click 1)
    (play-from-hand state :minion "Edge of World" "New remote")
    (play-from-hand state :minion "Edge of World" "New remote")
    (play-from-hand state :minion "Ice Wall" "Server 1")
    (play-from-hand state :minion "Ice Wall" "Server 1")
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (is (= :waiting (-> @state :hero :prompt first :prompt-type))
        "Runner waiting for Corp to act")
    (prompt-choice :minion "Yes")
    (prompt-choice :hero "Yes")
    (is (= 2 (:brain-damage (get-hero))) "Runner took 2 brain damage")
    (run-empty-server state "Server 2")
    (prompt-choice :minion "Yes")
    (prompt-choice :hero "Yes")
    (is (= 2 (:brain-damage (get-hero))) "Runner did not take brain damage when no ICE protected Edge of World")))

(deftest elizabeth-mills
  ;; Elizabeth Mills - Remove 1 bad publicity when rezzed; click-trash to trash a location
  (do-game
    (new-game (default-minion [(qty "Elizabeth Mills" 1)])
              (default-hero [(qty "Earthrise Hotel" 1)]))
    (core/gain state :minion :bad-publicity 1)
    (play-from-hand state :minion "Elizabeth Mills" "New remote")
    (take-credits state :minion)
    (play-from-hand state :hero "Earthrise Hotel")
    (take-credits state :hero)
    (let [liz (get-content state :remote1 0)
          hotel (get-in @state [:hero :rig :resource 0])]
      (core/rez state :minion liz)
      (is (= 0 (:bad-publicity (get-minion))) "1 bad publicity removed")
      (card-ability state :minion liz 0)
      (prompt-select :minion hotel)
      (is (= 1 (count (:discard (get-hero)))) "Earthrise trashed")
      (is (= 1 (count (:discard (get-minion)))) "Elizabeth Mills trashed")
      (is (= 1 (:bad-publicity (get-minion))) "1 bad publicity taken from trashing a location"))))

(deftest elizas-toybox
  ;; Eliza's Toybox - Rez a card ignoring all costs
  (do-game
    (new-game (default-minion [(qty "Eliza's Toybox" 1) (qty "Wotan" 1)])
              (default-hero))
    (core/gain state :minion :click 2)
    (play-from-hand state :minion "Wotan" "R&D")
    (play-from-hand state :minion "Eliza's Toybox" "New remote")
    (let [wotan (get-ice state :rd 0)
          eliza (get-content state :remote1 0)]
      (core/rez state :minion eliza)
      (is (= 1 (:credit (get-minion))))
      (card-ability state :minion eliza 0)
      (prompt-select :minion wotan)
      (is (get-in (refresh wotan) [:rezzed]))
      (is (= 0 (:click (get-minion))) "3 clicks spent")
      (is (= 1 (:credit (get-minion))) "No credits spent"))))

(deftest encryption-protocol
  ;; Encryption Protocol - Trash cost of installed cards increased by 1
  (do-game
    (new-game (default-minion [(qty "Encryption Protocol" 2)])
              (default-hero))
    (play-from-hand state :minion "Encryption Protocol" "New remote")
    (play-from-hand state :minion "Encryption Protocol" "New remote")
    (let [ep1 (get-content state :remote1 0)
          ep2 (get-content state :remote2 0)]
      (core/rez state :minion ep1)
      (core/rez state :minion ep2)
      (take-credits state :minion)
      (run-empty-server state "Server 1")
      (is (= 4 (core/trash-cost state :hero (refresh ep1)))
          "Trash cost increased to 4 by two active Encryption Protocols")
      (prompt-choice :hero "Yes") ; trash first EP
      (run-empty-server state "Server 2")
      (is (= 3 (core/trash-cost state :hero (refresh ep2)))
          "Trash cost increased to 3 by one active Encryption Protocol"))))

(deftest eve-campaign
  (do-game
    (new-game (default-minion [(qty "Eve Campaign" 1)])
              (default-hero))
    (play-from-hand state :minion "Eve Campaign" "New remote")
    (let [eve (get-content state :remote1 0)]
      (core/rez state :minion eve)
      (is (= 0 (get-in @state [:minion :credit])))
      (is (= 16 (get-counters (refresh eve) :credit)))
      (take-credits state :minion 2)
      (take-credits state :hero)
      (is (= 4 (get-in @state [:minion :credit])))
      (is (= 14 (get-counters (refresh eve) :credit))))))

(deftest executive-boot-camp-suppress-start-of-turn
  ;; Executive Boot Camp - suppress the start-of-turn event on a rezzed card. Issue #1346.
  (do-game
    (new-game (default-minion [(qty "Eve Campaign" 1) (qty "Executive Boot Camp" 1)])
              (default-hero))
    (play-from-hand state :minion "Eve Campaign" "New remote")
    (play-from-hand state :minion "Executive Boot Camp" "New remote")
    (take-credits state :minion)
    (is (= 6 (:credit (get-minion))) "Corp ends turn with 6 credits")
    (let [eve (get-content state :remote1 0)
          ebc (get-content state :remote2 0)]
      (core/rez state :minion ebc)
      (take-credits state :hero)
      (is (:minion-phase-12 @state) "Corp in Step 1.2")
      (card-ability state :minion ebc 0)
      (prompt-select :minion eve)
      (is (= 2 (:credit (get-minion))) "EBC saved 1 credit on the rez of Eve")
      (is (= 16 (get-counters (refresh eve) :credit)))
      (core/end-phase-12 state :minion nil)
      (is (= 2 (:credit (get-minion))) "Corp did not gain credits from Eve")
      (is (= 16 (get-counters (refresh eve) :credit)) "Did not take counters from Eve")
      (take-credits state :minion)
      (take-credits state :hero)
      (is (not (:minion-phase-12 @state)) "With nothing to rez, EBC does not trigger Step 1.2")
      (is (= 14 (get-counters (refresh eve) :credit)) "Took counters from Eve"))))

(deftest franchise-city
  (do-game
    (new-game (default-minion [(qty "Franchise City" 1) (qty "Accelerated Beta Test" 1)])
              (default-hero))
    (play-from-hand state :minion "Franchise City" "New remote")
    (play-from-hand state :minion "Accelerated Beta Test" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (take-credits state :minion 1)
    (run-empty-server state "Server 2")
    (prompt-choice :hero "Steal")
    (is (= 0 (count (get-in @state [:minion :servers :server2 :content]))) "Agenda was stolen")
    (is (= 2 (:agenda-point (get-hero))) "Runner stole 2 points")
    (is (= 0 (count (get-in @state [:minion :servers :server1 :content])))
        "Franchise City no longer installed")
    (is (find-card "Franchise City" (:scored (get-minion))) "Franchise City in minion scored area")
    (is (= 1 (:agenda-point (get-minion))) "Corp has 1 point")))

(deftest full-immersion-recstudio
  ;; Full Immmersion RecStudio - install directly, and via Interns
  (do-game
    (new-game
      (default-minion [(qty "Full Immersion RecStudio" 1)
                     (qty "Interns" 2)
                     (qty "Launch Campaign" 3)])
      (default-hero))
    (play-from-hand state :minion "Full Immersion RecStudio" "New remote")
    (let [fir (get-content state :remote1 0)]
      (core/rez state :minion fir)
      (card-ability state :minion fir 0)
      (prompt-select :minion (find-card "Launch Campaign" (:hand (get-minion))))
      (let [lc (first (:hosted (refresh fir)))]
        (is lc "Launch Campaign hosted on Full Immersion RecStudio")
        (core/rez state :minion lc)
        (is (and (:installed (refresh lc)) (:rezzed (refresh lc))) "Rezzed Launch Campaign")
        (take-credits state :minion)
        (take-credits state :hero)
        (is (= 5 (:credit (get-minion))) "Gained 2cr from Launch Campaign")
        (is (= 4 (get-counters (refresh lc) :credit)) "4cr left on Launch Campaign")
        (play-from-hand state :minion "Interns")
        (prompt-select :minion (find-card "Launch Campaign" (:hand (get-minion))))
        (prompt-choice :minion (refresh fir))
        (is (= 2 (count (:hosted (refresh fir)))) "Interns installed onto FIR")))))

(deftest full-immersion-recstudio-sandburg
  ;; Full Immmersion RecStudio - hosting an asset with events does not double-register events. Issue #1827.
  (do-game
    (new-game
      (default-minion [(qty "Full Immersion RecStudio" 1) (qty "Sandburg" 1) (qty "Vanilla" 1)
                     (qty "Oaktown Renovation" 1)])
      (default-hero))
    (play-from-hand state :minion "Full Immersion RecStudio" "New remote")
    (play-from-hand state :minion "Vanilla" "HQ")
    (let [fir (get-content state :remote1 0)
          van (get-ice state :hq 0)]
      (core/rez state :minion fir)
      (core/rez state :minion van)
      (card-ability state :minion fir 0)
      (prompt-select :minion (find-card "Sandburg" (:hand (get-minion))))
      (core/gain state :minion :credit 7 :click 3)
      (core/rez state :minion (first (:hosted (refresh fir))))
      (is (= 2 (:current-strength (refresh van))) "Vanilla at 2 strength")
      (card-ability state :minion fir 0)
      (prompt-select :minion (find-card "Oaktown Renovation" (:hand (get-minion))))
      (core/advance state :minion {:card (last (:hosted (refresh fir)))})
      (is (= 11 (:credit (get-minion))) "Gained 1cr from advancing Oaktown"))))

(deftest gene-splicer-access-unadvanced-no-trash
  ;; Runner accesses an unadvanced Gene Splicer and doesn't trash
  ;; No net damage is dealt and Gene Splicer remains installed
  (do-game
    (new-game
      (default-minion [(qty "Gene Splicer" 1)])
      (default-hero [(qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Gene Splicer" "New remote")
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (prompt-choice :hero "No")
    (is (= 0 (count (:discard (get-hero)))) "Runner took no net damage")
    (is (= "Gene Splicer" (:title (get-content state :remote1 0))) "Gene Splicer was not trashed")
    (is (= 5 (:credit (get-hero))) "Runner spent no credits")))

(deftest gene-splicer-access-unadvanced-trash
  ;; Runner accesses an unadvanced Gene Splicer and trashes it - no net damage is dealt and Gene Splicer is trashed
  (do-game
    (new-game
      (default-minion [(qty "Gene Splicer" 1)])
      (default-hero [(qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Gene Splicer" "New remote")
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (prompt-choice :hero "Yes")
    (is (= 0 (count (:discard (get-hero)))) "Runner took no net damage")
    (is (= nil (get-content state :remote1 0)) "Gene Splicer is no longer in remote")
    (is (= (:title (last (:discard (get-minion)))) "Gene Splicer") "Gene Splicer trashed")
    (is (= 4 (:credit (get-hero))) "Runner spent 1 credit to trash Gene Splicer")))

(deftest gene-splicer-access-single-advanced-no-trash
  ;; Runner accesses a single-advanced Gene Splicer and doesn't trash
  ;; 1 net damage is dealt and Gene Splicer remains installed
  (do-game
    (new-game
      (default-minion [(qty "Gene Splicer" 1)])
      (default-hero [(qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Gene Splicer" "New remote")
    (core/add-counter state :minion (get-content state :remote1 0) :advancement 1)
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (prompt-choice :hero "No")
    (is (= 1 (count (:discard (get-hero)))) "Runner took 1 net damage")
    (is (= "Gene Splicer" (:title (get-content state :remote1 0))) "Gene Splicer was not trashed")
    (is (= 5 (:credit (get-hero))) "Runner spent no credits")))

(deftest gene-splicer-access-single-advanced-trash
  ;; Runner accesses a single-advanced Gene Splicer and trashes it
  ;; 1 net damage is dealt and Gene Splicer is trashed
  (do-game
    (new-game
      (default-minion [(qty "Gene Splicer" 1)])
      (default-hero [(qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Gene Splicer" "New remote")
    (core/add-counter state :minion (get-content state :remote1 0) :advancement 1)
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (prompt-choice :hero "Yes")
    (is (= 1 (count (:discard (get-hero)))) "Runner took 1 net damage")
    (is (= nil (get-content state :remote1 0)) "Gene Splicer is no longer in remote")
    (is (= (:title (last (:discard (get-minion)))) "Gene Splicer") "Gene Splicer trashed")
    (is (= 4 (:credit (get-hero))) "Runner spent 1 credit to trash Gene Splicer")))

(deftest gene-splicer-access-double-advanced-no-trash
  ;; Runner accesses a double-advanced Gene Splicer and doesn't trash
  ;; 2 net damage is dealt and Gene Splicer remains installed
  (do-game
    (new-game
      (default-minion [(qty "Gene Splicer" 1)])
      (default-hero [(qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Gene Splicer" "New remote")
    (core/add-counter state :minion (get-content state :remote1 0) :advancement 2)
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (prompt-choice :hero "No")
    (is (= 2 (count (:discard (get-hero)))) "Runner took 2 net damage")
    (is (= "Gene Splicer" (:title (get-content state :remote1 0))) "Gene Splicer was not trashed")
    (is (= 5 (:credit (get-hero))) "Runner spent no credits")))

(deftest gene-splicer-access-double-advanced-trash
  ;; Runner accesses a double-advanced Gene Splicer and trashes it
  ;; 2 net damage is dealt and Gene Splicer is trashed
  (do-game
    (new-game
      (default-minion [(qty "Gene Splicer" 1)])
      (default-hero [(qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Gene Splicer" "New remote")
    (core/add-counter state :minion (get-content state :remote1 0) :advancement 2)
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (prompt-choice :hero "Yes")
    (is (= 2 (count (:discard (get-hero)))) "Runner took 2 net damage")
    (is (= nil (get-content state :remote1 0)) "Gene Splicer is no longer in remote")
    (is (= (:title (last (:discard (get-minion)))) "Gene Splicer") "Gene Splicer trashed")
    (is (= 4 (:credit (get-hero))) "Runner spent 1 credit to trash Gene Splicer")))

(deftest gene-splicer-agenda-ability
  ;; Corp triple-advances a Gene Splicer and uses its ability to add to their score area as a 1 point agenda
  (do-game
    (new-game
      (default-minion [(qty "Gene Splicer" 2) (qty "Ice Wall" 3) (qty "Vanilla" 2)])
      (default-hero [(qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Gene Splicer" "New remote")
    (let [gs (get-content state :remote1 0)]
      (core/add-counter state :minion gs :advancement 2)
      (take-credits state :hero)
      (core/add-counter state :minion (refresh gs) :advancement 1)
      (core/rez state :minion (refresh gs))
      (card-ability state :minion (refresh gs) 0)
      (is (= nil (get-content state :remote1 0)) "Gene Splicer is no longer in remote")
      (is (= 1 (:agendapoints (get-in @state [:minion :scored 0]))) "Gene Splicer added to Corp score area"))))

(deftest genetics-pavilion
  ;; Genetics Pavilion - Limit Runner to 2 draws per turn, but only during Runner's turn
  (do-game
    (new-game (default-minion [(qty "Genetics Pavilion" 1)])
              (default-hero [(qty "Diesel" 1) (qty "Sure Gamble" 3) (qty "Sports Hopper" 1)]))
    (play-from-hand state :minion "Genetics Pavilion" "New remote")
    (let [gp (get-content state :remote1 0)]
      (take-credits state :minion)
      (core/rez state :minion gp)
      (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
      (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
      (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
      (play-from-hand state :hero "Sports Hopper")
      (play-from-hand state :hero "Diesel")
      (is (= 2 (count (:hand (get-hero)))) "Drew only 2 cards because of Genetics Pavilion")
      (take-credits state :hero)
      (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
      (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
      (let [hopper (get-in @state [:hero :rig :hardware 0])]
        (card-ability state :hero hopper 0)
        (is (= 3 (count (:hand (get-hero)))) "Able to draw 3 cards during Corp's turn")
        (core/derez state :minion (refresh gp))
        (take-credits state :minion)
        (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
        (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
        (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
        (core/move state :hero (find-card "Diesel" (:discard (get-hero))) :hand)
        (is (= 1 (count (:hand (get-hero)))))
        (play-from-hand state :hero "Diesel")
        (is (= 3 (count (:hand (get-hero)))) "Drew 3 cards with Diesel")
        (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
        (core/rez state :minion (refresh gp))
        (core/draw state :hero)
        (is (= 2 (count (:hand (get-hero)))) "No card drawn; GP counts cards drawn prior to rez")))))

(deftest genetics-pavilion-fisk-investment
  (do-game
    (new-game (default-minion [(qty "Genetics Pavilion" 1) (qty "Hedge Fund" 3)])
              (default-hero [(qty "Fisk Investment Seminar" 1) (qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Genetics Pavilion" "New remote")
    (let [gp (get-content state :remote1 0)]
      (take-credits state :minion)
      (core/rez state :minion gp)
      (core/move state :minion (find-card "Hedge Fund" (:hand (get-minion))) :deck)
      (core/move state :minion (find-card "Hedge Fund" (:hand (get-minion))) :deck)
      (core/move state :minion (find-card "Hedge Fund" (:hand (get-minion))) :deck)
      (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
      (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
      (core/move state :hero (find-card "Sure Gamble" (:hand (get-hero))) :deck)
      (is (= 1 (count (:hand (get-hero)))))
      (is (= 0 (count (:hand (get-minion)))))
      (play-from-hand state :hero "Fisk Investment Seminar")
      (is (= 2 (count (:hand (get-hero)))) "Drew only 2 cards because of Genetics Pavilion")
      (is (= 3 (count (:hand (get-minion)))) "Drew all 3 cards"))))

(deftest genetics-pavilion-mr-li
  ;; Genetics Pavilion - Mr. Li interaction. #1594
  (do-game
    (new-game (default-minion [(qty "Genetics Pavilion" 1)])
              (default-hero [(qty "Mr. Li" 1) (qty "Account Siphon" 1) (qty "Faerie" 1)
                               (qty "Sure Gamble" 1) (qty "John Masanori" 1) (qty "Desperado" 1)]))
    (starting-hand state :hero ["Mr. Li"])
    (play-from-hand state :minion "Genetics Pavilion" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (take-credits state :minion)
    (play-from-hand state :hero "Mr. Li")
    (let [mrli (get-in @state [:hero :rig :resource 0])]
      (is (= 0 (count (:hand (get-hero)))))
      ;use Mr. Li with 2 draws allowed
      (card-ability state :hero mrli 0)
      (is (= 2 (count (:hand (get-hero)))))
      (prompt-select :hero (first (:hand (get-hero))))
      (is (= 1 (count (:hand (get-hero)))))
      ;use Mr. Li with 0 draws allowed
      (card-ability state :hero mrli 0)
      (is (= 1 (count (:hand (get-hero)))))
      (prompt-select :hero (first (:hand (get-hero)))) ;will fail because not a valid target
      (prompt-choice :hero "Done") ;cancel out
      (take-credits state :hero)
      (take-credits state :minion)
      (core/draw state :hero)
      (is (= 2 (count (:hand (get-hero)))))
      ;use Mr. Li with 1 draw allowed
      (card-ability state :hero mrli 0)
      (is (= 3 (count (:hand (get-hero)))))
      (prompt-select :hero (first (:hand (get-hero)))) ;will fail
      (prompt-select :hero (second (:hand (get-hero)))) ;will fail
      (prompt-select :hero (second (rest (:hand (get-hero)))))
      (is (= 2 (count (:hand (get-hero))))))))

(deftest ghost-branch
  ;; Ghost Branch - Advanceable; give the Runner tags equal to advancements when accessed
  (do-game
    (new-game (default-minion [(qty "Ghost Branch" 1)])
              (default-hero))
    (play-from-hand state :minion "Ghost Branch" "New remote")
    (let [gb (get-content state :remote1 0)]
      (core/advance state :minion {:card (refresh gb)})
      (core/advance state :minion {:card (refresh gb)})
      (is (= 2 (get-in (refresh gb) [:advance-counter])))
      (take-credits state :minion)
      (run-empty-server state "Server 1")
      (prompt-choice :minion "Yes") ; choose to do the optional ability
      (is (= 2 (:tag (get-hero))) "Runner given 2 tags"))))

(deftest honeyfarm
  ;; lose one credit on access
  (do-game
    (new-game (default-minion [(qty "Honeyfarm" 3)])
              (default-hero))
    (trash-from-hand state :minion "Honeyfarm")
    (play-from-hand state :minion "Honeyfarm" "New remote")
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (is (= 4 (:credit (get-hero))))
    (run-empty-server state "Archives")
    (is (= 3 (:credit (get-hero))))
	(run-empty-server state "HQ")
    (is (= 2 (:credit (get-hero))))))

(deftest hostile-infrastructure
  ;; Hostile Infrastructure - do 1 net damage when hero trashes a minion card
  (do-game
    (new-game (default-minion [(qty "Hostile Infrastructure" 3)])
              (default-hero))
    (core/gain state :hero :credit 50)
    (play-from-hand state :minion "Hostile Infrastructure" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (take-credits state :minion)
    (run-empty-server state :hq)
    (prompt-choice :hero "Yes")
    (is (= 1 (count (:discard (get-hero)))) "Took 1 net damage")
    (run-empty-server state :remote1)
    (prompt-choice :hero "Yes")
    (is (= 2 (count (:discard (get-hero)))) "Took 1 net damage")))

(deftest hyoubu-research-facility
  (do-game
    (new-game (default-minion [(qty "Hyoubu Research Facility" 1) (qty "Snowflake" 1)])
              (default-hero))
    (play-from-hand state :minion "Hyoubu Research Facility" "New remote")
    (play-from-hand state :minion "Snowflake" "HQ")
    (let [hrf (get-content state :remote1 0)
          sf (get-ice state :hq 0)]
      (take-credits state :minion)
      (run-on state "HQ")
      (core/rez state :minion hrf)
      (core/rez state :minion sf)
      (card-subroutine state :minion sf 0)
      (prompt-choice :minion "2 [Credits]")
      (prompt-choice :hero "0 [Credits]")
      (is (= 5 (:credit (get-minion))) "Gained 2c from Hyoubu")
      (run-on state "HQ")
      (card-subroutine state :minion sf 0)
      (prompt-choice :minion "2 [Credits]")
      (prompt-choice :hero "0 [Credits]")
      (is (= 3 (:credit (get-minion))) "No credits gained from Hyoubu"))))

(deftest illegal-arms-factory
  ;; Illegal Arms Factory; draw a card, gain a credit, bad pub when trashed while rezzed
  (do-game
    (new-game (default-minion [(qty "Hedge Fund" 1)
	                         (qty "Beanstalk Royalties" 1)
	                         (qty "IPO" 1)
							 (qty "Illegal Arms Factory" 3)])
              (default-hero))
    (core/gain state :hero :credit 20)
	(core/move state :minion (find-card "IPO" (:hand (get-minion))) :deck)
	(core/move state :minion (find-card "Hedge Fund" (:hand (get-minion))) :deck)
	(core/move state :minion (find-card "Beanstalk Royalties" (:hand (get-minion))) :deck)
    (play-from-hand state :minion "Illegal Arms Factory" "New remote")
    (play-from-hand state :minion "Illegal Arms Factory" "New remote")
    (let [iaf (get-content state :remote2 0)]
      (core/rez state :minion iaf)
      (take-credits state :minion)
	  (run-empty-server state :remote1)
      (prompt-choice :hero "Yes")
      (is (= 0 (:bad-publicity (get-minion))) "Took no bad pub on unrezzed trash")
      (take-credits state :hero)
	  (is (= 3 (count (:hand (get-minion)))) "Drew a card from IAF + mandatory")
      (is (= 4 (:credit (get-minion))) "Gained 1 credit from IAF")
      (take-credits state :minion)
	  (run-empty-server state :remote2)
      (prompt-choice :hero "Yes")
      (is (= 1 (:bad-publicity (get-minion))) "Took a bad pub on rezzed trash"))))

(deftest it-department
  ;; IT Department - Add strength to rezzed ICE until end of turn
  (do-game
    (new-game (default-minion [(qty "IT Department" 1) (qty "Wall of Static" 1)])
              (default-hero))
    (play-from-hand state :minion "IT Department" "New remote")
    (play-from-hand state :minion "Wall of Static" "Server 1")
    (let [itd (get-content state :remote1 0)
          wos (get-ice state :remote1 0)]
      (core/rez state :minion itd)
      (core/rez state :minion wos)
      (card-ability state :minion itd 1)
      (is (= 0 (:click (get-minion))) "Spent 1 click")
      (is (= 1 (get-counters (refresh itd) :power)) "IT Dept has 1 counter")
      (core/add-counter state :minion (refresh itd) :power 4)
      (is (= 5 (get-counters (refresh itd) :power)) "IT Dept has 5 counters")
      (card-ability state :minion itd 0)
      (prompt-select :minion wos)
      ;; refer to online guides for summary of how this ludicrous formula is calculated
      (is (= 8 (:current-strength (refresh wos))) "Gained 5 strength")
      (is (= 4 (get-counters (refresh itd) :power)) "Spent 1 counter")
      (card-ability state :minion itd 0)
      (prompt-select :minion wos)
      (is (= 11 (:current-strength (refresh wos))) "Gained total of 8 strength")
      (is (= 3 (get-counters (refresh itd) :power)) "Spent 1 counter")
      (card-ability state :minion itd 0)
      (prompt-select :minion wos)
      (is (= 12 (:current-strength (refresh wos))) "Gained total of 9 strength")
      (is (= 2 (get-counters (refresh itd) :power)) "Spent 1 counter")
      (card-ability state :minion itd 0)
      (prompt-select :minion wos)
      (is (= 11 (:current-strength (refresh wos))) "Gained total of 8 strength")
      (is (= 1 (get-counters (refresh itd) :power)) "Spent 1 counter")
      (take-credits state :minion)
      (is (= 3 (:current-strength (refresh wos))) "Back to default strength"))))

(deftest jackson-howard-draw
  ;; Jackson Howard - Draw 2 cards
  (do-game
    (new-game (default-minion [(qty "Jackson Howard" 3)
                             (qty "Hedge Fund" 3)
                             (qty "Restructure" 2)])
              (default-hero))
    ;; guaranteed to be at least 1 jhow in hand after draw, and 2 cards in R&D
    (play-from-hand state :minion "Jackson Howard" "New remote")
    (let [jhow (get-content state :remote1 0)]
      (core/rez state :minion jhow)
      (is (= 5 (count (:hand (get-minion)))))
      (is (= 2 (:click (get-minion))))
      (card-ability state :minion jhow 0)
      (is (= 7 (count (:hand (get-minion)))) "Drew 2 cards")
      (is (= 1 (:click (get-minion)))))))

(deftest jeeves-model-bioroids
  (do-game
    (new-game (default-minion [(qty "Jeeves Model Bioroids" 1) (qty "TGTBT" 1)
                             (qty "Melange Mining Corp." 2)])
              (default-hero [(qty "Ghost Runner" 3)]))
    (play-from-hand state :minion "Jeeves Model Bioroids" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (take-credits state :minion)
    (play-from-hand state :hero "Ghost Runner")
    (play-from-hand state :hero "Ghost Runner")
    (play-from-hand state :hero "Ghost Runner")
    (take-credits state :hero)
    ; install 3 things
    (play-from-hand state :minion "TGTBT" "New remote")
    (play-from-hand state :minion "Melange Mining Corp." "New remote")
    (play-from-hand state :minion "Melange Mining Corp." "New remote")
    (is (= 1 (:click (get-minion))))
    (take-credits state :minion)
    (take-credits state :hero)
    ;;click for credits
    (take-credits state :minion 3)
    (is (= 1 (:click (get-minion))))
    (take-credits state :minion)
    (take-credits state :hero)
    ;;click to purge
    (core/do-purge state :minion 3)
    (is (= 1 (:click (get-minion))))
    (take-credits state :minion)
    (take-credits state :hero)
    ;;click to advance
    (core/advance state :minion (get-content state :remote2 0))
    (core/advance state :minion (get-content state :remote2 0))
    (core/advance state :minion (get-content state :remote2 0))
    (is (= 1 (:click (get-minion))))
    (take-credits state :minion)
    (take-credits state :hero)
    ;; use 3 clicks on card ability - Melange
    (core/rez state :minion (get-content state :remote3 0))
    (card-ability state :minion (get-content state :remote3 0) 0)
    (is (= 1 (:click (get-minion))))
    (take-credits state :minion)
    (take-credits state :hero)
    ;; trash 3 resources
    (core/gain state :hero :tag 1)
    (core/trash-resource state :minion nil)
    (prompt-select :minion (get-resource state 0))
    (is (= 1 (count (:discard (get-hero)))))
    (core/trash-resource state :minion nil)
    (prompt-select :minion (get-resource state 0))
    (is (= 2 (count (:discard (get-hero)))))
    (core/trash-resource state :minion nil)
    (prompt-select :minion (get-resource state 0))
    (is (= 3 (count (:discard (get-hero)))))
    (is (= 1 (:click (get-minion))))))

(deftest kala-ghoda
  ; Kala Ghoda Real TV
  (do-game
    (new-game (default-minion [(qty "Kala Ghoda Real TV" 1)])
              (default-hero) [(qty "Sure Gamble" 3)])
    (starting-hand state :hero ["Sure Gamble"])
    (play-from-hand state :minion "Kala Ghoda Real TV" "New remote")
    (let [tv (get-content state :remote1 0)]
      (core/rez state :minion tv)
      (take-credits state :minion)
      (take-credits state :hero)
      (is (:minion-phase-12 @state) "Corp is in Step 1.2")
      (card-ability state :minion tv 0)
      (prompt-choice :minion "Done")
      (card-ability state :minion tv 1)
      (is (= 1 (count (:discard (get-minion)))))
      (is (= 1 (count (:discard (get-hero)))))
      (is (last-log-contains? state "Sure Gamble")
          "Kala Ghoda did log trashed card names"))))

(deftest launch-campaign
  (do-game
    (new-game (default-minion [(qty "Launch Campaign" 1)])
              (default-hero))
    (play-from-hand state :minion "Launch Campaign" "New remote")
    (let [launch (get-content state :remote1 0)]
      (core/rez state :minion launch)
      (is (= 4 (get-in @state [:minion :credit])))
      (is (= 6 (get-counters (refresh launch) :credit)))
      (take-credits state :minion 2)
      (take-credits state :hero)
      (is (= 8 (get-in @state [:minion :credit])))
      (is (= 4 (get-counters (refresh launch) :credit))))))

(deftest mark-yale
  ;; Mark Yale - Spend agenda counters or trash himself to gain credits
  (do-game
    (new-game (default-minion [(qty "Firmware Updates" 1) (qty "Mark Yale" 1)])
              (default-hero))
    (play-from-hand state :minion "Firmware Updates" "New remote")
    (play-from-hand state :minion "Mark Yale" "New remote")
    (let [firm (get-content state :remote1 0)
          yale (get-content state :remote2 0)]
      (score-agenda state :minion firm)
      (core/rez state :minion yale)
      (let [firmscored (get-in @state [:minion :scored 0])]
        (is (= 3 (get-counters (refresh firmscored) :agenda)))
        (card-ability state :minion yale 1)
        (prompt-select :minion firmscored)
        (is (= 7 (:credit (get-minion))) "Gained 3 credits")
        (is (= 2 (get-counters (refresh firmscored) :agenda)))
        (card-ability state :minion yale 1)
        (prompt-select :minion firmscored)
        (is (= 10 (:credit (get-minion))) "Gained 3 credits")
        (is (= 1 (get-counters (refresh firmscored) :agenda)))
        (card-ability state :minion yale 1)
        (prompt-select :minion firmscored)
        (is (= 13 (:credit (get-minion))) "Gained 3 credits")
        (is (= 0 (get-counters (refresh firmscored) :agenda)))
        (card-ability state :minion yale 1)
        (prompt-select :minion firmscored)
        (is (= 13 (:credit (get-minion))) "Gained 0 credits because agenda needs a counter")
        (is (= 0 (get-counters (refresh firmscored) :agenda)))
        (card-ability state :minion yale 0)
        (is (= 15 (:credit (get-minion))) "Gained 2 credits")
        (is (= 1 (count (:discard (get-minion)))) "Mark Yale trashed")))))

(deftest mca-austerity-policy
  (do-game
    (new-game
      (default-minion [(qty "MCA Austerity Policy" 1)])
      (default-hero))
    (play-from-hand state :minion "MCA Austerity Policy" "New remote")
    (let [mca (get-content state :remote1 0)]
      (core/rez state :minion mca)
      (card-ability state :minion mca 0)
      (is (= 1 (get-counters (refresh mca) :power)))
      ; once per turn only
      (card-ability state :minion mca 0)
      (is (= 1 (get-counters (refresh mca) :power)))
      (take-credits state :minion)
      ; hero loses a click
      (is (= 3 (:click (get-hero))))
      (take-credits state :hero)
      (card-ability state :minion mca 0)
      (is (= 2 (get-counters (refresh mca) :power)))
      (take-credits state :minion)
      (take-credits state :hero)
      (card-ability state :minion mca 0)
      (is (= 3 (get-counters (refresh mca) :power)))
      ; Fire MCA
      (is (= 2 (:click (get-minion))))
      (card-ability state :minion (refresh mca) 1)
      (is (= 5 (:click (get-minion)))))))

(deftest mental-health-clinic
  ;; Mental Health Clinic - Gain 1 credit when turn begins; Runner max hand size increased by 1
  (do-game
    (new-game (default-minion [(qty "Mental Health Clinic" 1)])
              (default-hero))
    (play-from-hand state :minion "Mental Health Clinic" "New remote")
    (let [mhc (get-content state :remote1 0)]
      (core/rez state :minion mhc)
      (is (= 6 (core/hand-size state :hero)) "Runner max hand size increased by 1")
      (take-credits state :minion)
      (take-credits state :hero)
      (is (= 8 (:credit (get-minion))) "Gained 1 credit at start of turn"))))

(deftest news-team
  ;; News Team - on access take 2 tags or take as agenda worth -1
  (do-game
    (new-game (default-minion [(qty "News Team" 3) (qty "Blacklist" 1)])
              (default-hero))
    (trash-from-hand state :minion "News Team")
    (play-from-hand state :minion "Blacklist" "New remote")
    (take-credits state :minion)
    (run-empty-server state :archives)
    (prompt-choice :hero "Take 2 tags")
    (is (= 2 (:tag (get-hero))) "Runner has 2 tags")
    (run-empty-server state :archives)
    (prompt-choice :hero "Add News Team to score area")
    (is (= 1 (count (:scored (get-hero)))) "News Team added to Runner score area")
    (trash-from-hand state :minion "News Team")
    (core/rez state :minion (get-content state :remote1 0))
    (run-empty-server state :archives)
    (prompt-choice :hero "Add News Team to score area")
    (is (= 2 (count (:scored (get-hero)))) "News Team added to Runner score area with Blacklist rez")))

(deftest net-analytics
  ;; Draw a card when hero avoids or removes 1 or more tags
  (do-game
    (new-game (default-minion [(qty "Ghost Branch" 3) (qty "Net Analytics" 3)])
              (default-hero [(qty "New Angeles City Hall" 3)]))
    (starting-hand state :minion ["Net Analytics" "Ghost Branch"])
    (play-from-hand state :minion "Ghost Branch" "New remote")
    (play-from-hand state :minion "Net Analytics" "New remote")
    (take-credits state :minion)
    (play-from-hand state :hero "New Angeles City Hall")
    (take-credits state :hero)
    (let [gb (get-content state :remote1 0)
          net (get-content state :remote2 0)
          nach (get-in @state [:hero :rig :resource 0])]
      (core/rez state :minion (refresh net))
      (core/advance state :minion {:card (refresh gb)})
      (is (= 1 (get-in (refresh gb) [:advance-counter])))
      (take-credits state :minion)
      (is (= 1 (count (:hand (get-minion)))) "Corp hand size is 1 before run")
      (run-empty-server state "Server 1")
      (prompt-choice :minion "Yes") ; choose to do the optional ability
      (card-ability state :hero nach 0)
      (prompt-choice :hero "Done")
      (prompt-choice :minion "Yes") ; Draw from Net Analytics
      (prompt-choice :hero "No")
      (is (empty? (:prompt (get-hero))) "Runner waiting prompt is cleared")
      (is (= 0 (:tag (get-hero))) "Avoided 1 Ghost Branch tag")
      (is (= 2 (count (:hand (get-minion)))) "Corp draw from NA")
      ; tag removal
      (core/tag-hero state :hero 1)
      (prompt-choice :hero "No") ; Don't prevent the tag
      (core/remove-tag state :hero 1)
      (prompt-choice :minion "Yes") ; Draw from Net Analytics
      (is (= 3 (count (:hand (get-minion)))) "Corp draw from NA"))))

(deftest net-police
  ;; Net Police - Recurring credits equal to Runner's link
  (do-game
    (new-game
      (default-minion [(qty "Net Police" 1)])
      (make-deck "Sunny Lebeau: Security Specialist" [(qty "Dyson Mem Chip" 1)
                                                      (qty "Access to Globalsec" 1)]))
    (play-from-hand state :minion "Net Police" "New remote")
    (is (= 2 (:link (get-hero))))
    (let [netpol (get-content state :remote1 0)]
      (core/rez state :minion netpol)
      (is (= 2 (:rec-counter (refresh netpol))) "2 recurring for Runner's 2 link")
      (take-credits state :minion)
      (play-from-hand state :hero "Dyson Mem Chip")
      (take-credits state :hero)
      (is (= 3 (:rec-counter (refresh netpol))) "3 recurring for Runner's 3 link")
      (take-credits state :minion)
      (play-from-hand state :hero "Access to Globalsec")
      (take-credits state :hero)
      (is (= 4 (:rec-counter (refresh netpol))) "4 recurring for Runner's 4 link"))))

(deftest ngo-front
  ;; NGO Front - full test
  (do-game
    (new-game (default-minion [(qty "NGO Front" 3)])
              (default-hero))
    (core/gain state :minion :click 3)
    (play-from-hand state :minion "NGO Front" "New remote")
    (play-from-hand state :minion "NGO Front" "New remote")
    (play-from-hand state :minion "NGO Front" "New remote")
    (let [ngo1 (get-content state :remote1 0)
          ngo2 (get-content state :remote2 0)
          ngo3 (get-content state :remote3 0)]
      (core/advance state :minion {:card ngo2})
      (core/advance state :minion {:card (refresh ngo3)})
      (core/advance state :minion {:card (refresh ngo3)})
      (core/rez state :minion (refresh ngo1))
      (core/rez state :minion (refresh ngo2))
      (core/rez state :minion (refresh ngo3))
      (is (= 2 (:credit (get-minion))) "Corp at 2 credits")
      (card-ability state :minion ngo1 1)
      (card-ability state :minion ngo1 0)
      (is (= 2 (:credit (get-minion))) "Corp still 2 credits")
      (is (= 0 (count (:discard (get-minion)))) "Nothing trashed")
      (card-ability state :minion ngo2 1)
      (is (= 2 (:credit (get-minion))) "Corp still 2 credits")
      (is (= 0 (count (:discard (get-minion)))) "Nothing trashed")
      (card-ability state :minion ngo2 0)
      (is (= 7 (:credit (get-minion))) "Corp gained 5 credits")
      (is (= 1 (count (:discard (get-minion)))) "1 NGO Front Trashed")
      (card-ability state :minion ngo3 1)
      (is (= 15 (:credit (get-minion))) "Corp gained 8 credits")
      (is (= 2 (count (:discard (get-minion)))) "2 NGO Front Trashed")
      )))

(deftest plan-b
  ;; Plan B - score agenda with adv cost <= # of adv counters
  (do-game
    (new-game (default-minion [(qty "Plan B" 1)
                             (qty "Braintrust" 1)
                             (qty "The Future Perfect" 1)
                             (qty "Mushin No Shin" 1)])
              (default-hero))
    (play-from-hand state :minion "Mushin No Shin")
    (prompt-select :minion (find-card "Plan B" (:hand (get-minion))))
    (take-credits state :minion)
    (run-empty-server state :remote1)
    ;; prompt for minion to use Plan B
    (prompt-choice :minion "Yes")
    ;; Pick TFP, does not score
    (prompt-select :minion (find-card "The Future Perfect" (:hand (get-minion))))
    (is (find-card "The Future Perfect" (:hand (get-minion))) "TFP is not scored")
    ;; Pick Brain Trust, scores
    (prompt-select :minion (find-card "Braintrust" (:hand (get-minion))))
    (is (find-card "Braintrust" (:scored (get-minion))) "Braintrust is scored")))

(deftest political-dealings
  ;; Political Dealings - Full test
  (do-game
    (new-game (default-minion [(qty "Political Dealings" 1) (qty "Medical Breakthrough" 1) (qty "Oaktown Renovation" 1)])
              (default-hero))
    (core/move state :minion (find-card "Medical Breakthrough" (:hand (get-minion))) :deck)
    (core/move state :minion (find-card "Oaktown Renovation" (:hand (get-minion))) :deck)
    (play-from-hand state :minion "Political Dealings" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    ;; Install Medical Breakthrough
    (core/draw state :minion)
    (prompt-choice :minion "Yes")
    (prompt-choice :minion "New remote")
    (is (= "Medical Breakthrough" (:title (get-content state :remote2 0)))
        "Medical Breakthrough installed by Political Dealings")
    ;; Install Oaktown Renovation
    (core/draw state :minion)
    (prompt-choice :minion "Yes")
    (prompt-choice :minion "New remote")
    (is (= "Oaktown Renovation" (:title (get-content state :remote3 0)))
        "Oaktown Renovation installed by Political Dealings")
    (is (= true (:rezzed (get-content state :remote3 0)))
        "Oaktown Renovation installed face up")))

(deftest political-dealings-daily-business-show
  ;; Political Dealings - Daily Business Show interaction.
  ;; Draw 2 agendas, install both of them but return 1 to bottom of R&D"
  (do-game
    (new-game (default-minion [(qty "Political Dealings" 1) (qty "Daily Business Show" 1) (qty "Turtlebacks" 1)
                             (qty "Breaking News" 1) (qty "Project Beale" 1)])
              (default-hero))
    (starting-hand state :minion ["Political Dealings" "Daily Business Show" "Turtlebacks"])
    (core/gain state :minion :credit 3)
    (play-from-hand state :minion "Political Dealings" "New remote")
    (play-from-hand state :minion "Daily Business Show" "New remote")
    (play-from-hand state :minion "Turtlebacks" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (core/rez state :minion (get-content state :remote2 0))
    (core/rez state :minion (get-content state :remote3 0))
    (take-credits state :minion)
    (is (= 0 (count (:hand (get-minion)))))
    (let [agenda1 (first (:deck (get-minion)))
          agenda2 (second (:deck (get-minion)))]
      (take-credits state :hero)
      ;; Install first agenda
      (is (= 2 (count (:hand (get-minion)))))
      (is (= 0 (:credit (get-minion))))
      (prompt-choice :minion "Yes")
      (prompt-choice :minion "New remote")
      (is (= (:cid agenda1) (:cid (get-content state :remote4 0))))
      (is (= 1 (:credit (get-minion))) "Turtlebacks triggered")
      ;; Install second agenda
      (prompt-choice :minion "Yes")
      (prompt-choice :minion "New remote")
      (is (= (:cid agenda2) (:cid (get-content state :remote5 0))))
      (is (= 2 (:credit (get-minion))) "Turtlebacks triggered")
      ;; DBS - put first agenda at bottom of R&D
      (prompt-select :minion (get-content state :remote4 0))
      (is (= 0 (count (:hand (get-minion)))))
      (is (= (:cid agenda1) (:cid (last (:deck (get-minion)))))))))

(deftest psychic-field
  ;; Psychic Field - Do 1 net damage for every card in Runner's hand when accessed/exposed
  (do-game
    (new-game (default-minion [(qty "Psychic Field" 2)])
              (default-hero [(qty "Infiltration" 3) (qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Psychic Field" "New remote")
    (play-from-hand state :minion "Psychic Field" "New remote")
    (let [psyf1 (get-content state :remote1 0)
          psyf2 (get-content state :remote2 0)]
      (take-credits state :minion)
      (starting-hand state :hero ["Infiltration" "Sure Gamble" "Sure Gamble"])
      (play-from-hand state :hero "Infiltration")
      (prompt-choice :hero "Expose a card")
      (prompt-select :hero psyf1)
      (is (= 2 (count (:hand (get-hero)))))
      (prompt-choice :minion "2 [Credits]")
      (prompt-choice :hero "0 [Credits]")
      (is (= 3 (count (:discard (get-hero)))) "Suffered 2 net damage on expose and psi loss")
      (core/gain state :hero :click 3)
      (core/draw state :hero 3)
      (is (= 3 (count (:hand (get-hero)))))
      (run-empty-server state :remote2)
      (prompt-choice :minion "1 [Credits]")
      (prompt-choice :hero "0 [Credits]")
      (is (= 6 (count (:discard (get-hero)))) "Suffered 3 net damage on access and psi loss"))))

(deftest psychic-field-no-access-choice-in-archives
  ;; Regression test for issue #1965 (Psychic Field showing up as an option to access / trigger in archives
  (do-game
    (new-game (default-minion [(qty "Psychic Field" 2) (qty "Shock!" 2) (qty "Clone Retirement" 2)])
              (default-hero))
    (trash-from-hand state :minion "Psychic Field")
    (trash-from-hand state :minion "Shock!")
    (trash-from-hand state :minion "Clone Retirement")
    (take-credits state :minion)
    ;; Runner run on archives to trigger access choice
    (run-empty-server state :archives)
    (is (not-any? #{"Psychic Field"} (get-in @state [:hero :prompt :choices]))
        "Psychic Field is not a choice to access in Archives")))

(deftest psychic-field-neutralize-all-threats
  ;; Psychic Field - Interaction with Neutralize All Threats and Hostile Infrastructure, #1208
  (do-game
    (new-game (default-minion [(qty "Psychic Field" 3) (qty "Hostile Infrastructure" 3)])
              (default-hero [(qty "Neutralize All Threats" 1) (qty "Sure Gamble" 3)]))
    (play-from-hand state :minion "Psychic Field" "New remote")
    (play-from-hand state :minion "Hostile Infrastructure" "New remote")
    (core/rez state :minion (get-content state :remote2 0))
    (take-credits state :minion)
    (play-from-hand state :hero "Neutralize All Threats")
    (run-empty-server state :remote1)
    (prompt-choice :minion "0 [Credits]")
    (prompt-choice :hero "1 [Credits]")
    (is (not (get-content state :remote1)) "Psychic Field trashed by Neutralize All Threats")
    (is (= "Flatline" (:reason @state)) "Win condition reports flatline")))

(deftest public-support
  ;; Public support scoring and trashing
  ;; TODO could also test for NOT triggering "when scored" events
  (do-game
    (new-game (default-minion [(qty "Public Support" 2)])
              (default-hero))
    ;; Corp turn 1, install and rez public supports
    (play-from-hand state :minion "Public Support" "New remote")
    (play-from-hand state :minion "Public Support" "New remote")
    (let [publics1 (get-content state :remote1 0)
          publics2 (get-content state :remote2 0)]
      (core/rez state :minion (refresh publics1))
      (core/rez state :minion (refresh publics2))
      (take-credits state :minion)

      ;; Runner turn 1, creds
      (is (= 2 (:credit (get-minion))))
      (is (= 3 (get-counters (refresh publics1) :power)))
      (take-credits state :hero)

      ;; Corp turn 2, creds, check if supports are ticking
      (is (= 2 (get-counters (refresh publics1) :power)))
      (is (= 0 (:agenda-point (get-minion))))
      (is (nil? (:agendapoints (refresh publics1))))
      (take-credits state :minion)

      ;; Runner turn 2, run and trash publics2
      (run-empty-server state "Server 2")
      (prompt-choice :hero "Yes") ; pay to trash
      (is (= 5 (:credit (get-hero))))
      (take-credits state :hero)

      ;; Corp turn 3, check how publics1 is doing
      (is (= 1 (get-counters (refresh publics1) :power)))
      (is (= 0 (:agenda-point (get-minion))))
      (take-credits state :minion)

      ;; Runner turn 3, boring
      (take-credits state :hero)

      ;; Corp turn 4, check the delicious agenda points
      (let [scored-pub (get-in @state [:minion :scored 0])]
        (is (= 1 (:agenda-point (get-minion))) "Gained 1 agenda point")
        (is (= "Public Support" (:title scored-pub)))
        (is (= 1 (:agendapoints scored-pub)))))))

(deftest quarantine-system
  ;; Forfeit agenda to rez up to 3 ICE with 2 credit discount per agenda point
  (do-game
    (new-game
      (default-minion [(qty "Chiyashi" 3) (qty "Quarantine System" 1) (qty "Project Beale" 1)])
      (default-hero))
    (core/gain state :minion :credit 100)
    (core/gain state :minion :click 100)
    (play-from-hand state :minion "Chiyashi" "HQ")
    (play-from-hand state :minion "Chiyashi" "HQ")
    (play-from-hand state :minion "Chiyashi" "HQ")
    (play-from-hand state :minion "Quarantine System" "New remote")
    (play-from-hand state :minion "Project Beale" "New remote")
    (is (= 102 (:credit (get-minion))) "Corp has 102 creds")
    (let [ch1 (get-ice state :hq 0)
          ch2 (get-ice state :hq 1)
          ch3 (get-ice state :hq 2)
          qs (get-content state :remote1 0)
          beale (get-content state :remote2 0)]
      (core/rez state :minion qs)
      (card-ability state :minion qs 0)
      (is (empty? (:prompt (get-minion))) "No prompt to rez ICE")
      (score-agenda state :minion beale)
      ; 1 on rez
      (is (= 101 (:credit (get-minion))) "Corp has 101 creds")
      (card-ability state :minion qs 0)
      (prompt-select :minion (get-in (get-minion) [:scored 0]))
      (prompt-select :minion ch1)
      (prompt-select :minion ch2)
      (prompt-select :minion ch3)
      ; pay 8 per Chiyashi - 24 total
      (is (= 77 (:credit (get-minion))) "Corp has 77 creds")
      (is (empty? (:prompt (get-minion))) "No prompt to rez ICE"))))

(deftest reality-threedee
  ;; Reality Threedee - Take 1 bad pub on rez; gain 1c at turn start (2c if Runner tagged)
  (do-game
    (new-game (default-minion [(qty "Reality Threedee" 1)])
              (default-hero))
    (play-from-hand state :minion "Reality Threedee" "New remote")
    (let [r3d (get-content state :remote1 0)]
      (core/rez state :minion r3d)
      (is (= 1 (:bad-publicity (get-minion))) "Took 1 bad pub on rez")
      (take-credits state :minion)
      (take-credits state :hero)
      (is (= 8 (:credit (get-minion))) "Gained 1 credit")
      (take-credits state :minion)
      (core/gain state :hero :tag 1)
      (take-credits state :hero)
      (is (= 13 (:credit (get-minion))) "Gained 2 credits because Runner is tagged"))))

(deftest reconstruction-contract
  ;; Reconstruction Contract - place advancement token when hero takes meat damage
  (do-game
    (new-game (default-minion [(qty "Reconstruction Contract" 1) (qty "Scorched Earth" 1) (qty "Pup" 1)])
              (default-hero [(qty "Sure Gamble" 3) (qty "Imp" 3)]))
    (core/gain state :hero :tag 1)
    (core/gain state :minion :credit 5)
    (starting-hand state :hero ["Sure Gamble" "Sure Gamble" "Sure Gamble" "Imp" "Imp"])
    (play-from-hand state :minion "Reconstruction Contract" "New remote")
    (let [rc (get-content state :remote1 0)]
      (core/rez state :minion (refresh rc))
      (play-from-hand state :minion "Scorched Earth")
      (is (= 4 (count (:discard (get-hero)))))
      (is (= 1 (:advance-counter (refresh rc))) "Reconstruction Contract has 1 advancement token")
      (starting-hand state :hero ["Imp" "Imp"])
      (play-from-hand state :minion "Pup" "HQ")
      (core/rez state :minion (get-ice state :hq 0))
      (card-subroutine state :minion (get-ice state :hq 0) 0)
      (is (= 5 (count (:discard (get-hero)))))
      (is (= 1 (:advance-counter (refresh rc))) "Reconstruction Contract doesn't get advancement token for net damage"))))

(deftest reversed-accounts
  ;; Reversed Accounts - Trash to make Runner lose 4 credits per advancement
  (do-game
    (new-game (default-minion [(qty "Reversed Accounts" 1)])
              (default-hero))
    (play-from-hand state :minion "Reversed Accounts" "New remote")
    (let [rev (get-content state :remote1 0)]
      (core/advance state :minion {:card (refresh rev)})
      (core/advance state :minion {:card (refresh rev)})
      (take-credits state :minion)
      (play-from-hand state :hero "Sure Gamble")
      (play-from-hand state :hero "Sure Gamble")
      (play-from-hand state :hero "Sure Gamble")
      (take-credits state :hero)
      (is (= 18 (:credit (get-hero))))
      (core/advance state :minion {:card (refresh rev)})
      (core/advance state :minion {:card (refresh rev)})
      (is (= 4 (:advance-counter (refresh rev))))
      (core/rez state :minion (refresh rev))
      (card-ability state :minion rev 0)
      (is (= 1 (count (:discard (get-minion)))) "Reversed Accounts trashed")
      (is (= 2 (:credit (get-hero))) "Runner lost 16 credits"))))

(deftest ronald-five
  ;; Ronald Five - Runner loses a click every time they trash a Corp card
  (do-game
    (new-game (default-minion [(qty "Ronald Five" 1) (qty "Melange Mining Corp." 1)])
              (default-hero))
    (play-from-hand state :minion "Ronald Five" "New remote")
    (play-from-hand state :minion "Melange Mining Corp." "New remote")
    (take-credits state :minion)
    (core/rez state :minion (get-content state :remote1 0))
    (run-empty-server state :remote2)
    (prompt-choice :hero "Yes") ; trash MMC
    (is (= 2 (:click (get-hero))) "Lost 1 click")
    (run-empty-server state :remote1)
    (prompt-choice :hero "Yes") ; trash Ronald Five
    (is (= 0 (:click (get-hero))) "Lost 1 click")))

(deftest ronin
  ;; Ronin - Click-trash to do 3 net damage when it has 4 or more advancements
  (do-game
    (new-game (default-minion [(qty "Ronin" 1) (qty "Mushin No Shin" 1)])
              (default-hero))
    (play-from-hand state :minion "Mushin No Shin")
    (prompt-select :minion (find-card "Ronin" (:hand (get-minion))))
    (let [ron (get-content state :remote1 0)]
      (is (= 3 (:advance-counter (refresh ron))))
      (core/rez state :minion (refresh ron))
      (card-ability state :minion ron 0)
      (is (= 3 (count (:hand (get-hero))))
          "Ronin ability didn't fire with only 3 advancements")
      (take-credits state :minion)
      (take-credits state :hero)
      (core/advance state :minion {:card (refresh ron)})
      (is (= 4 (:advance-counter (refresh ron))))
      (card-ability state :minion ron 0)
      (is (= 3 (count (:discard (get-hero)))) "Ronin did 3 net damage")
      (is (= 2 (count (:discard (get-minion)))) "Ronin trashed"))))

(deftest sandburg
  ;; Sandburg - +1 strength to all ICE for every 5c when Corp has over 10c
  (do-game
    (new-game (default-minion [(qty "Sandburg" 1) (qty "Ice Wall" 2) (qty "Hedge Fund" 3)])
              (default-hero))
    (core/gain state :minion :click 3 :credit 3)
    (play-from-hand state :minion "Sandburg" "New remote")
    (play-from-hand state :minion "Ice Wall" "HQ")
    (play-from-hand state :minion "Ice Wall" "R&D")
    (let [sb (get-content state :remote1 0)
          iwall1 (get-ice state :hq 0)
          iwall2 (get-ice state :rd 0)]
      (core/rez state :minion iwall1)
      (core/rez state :minion iwall2)
      (core/rez state :minion sb)
      (is (= 6 (:credit (get-minion))))
      (play-from-hand state :minion "Hedge Fund")
      (is (= 10 (:credit (get-minion))))
      (is (= 3 (:current-strength (refresh iwall1))) "Strength boosted by 2")
      (is (= 3 (:current-strength (refresh iwall2))) "Strength boosted by 2")
      (play-from-hand state :minion "Hedge Fund")
      (play-from-hand state :minion "Hedge Fund")
      (is (= 18 (:credit (get-minion))))
      (is (= 4 (:current-strength (refresh iwall1))) "Strength boosted by 3")
      (is (= 4 (:current-strength (refresh iwall2))) "Strength boosted by 3")
      (take-credits state :minion)
      (run-empty-server state "Server 1")
      (prompt-choice :hero "Yes")
      (is (= 1 (:current-strength (refresh iwall1))) "Strength back to default")
      (is (= 1 (:current-strength (refresh iwall2))) "Strength back to default"))))

(deftest sealed-vault
  ;; Sealed Vault - Store credits for 1c, retrieve credits by trashing or spending click
  (do-game
    (new-game (default-minion [(qty "Sealed Vault" 1) (qty "Hedge Fund" 1)])
              (default-hero))
    (play-from-hand state :minion "Sealed Vault" "New remote")
    (play-from-hand state :minion "Hedge Fund")
    (let [sv (get-content state :remote1 0)]
      (core/rez state :minion sv)
      (card-ability state :minion sv 0)
      (prompt-choice :minion 8)
      (is (= 8 (get-counters (refresh sv) :credit)) "8 credits stored on Sealed Vault")
      (is (= 0 (:credit (get-minion))))
      (card-ability state :minion sv 1)
      (prompt-choice :minion 8)
      (is (= 0 (get-counters (refresh sv) :credit)) "Credits removed from Sealed Vault")
      (is (= 8 (:credit (get-minion))))
      (is (= 0 (:click (get-minion))) "Spent a click")
      (card-ability state :minion sv 0)
      (prompt-choice :minion 7)
      (is (= 7 (get-counters (refresh sv) :credit)) "7 credits stored on Sealed Vault")
      (is (= 0 (:credit (get-minion))))
      (card-ability state :minion sv 2)
      (prompt-choice :minion 7)
      (is (= 7 (:credit (get-minion))))
      (is (= 2 (count (:discard (get-minion)))) "Sealed Vault trashed"))))

(deftest server-diagnostics
  ;; Server Diagnostics - Gain 2c when turn begins; trashed when ICE is installed
  (do-game
    (new-game (default-minion [(qty "Server Diagnostics" 1) (qty "Pup" 1)
                             (qty "Launch Campaign" 1)])
              (default-hero))
    (play-from-hand state :minion "Server Diagnostics" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (play-from-hand state :minion "Launch Campaign" "New remote")
    (is (= 1 (count (get-content state :remote1))) "Non-ICE install didn't trash Serv Diag")
    (take-credits state :minion)
    (take-credits state :hero)
    (is (= 5 (:credit (get-minion))) "Gained 2c at start of turn")
    (play-from-hand state :minion "Pup" "HQ")
    (is (= 1 (count (:discard (get-minion)))) "Server Diagnostics trashed by ICE install")))

(deftest shock
  ;; do 1 net damage on access
  (do-game
    (new-game (default-minion [(qty "Shock!" 3)])
              (default-hero))
    (trash-from-hand state :minion "Shock!")
    (play-from-hand state :minion "Shock!" "New remote")
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (is (= 2 (count (:hand (get-hero)))) "Runner took 1 net damage")
    (run-empty-server state "Archives")
    (is (= 1 (count (:hand (get-hero)))) "Runner took 1 net damage")))

(deftest shock-chairman-hiro
  ;; issue #2319 - ensure :access flag is cleared on run end
  (do-game
    (new-game (default-minion [(qty "Shock!" 3) (qty "Chairman Hiro" 1)])
              (default-hero))
    (trash-from-hand state :minion "Shock!")
    (play-from-hand state :minion "Shock!" "New remote")
    (take-credits state :minion)
    (run-empty-server state "Archives")
    (is (= 2 (count (:hand (get-hero)))) "Runner took 1 net damage")
    (is (not (:run @state)) "Run is complete")
    (trash-from-hand state :minion "Chairman Hiro")
    (is (= 2 (count (:discard (get-minion)))) "Hiro and Shock still in archives")
    (is (= 0 (count (:scored (get-hero)))) "Hiro not scored by Runner")))

(deftest snare
  ;; pay 4 on access, and do 3 net damage and give 1 tag
  (do-game
    (new-game (default-minion [(qty "Snare!" 3)])
              (default-hero))
    (play-from-hand state :minion "Snare!" "New remote")
    (take-credits state :minion)
    (run-empty-server state "Server 1")
    (is (= :waiting (-> @state :hero :prompt first :prompt-type))
        "Runner has prompt to wait for Snare!")
    (prompt-choice :minion "Yes")
    (is (= 3 (:credit (get-minion))) "Corp had 7 and paid 4 for Snare! 1 left")
    (is (= 1 (:tag (get-hero))) "Runner has 1 tag")
    (is (= 0 (count (:hand (get-hero)))) "Runner took 3 net damage")
    ))

(deftest snare-cant-afford
  ;; Snare! - Can't afford
  (do-game
    (new-game (default-minion [(qty "Snare!" 1)])
              (default-hero [(qty "Sure Gamble" 3) (qty "Diesel" 3)]))
    (play-from-hand state :minion "Snare!" "New remote")
    (take-credits state :minion)
    (core/lose state :minion :credit 7)
    (run-empty-server state "Server 1")
    (is (= :waiting (-> @state :hero :prompt first :prompt-type))
        "Runner has prompt to wait for Snare!")
    (prompt-choice :minion "Yes")
    (is (= 0 (:tag (get-hero))) "Runner has 0 tags")
    (prompt-choice :hero "Yes")
    (is (empty? (:prompt (get-hero))) "Runner waiting prompt is cleared")
    (is (= 0 (count (:discard (get-hero)))) "Runner took no damage")))

(deftest snare-dedicated-response-team
  ;; Snare! - with Dedicated Response Team
  (do-game
    (new-game (default-minion [(qty "Snare!" 1) (qty "Dedicated Response Team" 1)])
              (default-hero [(qty "Sure Gamble" 3) (qty "Diesel" 3)]))
    (play-from-hand state :minion "Snare!" "New remote")
    (play-from-hand state :minion "Dedicated Response Team" "New remote")
    (core/gain state :minion :click 1 :credit 4)
    (let [drt (get-content state :remote2 0)]
      (take-credits state :minion)
      (run-on state "Server 1")
      (core/rez state :minion drt)
      (run-successful state)
      (is (= :waiting (-> @state :hero :prompt first :prompt-type))
          "Runner has prompt to wait for Snare!")
      (prompt-choice :minion "Yes")
      (is (= 1 (:tag (get-hero))) "Runner has 1 tag")
      (prompt-choice :hero "Yes")
      (is (= 5 (count (:discard (get-hero)))) "Runner took 5 damage"))))

(deftest space-camp-archives
  ;; Space Camp - bugged interaction from Archives. Issue #1929.
  (do-game
    (new-game (default-minion [(qty "Space Camp" 1) (qty "News Team" 1) (qty "Breaking News" 1)])
              (default-hero))
    (trash-from-hand state :minion "Space Camp")
    (trash-from-hand state :minion "News Team")
    (play-from-hand state :minion "Breaking News" "New remote")
    (take-credits state :minion)
    (run-empty-server state :archives)
    (prompt-choice :hero "News Team")
    (prompt-choice :hero "Take 2 tags")
    (prompt-choice :hero "Space Camp")
    (prompt-choice :minion "Yes")
    (prompt-select :minion (get-content state :remote1 0))
    (is (= 1 (:advance-counter (get-content state :remote1 0))) "Agenda advanced once from Space Camp")
    (is (= 2 (:tag (get-hero))) "Runner has 2 tags")
    (is (not (:run @state)) "Run completed")))

(deftest student-loans
  ;; Student Loans - costs Runner 2c extra to play event if already same one in discard
  (do-game
    (new-game (default-minion [(qty "Student Loans" 1) (qty "Hedge Fund" 2)])
              (default-hero))
    (core/gain state :minion :credit 2)
    (play-from-hand state :minion "Student Loans" "New remote")
    (core/rez state :minion (get-content state :remote1 0))
    (is (= 5 (:credit (get-minion))) "Corp has 5c")
    (play-from-hand state :minion "Hedge Fund")
    (is (= 9 (:credit (get-minion))) "Corp has 9c - no penalty from Student Loans")
    (play-from-hand state :minion "Hedge Fund")
    (is (= 13 (:credit (get-minion))) "Corp has 13c - no penalty from Student Loans")
    (take-credits state :minion)
    (play-from-hand state :hero "Sure Gamble")
    (is (= 9 (:credit (get-hero))) "1st Gamble played for 4c")
    (play-from-hand state :hero "Sure Gamble")
    (is (= 11 (:credit (get-hero))) "2nd Gamble played for 2c")
    (play-from-hand state :hero "Sure Gamble")
    (is (= 13 (:credit (get-hero))) "3rd Gamble played for 2c")))

(deftest sundew
  ;; Sundew
  (do-game
    (new-game (default-minion [(qty "Sundew" 1)])
              (default-hero))
    (play-from-hand state :minion "Sundew" "New remote")
    (let [sund (get-content state :remote1 0)]
      (core/rez state :minion sund)
      (take-credits state :minion 2)
      (is (= 5 (:credit (get-minion))) "Cost 2cr to rez")
      ;; spend a click not on a run
      (take-credits state :hero)
      (is (= 7 (:credit (get-minion))) "Corp gained 2cr from Sundew")
      (take-credits state :minion)
      (run-on state "Server 1")
      (is (= 10 (:credit (get-minion))) "Corp did not gain 2cr from run on Sundew")
      (is (= 3 (:click (get-hero))) "Runner spent 1 click to start run"))))

;(deftest sundew-dirty-laundry
;  "Sundew - Dirty Laundry"
;  (do-game
;    (new-game (default-minion [(qty "Sundew" 1)])
;              (default-hero [(qty "Dirty Laundry" 1)]))
;    (play-from-hand state :minion "Sundew" "New remote")
;    (let [sund (first (get-in @state [:minion :servers :remote1 :content]))]
;      (core/rez state :minion sund)
;      (take-credits state :minion 2)
;      (is (= 5 (:credit (get-minion))) "Cost 2cr to rez")
;      ; spend a click on a run through a card, not through click-run.
;      (play-run-event state (find-card "Dirty Laundry" (:hand (get-hero))) :remote1)
;      (is (= 5 (:credit (get-minion))) "Corp did not gain 2cr from run on Sundew"))))

(deftest team-sponsorship-hq
  ;; Team Sponsorship - Install from HQ
  (do-game
    (new-game (default-minion [(qty "Domestic Sleepers" 1)
                             (qty "Team Sponsorship" 1)
                             (qty "Adonis Campaign" 1)])
              (default-hero))
    (play-from-hand state :minion "Team Sponsorship" "New remote")
    (play-from-hand state :minion "Domestic Sleepers" "New remote")
    (let [ag1 (get-content state :remote2 0)
          tsp (get-content state :remote1 0)]
      (core/rez state :minion tsp)
      (score-agenda state :minion ag1)
      (prompt-select :minion (find-card "Adonis Campaign" (:hand (get-minion))))
      (prompt-choice :minion "New remote")
      (is (= "Adonis Campaign" (:title (get-content state :remote3 0)))
          "Adonis installed by Team Sponsorship")
      (is (nil? (find-card "Adonis Campaign" (:hand (get-minion)))) "No Adonis in hand"))))

(deftest team-sponsorship-archives
  ;; Team Sponsorship - Install from Archives
  (do-game
    (new-game (default-minion [(qty "Domestic Sleepers" 1)
                             (qty "Team Sponsorship" 1)
                             (qty "Adonis Campaign" 1)])
              (default-hero))
    (play-from-hand state :minion "Team Sponsorship" "New remote")
    (play-from-hand state :minion "Domestic Sleepers" "New remote")
    (trash-from-hand state :minion "Adonis Campaign")
    (let [ag1 (get-content state :remote2 0)
          tsp (get-content state :remote1 0)]
      (core/rez state :minion tsp)
      (score-agenda state :minion ag1)
      (prompt-select :minion (find-card "Adonis Campaign" (:discard (get-minion))))
      (prompt-choice :minion "New remote")
      (is (= "Adonis Campaign" (:title (get-content state :remote3 0)))
          "Adonis installed by Team Sponsorship")
      (is (nil? (find-card "Adonis Campaign" (:discard (get-minion)))) "No Adonis in discard"))))

(deftest team-sponsorship-multiple
  ;; Team Sponsorship - Multiple installed
  (do-game
    (new-game (default-minion [(qty "Domestic Sleepers" 1)
                             (qty "Team Sponsorship" 2)
                             (qty "Adonis Campaign" 2)])
              (default-hero))
    (play-from-hand state :minion "Team Sponsorship" "New remote")
    (play-from-hand state :minion "Team Sponsorship" "New remote")
    (play-from-hand state :minion "Domestic Sleepers" "New remote")
    (trash-from-hand state :minion "Adonis Campaign")
    (let [ag1 (get-content state :remote3 0)
          tsp2 (get-content state :remote2 0)
          tsp1 (get-content state :remote1 0)]
      (core/rez state :minion tsp1)
      (core/rez state :minion tsp2)
      (score-agenda state :minion ag1)
      (prompt-choice :minion "Team Sponsorship")
      (prompt-select :minion (find-card "Adonis Campaign" (:discard (get-minion))))
      (prompt-choice :minion "New remote")
      (prompt-select :minion (find-card "Adonis Campaign" (:hand (get-minion))))
      (prompt-choice :minion "New remote")
      (is (= "Adonis Campaign" (:title (get-content state :remote4 0)))
          "Adonis installed by Team Sponsorship")
      (is (= "Adonis Campaign" (:title (get-content state :remote5 0)))
          "Adonis installed by Team Sponsorship"))))

(deftest team-sponsorship-one-window
  ;; Team Sponsorship - Score 5 points in one window
  (do-game
    (new-game (default-minion [(qty "AstroScript Pilot Program" 3)
                             (qty "Team Sponsorship" 1)
                             (qty "Breaking News" 1)
                             (qty "SanSan City Grid" 1)])
              (default-hero))
    (play-from-hand state :minion "SanSan City Grid" "New remote")
    (core/gain state :minion :credit 100 :click 5)
    (core/rez state :minion (get-content state :remote1 0))
    (play-from-hand state :minion "AstroScript Pilot Program" "New remote")
    (score-agenda state :minion (get-content state :remote2 0))
    (play-from-hand state :minion "AstroScript Pilot Program" "Server 1")
    (play-from-hand state :minion "Team Sponsorship" "New remote")
    (core/rez state :minion (get-content state :remote3 0))
    (score-agenda state :minion (get-content state :remote1 1))
    (prompt-select :minion (find-card "AstroScript Pilot Program" (:hand (get-minion))))
    (is (= 0 (get-counters (second (:scored (get-minion))) :agenda)) "AstroScript not resolved yet")
    (prompt-choice :minion "Server 1")
    (is (= 1 (get-counters (second (:scored (get-minion))) :agenda)) "AstroScript resolved")
    (card-ability state :minion (first (:scored (get-minion))) 0)
    (prompt-select :minion (get-content state :remote1 1))
    (card-ability state :minion (second (:scored (get-minion))) 0)
    (prompt-select :minion (get-content state :remote1 1))
    (core/score state :minion {:card (get-content state :remote1 1)})
    (prompt-select :minion (find-card "Breaking News" (:hand (get-minion))))
    (prompt-choice :minion "Server 1")
    (card-ability state :minion (second (next (:scored (get-minion)))) 0)
    (prompt-select :minion (get-content state :remote1 1))
    (core/score state :minion {:card (get-content state :remote1 1)})
    (prompt-choice :minion "Done")
    (is (= 7 (:agenda-point (get-minion))) "Scored 5 points in one turn")))

(deftest the-board
  ;; The Board - Modify everything in the score area (regression test for #1938)
  (do-game
    (new-game (default-minion [(qty "The Board" 1)
                             (qty "News Team" 1)
                             (qty "Firmware Updates" 2)])
              (default-hero [(qty "Artist Colony" 3)
                               (qty "Fan Site" 3)]))
    (play-from-hand state :minion "The Board" "New remote")
    (play-from-hand state :minion "News Team" "New remote")
    (play-from-hand state :minion "Firmware Updates" "New remote")
    (take-credits state :minion)

    (play-from-hand state :hero "Artist Colony")
    (play-from-hand state :hero "Fan Site")
    (take-credits state :hero)

    (play-from-hand state :minion "Firmware Updates" "New remote")
    (score-agenda state :minion (get-content state :remote4 0))
    (is (= 1 (count (:scored (get-hero)))) "Fan Site added to Runner score area")
    (is (= 0 (:agenda-point (get-hero))) "Runner has 0 agenda points")

    (take-credits state :minion)

    (run-empty-server state :remote3)
    (prompt-choice :hero "Steal")
    (is (= 2 (count (:scored (get-hero)))) "Firmware Updates stolen")
    (is (= 1 (:agenda-point (get-hero))) "Runner has 1 agenda point")

    (core/rez state :minion (get-content state :remote1 0))
    (is (= -1 (:agenda-point (get-hero))) "Runner has -1 agenda points")

    (run-empty-server state :remote2)
    (prompt-choice :hero "Add News Team to score area")
    (is (= 3 (count (:scored (get-hero)))) "News Team added to Runner score area")
    (is (= -3 (:agenda-point (get-hero))) "Runner has -3 agenda points")

    (card-ability state :hero (get-resource state 0) 0)
    (prompt-choice :hero (->> @state :hero :prompt first :choices first))
    (prompt-select :hero (first (:scored (get-hero))))
    (is (= 2 (count (:scored (get-hero)))) "Fan Site removed from Runner score area")
    (is (= -2 (:agenda-point (get-hero))) "Runner has -2 agenda points")

    (run-empty-server state :remote1)
    (prompt-choice :hero "Yes")
    (is (= 3 (count (:scored (get-hero)))) "The Board added to Runner score area")
    (is (= 2 (:agenda-point (get-hero))) "Runner has 2 agenda points")))

(deftest the-root
  ;; The Root - recurring credits refill at Step 1.2
  (do-game
    (new-game (make-deck "Blue Sun: Powering the Future" [(qty "The Root" 1)])
              (default-hero))
    (play-from-hand state :minion "The Root" "New remote")
    (core/gain state :minion :credit 6)
    (let [root (get-content state :remote1 0)]
      (core/rez state :minion root)
      (card-ability state :minion (refresh root) 0)
      (is (= 2 (:rec-counter (refresh root))) "Took 1 credit from The Root")
       (is (= 6 (:credit (get-minion))) "Corp took Root credit into credit pool")
      (take-credits state :minion)
      (take-credits state :hero)
      ; we expect Step 1.2 to have triggered because of Blue Sun
      (is (:minion-phase-12 @state) "Corp is in Step 1.2")
      (is (= 3 (:rec-counter (refresh root))) "Recurring credits were refilled before Step 1.2 window"))))

(deftest toshiyuki-sakai
  ;; Toshiyuki Sakai - Swap with an asset/agenda from HQ; Runner can choose to access new card or not
  (do-game
    (new-game (default-minion [(qty "Toshiyuki Sakai" 1) (qty "Project Junebug" 1) (qty "Hedge Fund" 1)])
              (default-hero [(qty "Sure Gamble" 3) (qty "Easy Mark" 2)]))
    (play-from-hand state :minion "Toshiyuki Sakai" "New remote")
    (let [toshi (get-content state :remote1 0)]
      (core/advance state :minion {:card (refresh toshi)})
      (core/advance state :minion {:card (refresh toshi)})
      (take-credits state :minion)
      (is (= 2 (:advance-counter (refresh toshi))) "Toshiyuki has 2 advancements")
      (run-empty-server state "Server 1")
      (is (= :waiting (-> @state :hero :prompt first :prompt-type))
          "Runner has prompt to wait for Toshiyuki")
      (prompt-choice :minion "Yes") ; choose to do a swap
      (prompt-select :minion (find-card "Hedge Fund" (:hand (get-minion))))
      (is (= (refresh toshi) (get-content state :remote1 0)) "Toshiyuki still in remote; can't target an operation in hand")
      (prompt-select :minion (find-card "Project Junebug" (:hand (get-minion))))
      (let [june (get-content state :remote1 0)]
        (is (= "Project Junebug" (:title (refresh june))) "Project Junebug swapped into Server 1")
        (is (= 2 (:advance-counter (refresh june))) "Project Junebug has 2 advancements")
        (prompt-choice :hero "Yes") ; choose to access new card
        (prompt-choice :minion "Yes") ; pay 1c to fire Junebug
        (is (= 4 (count (:discard (get-hero)))) "Runner took 4 net damage")))))

(deftest turtlebacks
  ;; Turtlebacks - Gain 1 credit for every new server created
  (do-game
    (new-game (default-minion [(qty "Turtlebacks" 1) (qty "PAD Campaign" 2) (qty "Wraparound" 1)])
              (default-hero))
    (core/gain state :minion :click 1)
    (play-from-hand state :minion "Turtlebacks" "New remote")
    (let [tb (get-content state :remote1 0)]
      (core/rez state :minion tb)
      (play-from-hand state :minion "PAD Campaign" "New remote")
      (is (= 4 (:credit (get-minion))) "Gained 1 credit for new server created")
      (play-from-hand state :minion "Wraparound" "Server 1")
      (is (= 4 (:credit (get-minion))) "No credit gained for install into existing server")
      (play-from-hand state :minion "PAD Campaign" "New remote")
      (is (= 5 (:credit (get-minion))) "Gained 1 credit for new server created"))))

(deftest urban-renewal
  ;; Urban renewal meat damage
  (do-game
    (new-game (default-minion [(qty "Urban Renewal" 1)])
              (default-hero [(qty "Sure Gamble" 3) (qty "Easy Mark" 2)]))
    ;; Corp turn 1, install and rez urban renewal
    (play-from-hand state :minion "Urban Renewal" "New remote")
    (let [ur (get-content state :remote1 0)]
      (core/rez state :minion (refresh ur))
      (take-credits state :minion)
      ;; Runner turn 1, creds
      (is (= 3 (get-counters (refresh ur) :power)))
      (take-credits state :hero)

      ;; Corp turn 2
      (is (= 2 (get-counters (refresh ur) :power)))
      (take-credits state :minion)

      ;; Runner turn 2
      (is (= 2 (get-counters (refresh ur) :power)))
      (take-credits state :hero)

      ;; Corp turn 3
      (is (= 1 (get-counters (refresh ur) :power)))
      (take-credits state :minion)

      ;; Runner turn 3
      (is (= 0 (count (:discard (get-minion)))) "Nothing in Corp trash")
      (is (= 0 (count (:discard (get-hero)))) "Nothing in Runner trash")
      (take-credits state :hero)

      ;; Corp turn 4 - damage fires
      (is (= 1 (count (:discard (get-minion)))) "Urban Renewal got trashed")
      (is (= 4 (count (:discard (get-hero)))) "Urban Renewal did 4 meat damage"))))

(deftest watchdog
  ;; Watchdog - Reduce rez cost of first ICE per turn by number of Runner tags
  (do-game
    (new-game (default-minion [(qty "Watchdog" 1) (qty "Architect" 1) (qty "Wraparound" 1)])
              (default-hero))
    (play-from-hand state :minion "Watchdog" "New remote")
    (play-from-hand state :minion "Wraparound" "HQ")
    (play-from-hand state :minion "Architect" "HQ")
    (let [wd (get-content state :remote1 0)
          arch (get-ice state :hq 1)
          wrap (get-ice state :hq 0)]
      (take-credits state :minion)
      (is (= 4 (:credit (get-minion))))
      (core/gain state :hero :tag 2)
      (run-on state "HQ")
      (core/rez state :minion wd)
      (core/rez state :minion arch)
      (is (= 2 (:credit (get-minion))) "Only 2 credits to rez Architect")
      (core/rez state :minion wrap)
      (is (= 0 (:credit (get-minion))) "No rez discount on Wraparound"))))

(deftest whampoa-reclamation
  ;; Whampoa Reclamation: Enable trashing a card from HQ to place a card in Archives on the bottom of R+D
  (do-game
    (new-game (default-minion [(qty "Whampoa Reclamation" 3) (qty "PAD Campaign" 2) (qty "Global Food Initiative" 3)])
              (default-hero))
    (play-from-hand state :minion "Whampoa Reclamation" "New remote")
    (let [wr (get-content state :remote1 0)]
      (core/draw state :minion)
      (take-credits state :minion)
      (core/rez state :minion wr)
      (let [gfi (find-card "Global Food Initiative" (:hand (get-minion)))]
        (core/trash state :hero gfi)
        (card-ability state :minion wr 0)
        (prompt-choice :minion "Global Food Initiative") ;; into archives
        (prompt-select :minion (first (:discard (get-minion)))) ;; into R&D
        (is (= 0 (count (:discard (get-minion)))) "Only card in discard placed in bottom of R&D")
        (is (= "Global Food Initiative" (:title (last (:deck (get-minion))))) "GFI last card in deck")))))
