...
(deftest au-revoir
  ...
  (do-game
    ...
    (play-from-hand state :hazPlayer "Au Revoir")
...
(deftest crescentus
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Crescentus")
    (run-on state "HQ")
    (let [cres (get-in @state [:hazPlayer :rig :program 0])
          q (get-ice state :hq 0)]
      (card-ability state :hazPlayer cres 0)
      (is (not (nil? (get-in @state [:hazPlayer :rig :program 0]))) "Crescentus could not be used because the ICE is not rezzed")
      (core/rez state :resPlayer q)
      (is (get-in (refresh q) [:rezzed]) "Quandary is now rezzed")
      (card-ability state :hazPlayer cres 0)
      (is (nil? (get-in @state [:hazPlayer :rig :program 0])) "Crescentus could be used because the ICE is rezzed")
...
(deftest datasucker
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Fire Wall" "New remote")
    (take-credits state :resPlayer)
    ...
    (let [ds (get-in @state [:hazPlayer :rig :program 0])
      ...
      (card-ability state :hazPlayer ds 0)
...
(deftest datasucker-trashed
  ...
  (do-game
    ...
    (let [sucker (get-program state 0)
      ...
      (core/add-counter state :hazPlayer sucker :virus 2)
      ...
      (run-on state "HQ")
      (run-continue state)
      (card-ability state :hazPlayer (refresh sucker) 0)
      (card-ability state :hazPlayer (refresh sucker) 0)
...
(deftest diwan
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Diwan")
    ...
    (play-from-hand state :resPlayer "Fire Wall" "HQ") ; 2cr cost from normal install cost
...
(deftest djinn-host-chakana
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Priority Requisition" "New remote")
    (take-credits state :resPlayer 2)
    (play-from-hand state :hazPlayer "Djinn")
    (let [djinn (get-in @state [:hazPlayer :rig :program 0])
          agenda (get-content state :remote1 0)]
      (is agenda "Agenda was installed")
      (card-ability state :hazPlayer djinn 1)
      (prompt-select :hazPlayer (find-card "Chakana" (:hand (get-runner))))
      (let [chak (first (:hosted (refresh djinn)))]
        (is (= "Chakana" (:title chak)) "Djinn has a hosted Chakana")
        ;; manually add 3 counters
        (core/add-counter state :hazPlayer (first (:hosted (refresh djinn))) :virus 3)
        (take-credits state :hazPlayer 2)
        (core/advance state :resPlayer {:card agenda})
...
(deftest djinn-tutor-virus
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Djinn")
    (core/move state :hazPlayer (find-card "Parasite" (:hand (get-runner))) :deck)
    (is (zero? (count (:hand (get-runner)))) "No cards in hand after moving Parasite to deck")
    (let [djinn (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer djinn 0)
      (prompt-card :hazPlayer (find-card "Parasite" (:deck (get-runner))))
...
(deftest equivocation
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Equivocation")
    (play-from-hand state :hazPlayer "Desperado")
    (run-empty-server state :rd)
    (prompt-choice :hazPlayer "Laramy Fisk: Savvy Investor")
    (prompt-choice :hazPlayer "Yes")
    (is (= 2 (count (:hand (get-corp)))) "Corp forced to draw by Fisk")
    (prompt-choice :hazPlayer "Yes") ; Equivocation prompt
    (prompt-choice :hazPlayer "Yes") ; force the draw
...
(deftest false-echo
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Ice Wall" "Archives")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "False Echo")
    (play-from-hand state :hazPlayer "False Echo")
    (run-on state "Archives")
    (run-continue state)
    (let [echo1 (get-program state 0)
          echo2 (get-program state 1)]
      (card-ability state :hazPlayer echo1 0)
      (prompt-choice :resPlayer "Add to HQ")
      (is (= 2 (count (:hand (get-corp)))) "Ice Wall added to HQ")
      (is (= 1 (count (:discard (get-runner)))) "False Echo trashed")
      (run-continue state)
      (card-ability state :hazPlayer echo2 0)
      (prompt-choice :resPlayer "Rez")
...
(deftest gravedigger
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Launch Campaign" "New remote")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Gravedigger")
    (let [gd (get-in @state [:hazPlayer :rig :program 0])]
      (core/trash state :resPlayer (get-content state :remote1 0))
      (is (= 1 (get-counters (refresh gd) :virus)) "Gravedigger gained 1 counter")
      (core/trash state :resPlayer (get-content state :remote2 0))
      (is (= 2 (get-counters (refresh gd) :virus)) "Gravedigger gained 1 counter")
      (core/move state :resPlayer (find-card "Enigma" (:hand (get-corp))) :deck)
      (core/move state :resPlayer (find-card "Enigma" (:hand (get-corp))) :deck)
      (is (= 2 (count (:deck (get-corp)))))
      (card-ability state :hazPlayer gd 0)
...
(deftest harbinger-blacklist
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Harbinger")
    (core/trash state :hazPlayer (-> (get-runner) :rig :program first))
...
(deftest hyperdriver
  ...
  (do-game
    ...
    (let [hyp (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer hyp 0)
  ...
  (do-game
    (new-game (default-corp)
              (default-runner [(qty "Hyperdriver" 1)
                               (qty "Dhegdheer" 1)]))
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Dhegdheer")
    (let [dheg (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer dheg 0)
      (prompt-select :hazPlayer (find-card "Hyperdriver" (:hand (get-runner))))
...
(deftest imp-the-future-perfect
  ...
  (do-game
    ...
    (testing "Trash before access click"
      ...
      (is (empty? (get-in @state [:hazPlayer :prompt])) "Should be no psi-game prompt for TFP")
      (is (= "The Future Perfect" (get-in @state [:resPlayer :discard 0 :title])) "TFP trashed")
      (is (= 0 (:agenda-point (get-runner))) "Runner did not steal TFP")
      (core/move state :resPlayer (find-card "The Future Perfect" (:discard (get-corp))) :hand))
    (take-credits state :hazPlayer)
    (take-credits state :resPlayer)
    (testing "Trashing after lose psi game"
      ...
      (prompt-choice :resPlayer "0 [Credit]")
      (prompt-choice :hazPlayer "1 [Credit]")
      ;; Fail psi game
      (card-ability state :hazPlayer (get-program state 0) 0)
      (is (empty? (get-in @state [:hazPlayer :prompt])) "Should be no steal prompt for TFP")
      (is (= "The Future Perfect" (get-in @state [:resPlayer :discard 0 :title])) "TFP trashed")
      (is (= 0 (:agenda-point (get-runner))) "Runner did not steal TFP"))))
...
(deftest incubator-transfer-virus-counters
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Datasucker")
    (play-from-hand state :hazPlayer "Incubator")
    (take-credits state :hazPlayer)
    (take-credits state :resPlayer)
    (let [ds (get-in @state [:hazPlayer :rig :program 0])
          incub (get-in @state [:hazPlayer :rig :program 1])]
      ...
      (card-ability state :hazPlayer incub 0)
      (prompt-select :hazPlayer ds)
...
(deftest ixodidae
  ...
  (do-game
    ...
    (play-from-hand state :hazPlayer "Ixodidae")
    ...
    (let [s (get-ice state :hq 0)]
      (core/rez state :resPlayer s)
      (card-subroutine state :resPlayer s 0)
      (is (prompt-is-card? :resPlayer s) "Corp prompt is on Snowflake")
      (is (prompt-is-card? :hazPlayer s) "Runner prompt is on Snowflake")
...
(deftest lamprey
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Lamprey")
    (let [lamp (get-in @state [:hazPlayer :rig :program 0])]
      (run-empty-server state :hq)
      ...
      (take-credits state :hazPlayer)
      (core/purge state :resPlayer)
      (is (empty? (get-in @state [:hazPlayer :rig :program])) "Lamprey trashed by purge"))))
...
(deftest leprechaun-mu-savings
  ...
  (do-game
    ...
    (let [lep (get-in @state [:hazPlayer :rig :program 0])]
      ...
      ;; Trash Hyperdriver
      (core/move state :hazPlayer (find-card "Hyperdriver" (:hosted (refresh lep))) :discard)
      (is (= 3 (:memory (get-runner))) "Hyperdriver 3 MU not added to available MU")
      (core/move state :hazPlayer (find-card "Imp" (:hosted (refresh lep))) :discard) ; trash Imp
...
(deftest magnum-opus-click
  ...
  (do-game
    ...
    (is (= 0 (:credit (get-runner))))
    (let [mopus (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer mopus 0)
...
(deftest origami
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Origami")
    (is (= 6 (core/hand-size state :hazPlayer)))
    (play-from-hand state :hazPlayer "Origami")
    (is (= 9 (core/hand-size state :hazPlayer)) "Max hand size increased by 2 for each copy installed")))
...
(deftest paintbrush
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Ice Wall" "HQ")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Paintbrush")
    (is (= 2 (:memory (get-runner))))
    (let [iwall (get-ice state :hq 0)
          pb (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer pb 0)
...
(deftest parasite-apex
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (core/end-phase-12 state :hazPlayer nil)
    (prompt-select :hazPlayer (find-card "Parasite" (:hand (get-runner))))
    (is (empty? (:prompt (get-runner))) "No prompt to host Parasite")
    (is (= 1 (count (get-in @state [:hazPlayer :rig :facedown]))) "Parasite installed face down")))
...
(deftest parasite-builder-moved
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Ice Wall" "HQ")
    (play-from-hand state :resPlayer "Builder" "Archives")
    (let [builder (get-ice state :archives 0)
          _ (core/rez state :resPlayer builder)
          _ (take-credits state :resPlayer)
          _ (play-from-hand state :hazPlayer "Parasite")
          _ (prompt-select :hazPlayer builder)
          psite (first (:hosted (refresh builder)))
          _ (take-credits state :hazPlayer)
          _ (take-credits state :resPlayer)
          ...
          _ (card-ability state :resPlayer builder 0)
          _ (prompt-choice :resPlayer "HQ")
...
(deftest plague
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Plague")
    (prompt-choice :hazPlayer "Server 1")
    (let [plague (get-in @state [:hazPlayer :rig :program 0])]
...
(deftest progenitor-host-hivemind
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Progenitor")
    (play-from-hand state :hazPlayer "Virus Breeding Ground")
    (is (= 4 (:memory (get-runner))))
    (let [prog (get-in @state [:hazPlayer :rig :program 0])
          vbg (get-in @state [:hazPlayer :rig :resource 0])]
      (card-ability state :hazPlayer prog 0)
      ...
      (let [hive (first (:hosted (refresh prog)))]
        ...
        (take-credits state :resPlayer)
        (card-ability state :hazPlayer vbg 0) ; use VBG to transfer 1 token to Hivemind
...
(deftest progenitor-mu-savings
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Progenitor")
    (let [pro (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer pro 0)
      (prompt-select :hazPlayer (find-card "Hivemind" (:hand (get-runner))))
      (is (= 2 (:click (get-runner))))
      (is (= 2 (:credit (get-runner))))
      (is (= 4 (:memory (get-runner))) "Hivemind 2 MU not deducted from available MU")
      ;; Trash Hivemind
      (core/move state :hazPlayer (find-card "Hivemind" (:hosted (refresh pro))) :discard)
...
(deftest reaver
  ...
  (do-game
    ...
    (starting-hand state :hazPlayer ["Reaver" "Fall Guy"])
    ...
    (play-from-hand state :hazPlayer "Reaver")
    ...
    (card-ability state :hazPlayer (get-resource state 0) 1)
    (is (= 0 (count (:hand (get-runner)))) "No cards in hand")
    (take-credits state :hazPlayer)
...
(deftest reaver-fcc
  ...
  (do-game
    ...
    (starting-hand state :hazPlayer ["Reaver" "Imp" "Snitch" "Freelance Coding Contract"])
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Reaver")
    ...
    (prompt-select :hazPlayer (find-card "Snitch" (:hand (get-runner))))
    (prompt-select :hazPlayer (find-card "Imp" (:hand (get-runner))))
...
(deftest rng-key
  ...
  (do-game
    ...
    (starting-hand state :resPlayer ["Hedge Fund"])
    (starting-hand state :hazPlayer ["RNG Key"])
    (take-credits state :resPlayer)

    (play-from-hand state :hazPlayer "RNG Key")
    (is (= 5 (:credit (get-runner))) "Starts at 5 credits")
    (run-on state "HQ")
    (run-successful state)
    (prompt-choice :hazPlayer "Yes")
    (prompt-choice :hazPlayer 5)
...
(deftest scheherazade
  ...
  (do-game
    ...
    (let [sch (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer sch 0)
      ...
      (card-ability state :hazPlayer sch 0)
      (prompt-select :hazPlayer (find-card "Cache" (:hand (get-runner))))
...
(deftest sneakdoor-nerve-agent
  ...
  (do-game
    ...
    (let [nerve (get-in @state [:hazPlayer :rig :program 0])
          sb (get-in @state [:hazPlayer :rig :program 1])]
      (card-ability state :hazPlayer sb 0)
      (run-successful state)
...
(deftest sneakdoor-ash
  ...
  (do-game
    ...
    (let [sb (get-in @state [:hazPlayer :rig :program 0])
...
(deftest sneakdoor-crisium
  ...
  (do-game
    ...
    (trash-from-hand state :resPlayer "Priority Requisition")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Sneakdoor Beta")
...
(deftest snitch
  ...
  (do-game
    ...
    (let [hqice (get-ice state :hq 0)]
      (core/rez state :resPlayer hqice))
    (take-credits state :resPlayer)
    ...
    (let [snitch (get-in @state [:hazPlayer :rig :program 0])]
      ...
      (is (prompt-is-card? :hazPlayer snitch) "Option to jack out")
      (prompt-choice :hazPlayer "Yes")
      ;; rezzed ice scenario
      (run-on state "HQ")
      (card-ability state :hazPlayer snitch 0)
      (is (empty? (get-in @state [:hazPlayer :prompt])) "No option to jack out")
      ;; no ice scenario
      (run-on state "Archives")
      (card-ability state :hazPlayer snitch 0)
      (is (empty? (get-in @state [:hazPlayer :prompt])) "No option to jack out"))))
...
(deftest surfer
  ...
  (do-game
   ...
   (play-from-hand state :resPlayer "Ice Wall" "HQ")
   (take-credits state :resPlayer)
   (play-from-hand state :hazPlayer "Surfer")
   (is (= 3 (:credit (get-runner))) "Paid 2 credits to install Surfer")
   (core/rez state :resPlayer (get-ice state :hq 1))
   (run-on state "HQ")
   (is (= 2 (get-in @state [:run :position])) "Starting run at position 2")
   (let [surf (get-in @state [:hazPlayer :rig :program 0])]
     (card-ability state :hazPlayer surf 0)
...
(deftest takobi
  ...
  (do-game
    ...
    (core/gain state :hazPlayer :credit 10)
    (play-from-hand state :hazPlayer "Takobi")
    (play-from-hand state :hazPlayer "Corroder")
    (play-from-hand state :hazPlayer "Faust")
    (let [tako (get-in @state [:hazPlayer :rig :program 0])
          corr (get-in @state [:hazPlayer :rig :program 1])
          faus (get-in @state [:hazPlayer :rig :program 2])]
      (dotimes [_ 3]
        (card-ability state :hazPlayer tako 0))
      (is (= 3 (get-counters (refresh tako) :power)) "3 counters on Takobi")

      (run-on state "HQ")
      (card-ability state :hazPlayer tako 1)
      (is (empty? (:prompt (get-runner))) "No prompt for un-rezzed ice")
      (core/rez state :resPlayer (get-ice state :hq 0))
      (card-ability state :hazPlayer tako 1)
      (prompt-select :hazPlayer (refresh faus))
      (is (not-empty (:prompt (get-runner))) "Can't select AI breakers")
      (prompt-select :hazPlayer (refresh corr))
...
(deftest upya
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Upya")
    (dotimes [_ 3]
    ...
    (let [upya (get-program state 0)]
      (card-ability state :hazPlayer upya 0)
...
(deftest wari
  (do-game
    ...
    (play-from-hand state :resPlayer "Ice Wall" "R&D")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Wari")
    (run-empty-server state "HQ")
    (prompt-choice :hazPlayer "Yes")
    (prompt-choice :hazPlayer "Barrier")
    (prompt-select :hazPlayer (get-ice state :rd 0))
    (is (= 1 (count (:discard (get-runner)))) "Wari in heap")
    (is (not (empty? (get-in @state [:hazPlayer :prompt]))) "Runner is currently accessing Ice Wall")))
