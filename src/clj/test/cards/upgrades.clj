...
(deftest amazon-industrial-zone
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Amazon Industrial Zone" "New remote")
    (let [aiz (get-content state :remote1 0)]
      (core/rez state :resPlayer aiz)
      (is (= 2 (:credit (get-corp))))
      (play-from-hand state :resPlayer "Spiderweb" "Server 1")
      (prompt-choice :resPlayer "Yes") ; optional ability
...
(deftest ben-musashi
  ...
  (do-game
    (new-game (default-corp [(qty "Ben Musashi" 1) (qty "House of Knives" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Ben Musashi" "New remote")
    (play-from-hand state :resPlayer "House of Knives" "Server 1")
    (take-credits state :resPlayer 1)
    (let [bm (get-content state :remote1 0)
          hok (get-content state :remote1 1)]
      (core/rez state :resPlayer bm)
      (run-empty-server state "Server 1")
      ;; runner now chooses which to access.
      (prompt-select :hazPlayer hok)
...
(deftest ben-musashi-rd
  ...
  (do-game
    (new-game (default-corp [(qty "Ben Musashi" 1) (qty "House of Knives" 1)])
              (default-runner))
    (starting-hand state :resPlayer ["Ben Musashi"])
    (play-from-hand state :resPlayer "Ben Musashi" "R&D")
    ...
    (let [bm (get-content state :rd 0)]
      ...
      (is (= 5 (:credit (get-runner))) "Runner did not pay 2 net damage")
      (is (= 0 (count (:scored (get-runner)))) "No scored agendas")
      (prompt-choice :hazPlayer "Ben Musashi")
      (prompt-choice :hazPlayer "No")
      (run-empty-server state "R&D")
      (prompt-choice :hazPlayer "Card from deck")
      (prompt-choice :hazPlayer "Yes")
...
(deftest ben-musashi-obokata
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Ben Musashi" "New remote")
    (play-from-hand state :resPlayer "Obokata Protocol" "Server 1")
    (take-credits state :resPlayer)
    (let [bm (get-content state :remote1 0)
      ...
      (prompt-choice :hazPlayer "2 net damage")
      (is (= 2 (count (:discard (get-runner)))) "Runner took 2 net damage")
      (is (= 0 (count (:scored (get-runner)))) "No scored agendas")
      (prompt-choice :hazPlayer "4 net damage")
...
(deftest ben-musashi-fetal-ai
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Ben Musashi" "New remote")
    (play-from-hand state :resPlayer "Fetal AI" "Server 1")
    (take-credits state :resPlayer)
    (let [bm (get-content state :remote1 0)
...
(deftest bernice-mai
  ...
  (do-game
    (new-game (default-corp [(qty "Bernice Mai" 3) (qty "Hedge Fund" 3) (qty "Wall of Static" 3)])
              (default-runner))
    (starting-hand state :resPlayer ["Bernice Mai" "Bernice Mai" "Bernice Mai"])
    (play-from-hand state :resPlayer "Bernice Mai" "New remote")
    (play-from-hand state :resPlayer "Bernice Mai" "New remote")
    (play-from-hand state :resPlayer "Bernice Mai" "R&D")
...
(deftest breaker-bay-grid
  ...
  (do-game
   ...
   (play-from-hand state :resPlayer "The Root" "Server 1")
   (let [bbg1 (get-content state :remote1 0)
         root (get-content state :remote1 1)]
     (core/rez state :resPlayer bbg1)
     (core/rez state :resPlayer root)
     (is (= 4 (:credit (get-corp))) "Paid only 1 to rez The Root")
     (play-from-hand state :resPlayer "Breaker Bay Grid" "R&D")
     (play-from-hand state :resPlayer "Strongbox" "R&D")
     (let [bbg2 (get-content state :rd 0)
           sbox (get-content state :rd 1)]
       (core/rez state :resPlayer bbg2)
       (core/rez state :resPlayer sbox)
...
(deftest calibration-testing
  ...
  (do-game
    ...
    (core/gain state :resPlayer :click 1)
    (play-from-hand state :resPlayer "Calibration Testing" "New remote")
    (play-from-hand state :resPlayer "Project Junebug" "Server 1")
    (let [ct (get-content state :remote1 0)
          pj (get-content state :remote1 1)]
      (core/rez state :resPlayer ct)
      (card-ability state :resPlayer ct 0)
    ...
    (play-from-hand state :resPlayer "Calibration Testing" "New remote")
    (play-from-hand state :resPlayer "PAD Campaign" "Server 2")
    (let [ct (get-content state :remote2 0)
          pad (get-content state :remote2 1)]
      (core/rez state :resPlayer ct)
      (card-ability state :resPlayer ct 0)
      (prompt-select :resPlayer pad)
...
(deftest caprice-nisei
  ...
  (do-game
   ...
   (let [caprice (get-content state :remote1 0)]
     ...
     (is (prompt-is-card? :resPlayer caprice)
     ...
     (play-from-hand state :resPlayer "Quandary" "Server 1")
     (play-from-hand state :resPlayer "Quandary" "Server 1")
     (take-credits state :resPlayer)
     ...
     (run-on state "Server 1")
     (run-continue state)
     (is (empty? (get-in @state [:resPlayer :prompt])) "Caprice not trigger on first ice")
     ...
     (is (empty? (get-in @state [:resPlayer :prompt])) "Caprice prompted cleared")

     ;; Check Caprice does not trigger on other servers
     (run-on state "HQ")
     (is (empty? (get-in @state [:resPlayer :prompt])) "Caprice does not trigger on other servers"))))
...
(deftest chilo-city-grid
  ...
  (do-game
    (new-game (default-corp [(qty "Caduceus" 2) (qty "ChiLo City Grid" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "ChiLo City Grid" "New remote")
    (play-from-hand state :resPlayer "Caduceus" "Server 1")
    (take-credits state :resPlayer)
    (let [chilo (get-content state :remote1 0)
          cad (get-ice state :remote1 0)]
      (run-on state "R&D")
      (core/rez state :resPlayer cad)
      (core/rez state :resPlayer chilo)
...
(deftest cyberdex-virus-suite-purge
  ...
  (do-game
    ...
    (let [virus-counters (fn [card] (core/get-virus-counters state :hazPlayer (refresh card)))
          cache (find-card "Cache" (get-in @state [:hazPlayer :rig :program]))
          cvs (get-content state :hq 0)]
      (is (= 3 (virus-counters cache)))
      (play-from-hand state :hazPlayer "Medium")
      (take-credits state :hazPlayer 2)
      (core/rez state :resPlayer cvs)
      (card-ability state :resPlayer cvs 0)
      ;; nothing in hq content
      (is (empty? (get-content state :hq)) "CVS was trashed")
      ;; purged counters
      (is (zero? (virus-counters cache))
          "Cache has no counters")
      (is (zero? (virus-counters (find-card "Medium" (get-in @state [:hazPlayer :rig :program]))))
...
(deftest cyberdex-virus-suite-archives-access
  ...
  (do-game
    ...
    (trash-from-hand state :resPlayer "Cyberdex Virus Suite")
    (trash-from-hand state :resPlayer "Braintrust")
    (take-credits state :resPlayer)
    ;; runner's turn
    ;; install cache
    (play-from-hand state :hazPlayer "Cache")
    (let [cache (get-program state 0)]
      ...
      (prompt-choice :hazPlayer "Cyberdex Virus Suite")
      (prompt-choice :resPlayer "Yes")
...
(deftest forced-connection
  ...
  (do-game
    (new-game (default-corp [(qty "Forced Connection" 3)])
              (default-runner))
    (starting-hand state :resPlayer ["Forced Connection" "Forced Connection"])
    (play-from-hand state :resPlayer "Forced Connection" "New remote")
    (take-credits state :resPlayer)
...
(deftest ghost-branch-dedicated-response-team
  ...
  (do-game
    ...
    (let [gb (get-content state :remote1 0)
      ...
      (core/advance state :resPlayer {:card gb})
      ...
      (run-successful state)
      (is (prompt-is-type? :hazPlayer :waiting) "Runner has prompt to wait for Ghost Branch")
      (prompt-choice :resPlayer "Yes")
...
(deftest georgia-emelyov
  ...
  (do-game
    (new-game (default-corp [(qty "Georgia Emelyov" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Georgia Emelyov" "New remote")
    (let [geo (get-content state :remote1 0)]
      (core/rez state :resPlayer geo)
      (take-credits state :resPlayer)
      (run-on state "Server 1")
      (run-jack-out state)
      (is (= 1 (count (:discard (get-runner)))) "Runner took 1 net damage")
      (card-ability state :resPlayer (refresh geo) 0)
...
(deftest helheim-servers
  ...
  (do-game
    (new-game (default-corp [(qty "Helheim Servers" 1) (qty "Gutenberg" 1) (qty "Vanilla" 1)
              ...
              (default-runner))
    (play-from-hand state :resPlayer "Helheim Servers" "R&D")
    (play-from-hand state :resPlayer "Gutenberg" "R&D")
    ...
    (let [helheim (get-content state :rd 0)
      ...
      (core/rez state :resPlayer helheim)
      (core/rez state :resPlayer gutenberg)
      (core/rez state :resPlayer vanilla)
      (is (= 6 (:current-strength (refresh gutenberg))))
      (is (= 0 (:current-strength (refresh vanilla))))
      (card-ability state :resPlayer helheim 0)
      (prompt-select :resPlayer (find-card "Jackson Howard" (:hand (get-corp))))
...
(deftest hokusai-grid
  ...
  (do-game
    (new-game (default-corp [(qty "Hokusai Grid" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Hokusai Grid" "HQ")
    (take-credits state :resPlayer)
    (core/rez state :resPlayer (get-content state :hq 0))
    (run-empty-server state :rd)
...
(deftest jinja-city-grid
  ...
  (do-game
    (new-game (default-corp [(qty "Jinja City Grid" 1) (qty "Vanilla" 3) (qty "Ice Wall" 3)])
              (default-runner))
    (starting-hand state :resPlayer ["Jinja City Grid"])
    (core/gain state :resPlayer :click 6)
    (play-from-hand state :resPlayer "Jinja City Grid" "New remote")
    (core/rez state :resPlayer (get-content state :remote1 0))
    (dotimes [n 5]
      (core/click-draw state :resPlayer 1)
      (prompt-choice :resPlayer "Yes")
      (is (= 4 (:credit (get-corp))) "Not charged to install ice")
      (is (= (inc n) (count (get-in @state [:resPlayer :servers :remote1 :ices]))) (str n " ICE protecting Remote1")))
    (core/click-draw state :resPlayer 1)
    (prompt-choice :resPlayer "Yes")
    (is (= 3 (:credit (get-corp))) "Charged to install ice")
    (is (= 6 (count (get-in @state [:resPlayer :servers :remote1 :ices]))) "6 ICE protecting Remote1")))
...
(deftest keegan-lane
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Keegan Lane" "HQ")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Corroder")
    (run-on state :hq)
    (let [keeg (get-content state :hq 0)]
      (core/rez state :resPlayer keeg)
      (card-ability state :resPlayer keeg 0)
...
(deftest manta-grid
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Manta Grid" "HQ")
    (core/rez state :resPlayer (get-content state :hq 0))
    (take-credits state :resPlayer)
    (core/click-draw state :hazPlayer nil)
    (core/click-draw state :hazPlayer nil)
    (run-empty-server state "HQ")
    (prompt-choice :hazPlayer "No") ; don't trash Manta Grid
...
(deftest marcus-batty-security-nexus
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Marcus Batty" "HQ")
    (play-from-hand state :resPlayer "Enigma" "HQ")
    (take-credits state :resPlayer)
    (core/gain state :hazPlayer :credit 8)
    (play-from-hand state :hazPlayer "Security Nexus")
    (let [mb (get-content state :hq 0)
          en (get-ice state :hq 0)
          sn (-> @state :hazPlayer :rig :hardware first)]
      (run-on state "HQ")
      (core/rez state :resPlayer mb)
      (core/rez state :resPlayer en)
      (card-ability state :resPlayer mb 0)
      (card-ability state :hazPlayer sn 0)
      ;; both prompts should be on Batty
      (is (prompt-is-card? :resPlayer mb) "Corp prompt is on Marcus Batty")
      (is (prompt-is-card? :hazPlayer mb) "Runner prompt is on Marcus Batty")
      (prompt-choice :resPlayer "0")
      (prompt-choice :hazPlayer "0")
      (is (prompt-is-card? :resPlayer sn) "Corp prompt is on Security Nexus")
      (is (prompt-is-type? :hazPlayer :waiting) "Runner prompt is waiting for Corp"))))
...
(deftest mumbad-virtual-tour-force-trash
  ...
  (do-game
    (new-game (default-corp [(qty "Mumbad Virtual Tour" 2)])
              (default-runner))
    (play-from-hand state :resPlayer "Mumbad Virtual Tour" "New remote")
    (take-credits state :resPlayer)
    (run-empty-server state "HQ")
...
(deftest mumbad-virtual-tour-slow-trash
  ...
  (do-game
    ...
    (let [imp (get-program state 0)]
      ...
      (core/move state :hazPlayer (refresh imp) :discard)
      (is (not (core/any-flag-fn? state :hazPlayer :slow-trash true))))))
...
(deftest neotokyo-grid
  ...
  (do-game
    ...
    (let [nis (get-content state :remote1 1)]
      ...
      (core/advance state :resPlayer {:card (refresh (get-ice state :remote1 0))})
...
(deftest off-the-grid
  ...
  (do-game
   (new-game
    ...
    (default-runner))
   (play-from-hand state :resPlayer "Off the Grid" "New remote")
   (play-from-hand state :resPlayer "Mental Health Clinic" "Server 1")
   (let [otg (get-content state :remote1 0)]
     (take-credits state :resPlayer)
     (core/rez state :resPlayer (refresh otg))
...
(deftest old-hollywood-grid
  ...
  (do-game
    (new-game (default-corp [(qty "Old Hollywood Grid" 1) (qty "House of Knives" 3)])
              (default-runner))
    (play-from-hand state :resPlayer "Old Hollywood Grid" "New remote")
    (play-from-hand state :resPlayer "House of Knives" "Server 1")
    (take-credits state :resPlayer 1)
    (let [ohg (get-content state :remote1 0)
          hok (get-content state :remote1 1)]
      (run-on state "Server 1")
      (core/rez state :resPlayer ohg)
      (run-successful state)
      ;; runner now chooses which to access.
      (prompt-select :hazPlayer hok)
      ...
      (prompt-select :hazPlayer ohg)
      (prompt-choice :hazPlayer "No")
      (core/steal state :hazPlayer (find-card "House of Knives" (:hand (get-corp))))
      (run-empty-server state "Server 1")
      (prompt-select :hazPlayer hok)
      (prompt-choice :hazPlayer "Yes")
...
(deftest old-hollywood-grid-central
  ...
  (do-game
    (new-game (default-corp [(qty "Old Hollywood Grid" 1) (qty "House of Knives" 3)])
              (default-runner))
    (play-from-hand state :resPlayer "Old Hollywood Grid" "HQ")
    (take-credits state :resPlayer 2)
    (let [ohg (get-content state :hq 0)]
      ...
      (prompt-choice :hazPlayer "OK")
      (is (= 0 (count (:scored (get-runner)))) "No stolen agendas")
      (prompt-choice :hazPlayer "Old Hollywood Grid")
      ;; trash OHG
      (prompt-choice :hazPlayer "Yes")
      (run-empty-server state "HQ")
...
(deftest prisec-dedicated-response-team
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Dedicated Response Team" "New remote")
    (play-from-hand state :resPlayer "Prisec" "Archives")
    (play-from-hand state :resPlayer "Prisec" "Archives")
    (core/gain state :resPlayer :click 1 :credit 14)
    (core/rez state :resPlayer (get-content state :remote1 0))
    (take-credits state :resPlayer)

    (run-empty-server state :archives)
    (is (:run @state) "Run still active")
    (prompt-choice :hazPlayer "Unrezzed upgrade in Archives")
    (prompt-select :hazPlayer (get-content state :archives 0))
    (prompt-choice :resPlayer "Yes") ; corp pay for PriSec
    (prompt-choice :hazPlayer "No") ; runner don't pay to trash
    (is (:run @state) "Run still active")
    (prompt-choice :hazPlayer "Unrezzed upgrade in Archives")
    (prompt-choice :resPlayer "Yes") ; corp pay for PriSec
    (prompt-choice :hazPlayer "No") ; runner don't pay to trash
...
(deftest red-herrings
  ...
  (do-game
    (new-game (default-corp [(qty "Red Herrings" 1) (qty "House of Knives" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Red Herrings" "New remote")
    (play-from-hand state :resPlayer "House of Knives" "Server 1")
    (take-credits state :resPlayer 1)

    (let [rh (get-content state :remote1 0)
          hok (get-content state :remote1 1)]
      (core/rez state :resPlayer rh)
      (run-empty-server state "Server 1")
      ;; runner now chooses which to access.
      (prompt-select :hazPlayer hok)
...
(deftest red-herrings-trash-from-hand
  ...
  (do-game
    (new-game (default-corp [(qty "Red Herrings" 1) (qty "House of Knives" 1)])
              (default-runner))
    (trash-from-hand state :resPlayer "Red Herrings")
...
(deftest ruhr-valley
  ...
  (do-game
    (new-game (default-corp [(qty "Ruhr Valley" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Ruhr Valley" "HQ")
    (take-credits state :resPlayer)
    (let [ruhr (get-content state :hq 0)]
      (core/rez state :resPlayer ruhr)
      ...
      (run-on state :hq)
      (run-successful state)
      (prompt-choice :hazPlayer "Yes") ; pay to trash / 7 cr - 4 cr
      (is (= 2 (:click (get-runner))))
...
(deftest ruhr-valley-enable-state
  ...
  (do-game
    ...
    (let [ruhr (get-content state :hq 0)]
      ...
      (run-on state :hq)
      (run-successful state)
      (prompt-choice :hazPlayer "Yes") ; pay to trash / 6 cr - 4 cr
      (is (= 1 (:click (get-runner))))
      (run-on state :hq))))
...
(deftest ryon-knight
  ...
  (do-game
    (new-game (default-corp [(qty "Ryon Knight" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Ryon Knight" "HQ")
    (take-credits state :resPlayer)
    (let [ryon (get-content state :hq 0)]
      (run-on state :hq)
      (core/rez state :resPlayer ryon)
      (card-ability state :resPlayer ryon 0)
      (is (= 3 (:click (get-runner))))
...
(deftest satellite-grid
  ...
  (do-game
    (new-game (default-corp [(qty "Satellite Grid" 1) (qty "Ice Wall" 2)])
              (default-runner))
    (play-from-hand state :resPlayer "Satellite Grid" "HQ")
    ...
    (let [iw1 (get-ice state :hq 0)
      ...
      (core/gain state :resPlayer :click 1)
      (advance state iw1)
      (core/rez state :resPlayer sg)
      (core/rez state :resPlayer (refresh iw1))
...
(deftest signal-jamming
  ...
  (do-game
    ...
    (starting-hand state :hazPlayer ["Self-modifying Code" "Self-modifying Code"])
    (play-from-hand state :resPlayer "Signal Jamming" "HQ")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Self-modifying Code")
    (play-from-hand state :hazPlayer "Self-modifying Code")
    (let [smc1 (get-in @state [:hazPlayer :rig :program 0])
          smc2 (get-in @state [:hazPlayer :rig :program 1])
          sj (get-content state :hq 0)]
      (core/rez state :resPlayer sj)
      (run-on state "HQ")
      (run-continue state)
      (card-ability state :resPlayer sj 0)
      (card-ability state :hazPlayer smc1 0)
      (is (empty? (:prompt (get-runner))) "SJ blocking SMC")
      (run-jack-out state)
      (card-ability state :hazPlayer smc2 0)
      (prompt-card :hazPlayer (find-card "Reaver" (:deck (get-runner)))))))
...
(deftest strongbox
  ...
  (do-game
    (new-game (default-corp [(qty "Strongbox" 1) (qty "House of Knives" 1)])
              (default-runner))
    (play-from-hand state :resPlayer "Strongbox" "New remote")
    (play-from-hand state :resPlayer "House of Knives" "Server 1")
    (take-credits state :resPlayer 1)
...
(deftest tori-hanzo-hokusai
  ...
  (do-game
    (new-game (default-corp [(qty "Tori Hanzō" 1) (qty "Hokusai Grid" 1)])
              (default-runner))
    (core/gain state :resPlayer :credit 5)
    (play-from-hand state :resPlayer "Hokusai Grid" "Archives")
    (play-from-hand state :resPlayer "Tori Hanzō" "Archives")
    (take-credits state :resPlayer)
    (run-on state "Archives")
    (let [hg (get-content state :archives 0)
          tori (get-content state :archives 1)]
      (core/rez state :resPlayer hg)
      (core/rez state :resPlayer tori)
      (run-successful state)
      (prompt-choice :resPlayer "No") ; Tori prompt to pay 2c to replace 1 net with 1 brain
      (is (= 1 (count (:discard (get-runner)))) "1 net damage suffered")
      (prompt-choice :hazPlayer "Hokusai Grid")
      (prompt-choice :hazPlayer "No")
      (prompt-choice :hazPlayer "Tori Hanzō")
      (prompt-choice :hazPlayer "No")
      (is (and (empty (:prompt (get-runner))) (not (:run @state))) "No prompts, run ended")
      (run-empty-server state "Archives")
      (prompt-choice :resPlayer "Yes") ; Tori prompt to pay 2c to replace 1 net with 1 brain
...
(deftest underway-grid
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Underway Grid" "New remote")
...
(deftest valley-grid-trash
  ...
  (do-game
    (new-game (default-corp [(qty "Valley Grid" 3) (qty "Ice Wall" 3)])
              (default-runner))
    (play-from-hand state :resPlayer "Valley Grid" "New remote")
    (take-credits state :resPlayer 2)
    (run-on state "Server 1")
    (let [vg (get-content state :remote1 0)]
      (core/rez state :resPlayer vg)
      (card-ability state :resPlayer vg 0)
      (card-ability state :resPlayer vg 0) ; only need the run to exist for test, just pretending the Runner has broken all subs on 2 ice
      ...
      (prompt-choice :hazPlayer "Yes") ; pay to trash
      (take-credits state :hazPlayer 3)
      (is (= 5 (core/hand-size state :hazPlayer)) "Runner max hand size increased by 2 at start of Corp turn"))))
