...
(deftest adjusted-chronotype
  ...
  (do-game
   ...
   (take-credits state :resPlayer)
   (play-from-hand state :hazPlayer "Adjusted Chronotype")
   (play-from-hand state :hazPlayer "Beach Party")
   (take-credits state :hazPlayer)
   (take-credits state :resPlayer)
...
(deftest adjusted-chronotype-gcs
  ...
  (do-game
   ...
   (is (= 4 (:click (get-runner))) "Should have lost 1 click and gained 1 click")
   (play-from-hand state :hazPlayer "Beach Party")
   (play-from-hand state :hazPlayer "Gene Conditioning Shoppe")
   (take-credits state :hazPlayer)
   (take-credits state :resPlayer)
...
(deftest aesops-pawnshop
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Aesop's Pawnshop")
    (play-from-hand state :hazPlayer "Cache")
    (let [orig-credits (:credit (get-runner))
          ap (get-in @state [:hazPlayer :rig :resource 0])
          cache (get-in @state [:hazPlayer :rig :program 0])]
      (card-ability state :hazPlayer ap 0)
      (prompt-select :hazPlayer cache)
      (card-ability state :hazPlayer ap 0)
      (prompt-select :hazPlayer ap)
      (let [ap (get-in @state [:hazPlayer :rig :resource 0])
            cache (get-in @state [:hazPlayer :discard 0])]
...
(deftest all-nighter
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "All-nighter")
...
(deftest bank-job-manhunt
  ...
  (do-game
    ...
    (prompt-choice :resPlayer 2) ; Manhunt trace active
    (prompt-choice :hazPlayer 0)
    (prompt-choice :hazPlayer "Run ability")
    (is (= "Bank Job" (:title (:card (first (get-in @state [:hazPlayer :prompt])))))
        "Bank Job prompt active")
    (prompt-choice :hazPlayer 8)
    (is (empty? (get-in @state [:hazPlayer :rig :resource])) "Bank Job trashed after all credits taken")
...
(deftest bank-job-multiple-copies
  ...
  (do-game
    ...
    (let [bj1 (get-resource state 0)
      ...
      (run-empty-server state "Server 1")
      (prompt-choice :hazPlayer "Run ability")
      (prompt-select :hazPlayer bj2)
      (prompt-choice :hazPlayer 6)
  ...
  (do-game
    (new-game (default-corp)
              (default-runner [(qty "Street Peddler" 1)
                               ...
                               (qty "Spy Camera" 6)]))
    (take-credits state :resPlayer)
    (starting-hand state :hazPlayer ["Street Peddler" "Bazaar" "Spy Camera" "Spy Camera" "Spy Camera"])
    (play-from-hand state :hazPlayer "Bazaar")
    (play-from-hand state :hazPlayer "Street Peddler")
    (let [peddler (get-resource state 1)]
      (card-ability state :hazPlayer peddler 0)
      (prompt-card :hazPlayer (first (:hosted peddler)))
...
(deftest beach-party
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Beach Party")
    (is (= 10 (core/hand-size state :hazPlayer)) "Max hand size increased by 5")
    (take-credits state :hazPlayer)
    (take-credits state :resPlayer)
...
(deftest bhagat
  ...
  (do-game
    ...
    (play-from-hand state :hazPlayer "Bhagat")
    (run-empty-server state :hq)
...
(deftest chrome-parlor
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Chrome Parlor")
    ...
    (play-from-hand state :hazPlayer "Brain Cage")
    (is (= 2 (count (:hand (get-runner)))) "No cards lost")
    (is (= 0 (:brain-damage (get-runner))))
    (is (= 8 (core/hand-size state :hazPlayer)) "Runner hand size boosted by Brain Cage")
    (take-credits state :hazPlayer)
    (core/gain state :hazPlayer :tag 2)
    (core/trash state :hazPlayer (get-hardware state 0))
    (play-from-hand state :resPlayer "Traffic Accident")
...
(deftest councilman
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Councilman")
    (let [jesus (get-content state :remote1 0)
          judas (get-resource state 0)]
      (core/rez state :resPlayer jesus)
      ;; Runner triggers Councilman
      (card-ability state :hazPlayer judas 0)
      (prompt-select :hazPlayer jesus)
      (is (not (core/rezzed? (refresh jesus))) "Jackson Howard no longer rezzed")
      (core/rez state :resPlayer (refresh jesus))
...
(deftest counter-surveillance
  ...
  (do-game
    ...
    (-> @state :hazPlayer :credit (= 4) (is "Runner has 4 credits"))
    (let [cs (get-in @state [:hazPlayer :rig :resource 0])]
      (card-ability state :hazPlayer cs 0)
      ...
      (-> (get-runner) :prompt first :msg (= "You accessed Hedge Fund") is)
      (prompt-choice :hazPlayer "OK")
      (-> @state :hazPlayer :discard count (= 1) (is "Counter Surveillance trashed"))
      (-> @state :hazPlayer :credit (= 2) (is "Runner has 2 credits")))))
...
(deftest counter-surveillance-obelus
  ...
  (do-game
    ...
    (starting-hand state :hazPlayer ["Counter Surveillance" "Obelus"])
    (take-credits state :resPlayer)
    (core/gain state :hazPlayer :tag 2)
...
(deftest-pending councilman-zone-change
  ...
  (do-game
    ...
    (let [jesus (get-content state :remote1 0)
      ...
      (prompt-select :hazPlayer jesus)
      (is (not (core/rezzed? (refresh jesus))) "Jackson Howard no longer rezzed")
      (core/move state :resPlayer (refresh jesus) :hand))
    (play-from-hand state :resPlayer "Jackson Howard" "New remote")
...
(deftest daily-casts
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Daily Casts")
    (let [dc (get-in @state [:hazPlayer :rig :resource 0])]
      ;; Number of credits
      (is (= 8 (get-counters dc :credit)))
      (is (= 2 (get-in @state [:hazPlayer :credit])))
      ;; End turn
      (take-credits state :hazPlayer)
      (take-credits state :resPlayer)
      (is (= 6 (get-counters (refresh dc) :credit)))
      (is (= 7 (get-in @state [:hazPlayer :credit])))
      (take-credits state :hazPlayer)
      (take-credits state :resPlayer)
      (is (= 4 (get-counters (refresh dc) :credit)))
      (is (= 13 (get-in @state [:hazPlayer :credit])))
      (take-credits state :hazPlayer)
      (take-credits state :resPlayer)
      (is (= 2 (get-counters (refresh dc) :credit)))
      (is (= 19 (get-in @state [:hazPlayer :credit])))
      (take-credits state :hazPlayer)
      (take-credits state :resPlayer)
      (is (nil? (get-in @state [:hazPlayer :rig :resource 0]))))))
...
(deftest data-folding
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Data Folding")
    (take-credits state :hazPlayer)
    (take-credits state :resPlayer)
...
(deftest ddos
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Ice Wall" "HQ")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "DDoS")
    (let [ddos (get-in @state [:hazPlayer :rig :resource 0])
          iwall (get-ice state :hq 1)]
      (card-ability state :hazPlayer ddos 0)
      (is (= (:title ddos) (get-in @state [:hazPlayer :discard 0 :title])))
      (run-on state "HQ")
...
(deftest decoy
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Decoy")
...
(deftest donut-taganes
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Donut Taganes")
...
(deftest fan-site-eoi
  ...
  (do-game
    ...
    (prompt-select :resPlayer (find-card "Fan Site" (:scored (get-runner))))
    (prompt-select :resPlayer (find-card "Hostile Takeover" (:scored (get-corp))))
...
(deftest fester
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Fester")
    (take-credits state :hazPlayer)
    (core/lose state :resPlayer :credit 5)
...
(deftest film-critic-discarded-executives
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Film Critic")
    (let [fc (first (get-in @state [:hazPlayer :rig :resource]))]
      (run-empty-server state "Server 1")
      (card-ability state :hazPlayer fc 0)
      (is (= 1 (count (:hosted (refresh fc)))) "Agenda hosted on FC")
      (take-credits state :hazPlayer)
      (trash-from-hand state :resPlayer "Director Haas")
...
(deftest film-critic-fetal-ai
  ...
  (do-game
    ...
    (let [fc (first (get-in @state [:hazPlayer :rig :resource]))]
      ...
      (is (= 1 (count (:hosted (refresh fc)))) "Agenda hosted on FC")
      (card-ability state :hazPlayer fc 1)
...
(deftest gang-sign
  ...
  (do-game
    ...
    (play-from-hand state :hazPlayer "Gang Sign")
    (play-from-hand state :hazPlayer "HQ Interface")
    (take-credits state :hazPlayer)
    (play-from-hand state :resPlayer "Hostile Takeover" "New remote")
    (score-agenda state :resPlayer (get-content state :remote1 0))
    (prompt-choice :hazPlayer "Gang Sign") ; simultaneous effect resolution
...
(deftest gene-conditioning-shoppe
  ...
  (do-game
   ...
   (take-credits state :resPlayer)
   (play-from-hand state :hazPlayer "Adjusted Chronotype")
   (is (not (core/has-flag? state :hazPlayer :persistent :genetics-trigger-twice)))
   (play-from-hand state :hazPlayer "Gene Conditioning Shoppe")
   (is (core/has-flag? state :hazPlayer :persistent :genetics-trigger-twice))
   (core/trash state :hazPlayer (get-in @state [:hazPlayer :rig :resource 1]))
   (is (not (core/has-flag? state :hazPlayer :persistent :genetics-trigger-twice)))))
...
(deftest gene-conditioning-shoppe-redundancy
  ...
  (do-game
   ...
   (take-credits state :resPlayer)
   (play-from-hand state :hazPlayer "Adjusted Chronotype")
   (let [adjusted-chronotype (get-in @state [:hazPlayer :rig :resource 0])]
     (is (not (core/has-flag? state :hazPlayer :persistent :genetics-trigger-twice)))
     (play-from-hand state :hazPlayer "Gene Conditioning Shoppe")
     (play-from-hand state :hazPlayer "Gene Conditioning Shoppe")
     (let [gcs1 (get-in @state [:hazPlayer :rig :resource 1])
           gcs2 (get-in @state [:hazPlayer :rig :resource 2])]
       (is (core/has-flag? state :hazPlayer :persistent :genetics-trigger-twice))
       (core/trash state :hazPlayer gcs1)
       (is (core/has-flag? state :hazPlayer :persistent :genetics-trigger-twice))
       (core/trash state :hazPlayer gcs2)
       (is (not (core/has-flag? state :hazPlayer :persistent :genetics-trigger-twice)))))))
...
(deftest globalsec-security-clearance
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (core/gain state :hazPlayer :link 2)
    (play-from-hand state :hazPlayer "Globalsec Security Clearance")
    (take-credits state :hazPlayer)
    (starting-hand state :resPlayer ["Hedge Fund"]) ; Hedge Fund on top
    (take-credits state :resPlayer)
    (is (:hazPlayer-phase-12 @state) "Runner in Step 1.2")
    (let [gsec (-> (get-runner) :rig :resource first)]
      (card-ability state :hazPlayer gsec 0)
...
(deftest grifter
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Grifter")
    (run-empty-server state :hq)
    (take-credits state :hazPlayer)
...
(deftest hard-at-work
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Hard at Work")
    (take-credits state :hazPlayer)
    (take-credits state :resPlayer)
...
(deftest investigative-journalism
  ...
  (do-game
    ...
    (is (empty? (get-in @state [:hazPlayer :rig :resource])) "Corp has no bad pub, couldn't install")
...
(deftest john-masanori
  ...
  (do-game
    ...
    (run-empty-server state "HQ")
    (prompt-choice :hazPlayer "Yes") ; trash crisium #2433
    (run-empty-server state "Archives")
...
(deftest joshua-b
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Joshua B.")
    (take-credits state :hazPlayer)
    (take-credits state :resPlayer)
...
(deftest kati-jones
  ...
  (do-game
    ...
    (let [kati (get-in @state [:hazPlayer :rig :resource 0])]
      (card-ability state :hazPlayer kati 0)
      ...
      (take-credits state :resPlayer)
      (card-ability state :hazPlayer (refresh kati) 1)
...
(deftest lewi-guilherme
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Lewi Guilherme")
    ...
    (play-from-hand state :hazPlayer "Lewi Guilherme")
    (take-credits state :hazPlayer)
    (core/lose state :hazPlayer :credit 8)
...
(deftest london-library
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (core/gain state :hazPlayer :click 2)
    (play-from-hand state :hazPlayer "London Library")
    (let [lib (get-in @state [:hazPlayer :rig :resource 0])]
      (is (= 0 (count (:hosted (refresh lib)))) "0 programs hosted")
      (card-ability state :hazPlayer lib 0) ; Install a non-virus program on London Library
      (prompt-select :hazPlayer (find-card "Femme Fatale" (:hand (get-runner))))
      (prompt-choice :hazPlayer "Done") ; Cancel out of Femme's bypass
      (is (= 1 (count (:hosted (refresh lib)))) "1 program hosted")
      (card-ability state :hazPlayer lib 0)
      (prompt-select :hazPlayer (find-card "Study Guide" (:hand (get-runner))))
      ...
      (let [sg (second (:hosted (refresh lib)))]
        ...
        (card-ability state :hazPlayer sg 1) ; Place 1 power counter
      ...
      (card-ability state :hazPlayer lib 0)
      (prompt-select :hazPlayer (find-card "Darwin" (:hand (get-runner)))) ; Darwin is a virus
      (is (= 3 (count (:hosted (refresh lib)))) "Still 3 programs hosted")
      (is (= 2 (:click (get-runner))) "Failed Darwin didn't use a click")
      (is (= 1 (count (:hand (get-runner)))))
      (card-ability state :hazPlayer lib 1) ; Add a program hosted on London Library to your Grip
      (prompt-card :hazPlayer nil)
      (prompt-select :hazPlayer (find-card "Study Guide" (:hosted (refresh lib))))
...
(deftest muertos-trashed
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Tollbooth" "HQ")
    (play-from-hand state :resPlayer "Ice Wall" "Archives")
    (take-credits state :resPlayer)
    (let [toll (get-ice state :hq 0)
          iw (get-ice state :archives 0)]
      (core/rez state :resPlayer iw)
      (core/move state :hazPlayer (find-card "Hedge Fund" (:hand (get-runner))) :deck)

      (play-from-hand state :hazPlayer "Muertos Gang Member")
      (prompt-select :resPlayer (refresh iw))
      (is (not (:rezzed (refresh iw))) "Ice Wall derezzed")
      (is (= 2 (count (:hand (get-runner)))) "2 cards in Runner's hand")
      (let [muer (get-in @state [:hazPlayer :rig :resource 0])]
        (card-ability state :hazPlayer muer 0)
        (is (= 3 (count (:hand (get-runner)))) "Runner drew a card from Muertos")
        (prompt-select :resPlayer toll)
...
(deftest muertos-reina
  ...
  (do-game
    ...
    (let [toll (get-ice state :hq 0)
      ...
      (core/rez state :resPlayer iw)
      (take-credits state :resPlayer)
      (core/lose state :resPlayer :credit 100)
      (core/move state :hazPlayer (find-card "Hedge Fund" (:hand (get-runner))) :deck)
...
(deftest net-mercur
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (core/gain state :hazPlayer :click 4 :credit 10)
    (play-from-hand state :hazPlayer "Silencer")
    (play-from-hand state :hazPlayer "Net Mercur")
    (play-from-hand state :hazPlayer "Ghost Runner")
    (let [sil (get-hardware state 0)
          nm (get-resource state 0)
          gr (get-resource state 1)]
      (card-ability state :hazPlayer gr 0)
      (is (empty? (:prompt (get-runner))) "No Net Mercur prompt from stealth spent outside of run")
      (run-on state :hq)
      (card-ability state :hazPlayer sil 0)
      (prompt-choice :hazPlayer "Place 1 [Credits]")
      ...
      (take-credits state :resPlayer)
      (run-on state :hq)
      (card-ability state :hazPlayer nm 0)
      (is (= "Net Mercur" (:title (:card (first (get-in @state [:hazPlayer :prompt]))))) "Net Mercur triggers itself"))))
...
(deftest network-exchange
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Network Exchange")
    (take-credits state :hazPlayer)
...
(deftest network-exchange-architect
  ...
  (do-game
    ...
    (take-credits state :resPlayer) ; corp has 7 credits
    (play-from-hand state :hazPlayer "Network Exchange")
    (take-credits state :hazPlayer)
    (let [architect (get-ice state :hq 0)]
      (core/rez state :resPlayer architect)
      (is (= 3 (:credit (get-corp))) "Corp has 3 credits after rez")
      (core/move state :resPlayer (find-card "Architect" (:hand (get-corp))) :deck)
      (card-subroutine state :resPlayer architect 0)
      (prompt-choice :resPlayer (find-card "Architect" (:deck (get-corp))))
...
(deftest neutralize-all-threats
  ...
  (do-game
    ...
    (run-empty-server state "HQ")
    (prompt-choice :hazPlayer "Card from hand")
    (prompt-choice :hazPlayer "OK") ; access first Hedge Fund
    (prompt-choice :hazPlayer "Card from hand")
    (prompt-choice :hazPlayer "OK") ; access second Hedge Fund
    (run-empty-server state "Server 1")
...
(deftest new-angeles-city-hall
  ...
  (do-game
    ...
    (let [nach (get-in @state [:hazPlayer :rig :resource 0])]
      ...
      (prompt-choice :hazPlayer "Steal")
      (is (= 1 (:agenda-point (get-runner))))
      (is (empty? (get-in @state [:hazPlayer :rig :resource])) "NACH trashed by agenda steal"))))
...
(deftest off-campus-apartment-simultaneous
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (starting-hand state :hazPlayer ["Street Peddler" "Off-Campus Apartment" "Underworld Contact"])
    (play-from-hand state :hazPlayer "Off-Campus Apartment")
    (let [oca (get-resource state 0)]
      (card-ability state :hazPlayer oca 0)
      (prompt-select :hazPlayer (find-card "Underworld Contact" (:hand (get-runner))))
      (is (= 2 (count (:hand (get-runner)))) "Drew a card from OCA")
      (card-ability state :hazPlayer oca 0)
      (prompt-select :hazPlayer (find-card "Street Peddler" (:hand (get-runner))))
      ...
      (prompt-choice :hazPlayer "Off-Campus Apartment")
      (is (= 2 (count (:hand (get-runner)))) "Drew a card from OCA"))))
...
(deftest off-campus-peddler
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (starting-hand state :hazPlayer ["Street Peddler" "Street Peddler" "Off-Campus Apartment"])
    (core/move state :hazPlayer (find-card "Street Peddler" (:hand (get-runner))) :deck {:front true})
    (play-from-hand state :hazPlayer "Off-Campus Apartment")
    (let [oca (get-resource state 0)]
      (card-ability state :hazPlayer oca 0)
      (prompt-select :hazPlayer (find-card "Street Peddler" (:hand (get-runner))))
      (prompt-choice :hazPlayer "Street Peddler")
      (let [ped1 (first (:hosted (refresh oca)))]
        (card-ability state :hazPlayer ped1 0)
        (prompt-card :hazPlayer (-> (get-runner) :prompt first :choices second)) ; choose Street Peddler
        (card-ability state :hazPlayer (refresh oca) 1)
        (prompt-select :hazPlayer (get-resource state 1))
        (let [ped2 (first (:hosted (refresh oca)))]
          (card-ability state :hazPlayer ped2 0)
          (prompt-card :hazPlayer (-> (get-runner) :prompt first :choices first)) ; choose Spy Camera
...
(deftest officer-frank
  ...
  (do-game
   ...
   (play-from-hand state :resPlayer "Swordsman" "Archives")
   (take-credits state :resPlayer)
   (starting-hand state :hazPlayer ["Officer Frank" "Skulljack" "Respirocytes" "Respirocytes" "Respirocytes" "Respirocytes"])
   (play-from-hand state :hazPlayer "Officer Frank")
   (card-ability state :hazPlayer (get-resource state 0) 0)
   (is (= 0 (count (:discard (get-corp)))) "Nothing discarded from HQ")
   (play-from-hand state :hazPlayer "Skulljack")
   (is (= 3 (count (:hand (get-runner)))) "Took 1 brain damage")
   (card-ability state :hazPlayer (get-resource state 0) 0)
   (is (= 0 (count (:discard (get-corp)))) "Nothing discarded from HQ")
   (let [sm (get-ice state :archives 0)]
     (run-on state :archives)
     (core/rez state :resPlayer sm)
     (card-subroutine state :resPlayer sm 0)
     (run-jack-out state))
...
(deftest paige-piper-frantic-coding
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (starting-hand state :hazPlayer ["Paige Piper" "Frantic Coding" "Frantic Coding"])
    (play-from-hand state :hazPlayer "Paige Piper")
    (prompt-choice :hazPlayer "No")
    (take-credits state :hazPlayer) ; now 8 credits
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Frantic Coding")
    (prompt-choice :hazPlayer "OK")
    (prompt-card :hazPlayer (find-card "Gordian Blade" (:deck (get-runner))))
    ...
    (play-from-hand state :hazPlayer "Frantic Coding")
    (prompt-choice :hazPlayer "OK")
    (prompt-card :hazPlayer (find-card "Ninja" (:deck (get-runner))))
...
(deftest patron
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Jackson Howard" "New remote")
    (take-credits state :resPlayer 2)
    (play-from-hand state :hazPlayer "Patron")
    (let [p (get-in @state [:hazPlayer :rig :resource 0])]
      (take-credits state :hazPlayer 3)
      (take-credits state :resPlayer)
      (prompt-choice :hazPlayer "Server 1")
...
(deftest patron-manual
  ...
  (do-game
    ...
    (starting-hand state :hazPlayer ["Patron" "Jak Sinclair"])
    (play-from-hand state :hazPlayer "Patron")
    (play-from-hand state :hazPlayer "Jak Sinclair")
    (take-credits state :hazPlayer)
    (let [p (get-resource state 0)
          j (get-resource state 1)]
      (take-credits state :resPlayer)
      (is (:hazPlayer-phase-12 @state) "Runner in Step 1.2")
      (card-ability state :hazPlayer p 0)
      (prompt-choice :hazPlayer "Archives")
      (card-ability state :hazPlayer j 0)
      (prompt-choice :hazPlayer "Archives")
      (run-successful state)
      (core/end-phase-12 state :hazPlayer nil)
  ...
  (do-game
    (new-game (default-corp)
              (default-runner [(qty "Professional Contacts" 3)
                               ...
                               (qty "Shiv" 2)]))
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Professional Contacts")
    (let [proco (get-in @state [:hazPlayer :rig :resource 0])]
      (card-ability state :hazPlayer proco 0)
...
(deftest rolodex
  ...
  (do-game
    ...
    (starting-hand state :hazPlayer ["Rolodex"])
    (is (= 1 (count (:hand (get-runner)))))
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Rolodex")
    ...
    (core/trash state :hazPlayer (get-resource state 0))
...
(deftest sacrificial-construct
  ...
  (do-game
    ...
    (play-from-hand state :hazPlayer "Sacrificial Construct")
    (play-from-hand state :hazPlayer "Cache")
    (play-from-hand state :hazPlayer "Motivation")
    (play-from-hand state :hazPlayer "Astrolabe")
    (take-credits state :hazPlayer)
    (core/trash state :hazPlayer (get-resource state 2))
    (is (empty? (:prompt (get-runner))) "Sac Con not prompting to prevent resource trash")
    (core/trash state :hazPlayer (get-program state 0))
    (card-ability state :hazPlayer (get-resource state 0) 0)
    (is (= 2 (count (:discard (get-runner)))) "Sac Con trashed")
    (is (= 1 (count (get-in @state [:hazPlayer :rig :program]))) "Cache still installed")
    (core/trash state :hazPlayer (get-hardware state 0))
    (card-ability state :hazPlayer (get-resource state 0) 0)
    (is (= 3 (count (:discard (get-runner)))) "Sac Con trashed")
    (is (= 1 (count (get-in @state [:hazPlayer :rig :hardware]))) "Astrolabe still installed")))
...
(deftest safety-first
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (starting-hand state :hazPlayer ["Safety First" "Safety First" "Cache"])
    (play-from-hand state :hazPlayer "Safety First")
    (is (= 3 (core/hand-size state :hazPlayer)) "Max hand size reduced by 2")
    (take-credits state :hazPlayer)
...
(deftest salsette-slums
  ...
  (do-game
    ...
    (core/move state :resPlayer (find-card "Hostile Infrastructure" (:deck (get-corp))) :hand)
    (core/move state :resPlayer (find-card "Tech Startup" (:deck (get-corp))) :hand)
    (core/move state :resPlayer (find-card "Thomas Haas" (:deck (get-corp))) :hand)
    (play-from-hand state :resPlayer "Tech Startup" "New remote")
    (play-from-hand state :resPlayer "Hostile Infrastructure" "New remote")
    (play-from-hand state :resPlayer "Thomas Haas" "New remote")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Salsette Slums")
    (play-from-hand state :hazPlayer "Salsette Slums")
    (core/gain state :hazPlayer :credit 2)
    (core/gain state :hazPlayer :click 4)
    (let [ts1 (get-content state :remote1 0)
      ...
      (core/rez state :resPlayer hostile2)
      (run-empty-server state "Server 1")
      (is (not (empty? (:prompt (get-runner)))) "Prompting to trash.")
      (card-ability state :hazPlayer salsette1 0)
      ...
      (card-ability state :hazPlayer salsette2 0)
      (is (not (empty? (:prompt (get-runner)))) "Still prompting to trash")
      (is (:run @state) "Run is still occurring")
      (prompt-choice :hazPlayer "No")
      ...
      (card-ability state :hazPlayer salsette1 1)
      ...
      (card-ability state :hazPlayer salsette2 1)
      (is (not (empty? (:prompt (get-runner)))) "Prompting the runner to choose a card")
      (prompt-select :hazPlayer (find-card "Thomas Haas" (:discard (get-corp))))
    ...
    (let [salsette1 (get-resource state 0)
          hostile2 (get-content state :remote2 0)]
      (card-ability state :hazPlayer salsette1 1)
      (prompt-select :hazPlayer (find-card "Hostile Infrastructure" (:discard (get-corp))))
...
(deftest spoilers
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Spoilers")
    (take-credits state :hazPlayer)
...
(deftest stim-dealer
  ...
  (do-game
    ...
    (play-from-hand state :hazPlayer "Stim Dealer")
    (take-credits state :hazPlayer)
    (take-credits state :resPlayer)
  ...
  (do-game
    (new-game (default-corp)
              (default-runner [(qty "Street Peddler" 1)
                               ...
                               (qty "Sure Gamble" 2)]))
    (take-credits state :resPlayer)
    (starting-hand state :hazPlayer ["Street Peddler" "Sure Gamble"])
    (play-from-hand state :hazPlayer "Street Peddler")
    (let [sp (get-in @state [:hazPlayer :rig :resource 0])]
      (is (= 3 (count (:hosted sp))) "Street Peddler is hosting 3 cards")
      (card-ability state :hazPlayer sp 0)
      (prompt-card :hazPlayer (find-card "Gordian Blade" (:hosted sp))) ; choose to install Gordian
      (is (= "Gordian Blade" (:title (get-in @state [:hazPlayer :rig :program 0])))
...
(deftest street-peddler-memory-units
  ...
  (do-game
    ...
    (let [sp (get-in @state [:hazPlayer :rig :resource 0])]
      ...
      (card-ability state :hazPlayer sp 0)
      (prompt-card :hazPlayer (first (:hosted sp))) ; choose to install Gordian
      (is (= "Corroder" (:title (get-in @state [:hazPlayer :rig :program 0])))
...
(deftest street-peddler-muertos-brain-chip
  ...
  (do-game
    ...
    (core/move state :hazPlayer (find-card "Muertos Gang Member" (:hand (get-runner))) :deck {:front true})
    (core/move state :hazPlayer (find-card "Brain Chip" (:hand (get-runner))) :deck {:front true})
    (core/move state :hazPlayer (find-card "Street Peddler" (:hand (get-runner))) :deck {:front true})
    (play-from-hand state :resPlayer "Jackson Howard" "New remote")
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Street Peddler")
    (core/gain state :hazPlayer :agenda-point 1)
    (let [jh (get-content state :remote1 0)
          sp (get-in @state [:hazPlayer :rig :resource 0])]
      (core/rez state :resPlayer jh)
      (card-ability state :hazPlayer sp 0)
      (prompt-card :hazPlayer (find-card "Street Peddler" (:hosted sp))) ; choose to another Peddler
...
(deftest street-peddler-in-play-effects
  ...
  (do-game
   ...
   (let [sp (get-in @state [:hazPlayer :rig :resource 0])]
     (card-ability state :hazPlayer sp 0)
     (prompt-card :hazPlayer (first (:hosted sp))) ; choose to install HQ Interface
...
(deftest street-peddler-parasite-1cr
  ...
  (do-game
    ...
    (play-from-hand state :resPlayer "Pop-up Window" "HQ")
    (take-credits state :resPlayer 2)
    (starting-hand state :hazPlayer ["Street Peddler"])
    (core/lose state :hazPlayer :credit 4) ; go down to 1 credit
    (is (= 1 (:credit (get-runner))) "Runner has 1 credit")
    (play-from-hand state :hazPlayer "Street Peddler")
    (let [sp (get-in @state [:hazPlayer :rig :resource 0])
          pu (get-ice state :hq 0)]
      (core/rez state :resPlayer pu)
      (card-ability state :hazPlayer sp 0)
      (prompt-card :hazPlayer (first (:hosted sp))) ; choose to install Parasite
      (is (= "Parasite" (:title (:card (first (get-in @state [:hazPlayer :prompt])))))
          "Parasite target prompt")
      (prompt-select :hazPlayer pu)
...
(deftest street-peddler-tech-trader
  ...
  (do-game
    ...
    (let [sp (get-in @state [:hazPlayer :rig :resource 0])]
      ...
      (card-ability state :hazPlayer sp 0)
      (prompt-card :hazPlayer (find-card "Tech Trader" (:hosted sp))) ; choose to install Tech Trader
      (is (= "Tech Trader" (:title (get-in @state [:hazPlayer :rig :resource 0])))
...
(deftest-pending street-peddler-trash-while-choosing-card
  ...
  (do-game
    ...
    (starting-hand state :hazPlayer ["Street Peddler" "Sure Gamble"])
    (play-from-hand state :hazPlayer "Street Peddler")
    (let [street-peddler (get-in @state [:hazPlayer :rig :resource 0])]
      (is (= 3 (count (:hosted street-peddler))) "Street Peddler is hosting 3 cards")
      (card-ability state :hazPlayer street-peddler 0)
      (trash-resource state "Street Peddler")
      (is (zero? (count (get-in @state [:hazPlayer :prompt])))))))
  ...
  (do-game
    (new-game (default-corp)
              (default-runner [(qty "Symmetrical Visage" 3)
                               ...
                               (qty "Fall Guy" 1)]))
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Symmetrical Visage")
    (is (= 3 (:credit (get-runner))))
...
(deftest symmetrical-visage-gcs
  ...
  (do-game
   ...
   (core/move state :hazPlayer (find-card "Fall Guy" (:hand (get-runner))) :deck)
   (core/click-draw state :hazPlayer nil)
   (is (= 3 (:credit (get-runner)))
  ...
  (do-game
   (new-game (default-corp [(qty "Data Mine" 3) (qty "Hedge Fund" 3)])
             (default-runner [(qty "Synthetic Blood" 3)
                              (qty "Sure Gamble" 3)
                              (qty "Fall Guy" 1)]))
   (play-from-hand state :resPlayer "Data Mine" "HQ")
   (play-from-hand state :resPlayer "Data Mine" "HQ")
   (take-credits state :resPlayer)
   (let [first-dm (get-ice state :hq 1)
         second-dm (get-ice state :hq 0)]
     (play-from-hand state :hazPlayer "Synthetic Blood")
     (run-on state "HQ")
     (core/rez state :resPlayer first-dm)
     (card-subroutine state :resPlayer first-dm 0)
     (is (= 4 (count (:hand (get-runner)))) "1 card drawn when receiving damage (1st time)")
     (run-continue state)
     (core/rez state :resPlayer second-dm)
     (card-subroutine state :resPlayer second-dm 0)
...
(deftest technical-writer
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (core/gain state :hazPlayer :click 2)
    (play-from-hand state :hazPlayer "Technical Writer")
    (let [tw (get-resource state 0)]
      ...
      (play-from-hand state :hazPlayer "Same Old Thing")
      (is (= 3 (get-counters (refresh tw) :credit)) "No credit gained for resource install")
      (card-ability state :hazPlayer tw 0)
...
(deftest the-source
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Sure Gamble")
    (play-from-hand state :hazPlayer "The Source")
    (run-empty-server state :remote1)
    (prompt-choice :hazPlayer "Yes") ; pay 3c extra to steal
    ...
    (let [ht (get-content state :remote2 0)]
      (core/advance state :resPlayer {:card (refresh ht)})
      (core/advance state :resPlayer {:card (refresh ht)})
      (core/score state :resPlayer {:card (refresh ht)})
  ...
  (do-game
    (new-game (default-corp)
              (default-runner [(qty "The Supplier" 1)
                               ...
                               (qty "Hedge Fund" 1)]))
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "The Supplier")
    (let [ts (get-in @state [:hazPlayer :rig :resource 0])]
      (card-ability state :hazPlayer ts 0)
      (prompt-select :hazPlayer (find-card "Plascrete Carapace" (:hand (get-runner))))
      (card-ability state :hazPlayer ts 0)
      (is (= 1 (count (-> @state :hazPlayer :prompt first :choices))))
      (prompt-select :hazPlayer (find-card "Utopia Shard" (:hand (get-runner))))
      ...
      (prompt-select :hazPlayer (find-card "Plascrete Carapace" (:hosted (refresh ts))))
...
(deftest the-supplier-kate-discount
  ...
  (do-game
    ...
    (let [ts (get-in @state [:hazPlayer :rig :resource 0])]
      (card-ability state :hazPlayer ts 0)
      (prompt-select :hazPlayer (find-card "Plascrete Carapace" (:hand (get-runner))))
      (core/lose state :hazPlayer :credit (:credit (get-runner)))
      (core/end-turn state :hazPlayer nil)
      (take-credits state :resPlayer)
      (prompt-select :hazPlayer (find-card "Plascrete Carapace" (:hosted (refresh ts))))
      (is (= 0 (:credit (get-runner))) "Kate discount applied")
      (is (= 1 (count (get-in @state [:hazPlayer :rig :resource]))) "Plascrete installed"))))
...
(deftest the-supplier-trashed
  ...
  (do-game
    ...
    (let [ts (get-resource state 0)]
      (card-ability state :hazPlayer ts 0)
      (prompt-select :hazPlayer (find-card "Brain Chip" (:hand (get-runner))))
      (is (= 4 (:memory (get-runner))) "Runner has 4 MU")
      (run-empty-server state "Server 1")
      (prompt-choice :hazPlayer "Steal")
      (take-credits state :hazPlayer)
...
(deftest tech-trader
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Tech Trader")
    (play-from-hand state :hazPlayer "Fall Guy")
    (is (= 4 (:credit (get-runner))))

    (let [fall (get-in @state [:hazPlayer :rig :resource 1])]
      (card-ability state :hazPlayer fall 1)
...
(deftest the-black-file
  ...
  (do-game
    ...
    (starting-hand state :resPlayer ["Vanity Project"])
    (core/gain state :resPlayer :agenda-point 3)
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "The Black File")
    (take-credits state :hazPlayer)
    (play-from-hand state :resPlayer "Vanity Project" "New remote")
    (score-agenda state :resPlayer (get-content state :remote1 0))
...
(deftest the-black-file-flatline
  ...
  (do-game
    (new-game (default-corp [(qty "Vanity Project" 3) (qty "Scorched Earth" 3)])
              (default-runner [(qty "The Black File" 1)]))
    (starting-hand state :resPlayer ["Vanity Project" "Scorched Earth"])
    (core/gain state :resPlayer :agenda-point 3)
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "The Black File")
    (take-credits state :hazPlayer)
...
(deftest tri-maf-contact
  ...
  (do-game
    ...
    (let [tmc (get-resource state 0)]
      (card-ability state :hazPlayer tmc 0)
      (is (= 5 (:credit (get-runner))) "Gained 2c")
      (is (= 2 (:click (get-runner))) "Spent 1 click")
      (card-ability state :hazPlayer tmc 0)
      (is (= 5 (:credit (get-runner))) "No credits gained; already used this turn")
      (core/move state :hazPlayer tmc :hand)
...
(deftest virus-breeding-ground-gain
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Virus Breeding Ground")
    (let [vbg (get-in @state [:hazPlayer :rig :resource 0])]
...
(deftest virus-breeding-ground-gain
  ...
  (do-game
    ...
    (let [hive (get-in @state [:hazPlayer :rig :program 0])
          vbg (get-in @state [:hazPlayer :rig :resource 0])]
      (is (= 1 (get-counters hive :virus)) "Hivemind starts with 1 counter")
      (is (zero? (get-counters vbg :virus)) "Virus Breeding Ground starts with 0 counters")
      (take-credits state :hazPlayer 3)
      (take-credits state :resPlayer)
      (is (= 1 (get-counters (refresh vbg) :virus)) "Virus Breeding Ground gains 1 counter per turn")
      (card-ability state :hazPlayer vbg 0)
      (prompt-select :hazPlayer hive)
...
(deftest wasteland
  ...
  (do-game
    ...
    (play-from-hand state :hazPlayer "Fall Guy")
    (play-from-hand state :hazPlayer "Fall Guy")
    (card-ability state :hazPlayer (get-resource state 1) 1)
...
(deftest zona-sul-shipping
  ...
  (do-game
    ...
    (take-credits state :resPlayer)
    (play-from-hand state :hazPlayer "Zona Sul Shipping")
    (let [zss (get-resource state 0)]
      ...
      (card-ability state :hazPlayer zss 0)
