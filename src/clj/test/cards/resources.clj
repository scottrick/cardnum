...
(deftest aesops-pawnshop
  ...
  (do-game
    ...
    (let [orig-credits (:credit (get-challenger))
          ap (get-in @state [:challenger :rig :radicle 0])
      ...
      (let [ap (get-in @state [:challenger :rig :radicle 0])
...
(deftest bank-job-manhunt
  ...
  (do-game
    ...
    (is (empty? (get-in @state [:challenger :rig :radicle])) "Bank Job discarded after all credits taken")
...
(deftest counter-surveillance
  ...
  (do-game
    ...
    (let [cs (get-in @state [:challenger :rig :radicle 0])]
...
(deftest daily-casts
  ...
  (do-game
    ...
    (let [dc (get-in @state [:challenger :rig :radicle 0])]
      ;; Number of credits
      ...
      (take-credits state :challenger)
      (take-credits state :contestant)
      (is (nil? (get-in @state [:challenger :rig :radicle 0]))))))
...
(deftest ddos
  ...
  (do-game
    ...
    (let [ddos (get-in @state [:challenger :rig :radicle 0])
...
(deftest film-critic-discarded-executives
  ...
  (do-game
    ...
    (let [fc (first (get-in @state [:challenger :rig :radicle]))]
      (run-empty-locale state "Locale 1")
...
(deftest gene-conditioning-shoppe
  ...
  (do-game
   ...
   (core/discard state :challenger (get-in @state [:challenger :rig :radicle 1]))
   (is (not (core/has-flag? state :challenger :persistent :genetics-trigger-twice)))))
...
(deftest gene-conditioning-shoppe-redundancy
  ...
  (do-game
   ...
   (take-credits state :contestant)
   (play-from-hand state :challenger "Adjusted Chronotype")
   (let [adjusted-chronotype (get-in @state [:challenger :rig :radicle 0])]
     (is (not (core/has-flag? state :challenger :persistent :genetics-trigger-twice)))
     (play-from-hand state :challenger "Gene Conditioning Shoppe")
     (play-from-hand state :challenger "Gene Conditioning Shoppe")
     (let [gcs1 (get-in @state [:challenger :rig :radicle 1])
           gcs2 (get-in @state [:challenger :rig :radicle 2])]
       (is (core/has-flag? state :challenger :persistent :genetics-trigger-twice))
...
(deftest globalsec-security-clearance
  ...
  (do-game
    ...
    (let [gsec (-> (get-challenger) :rig :radicle first)]
...
(deftest investigative-journalism
  ...
  (do-game
    ...
    (is (empty? (get-in @state [:challenger :rig :radicle])) "Contestant has no bad pub, couldn't place")
...
(deftest london-library
  ...
  (do-game
    ...
    (let [lib (get-in @state [:challenger :rig :radicle 0])]
...
(deftest muertos-discarded
  ...
  (do-game
    ...
    (let [toll (get-character state :hq 0)
      ...
      (let [muer (get-in @state [:challenger :rig :radicle 0])]
...
(deftest new-angeles-city-hall
  ...
  (do-game
    ...
    (let [nach (get-in @state [:challenger :rig :radicle 0])]
      ...
      (prompt-choice :challenger "Steal")
      (is (= 1 (:agenda-point (get-challenger))))
      (is (empty? (get-in @state [:challenger :rig :radicle])) "NACH discarded by agenda steal"))))
...
(deftest patron
  ...
  (do-game
    ...
    (let [p (get-in @state [:challenger :rig :radicle 0])]
      (take-credits state :challenger 3)
      (take-credits state :contestant)
      (prompt-choice :challenger "Locale 1")
...
(deftest professional-contacts
  ...
  (do-game
    ...
    (let [proco (get-in @state [:challenger :rig :radicle 0])]
...
(deftest street-peddler-ability
  ...
  (do-game
    ...
    (play-from-hand state :challenger "Street Peddler")
    (let [sp (get-in @state [:challenger :rig :radicle 0])]
...
(deftest street-peddler-muertos-brain-chip
  ...
  (do-game
    ...
    (let [jh (get-content state :party1 0)
          sp (get-in @state [:challenger :rig :radicle 0])]
...
(deftest street-peddler-tech-trader
  ...
  (do-game
    ...
    (let [sp (get-in @state [:challenger :rig :radicle 0])]
      ...
      (is (= "Tech Trader" (:title (get-in @state [:challenger :rig :radicle 0])))
...
(deftest-pending street-peddler-discard-while-choosing-card
  ...
  (do-game
    ...
    (starting-hand state :challenger ["Street Peddler" "Sure Gamble"])
    (play-from-hand state :challenger "Street Peddler")
    (let [street-peddler (get-in @state [:challenger :rig :radicle 0])]
...
(deftest the-supplier-ability
  ...
  (do-game
    ...
    (let [ts (get-in @state [:challenger :rig :radicle 0])]
...
(deftest the-supplier-kate-discount
  ...
  (do-game
    ...
    (let [ts (get-in @state [:challenger :rig :radicle 0])]
      ...
      (is (= 1 (count (get-in @state [:challenger :rig :radicle]))) "Plascrete placed"))))
...
(deftest tech-trader
  ...
  (do-game
    ...
    (let [fall (get-in @state [:challenger :rig :radicle 1])]
...
(deftest virus-breeding-ground-gain
  ...
  (do-game
    ...
    (let [vbg (get-in @state [:challenger :rig :radicle 0])]
...
(deftest virus-breeding-ground-gain
  ...
  (do-game
    ...
    (let [hive (get-in @state [:challenger :rig :resource 0])
          vbg (get-in @state [:challenger :rig :radicle 0])]
