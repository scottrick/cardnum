...
(deftest activist-support
  ...
  (do-game
    ...
    (take-credits state :contestant)
    (is (= 1 (:bad-publicity (get-contestant))) "Corp took 1 bad pub; had none")
    (take-credits state :hero)
    (is (= 1 (:tag (get-hero))) "Runner had 1 tag; didn't take another")
    (take-credits state :contestant)
    (is (= 1 (:bad-publicity (get-contestant))) "Corp had 1 bad pub; didn't take another")))
...
(deftest donut-taganes
  ...
  (do-game
    ...
    (take-credits state :hero)
    (is (= 8 (:credit (get-contestant))) "Corp has 8c")
    (play-from-hand state :contestant "Hedge Fund")
    (is (= 11 (:credit (get-contestant))) "Corp has 11c")))
...
(deftest eden-shard
  ...
  (do-game
    ...
    (card-ability state :hero (get-resource state 0) 0)
    (is (= 3 (count (:hand (get-contestant)))) "Corp drew 2 cards")
...
(deftest fan-site-eoi
  ...
  (do-game
    ...
    (prompt-select :contestant (find-card "Fan Site" (:scored (get-hero))))
    ...
    (is (find-card "Fan Site" (:scored (get-contestant))) "Fan Site swapped into Corp score area")
    (play-from-hand state :contestant "Hostile Takeover" "New remote")
    (score-agenda state :contestant (get-content state :remote2 0))
    (is (find-card "Fan Site" (:scored (get-contestant))) "Fan Site not removed from Corp score area")))
...
(deftest investigative-journalism
  ...
  (do-game
    ...
    (is (= 2 (:bad-publicity (get-contestant))) "Corp took 1 bad publicity")))
...
(deftest lewi-guilherme
  ...
  (do-game
    ...
    (is (= -1 (:hand-size-modification (get-contestant))) "Corp hand size reduced by 1")
    (take-credits state :hero)
    ...
    (is (= 0 (:hand-size-modification (get-contestant))) "Corp hand size normal again")
    (play-from-hand state :hero "Lewi Guilherme")
    (take-credits state :hero)
...
(deftest london-library
  ...
  (do-game
    (new-game (default-contestant) (default-hero [(qty "London Library" 1) (qty "Darwin" 1) (qty "Study Guide" 1)
...
(deftest network-exchange-architect
  ...
  (do-game
    ...
    (let [architect (get-ice state :hq 0)]
      ...
      (is (= 3 (:credit (get-contestant))) "Corp has 3 credits after rez")
      ...
      (is (= 3 (:credit (get-contestant))) "Corp has 7 credits"))))
...
(deftest salsette-slums
  ...
  (do-game
    ...
    (let [ts1 (get-content state :remote1 0)
      ...
      (prompt-select :hero (find-card "Thomas Haas" (:discard (get-contestant))))
    ...
    (let [salsette1 (get-resource state 0)
          hostile2 (get-content state :remote2 0)]
      (card-ability state :hero salsette1 1)
      (prompt-select :hero (find-card "Hostile Infrastructure" (:discard (get-contestant))))
...
(deftest street-peddler-muertos-brain-chip
  ...
  (do-game
    ...
    (let [jh (get-content state :remote1 0)
      ...
      (is (empty? (:prompt (get-contestant))) "Corp not prompted to rez Jackson")
...
(deftest wasteland
  ...
  (do-game
    ...
    (is (= 0 (:credit (get-hero))) "Gained nothing from Wasteland on contestant trash")
