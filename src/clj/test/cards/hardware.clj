...
(deftest archives-interface
  ...
  (do-game
    ...
    (prompt-choice :hero (find-card "Shock!" (:discard (get-contestant))))
  ...
  (do-game
    ...
    (let [arch (get-ice state :hq 0)
      ...
      (let [chip (get-hardware state 1)]
        ...
        (is (= 0 (count (:discard (get-contestant)))) "No Corp cards trashed")
        (card-ability state :hero chip 0)
  ...
  (do-game
    ...
    (let [pup (get-ice state :hq 0)]
      ...
      (let [kati (find-card "Kati Jones" (:hand (get-hero)))]
        (prompt-choice :contestant kati) ; Chronos Protocol takes precedence over Ribs on Corp turn
  ...
  (do-game
    ...
    (let [ag1 (get-content state :remote1 0)]
      ...
      (let [tt (get-in @state [:hero :rig :hardware 0])]
        ...
        (prompt-choice :hero "Yes")
        (prompt-select :hero (find-card "Project Vitruvius" (:scored (get-contestant))))
        (is (= 2 (:agenda-point (get-hero))) "Took Project Vitruvius from Corp")
        (is (= 0 (:agenda-point (get-contestant))) "Swapped Domestic Sleepers to Corp")))))
...
(deftest turntable-mandatory-upgrades
  ...
  (do-game
    ...
    (let [tt (get-in @state [:hero :rig :hardware 0])]
      ...
      (core/steal state :hero (find-card "Project Vitruvius" (:hand (get-contestant))))
      (is (prompt-is-card? :hero tt))
      (prompt-choice :hero "Yes")
      (prompt-select :hero (find-card "Mandatory Upgrades" (:scored (get-contestant))))
      ...
      (prompt-choice :hero "Yes")
      (prompt-select :hero (find-card "Project Vitruvius" (:scored (get-contestant))))
      (is (= 0 (:click (get-contestant))) "Corp doesn't gain a click on Runner's turn")
...
(deftest vigil
  ...
  (do-game
    ...
    (take-credits state :hero)
    (is (= (count (:hand (get-contestant))) (core/hand-size state :contestant)) "Corp hand filled to max")
    (take-credits state :contestant)
    (is (= 1 (count (:hand (get-hero)))) "Drew 1 card")
    (take-credits state :hero)
    (play-from-hand state :contestant "Hedge Fund")
    (take-credits state :contestant)
    (is (not= (count (:hand (get-contestant))) (core/hand-size state :contestant)) "Corp hand below max")
