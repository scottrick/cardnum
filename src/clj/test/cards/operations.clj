...
(deftest twenty-four-seven-news-cycle-swaps
  ...
  (do-game
    ...
    (core/steal state :hero (find-card "Profiteering" (:hand (get-contestant))))
    (prompt-choice :hero "Yes")
    (prompt-select :hero (find-card "Philotic Entanglement" (:scored (get-contestant))))
    ...
    (prompt-select :contestant (find-card "Philotic Entanglement" (:scored (get-hero))))
    (is (= 2 (count (:hand (get-hero)))))
...
(deftest accelerated-diagnostics
  ...
  (do-game
    ...
    (let [playarea (get-in @state [:contestant :play-area])
      ...
      (is (= 9 (:credit (get-contestant))) "Corp gained credits from Hedge Fund")
      (prompt-select :contestant bc)
      (prompt-select :contestant (refresh co))
      (is (= 15 (:credit (get-contestant))) "Corp gained 6 credits for Back Channels"))))
...
(deftest an-offer-you-cant-refuse
  ...
  (do-game
    ...
    (is (= 1 (:agenda-point (get-contestant))) "An Offer the Runner refused")
...
(deftest big-brother
  ...
  (do-game
    ...
    (is (= 1 (count (:hand (get-contestant)))) "Card not played because Runner has no tags")
...
(deftest economic-warfare
  ...
  (do-game
    ...
    (is (= 3 (count (:hand (get-contestant)))) "Corp still has 3 cards")
    (take-credits state :contestant)
...
(deftest exchange-of-information
  ...
  (do-game
    (new-game (default-contestant [(qty "Exchange of Information" 1)
      ...
      (core/steal state :hero (find-card "Project Beale" (:hand (get-contestant))))
      (core/steal state :hero (find-card "Explode-a-palooza" (:hand (get-contestant))))
      (take-credits state :hero)
      ...
      (prompt-select :contestant (find-card "Project Beale" (:scored (get-hero))))
      (prompt-select :contestant (find-card "Breaking News" (:scored (get-contestant))))
...
(deftest exchange-of-information-fifteen-minutes
  ...
  (do-game
    ...
    (take-credits state :contestant)
    (core/steal state :hero (find-card "15 Minutes" (:deck (get-contestant))))
    (take-credits state :hero)
    (is (= 2 (:agenda-point (get-contestant))))
    (is (= 1 (:agenda-point (get-hero))))
    (play-from-hand state :contestant "Exchange of Information")
    (prompt-select :contestant (find-card "15 Minutes" (:scored (get-hero))))
...
(deftest exchange-of-information-mandatory-upgrades
  ...
  (do-game
    ...
    (take-credits state :contestant)
    (core/gain state :hero :tag 1)
    (core/steal state :hero (find-card "Mandatory Upgrades" (:hand (get-contestant))))
    (take-credits state :hero)
    (is (= 3 (:agenda-point (get-contestant))))
    (is (= 2 (:agenda-point (get-hero))))
    ...
    (play-from-hand state :contestant "Exchange of Information")
    (prompt-select :contestant (find-card "Mandatory Upgrades" (:scored (get-hero))))
    ...
    (is (= 4 (:click-per-turn (get-contestant))))
    (play-from-hand state :contestant "Exchange of Information")
    (prompt-select :contestant (find-card "Global Food Initiative" (:scored (get-hero))))
...
(deftest election-day
  (do-game
    (new-game (default-contestant [(qty "Election Day" 7)])
                (default-hero))
      (is (= 6 (count (:hand (get-contestant)))) "Corp starts with 5 + 1 cards")
      ...
      (take-credits state :contestant)
      (take-credits state :hero)
      (is (= 2 (count (:hand (get-contestant)))) "Corp has now 1 + 1 cards before Election Day")
      (play-from-hand state :contestant "Election Day")
      (is (= 5 (count (:hand (get-contestant)))) "Corp has now 5 cards due to Election Day")))

(deftest hedge-fund
  (do-game
    (new-game (default-contestant) (default-hero))
    (is (= 5 (:credit (get-contestant))))
...
(deftest invasion-of-privacy
  ...
  (do-game
    ...
    (let [get-prompt (fn [] (first (#(get-in @state [:contestant :prompt]))))
      ...
      (prompt-choice :contestant (find-card "Fall Guy" (:hand (get-hero))))
...
(deftest midseason-replacements
  ...
  (do-game
    ...
    (is (= 3 (:click (get-contestant))) "Midseason precondition not met; Corp not charged a click")
    (play-from-hand state :contestant "Breaking News" "New remote")
    (take-credits state :contestant)
    (is (= 7 (:credit (get-contestant))))
...
(deftest scorched-earth-no-tag
  ...
  (do-game
    (new-game (default-contestant [(qty "Scorched Earth" 1)])
              (default-hero [(qty "Sure Gamble" 3) (qty "Lucky Find" 3)]))
    (play-from-hand state :contestant "Scorched Earth")
    (is (= 3 (:click (get-contestant))) "Corp not charged a click")
...
(deftest subcontract-scorched
  ...
  (do-game
    ...
    (is (not-empty (:prompt (get-contestant))) "Corp can now play second Subcontract operation")))
...
(deftest subcontract-terminal
  ...
  (do-game
    ...
    (is (empty? (:prompt (get-contestant))) "Corp does not have a second Subcontract selection prompt")))
...
(deftest subliminal-messaging
  ...
  (do-game
    ...
    (run-jack-out state)
    (take-credits state :hero)
    (is (empty? (get-in @state [:contestant :prompt])) "No prompt here because hero made a run last turn")
    (take-credits state :contestant)
    (is (= 2 (count (:hand (get-contestant)))))
    (is (= 1 (count (:discard (get-contestant)))) "1 Subliminal not returned because hero made a run last turn")))
...
(deftest success-jemison
  ...
  (do-game
    ...
    (play-from-hand state :contestant "Government Takeover" "New remote")
    (play-from-hand state :contestant "Success")
...
(deftest threat-assessment
  ...
  (do-game
    ...
    (prompt-select :contestant (find-card "Desperado" (-> (get-hero) :rig :hardware)))
    ...
    (play-from-hand state :contestant "Threat Assessment")
    (prompt-select :contestant (find-card "Corroder" (-> (get-hero) :rig :program)))
