...
(def ^:const meccgdb-image-url "https://netrunnerdb.com/card_image/")
...
(defn- make-image-url
  ...
  (if (:ffg_id set)
    ...
    (str meccgdb-image-url (:code card) ".png")))
...
(defn- card-image-file
  ...
  [card]
  (io/file "radicles" "public" "img" "cards" (str (:code card) ".png")))
...
(defn download-card-images
  ...
  [card-map]
   (let [img-dir (io/file "radicles" "public" "img" "cards")
         cards (vals card-map)]
