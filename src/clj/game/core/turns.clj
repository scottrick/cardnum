...
(defn- init-game-state
  ...
  [{:keys [players gameid use-dce use-ice eot-auto-save spectatorhands room] :as game}]
    ...
    (atom
      {:gameid gameid :log [] :active-player :challenger :end-turn true
       ...
       :options {:spectatorhands spectatorhands
                 ...
                 :use-dce use-dce :use-ice use-ice}
       :contestant {:user (:user contestant) :identity contestant-identity
                    ...
                    :opt-key false :fn12-key false :bonus-key false :minus-key false
                    :hand-size-base 0 :hand-size-modification 8 :hpf false
...
(defn load-game
  ...
  (let [state (atom game)
    ...
    (let [side :contestant]
      (swap! state assoc-in [side :opensite] true)
...
(defn eot-discard
  [state side args]
  (swap! state assoc-in [side :eot] true)
  (system-msg state side "ends their EOT discard.")
  (gain state side :click -5)
  )
