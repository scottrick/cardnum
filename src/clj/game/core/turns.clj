...
(defn- init-game-state
  ...
  (let [contestant (some #(when (= (:side %) "Contestant") %) players)
        ...
        contestant-quote (quotes/make-quote contestant-identity challenger-identity)
        challenger-quote (quotes/make-quote challenger-identity contestant-identity)]
    (atom
...
(defn create-deck
  ...
  ([deck user]
   (shuffle (mapcat #(map (fn [card]
                            (let [locale-card (or (locale-card (:title card) user) card)
                                  c (assoc (make-card locale-card) :art (:art card))]
     ...
     (when (<= (count (get-in @state [side :hand])) max-hand-size)
       ...
       (wait-for
         ...
         (do (when (= side :challenger)
             ...
             (doseq [card (all-placed state :contestant)]
               ;; Clear :revealed :this-turn as turn has ended
               (when (= :this-turn (:revealed card))
                 (update! state side (assoc card :revealed true)))
               ;; Update strength of all character every turn
               (when (character? card)
