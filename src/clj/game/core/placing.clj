...
(declare host in-play? place-locked? make-rid reveal run-flag? locale-list locale->zone set-prop system-msg
...
(defn deactivate
  ...
  ([state side card keep-counter]
   ...
   (when (and (:memoryunits card) (:placed card) (not (:facedown card)))
     (gain state :challenger :memory (:memoryunits card)))
   (when (and (find-cid (:cid card) (all-placed state side))
              (not (:disabled card))
              (or (:revealed card) (:placed card)))
...
(defn- contestant-can-place?
  ...
  (let [reason (contestant-can-place-reason state side card dest-zone)
    ...
    (case reason
      ...
      ;; failed region check
      :region
      (reason-toast (str "Cannot place " (:title card) ", limit of one Region per locale"))
...
(defn- contestant-place-site-agenda
  ...
  (let [prev-card (some #(when (#{"Hazard"} (:type %)) %) dest-zone)]
    (if (and (#{"Hazard"} (:type card))
             prev-card
             (not (:host card)))
      (resolve-ability state side eid {:prompt (str "The " (:title prev-card) " in " locale " will now be discarded.")
                                       :choices ["OK"]
                                       :effect (req (system-msg state :contestant (str "discards " (card-str state prev-card)))
                                                    (when (get-card state prev-card) ; make sure they didn't discard the card themselves
                                                    (discard state :contestant prev-card {:keep-locale-alive true})))}
                       nil nil)
      (effect-completed state side eid))))
   ...
   (cond
     ...
     (let [cdef (card-def card)
       ...
       (let [character-cost (if (and (character? card)
         ...
         (when (and (contestant-can-place? state side card dest-zone) (not (place-locked? state :contestant)))
           (if-let [cost-str (pay state side card end-cost action)]
             (do (let [c (-> card
                   ...
                   (let [moved-card (if host-card
                     ...
                     (when-completed (contestant-place-site-agenda state side moved-card dest-zone locale)
                                     (do (cond
  ...
  (let [req (:req (card-def card))
    ...
    (cond
      ...
      (and (has-subtype? card "Console")
           (some #(has-subtype? % "Console") (all-placed state :challenger)))
      :console
      ;; Placing not locked
      (place-locked? state side) :lock-place
      ;; Uniqueness check
...
(defn- handle-virus-counter-flag
  ...
  (if (and (has-subtype? placed-card "Virus")
           (pos? (get-in placed-card [:counter :virus] 0)))
...
(defn challenger-place
  ...
  ([state side eid card {:keys [host-card facedown] :as params}]
   (if (and (empty? (get-in @state [side :locked (-> card :zone first)]))
            (not (seq (get-in @state [:challenger :lock-place]))))
     (if-let [hosting (and (not host-card) (not facedown) (:hosting (card-def card)))]
       (continue-ability state side
                         ...
                         card nil)
       (do (trigger-event state "Challenger" :pre-place card facedown)
