...
(defn- contestant-place-message
  ...
  [state side card locale place-state cost-str args]
...
(defn- contestant-place-continue
  ...
  (let [cdef (card-def card)
    ...
    (when-not host-card
      (contestant-place-message state side c locale place-state cost-str args))
...
(defn challenger-can-place?
  ...
  ([state side card] (challenger-can-place? state side card false))
  ([state side card facedown]
   ...
   (if (and (empty? (get-in @state [side :locked (-> card :zone first)]))
     ...
     (if-let [hosting (and (not host-card) (not facedown) (:hosting (card-def card)))]
       (continue-ability state side
                         ...
                         card nil)
       (wait-for (trigger-event-simult state side :pre-place nil card facedown)
