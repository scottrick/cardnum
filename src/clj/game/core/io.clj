...
(defn card-str
  ...
  (str (if (card-is? card :side :resPlayer)
         ; Corp card messages
...
(defn name-zone
  ...
  [side zone]
  (match (vec zone)
         [:hand] (if (= side "HazPlayer") "Grip" "HQ")
         [:discard] (if (= side "HazPlayer") "Heap" "Archives")
         [:deck] (if (= side "HazPlayer") "Stack" "R&D")
         [:rig _] "Rig"
  ...
  (resolve-ability
    ...
    {:effect (req (let [existing (:counter target)
                        ...
                        c-type (cond (= 1 (count existing)) (first (keys existing))
                                     ...
                                     (and (card-is? target :side :hazPlayer) (has-subtype? target "Virus")) :virus)
    ...
    (when (<= (count args) 2)
      (if (= (first (first args)) \#)
        ...
        (case command
          ...
          "/facedown"   #(when (= %2 :hazPlayer)
                           (command-facedown %1 %2))
          "/roll"       #(command-roll %1 %2 value)
          "/tag"        #(swap! %1 assoc-in [%2 :tag] (max 0 value))
          "/take-brain" #(when (= %2 :hazPlayer) (damage %1 %2 :brain (max 0 value)))
          "/take-meat"  #(when (= %2 :hazPlayer) (damage %1 %2 :meat  (max 0 value)))
          "/take-net"   #(when (= %2 :hazPlayer) (damage %1 %2 :net   (max 0 value)))
          "/trace"      #(when (= %2 :resPlayer) (corp-trace-prompt %1
...
(defn corp-install-msg
  ...
  [card]
  (str "install " (if (:seen card) (:title card) "an unseen card") " from " (name-zone :resPlayer (:zone card))))
...
(defn turn-message
  ...
  (let [pre (if start-of-turn "started" "is ending")
        hand (if (= side :hazPlayer) "their Grip" "HQ")
