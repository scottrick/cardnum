...
(def cards-resources
  {"Aaron Marrón"
   (let [am {:effect (effect (add-counter card :power 2)
                             (system-msg :hazPlayer (str "places 2 power counters on Aaron Marrón")))}]
   ...
   "Activist Support"
   {:events
    {:resPlayer-turn-begins {:req (req (= 0 (:tag runner)))
                        :msg "take 1 tag"
                        :delayed-completion true
                        :effect (effect (tag-runner :hazPlayer eid 1))}
     :hazPlayer-turn-begins {:req (req (not has-bad-pub))
                          :msg "give the Corp 1 bad publicity"
                          :effect (effect (gain :resPlayer :bad-publicity 1))}}}

   "Adjusted Chronotype"
   {:events {:hazPlayer-loss {:req (req (and (some #{:click} target)
                                          (let [click-losses (filter #(= :click %) (mapcat first (turn-events state side :hazPlayer-loss)))]
                                            (or (empty? click-losses)
                                                (and (= (count click-losses) 1)
                                                     (has-flag? state side :persistent :genetics-trigger-twice))))))
                           :msg "gain [Click]" :effect (effect (gain :hazPlayer :click 1))}}}
   ...
   "Aesops Pawnshop"
   {:flags {:hazPlayer-phase-12 (req (>= 2 (count (all-installed state :hazPlayer))))}
    :abilities [{:effect (req (resolve-ability
                                state side
                                {:msg (msg "trash " (:title target) " and gain 3 [Credits]")
                                 :choices {:req #(and (card-is? % :side :hazPlayer) (installed? %) (not (card-is? % :cid (:cid card))))}
   ...
   {:in-play [:click 1 :click-per-turn 1]
    ...
    :effect (effect (gain :resPlayer :click-per-turn 1))
    :leave-play (effect (lose :resPlayer :click-per-turn 1))}
   ...
   {:flags {:hazPlayer-phase-12 (req (> (:credit runner) 0))}
    ...
    :events {:hazPlayer-turn-begins {:req (req (>= (get-in card [:counter :credit] 0) 6))
   ...
   {:prompt "How many power counters?"
    ...
    :abilities [{:counter-cost [:power 1]
                 ...
                 :effect (req (when (zero? (get-in card [:counter :power] 0))
                                (trash state :hazPlayer card {:unpreventable true})))
   ...
   {:data {:counter {:credit 12}}
    :abilities [{:cost [:click 1]
                 ...
                 :msg "gain 2 [Credits]"
                 :effect (req (gain state :hazPlayer :credit 2)
              ...
              :effect (req (let [bj card]
                             (when-not (:replace-access (get-in @state [:run :run-effect]))
                               (swap! state assoc-in [:run :run-effect :replace-access]
                                      {:effect (req (if (> (count (filter #(= (:title %) "Bank Job") (all-installed state :hazPlayer))) 1)
   ...
   {:in-play [:hand-size-modification 5]
    :events {:hazPlayer-turn-begins {:msg "lose [Click]" :effect (effect (lose :click 1))}}}
   ...
   {:flags {:hazPlayer-phase-12 (req true)}
    :abilities [{:req (req (and (:hazPlayer-phase-12 @state)
                                (not (seq (get-in @state [:hazPlayer :locked :discard])))))
                 :once :per-turn
                 :prompt "Choose a card in the Heap to remove from the game and gain 2 [Credits]"
                 :show-discard true
                 :choices {:req #(and (in-discard? %) (= (:side %) "HazPlayer"))}
   ...
   {:prompt "How many power counters?"
    ...
    :msg (msg "add " target " power counters")
    :effect (effect (add-counter card :power target))
    :events {:hazPlayer-turn-ends {:req (req (zero? (count (:hand runner))))
   ...
   {:abilities [{:cost [:click 2]
                ...
                {:label "Pay credits equal to strength of approached rezzed ICE to bypass it"
                 ...
                 :effect (effect (pay :hazPlayer card :credit (:current-strength current-ice)))}]}
   ...
   {:prevent {:damage [:meat]}
    ...
    :events {:hazPlayer-turn-ends
             {:req (req (pos? (:tag runner)))
              :msg "force the Corp to initiate a trace"
              :label "Trace 1 - If unsuccessful, Runner removes 1 tag"
              :trace {:base 1 :unsuccessful {:effect (effect (lose :hazPlayer :tag 1))
                                             :msg "remove 1 tag"}}}}}

   "Clan Vengeance"
   {:events {:pre-resolve-damage {:req (req (pos? (last targets)))
                                  :effect (effect (add-counter card :power 1)
                                                  (system-msg :hazPlayer (str "places 1 power counter on Clan Vengeance")))}}
   ...
   "Corporate Defector"
   {:events {:resPlayer-click-draw {:msg (msg "reveal " (-> target first :title))}}}

   "Councilman"
   {:implementation "Does not restrict Runner to Asset / Upgrade just rezzed"
    :events {:rez {:req (req (and (#{"Asset" "Upgrade"} (:type target))
                                  (can-pay? state :hazPlayer nil [:credit (rez-cost state :resPlayer target)])))
                   :effect (req (toast state :hazPlayer (str "Click Councilman to derez " (card-str state target {:visible true})
                                                          " that was just rezzed") "info")
                                (toast state :resPlayer (str "Runner has the opportunity to derez with Councilman.") "error"))}}
    :abilities [{:prompt "Select an asset or upgrade that was just rezzed"
                 :choices {:req #(and (rezzed? %)
                                      (or (is-type? % "Asset") (is-type? % "Upgrade")))}
                 :effect (req (let [c target
                                    creds (rez-cost state :resPlayer c)]
                                (when (can-pay? state side nil [:credit creds])
                                  (resolve-ability
                                    state :hazPlayer
                                    {:msg (msg "pay " creds " [Credit] and derez " (:title c) ". Councilman is trashed")
                                     :effect (req (lose state :hazPlayer :credit creds)
                                                  (derez state :resPlayer c)
                                                  (register-turn-flag!
                                                    state side card :can-rez
                                                    (fn [state side card]
                                                      (if (= (:cid card) (:cid c))
                                                        ((constantly false)
                                                         (toast state :resPlayer "Cannot rez the rest of this turn due to Councilman"))
                                                        true)))
                                                  (trash state side card {:unpreventable true}))}
                                   card nil))))}]}
   ...
   (let [trashme {:effect (effect (system-msg "trashes Dadiana Chacon and suffers 3 meat damage")
         ...
         ability {:once :per-turn
                  :msg "gain 1 [Credits]"
                  :req (req (< (get-in @state [:hazPlayer :credit]) 6))
                  :effect (req (gain state :hazPlayer :credit 1))}]
     {:effect (req (if (zero? (get-in @state [:hazPlayer :credit]))
                     (resolve-ability state side trashme card nil)
                     (add-watch state :dadiana
                                (fn [k ref old new]
                                  (when (and (not (zero? (get-in old [:hazPlayer :credit])))
                                             (zero? (get-in new [:hazPlayer :credit])))
   ...
   (let [ability {:label "Gain 1 [Credits] (start of turn)"
                  :msg "gain 1 [Credits]"
                  :once :per-turn
                  :req (req (and (>= (:memory runner) 2) (:hazPlayer-phase-12 @state)))
                  :effect (effect (gain :credit 1))}]
    {:flags {:drip-economy true}
    :abilities [ability]
    :events {:hazPlayer-turn-begins ability}})
   ...
   {:req (req (some #{:hq :rd :archives} (:successful-run runner-reg)))
    :abilities [{:req (req tagged) :cost [:click 1] :effect (effect (mill :resPlayer))
                 :msg "force the Corp to trash the top card of R&D"}]}
   ...
   {:abilities [{:msg "prevent the corp from rezzing the outermost piece of ice during a run on any server this turn"
                 :effect (effect
                           (register-turn-flag!
                             ...
                             (fn [state side card]
                               (if (and (ice? card)
                                        (= (count (get-in @state (concat [:resPlayer :servers] (:server (:run @state)) [:ices])))
                                           (inc (ice-index state card))))
                                 ((constantly false) (toast state :resPlayer "Cannot rez any outermost ICE due to DDoS." "warning"))
                                 true)))
                           (trash card {:cause :ability-cost}))}]}
   ...
   "Dr. Lovegood"
   {:flags {:hazPlayer-phase-12 (req (> (count (all-installed state :hazPlayer)) 1))}
    :abilities [{:req (req (:hazPlayer-phase-12 @state))
    ...
    :events {:hazPlayer-turn-ends nil}}

   "Drug Dealer"
   {:flags {:hazPlayer-phase-12 (req (some #(card-flag? % :drip-economy true) (all-installed state :hazPlayer)))}
    :abilities [{:label "Lose 1 [Credits] (start of turn)"
                 :msg (msg (if (= (get-in @state [:hazPlayer :credit]) 0) "lose 0 [Credits] (runner has no credits to lose)" "lose 1 [Credits]"))
                 :req (req (:hazPlayer-phase-12 @state))
                 :once :per-turn
                 :effect (effect (lose :credit 1))}]
    :events {:resPlayer-turn-begins {:msg (msg "draw " (if (= (count (get-in @state [:hazPlayer :deck])) 0)
                                                   "0 cards (runner's stack is empty)"
                                                   "1 card"))
                                :effect (effect (draw :hazPlayer 1))}
             :hazPlayer-turn-begins {:msg (msg "lose " (if (= (get-in @state [:hazPlayer :credit]) 0)
   ...
   (let [ability {:msg "draw 2 cards"
                  ...
                  :req (req (:hazPlayer-phase-12 @state))
                  :effect (req (draw state :hazPlayer 2)
                               (when (zero? (get-in card [:counter :power] 0))
                                 (trash state :hazPlayer card {:unpreventable true})))}]
   {:flags {:hazPlayer-turn-draw true
            :hazPlayer-phase-12 (req (< 1 (count (filter #(card-flag? % :hazPlayer-turn-draw true)
                                                      (cons (get-in @state [:hazPlayer :identity])
   ...
   "Eden Shard"
   (shard-constructor :rd "force the Corp to draw 2 cards" (req (draw state :resPlayer 2)))
   ...
   {:events {:agenda-scored {:msg "add it to their score area as an agenda worth 0 agenda points"
                             ...
                             :effect (effect (as-agenda :hazPlayer card 0))}}}

   "Fester"
   {:events {:purge {:msg "force the Corp to lose 2 [Credits] if able"
                     :effect (effect (pay :resPlayer card :credit 2))}}}
     ...
     {:implementation "Use hosting ability when presented with Access prompt for an agenda"
      :abilities [{:req (req (and (empty? (filter #(= "Agenda" (:type %)) (:hosted card)))
                  ...
                  {:cost [:click 2] :label "Add hosted agenda to your score area"
                   ...
                   :effect (req (let [c (move state :hazPlayer (get-agenda card) :scored)]
                                  (gain-agenda-point state :hazPlayer (get-agenda-points state :hazPlayer c))))
                   :msg (msg (let [c (get-agenda card)]
                               (str "add " (:title c) " to their score area and gain "
                                    (quantify (get-agenda-points state :hazPlayer c) "agenda point"))))}]})

   "Find the Truth"
   {:events {:post-runner-draw {:msg (msg "reveal that they drew: "
                                          (join ", " (map :title (get-in @state [:hazPlayer :register :most-recent-drawn]))))}
             :successful-run {:interactive (req true)
                              :optional
                              {:delayed-completion true
                               :req (req (= 1 (count (get-in @state [:hazPlayer :register :successful-run]))))
                               :prompt "Use Find the Truth to look at the top card of R&D?"
                               :yes-ability {:msg "look at the top card of R&D"
                                             :effect (req (prompt! state :hazPlayer card (str "The top card of R&D is "
   ...
   {:abilities [{:cost [:credit 2]
                 :req (req (some #(= (:side %) "ResPlayer") (map second (turn-events state :hazPlayer :damage))))
                 :msg "draw 1 card"
                 :effect (effect (draw))}]}
   ...
   {:events {:agenda-scored
             {:delayed-completion true
              ...
              :msg (msg "access " (quantify (get-in @state [:hazPlayer :hq-access]) "card") " from HQ")
                             ...
                             (let [from-hq (access-count state side :hq-access)]
                               (continue-ability
                                 ...
                                 (access-helper-hq
                                   ...
                                   (set (get-in @state [:resPlayer :servers :hq :content])))
                                 card nil))))}}}
   ...
   {:req (req (> (:link runner) 1))
    ...
    :abilities [{:msg "lose [Click] and look at the top card of R&D"
                 ...
                 :effect (effect (prompt! card (str "The top card of R&D is "
                                                    (:title (first (:deck corp)))) ["OK"] {}))}]
    :events {:hazPlayer-turn-begins {:req (req (get-in @state [:per-turn (:cid card)]))
                                  :effect (effect (lose :click 1))}}}

   "Grifter"
   {:events {:hazPlayer-turn-ends
             {:effect (req (let [ab (if (get-in @state [:hazPlayer :register :successful-run])
                             ...
                             (resolve-ability
                               ...
                               {:optional
                                {:prompt "Pay 4 [Credits] to prevent trashing Guru Davinder?"
                                 :player :hazPlayer
                                 :yes-ability {:effect (effect (lose :hazPlayer :credit 4)
   ...
   (shard-constructor :archives "access all cards in Archives" {:delayed-completion true}
                      (req (trash state side card {:cause :ability-cost})
                           ...
                           (when-completed (trigger-event-sync state side :pre-access :archives)
                                           (resolve-ability state :hazPlayer
                                                            (choose-access (get-in @state [:resPlayer :discard])
                                                                           '(:archives)) card nil))))
   ...
   "Human First"
   {:events {:agenda-scored {:msg (msg "gain " (get-agenda-points state :resPlayer target) " [Credits]")
                             :effect (effect (gain :hazPlayer :credit (get-agenda-points state :resPlayer target)))}
             :agenda-stolen {:msg (msg "gain " (get-agenda-points state :hazPlayer target) " [Credits]")
                             :effect (effect (gain :credit (get-agenda-points state :hazPlayer target)))}}}
   ...
   {:abilities [{:label "Prevent a \"when encountered\" ability on a piece of ICE"
                 ...
                 {:label "[Trash]: Install the top 3 cards of your Stack facedown"
                  ...
                  :effect (req (trash state side card {:cause :ability-cost})
                               (doseq [c (take 3 (get-in @state [:hazPlayer :deck]))]
   ...
   {:implementation "Credit use restriction is not enforced"
    :events {:rez {:req (req (ice? target))
                   ...
                   :effect (effect (add-counter :hazPlayer card :credit 1))}}
    :abilities [{:counter-cost [:credit 1]
                 :effect (effect (gain :credit 1))
   ...
   {:req (req has-bad-pub)
    :abilities [{:cost [:click 4] :msg "give the Corp 1 bad publicity"
                 :effect (effect (gain :resPlayer :bad-publicity 1) (trash card {:cause :ability-cost}))}]}
   ...
   "Jarogniew Mercs"
   {:effect (effect (tag-runner :hazPlayer eid 1)
                    (add-counter card :power (-> @state :hazPlayer :tag (+ 3))))
   ...
   "John Masanori"
   {:events {:successful-run {:req (req (= 1 (count (get-in @state [:hazPlayer :register :successful-run]))))
                              :msg "draw 1 card" :once-key :john-masanori-draw
                              :effect (effect (draw))}
             :unsuccessful-run {:req (req (= 1 (count (get-in @state [:hazPlayer :register :unsuccessful-run]))))
                                :delayed-completion true
   ...
   {:abilities [{:label "[Trash]: Break the first subroutine on the encountered piece of ice"
                 :req (req (and (:run @state) (rezzed? current-ice)))
                 :effect (effect (trash card {:cause :ability-cost})
                                 (system-msg :resPlayer
                                             (str "trashes Kongamato to break the first subroutine on "
                                                  (:title current-ice))))}]}

   "Laguna Velasco District"
   {:events {:hazPlayer-click-draw {:msg "draw 1 card" :effect (effect (draw))}}}
   ...
   {:flags {:drip-economy true ;; for Drug Dealer
            :hazPlayer-phase-12 (req (< 1 (count (filter #(card-flag? % :drip-economy true)
                                                      (all-installed state :hazPlayer)))))}

    ;; KNOWN ISSUE: :effect is not fired when Assimilator turns cards over.
    :effect (effect (lose :resPlayer :hand-size-modification 1))
    :leave-play (effect (gain :resPlayer :hand-size-modification 1))
    :abilities [(assoc-in ability [:req] (req (:hazPlayer-phase-12 @state)))]
    :events {:hazPlayer-turn-begins ability}})
              ...
              :effect (req (when (not= target "None")
                           ...
                           (if (not-empty cards)
                             (let [tobottom (remove #(= % target) cards)]
                               (continue-ability state side (reorder-choice :hazPlayer :resPlayer tobottom '()
   ...
   {:abilities [{:cost [:click 1]
                 ...
                 :delayed-completion true
                 :effect (req (show-wait-prompt state :resPlayer "Runner to choose card to keep")
   ...
   {:data {:counter {:credit 16}}
    :abilities [{:cost [:click 1]
                 ...
                 :msg "gain 4 [Credits]"
                 :effect (req (gain state :hazPlayer :credit 4)
   ...
   {:abilities [{:cost [:click 5 :forfeit]
                 ...
                 :effect (req (if (not (empty? (:scored corp)))
                                (do (show-wait-prompt state :hazPlayer "Corp to decide whether or not to prevent Liberated Chela")
                                    (resolve-ability
                                      state side
                                      {:prompt (msg "Forfeit an agenda to prevent Liberated Chela from being added to Runner's score area?")
                                       :choices ["Yes" "No"] :player :resPlayer
                                                               ...
                                                               (if (= target "Yes")
                                                                 {:player :resPlayer
                                                                  ...
                                                                  :effect (effect (forfeit target)
                                                                                  (move :hazPlayer card :rfg)
                                                                                  (clear-wait-prompt :hazPlayer))}
                                                                 {:effect (effect (as-agenda :hazPlayer card 2)
                                                                                  (clear-wait-prompt :hazPlayer))
   ...
   {:abilities [{:label "Install a non-virus program on London Library"
    ...
    :events {:hazPlayer-turn-ends {:effect (req (doseq [c (:hosted card)]
   ...
   {:flags {:hazPlayer-turn-draw true
            :hazPlayer-phase-12 (req (some #(card-flag? % :hazPlayer-turn-draw true) (all-installed state :hazPlayer)))}
    :events {:hazPlayer-turn-begins ability}
    :abilities [ability]})
   ...
   {:abilities [{:cost [:click 1]
                 ...
                 :effect (req (draw state side 2)
                              (let [drawn (get-in @state [:hazPlayer :register :most-recent-drawn])]
                                (resolve-ability
                                  state side
   ...
   {:effect (req (resolve-ability
                   ...
                   {:prompt "Select a card to derez"
                    :choices {:req #(and (= (:side %) "ResPlayer")
                                         (not (is-type? % "Agenda"))
                                         (:rezzed %))}
   ...
   {:abilities [{:counter-cost [:credit 1]
    ...
    :events {:spent-stealth-credit
             {:req (req (and (:run @state)
              ...
              :once :per-run
              :delayed-completion true
              :effect (effect (show-wait-prompt :resPlayer "Runner to use Net Mercur")
                              (continue-ability
                                {:prompt "Place 1 [Credits] on Net Mercur or draw 1 card?"
                                 ...
                                 :effect (req (if (= target "Draw 1 card")
                                                (do (draw state side)
                                                    ...
                                                    (system-msg state :hazPlayer (str "uses Net Mercur to draw 1 card")))
                                                (do (add-counter state :hazPlayer card :credit 1)
                                                    (clear-wait-prompt state :resPlayer)
                                                    (system-msg state :hazPlayer (str "places 1 [Credits] on Net Mercur")))))}
                               card nil))}}}
   ...
   {:msg "increase the install cost of non-innermost ICE by 1"
    :events {:pre-corp-install {:req (req (is-type? target "ICE"))
                                :effect (req (when (pos? (count (:dest-zone (second targets))))
                                               (install-cost-bonus state :resPlayer [:credit 1])))}}}
   ...
   {:in-play [:hq-access 1]
    :events {:pre-access {:req (req (and (= target :archives)
             ...
             :access {:effect (req (swap! state assoc-in [:hazPlayer :register :force-trash] false))}
             :pre-trash {:req (req (let [cards (map first (turn-events state side :pre-trash))]
                                     (and (empty? (filter #(:trash %) cards))
                                          (number? (:trash target)))))
                         :once :per-turn
                         :effect (req (swap! state assoc-in [:hazPlayer :register :force-trash] true))}}}
   ...
   {:flags {:hazPlayer-install-draw true}
    ...
    :events {:hazPlayer-install {:req (req (= (:cid card) (:cid (:host target))))
                              :effect (effect (draw))}}}

   "Officer Frank"
   {:abilities [{:cost [:credit 1]
                 :req (req (some #(= :meat %) (map first (turn-events state :hazPlayer :damage))))
                 :msg "force the Corp to trash 2 random cards from HQ"
                 :effect (effect (trash-cards :resPlayer (take 2 (shuffle (:hand corp))))
                                 (trash card {:cause :ability-cost}))}]}
   ...
   {:abilities [{:cost [:click 1]
                 ...
                 :effect (req (let [c (first (get-in @state [:hazPlayer :deck]))]
   ...
   {:effect (req (add-watch state :order-of-sol
    ...
    :events {:hazPlayer-turn-begins {:req (req (= (:credit runner) 0)) :msg "gain 1 [Credits]"
                                  :effect (req (gain state :hazPlayer :credit 1)
                                               (swap! state assoc-in [:per-turn (:cid card)] true))}
             :resPlayer-turn-begins {:req (req (= (:credit runner) 0)) :msg "gain 1 [Credits]"
                                :effect (req (gain state :hazPlayer :credit 1)
                                             (swap! state assoc-in [:per-turn (:cid card)] true))}
             :hazPlayer-install {:silent (req (pos? (:credit runner)))
                                      ...
                                      :effect (req (let [target (Integer/parseInt target)]
                                                     ...
                                                     (when (> (int target) 0)
                                                       (system-msg state side (str "trashes "
                                                                                   ...
                                                                                   " of " title)))))}}}))]
     {:events {:hazPlayer-install {:req (req (first-event? state side :hazPlayer-install))
                                :delayed-completion true
   ...
   (let [ability {:prompt "Choose a server for Patron" :choices (req (conj servers "No server"))
                  :req (req (and (not (click-spent? :hazPlayer state)) (not (used-this-turn? (:cid card) state))))
                  ...
                  {:label "X[Credit]: Remove counters from a hosted card"
                   ...
                   :effect (req (let [paydowntarget target
                                  ...
                                  (resolve-ability
                                    ...
                                    card nil)))}]
      :events {:hazPlayer-turn-begins remove-counter}})
   ...
   {:req (req (some #{:hq} (:successful-run runner-reg)))
    :abilities [{:prompt "Select a rezzed card with a trash cost"
                 ...
                 :effect (req (let [c target]
                                ...
                                (let [cost (trash-cost state :hazPlayer c)]
   ...
   "Power Tap"
   {:events {:trace {:msg "gain 1 [Credits]" :effect (effect (gain :hazPlayer :credit 1))}}}
   ...
   {:delayed-completion true
    ...
    :effect (req (show-wait-prompt state :resPlayer "Runner to rearrange the top cards of their Stack")
                 (let [from (take 5 (:deck runner))]
                   (if (pos? (count from))
                     (continue-ability state side (reorder-choice :hazPlayer :resPlayer from '()
                                                                  (count from) from) card nil)
                     (do (clear-wait-prompt state :resPlayer)
                         (effect-completed state side eid card)))))
    :trash-effect {:effect (effect (system-msg :hazPlayer (str "trashes "
   ...
   {:abilities [{:req (req (and (not (install-locked? state side))
                                (some #(is-type? % "Program") (all-installed state :hazPlayer))))
                 :cost [:click 1]
   ...
   "Safety First"
   {:in-play [:hand-size-modification -2]
    :events {:hazPlayer-turn-ends {:req (req (< (count (:hand runner)) (hand-size state :hazPlayer)))
                                :msg (msg "draw a card")
                                :effect (effect (draw 1))}}}

   "Salvaged Vanadis Armory"
   {:events {:damage
             {:effect (req (show-wait-prompt state :resPlayer "Runner to use Salvaged Vanadis Armory")
                           (resolve-ability state :hazPlayer
                                            {:optional
                                             {:prompt "Use Salvaged Vanadis Armory?"
                                              :yes-ability {:msg (msg "force the Corp to trash the top " (get-turn-damage state :hazPlayer) " cards of R&D and trash itself")
                                                            :effect (effect (mill :resPlayer (get-turn-damage state :hazPlayer))
                                                                            (clear-wait-prompt :resPlayer)
                                                                            (trash card {:unpreventable true}))}
                                              :no-ability {:effect (effect (clear-wait-prompt :resPlayer))}}}
                                            card nil))}}}

   "Salsette Slums"
   {:flags {:slow-trash (req true)}
    :implementation "Will not trigger Maw when used on card already trashed (2nd ability)"
    :events {:hazPlayer-install
             {:req (req (= card target))
              :silent (req true)
             ...
             {:req (req (and (:slums-active card)
                             ...
                             (:trash target)
                             (= (:side target) "ResPlayer")))
              :effect (req (toast state :hazPlayer (str "Click Salsette Slums to remove " (:title target)
    ...
    :abilities [{:label "Remove the currently accessed card from the game instead of trashing it"
                 :req (req (let [c (:card (first (get-in @state [:hazPlayer :prompt])))]
                             (if-let [trash-cost (trash-cost state side c)]
                               (if (can-pay? state :hazPlayer nil :credit trash-cost)
                                 (if (:slums-active card)
                                   true
                                   ((toast state :hazPlayer "Can only use a copy of Salsette Slums once per turn.") false))
                                 ((toast state :hazPlayer (str "Unable to pay for " (:title c) ".")) false))
                               ((toast state :hazPlayer "Not currently accessing a card with a trash cost.") false))))
                 :msg (msg (let [c (:card (first (get-in @state [:hazPlayer :prompt])))]
                             (str "pay " (trash-cost state side c) " [Credits] and remove " (:title c) " from the game")))
                 :effect (req (let [c (:card (first (get-in @state [:hazPlayer :prompt])))]
                                (deactivate state side c)
                                (move state :resPlayer c :rfg)
                                (pay state :hazPlayer card :credit (trash-cost state side c))
                 ...
                 :effect (effect (resolve-ability
                                   {; only allow targeting cards that were trashed this turn -- not perfect, but good enough?
                                    :choices {:req #(some (fn [c] (= (:cid %) (:cid c)))
                                                          (map first (turn-events state side :hazPlayer-trash)))}
                                    :msg (msg "remove " (:title target) " from the game")
                                    :effect (req (deactivate state side target)
                                                 (trigger-event state side :no-trash target)
                                                 (move state :resPlayer target :rfg)
                                                 (update! state side (dissoc card :slums-active)))}
                                   card nil))}]}

   "Same Old Thing"
   {:abilities [{:cost [:click 2]
                 :req (req (and (not (seq (get-in @state [:hazPlayer :locked :discard])))
   ...
   {:events {:agenda-scored {:interactive (req true)
                             :msg "trash the top card of R&D" :effect (effect (mill :resPlayer))}}}

   "Starlight Crusade Funding"
   {:msg "ignore additional costs on Double events"
    :effect (req (swap! state assoc-in [:hazPlayer :register :double-ignore-additional] true))
    :events {:hazPlayer-turn-begins
             {:msg "lose [Click] and ignore additional costs on Double events"
              :effect (req (lose state :hazPlayer :click 1)
                           (swap! state assoc-in [:hazPlayer :register :double-ignore-additional] true))}}
    :leave-play (req (swap! state update-in [:hazPlayer :register] dissoc :double-ignore-additional))}
   ...
   "Symmetrical Visage"
   {:events {:hazPlayer-click-draw {:req (req (genetics-trigger? state side :hazPlayer-click-draw))
                                 :msg "gain 1 [Credits]"
                                 :effect (effect (gain :credit 1))}}}
   ...
   {:abilities [{:label "Draw 1 card for each Corp bad publicity"
    ...
    :events {:play-operation
             {:req (req (or (has-subtype? target "Black Ops")
              ...
              :effect (req (show-wait-prompt state :resPlayer "Runner to use Tallie Perrault")
                           (resolve-ability
                             state :hazPlayer
                             {:optional
                              {:prompt "Use Tallie Perrault to give the Corp 1 bad publicity and take 1 tag?"
                               ...
                               :yes-ability {:msg "give the Corp 1 bad publicity and take 1 tag"
                                             :delayed-completion true
                                             :effect (effect (gain :resPlayer :bad-publicity 1)
                                                             (tag-runner :hazPlayer eid 1)
                                                             (clear-wait-prompt :resPlayer))}
                               :no-ability {:effect (effect (clear-wait-prompt :resPlayer))}}}
                            card nil))}}}

   "Tech Trader"
   {:events {:hazPlayer-trash {:req (req (and (= side :hazPlayer) (= (second targets) :ability-cost)))
                            :msg "gain 1 [Credits]"
                            :effect (effect (gain :credit 1))}}}

   "Technical Writer"
   {:events {:hazPlayer-install {:silent (req true)
   ...
   {:abilities [{:cost [:click 1]
                ...
                {:label "Gain [Click]"
                 :counter-cost [:power 1]
                 :req (req (= (:active-player @state) :hazPlayer))
   ...
   {:in-play [:link 1]
    :events {:agenda-scored {:req (req (or (has-subtype? target "Initiative")
                             ...
                             :trace {:base 1
                                     :unsuccessful {:effect (effect (gain :resPlayer :bad-publicity 1)
                                                                    (system-msg :resPlayer (str "takes 1 bad publicity")))}}}}}

   "The Black File"
   {:msg "prevent the Corp from winning the game unless they are flatlined"
    :effect (req (swap! state assoc-in [:resPlayer :cannot-win-on-points] true))
    :events {:hazPlayer-turn-begins
             {:effect (req (if (>= (get-in card [:counter :power] 0) 2)
                             (do (move state side (dissoc card :counter) :rfg)
                                 (swap! state update-in [:resPlayer] dissoc :cannot-win-on-points)
                                 (system-msg state side "removes The Black File from the game")
                                 (gain-agenda-point state :resPlayer 0))
                             (add-counter state side card :power 1)))}}
    :trash-effect {:effect (req (swap! state update-in [:resPlayer] dissoc :cannot-win-on-points)
                                (gain-agenda-point state :resPlayer 0))}
    :leave-play (req (swap! state update-in [:resPlayer] dissoc :cannot-win-on-points)
                     (gain-agenda-point state :resPlayer 0))}
   ...
   {:in-play [:link 1]
    :abilities [{:msg (msg "give +2 strength to " (:title target))
                 ...
                 :effect (effect (update! (assoc card :hai-target target))
                                 (trash (get-card state card) {:cause :ability-cost})
                                 (update-breaker-strength target))}]
    :events {:hazPlayer-turn-ends nil :resPlayer-turn-ends nil :pre-breaker-strength nil}
    :trash-effect {:effect
                   (effect (register-events
                             (let [hai {:effect (effect (unregister-events card)
                                                        (update! (dissoc card :hai-target))
                                                        (update-breaker-strength (:hai-target card)))}]
                               {:hazPlayer-turn-ends hai :resPlayer-turn-ends hai
   ...
   {:flags {:drip-economy true}  ; not technically drip economy, but has an interaction with Drug Dealer
    ...
    :suppress {:hazPlayer-turn-begins {:req (req (= (:cid target) (:supplier-installed (get-card state card))))}}
    :events {:hazPlayer-turn-begins ability
             :hazPlayer-turn-ends {:req (req (:supplier-installed card))
   ...
   {:events {:agenda-stolen {:effect (effect (update! (assoc card :agenda-stolen true)))
             ...
             :run-ends {:effect (req (when (and (not (:agenda-stolen card))
                                                (#{:hq :rd} target))
                                       (add-counter state side card :power 1)
                                       (system-msg state :hazPlayer (str "places a power counter on " (:title card))))
   ...
   {:effect (req (lose state :hazPlayer :credit :all :run-credit :all)
                 (add-watch state :theophilius-bagbiter
                            (fn [k ref old new]
                              (let [credit (get-in new [:hazPlayer :credit])]
                                (when (not= (get-in old [:hazPlayer :credit]) credit)
                                  (swap! ref assoc-in [:hazPlayer :hand-size-base] credit))))))
    :leave-play (req (remove-watch state :theophilius-bagbiter)
                     (swap! state assoc-in [:hazPlayer :hand-size-base] 5))}
   ...
   (let [ability {:label "Gain 1 [Credits] (start of turn)"
                  :msg "gain 1 [Credits]"
                  :once :per-turn
                  :req (req (and (>= (:link runner) 2) (:hazPlayer-phase-12 @state)))
                  :effect (effect (gain :credit 1))}]
   {:flags {:drip-economy true}
    :abilities [ability]
    :events {:hazPlayer-turn-begins ability}})
   ...
   (shard-constructor :hq "force the Corp to discard 2 cards from HQ at random"
                      (effect (trash-cards :resPlayer (take 2 (shuffle (:hand corp))))))
   ...
   "Wasteland"
   {:events {:hazPlayer-trash {:req (req (and (first-installed-trash-own? state :hazPlayer)
                                           (installed? target)
                                           (= (:side target) "HazPlayer")))
                            :effect (effect (gain :credit 1))
                            :msg "gain 1 [Credits]"}}}
   ...
   (let [ability {:msg (msg "reveal " (:title (first (:deck corp))) " on the top of R&D")
                  ...
                  :once :per-turn
                  :req (req (:hazPlayer-phase-12 @state))
                  :effect (effect (show-wait-prompt :hazPlayer "Corp to decide whether or not to draw with Woman in the Red Dress")
                                  (resolve-ability
                                    {:optional
                                     {:player :resPlayer
                                      :prompt (msg "Draw " (:title (first (:deck corp))) "?")
                                      :yes-ability {:effect (effect (clear-wait-prompt :hazPlayer)
                                                                    (system-msg (str "draws " (:title (first (:deck corp)))))
                                                                    (draw))}
                                      :no-ability {:effect (effect (clear-wait-prompt :hazPlayer)
   ...
   {:flags {:hazPlayer-turn-draw true
    ...
    :events {:hazPlayer-turn-begins {:effect (req (lose state side :click 1)
   ...
   "Zona Sul Shipping"
   {:events {:hazPlayer-turn-begins {:effect (effect (add-counter card :credit 1))}}
    :abilities [{:cost [:click 1]
                 :msg (msg "gain " (get-in card [:counter :credit] 0) " [Credits]")
                 :label "Take all credits"
                 :effect (effect (gain :credit (get-in card [:counter :credit] 0))
