  ...
  {"Algernon"
   {:events
    {:challenger-turn-begins
     {:req (req (can-pay? state :challenger nil [:credit 2]))
      :optional
      {:prompt (msg "Pay 2 [Credits] to gain [Click]")
       :player :challenger
       :yes-ability {:msg "gain [Click]"
                     :effect (req (gain state :challenger :click 1)
                                  (update! state :challenger (assoc-in (get-card state card) [:special :used-algernon] true)))
                     :cost [:credit 2]}}}
     :challenger-turn-ends
     {:async true
      :effect (req (if (get-in card [:special :used-algernon])
                     (do
                       (update! state :challenger (dissoc-in card [:special :used-algernon]))
                       (if-not (:successful-run challenger-reg)
                         (do
                           (system-msg state :challenger "discards Algernon because a successful run did not occur")
   ...
   {:implementation "Bankroll gains credits automatically."
    ...
    :abilities [{:label "[Discard]: Take all credits from Bankroll"
                 :async true
                 :effect (req (let [credits-on-bankroll (get-counters card :credit)]
                                (wait-for (discard state :challenger card {:cause :ability-cost})
                                          (take-credits state :challenger credits-on-bankroll)
                                          (system-msg state :challenger (str "discards Bankroll and takes "
   ...
   {:hosting {:req #(and (character? %) (can-host? %))}
    :events {:pass-character {:req (req (same-card? target (:host card)))
                        :msg "gain 2 credits"
