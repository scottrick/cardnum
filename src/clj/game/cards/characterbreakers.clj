  ...
  {"Abagnale"
   ...
   {:abilities [(break-sub 2 0 "Code Gate")]
    :events {:card-moved {:silent (req true)
                          :req (req (and (= "Challenger" (:side target))
                                         (= [:hand] (or (:zone target)
                                                        (:previous-zone target)))))
                          :effect (effect (update-breaker-strength card))}
             :challenger-draw {:silent (req true)
                           :req (req (when-let [drawn (-> @state :challenger :register :most-recent-drawn first)]
                                       (= [:hand] (or (:zone drawn)
                                                      (:previous-zone drawn)))))
                           :effect (effect (update-breaker-strength card))} }
    :strength-bonus (req (-> @state :challenger :hand count -))}
   ...
   (auto-characterbreaker ["Sentry"]
                    {:abilities [(strength-pump 1 1)
                                 {:label "Hide a Sentry being encountered"
                                  :cost [:credit 6]
   ...
   (auto-characterbreaker ["Sentry"]
                    {:abilities [(break-sub 2 2 "Sentry")
                                 (strength-pump 2 4)
                                 {:label "Hide a Sentry and return Golden to your Grip"
                                  :cost [:credit 2]
                                  :req (req (and (revealed? current-character) (has-subtype? current-character "Sentry")))
   ...
   (auto-characterbreaker ["Sentry"]
                    {:abilities [(break-sub 1 2 "Sentry")
                                 (strength-pump 2 3)
                                 {:label "Host Ika on a piece of Character"
                                  :prompt (msg "Host Ika on a piece of Character")
                                  :cost [:credit 2]
   ...
   (auto-characterbreaker ["Code Gate"]
                    {:abilities [(break-sub 1 1 "Code Gate")
                                 (strength-pump 3 3)
                                 {:label "Hide a Code Gate and return Peregrine to your Grip"
                                  :cost [:credit 2]
   ...
   (auto-characterbreaker ["Barrier"]
                    {:abilities [(break-sub 1 1 "Barrier")
                                 (strength-pump 2 2)
                                 {:label "Hide a Barrier and return Saker to your Grip"
                                  :cost [:credit 2]
   ...
   (auto-characterbreaker ["Barrier"]
                    {:abilities [(break-sub 1 2 "Barrier" (effect (update! (assoc-in card [:special :tycoon-used] true))))
                                 (strength-pump 2 3)]
                     :events {:pass-character {:req (req (get-in card [:special :tycoon-used]))
                                         :msg "give the Contestant 2 [Credits]"
