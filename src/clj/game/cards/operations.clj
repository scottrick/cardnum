...
(def card-definitions
  {"24/7 News Cycle"
   ...
   (let [abhelp (fn ab [n total]
                  {:prompt "Select an Advertisement to place and reveal" :show-discard true
                   :async true
                   :choices {:req #(and (= (:side %) "Contestant")
                                        ...
                                        (or (in-hand? %)
                                            (= (:zone %) [:discard])))}
                   :effect (req (wait-for
                                  (contestant-place state side target nil {:place-state :revealed})
                                  (if (< n total)
     ...
     {:prompt "How many Advertisements?"
      :async true
      :choices :credit
      :msg (msg "place and reveal " target " Advertisements")
   ...
   {:async true
    :prompt "Choose a locale" :choices ["Archives" "R&D" "HQ"]
   ...
   {:async true
    :req (req (let [h (:hand contestant)
                ...
                (if (some #(= (:cid %) (:cid card)) p)
                  (>= (count h) 2)
                  (>= (count h) 3))))
    :effect (req (system-msg state side "discards all cards in HQ due to Audacity")
   ...
   "Back Channels"
   {:async true
    :prompt "Select an placed card in a locale to discard"
    ...
    :msg (msg "discard " (card-str state target) " and gain "
              (* 3 (get-counters target :advancement)) " [Credits]")}

   "Bad Times"
   {:implementation "Any required resource discarding is manual"
    :req (req tagged)
    ...
    :effect (req (lose state :challenger :memory 2)
                 (when (neg? (available-mu state))
                   ...
                   (system-msg state :challenger "must discard resources to free up [mu]")))
   ...
   {:async true
    ...
    :effect (req (let [t (:tag challenger)]
                   (continue-ability state side
                     {:prompt (msg "Choose a Challenger card with an place cost of " t " or less to discard")
   ...
   "Biased Reporting"
   (letfn [(num-placed [state t]
             (count (filter #(is-type? % t) (all-active-placed state :challenger))))]
     {:async true
      :req (req (not-empty (all-active-placed state :challenger)))
      :prompt "Choose a card type"
      :choices ["Radicle" "Hazard" "Resource"]
      :effect (req (let [t target
                         n (num-placed state t)]
                     (show-wait-prompt state :contestant "Challenger to choose cards to discard")
                     (wait-for
                       (resolve-ability
                         state :challenger
                         {:prompt (msg "Choose any number of cards of type " t " to discard")
                          :choices {:max n
                                    :req #(and (placed? %) (is-type? % t))}
                          :effect (req (doseq [c targets]
                                         (discard state :challenger c {:unpreventable true}))
   ...
   {:implementation "Hide is manual"
    :choices {:req #(and (character? %)
                         (has-subtype? % "Bioroid")
                         (placed? %)
                         (not (revealed? %)))}
    :msg (msg "reveal " (card-str state target {:visible true}) " at no cost")
   ...
   {:choices {:req #(and (= (:side %) "Contestant")
                         ...
                         (in-hand? %))}
    :async true
    :effect (req (contestant-place state side eid target nil {:no-place-cost true :place-state :revealed-no-cost}))}
   ...
   {:choices {:req #(and (is-type? % "Agenda")
    ...
    :effect (req (let [agenda target]
                   (continue-ability
                     state side {:prompt (str "Choose a locale to place " (:title agenda))
                                 :choices (placeable-locales state agenda)
                                 :effect (req (contestant-place state side agenda target {:place-state :face-up})
   ...
   (let [gain-cred-effect {:msg "gain 1 [Credits]"
     ...
     {:implementation "Credit gain mandatory to save on wait-prompts, adjust credits manually if credit not wanted."
      :events {:challenger-place gain-cred-effect
               :challenger-discard (assoc gain-cred-effect :req (req (placed? target)))}})
   ...
   (let [shuffle-two {:async true
         ...
         discard-from-hq {:async true
                        :prompt "Select up to 2 cards in HQ to discard"
                        :choices {:max 2
                                  :req #(and (= (:side %) "Contestant")
                                             (in-hand? %))}
                        :msg (msg "discard " (quantify (count targets) "card") " from HQ")
                        :effect (req (wait-for
                                       (discard-cards state side targets nil)
     ...
     {:async true
      ...
      :effect (effect (gain-credits :challenger 2)
                      (continue-ability discard-from-hq card nil))})

   "Diversified Portfolio"
   (letfn [(number-of-non-empty-parties [state]
             (count (filter #(not (empty? %))
                            (map #(:content (second %))
                                 (get-parties state)))))]
     {:msg (msg "gain " (number-of-non-empty-parties state)
                " [Credits]")
      :effect (effect (gain-credits (number-of-non-empty-parties state)))})

   "Divert Power"
   {:async true
    :prompt "Select any number of cards to hide"
    :choices {:req #(and (placed? %)
                         (revealed? %))
              :max (req (count (filter revealed? (all-placed state :contestant))))}
    :effect (req (doseq [c targets]
                   (hide state side c))
                   ...
                   (continue-ability
                     ...
                     {:async true
                      ...
                      :choices {:req #(and (placed? %)
                                           ...
                                           (not (revealed? %))
                                           (not (is-type? % "Agenda")))}
                      :effect (effect (reveal-cost-bonus discount)
                                      (reveal eid target nil))}
                     card nil)))}
   ...
   "Eavesdrop"
   (let [new-sub {:label "[Eavesdrop]: Trace 3 - Give the Challenger 1 tag"}]
     {:implementation "On encounter effect is manual"
      :sub-effect {:label "Give the Challenger 1 tag"
                   :trace {:base 3
      ...
      :events {:reveal {:req (req (= (:cid target) (:cid (:host card))))
                     :effect (req (add-extra-sub state :contestant (:cid card) (get-card state target) -1 new-sub))}}})
   ...
   {:req (req (->> (get-in @state [:contestant :hand])
                   ...
                   pos?))
    :async true
    :msg (msg "discard all cards in HQ and draw 5 cards")
    :effect (effect (discard-cards (get-in @state [:contestant :hand]))
                    (draw eid 5 nil))}
   ...
   "Enforcing Loyalty"
   {:trace {:base 3
            :label "Discard a card not matching the faction of the Challenger's identity"
            :successful
            {:async true
             :effect (req (let [f (:faction (:identity challenger))]
                            (continue-ability
                              state side
                              {:prompt "Select an placed card not matching the faction of the Challenger's identity"
                               :choices {:req #(and (placed? %)
                                                    (not= f (:faction %))
   ...
   "Fast Break"
   {:async true
    :req (req (-> challenger :scored count pos?))
    :effect
    (req (let [X (-> challenger :scored count)
               draw {:async true
               ...
               place-cards (fn place-cards
                               [locale n]
                               {:prompt "Select a card to place"
                                :choices {:req #(and (= (:side %) "Contestant")
                                                     (not (is-type? % "Operation"))
                                                     (in-hand? %)
                                                     (seq (filter (fn [c] (= locale c)) (contestant-place-list state %))))}
                                :effect (req (wait-for
                                               (contestant-place state side target locale nil)
                                               (let [locale (if (= "New party" locale)
                                                              (-> (turn-events state side :contestant-place)
                                                                  ffirst :zone second zone->name)
                                                              locale)]
                                                 (if (< n X)
                                                   (continue-ability state side (place-cards locale (inc n)) card nil)
                                                   (effect-completed state side eid)))))
                                :cancel-effect (effect (effect-completed eid))})
               select-locale {:async true
                              :prompt "Place cards in which locale?"
                              :choices (req (conj (vec (get-party-names state)) "New party"))
                              :effect (effect (continue-ability (place-cards target 1) card nil))}]
           (gain-credits state :contestant X)
           (wait-for (resolve-ability state side draw card nil)
                     (continue-ability state side select-locale card nil))))}
   ...
   "Financial Collapse"
   {:async true
    :req (req (and (>= (:credit challenger) 6) (seq (filter #(is-type? % "Radicle") (all-active-placed state :challenger)))))
    :effect (req (let [rcount (count (filter #(is-type? % "Radicle") (all-active-placed state :challenger)))]
                   (if (pos? rcount)
                     (do (show-wait-prompt state :contestant "Challenger to discard a radicle to prevent Financial Collapse")
                         (continue-ability
                           state side
                           {:prompt (msg "Discard a radicle to prevent Financial Collapse?")
                            ...
                            :effect (effect (continue-ability
                                              (if (= target "Yes")
                                                {:player :challenger
                                                 ...
                                                 :choices {:req #(and (is-type? % "Radicle") (placed? %))}
                                                 :effect (req (discard state side target {:unpreventable true})
                                                              (system-msg state :challenger
                                                                          (str "discards " (:title target)
   ...
   "Foxfire"
   {:trace {:base 7
            :successful {:prompt "Select 1 card to discard"
                         :not-distinct true
                         :choices {:req #(and (placed? %)
                                              (or (has-subtype? % "Virtual")
                                                  (has-subtype? % "Link")))}
                         :msg "discard 1 virtual radicle or link"
                         :effect (effect (discard target)
                                         (system-msg (str "discards " (:title target))))}}}

   "Freelancer"
   {:req (req tagged)
    :msg (msg "discard " (join ", " (map :title (sort-by :title targets))))
    :choices {:max 2
              :req #(and (placed? %)
                         (is-type? % "Radicle"))}
    :effect (effect (discard-cards :challenger targets))}

   "Friends in High Places"
   (let [fhelper (fn fhp [n] {:prompt "Select a card in Archives to place with Friends in High Places"
                              :priority -1
                              :async true
                              :show-discard true
                              :choices {:req #(and (= (:side %) "Contestant")
   ...
   "Game Changer"
   {:effect (req (gain state side :click (count (:scored challenger)))
                 (move state side (first (:play-area contestant)) :rfg))}

   "Genotyping"
   {:async true
    :effect (effect (mill :contestant 2)
                    (system-msg "discards the top 2 cards of R&D")
   ...
   "Hangeki"
   {:req (req (last-turn? state :challenger :discarded-card))
    :async true
    :prompt "Choose an placed Contestant card"
    :choices {:req #(and (= (:side %) "Contestant")
                         (placed? %))}
    :effect (effect (show-wait-prompt :contestant "Challenger to resolve Hangeki")
                    (continue-ability
                      {:optional
                       {:player :challenger
                        :async true
                        ...
                        {:effect (req (clear-wait-prompt state :contestant)
                                      (wait-for (access-card state side target)
                                                (move state :contestant (find-latest state card) :rfg)
                                                (system-msg state :contestant "removes Hangeki from the game")
                                                (effect-completed state side eid)))}
                        :no-ability
                        {:msg "add it to the Challenger's score area as an agenda worth -1 agenda point"
                         :effect (effect (clear-wait-prompt :contestant)
                                         (as-agenda :challenger eid (find-latest state card) -1))}}}
                      card targets))}
   ...
   (letfn [(hr-final [chosen original]
     ...
     {:additional-cost [:mill 1]
      :async true
      :msg "discard the top card of R&D, draw 3 cards, and add 3 cards in HQ to the top of R&D"
   ...
   {:trace {:base 5
            :successful {:choices {:req #(and (placed? %)
                         ...
                         :msg "add an placed non-virtual card to the Challenger's grip"
   ...
   "Hellion Alpha Test"
   {:req (req (last-turn? state :challenger :placed-radicle))
    :trace {:base 2
            :successful {:msg "add a Radicle to the top of the Stack"
                         :choices {:req #(and (placed? %)
                                              (is-type? % "Radicle"))}
   ...
   {:req (req (last-turn? state :challenger :discarded-card))
    :trace {:base 2
            :label "Trace 2 - Discard 2 placed non-resource cards or take 1 bad publicity"
            :successful {:choices {:max (req (min 2 (count (filter #(or (:facedown %)
                                                                        (not (is-type? % "Resource")))
                                                                   (concat (all-placed state :contestant)
   ...
   "Housekeeping"
   {:events {:challenger-place {:player :challenger
                              :prompt "Select a card from your Grip to discard for Housekeeping" :once :per-turn
                              :choices {:req #(and (= (:side %) "Challenger")
                                                   (in-hand? %))}
                              :msg (msg "force the Challenger to discard " (:title target) " from their Grip")
   ...
   {:prompt "Select a card to place from Archives or HQ"
    ...
    :choices {:req #(and (not (is-type? % "Operation"))
                         (= (:side %) "Contestant")
                         (#{[:hand] [:discard]} (:zone %)))}
    :effect (effect (contestant-place target nil {:no-place-cost true}))
   ...
   (letfn [(iop [x]
             {:async true
              :req (req (->> (:hand challenger)
                             (filter #(or (is-type? % "Radicle")
                                          (is-type? % "Event")))
                             count
                             pos?))
              :prompt "Choose a radicle or event to discard"
              :msg (msg "discard " (:title target))
              :choices (req (cancellable
                              (filter #(or (is-type? % "Radicle")
                                           (is-type? % "Event"))
              ...
              :effect (req (discard state side target)
                           (if (pos? x)
                             ...
                             (effect-completed state side eid)))})]
     {:trace {:base 2
              :successful {:msg "reveal the Challenger's Grip and discard up to X radicles or events"
                           :effect (req (let [x (- target (second targets))]
                                          (system-msg
                                            state :contestant
                                            (str "reveals the Challenger's Grip ( "
                                                 (join ", " (map :title (sort-by :title (:hand challenger))))
                                                 " ) and can discard up to " x " radicles or events"))
                                          (continue-ability state side (iop (dec x)) card nil)))}
              :unsuccessful {:msg "take 1 bad publicity"
                             :effect (effect (gain-bad-publicity :contestant 1))}}})
   ...
   {:async true
    ...
    :effect (effect (gain-credits 4)
                    (continue-ability {:player :contestant
                                       ...
                                       :async true
                                       :msg (msg (contestant-place-msg target))
                                       :effect (effect (contestant-place eid target nil nil))}
                                      card nil))}

   "Liquidation"
   {:async true
    :effect (req (let [n (count (filter #(not (is-type? % "Agenda")) (all-active-placed state :contestant)))]
                   (continue-ability state side
                     {:prompt "Select any number of revealed cards to discard"
                      :choices {:max n
                                :req #(and (revealed? %)
                                           (not (is-type? % "Agenda")))}
                      :msg (msg "discard " (join ", " (map :title (sort-by :title targets))) " and gain " (* (count targets) 3) " [Credits]")
                      :effect (req (doseq [c targets]
                                     (discard state side c))
   ...
   {:msg (msg "gain " (* 2 (count (filter #(pos? (+ (get-counters % :advancement) (:extra-advance-counter % 0)))
                                          (get-all-placed state)))) " [Credits]")
    :effect (effect (gain-credits (* 2 (count (filter #(pos? (+ (get-counters % :advancement) (:extra-advance-counter % 0)))
                                                      (get-all-placed state))))))}

   "MCA Informant"
   {:implementation "Challenger must deduct 1 click and 2 credits, then discard host manually"
    :req (req (not-empty (filter #(has-subtype? % "Connection") (all-active-placed state :challenger))))
    :prompt "Choose a connection to host MCA Informant on it"
    :choices {:req #(and (= (:side %) "Challenger") (has-subtype? % "Connection") (placed? %))}
    ...
    :leave-play (req (swap! state update-in [:challenger :tag] dec)
                     (system-msg state :contestant "discards MCA Informant"))}
   ...
   {:prompt "Select a card to place from HQ"
    :choices {:req #(and (#{"Site" "Agenda" "Region"} (:type %))
                         (= (:side %) "Contestant")
                         (in-hand? %))}
    :effect (req (contestant-place state side (assoc target :advance-counter 3) "New party")
                 (effect-completed state side eid)
                 (let [tgtcid (:cid target)]
                   (register-turn-flag! state side
                     card :can-reveal
                     (fn [state side card]
                       (if (= (:cid card) tgtcid)
                         ((constantly false) (toast state :contestant "Cannot reveal due to Mushin No Shin." "warning"))
                         true)))
                   (register-turn-flag! state side
                     card :can-score
                     (fn [state side card]
   ...
   {:req (req (some #(and (character? %)
                          (revealed? %))
                    (all-placed state :contestant)))
    :prompt "Select a revealed piece of character to discard"
    :choices {:req #(and (character? %)
                         (revealed? %))}
    :async true
    :effect (req (let [i (character-index state target)
                       ...
                       titles (->> (conj (vec reveal) (first r))
                                   (filter identity)
                                   (map :title))]
                   (wait-for (discard state :contestant target nil)
                             (do
                               (system-msg state side (str "uses Mutate to discard " (:title target)))
                               (when (seq titles)
                                 (system-msg state side (str "reveals " (clojure.string/join ", " titles) " from R&D")))
                               (if-let [character (first r)]
                                 (let [newcharacter (assoc character :zone (:zone target) :revealed true)
                                   ...
                                   (card-init state side newcharacter {:resolve-effect false
                                                                 :init-data true})
                                   (system-msg state side (str "uses Mutate to place and reveal " (:title newcharacter) " from R&D at no cost"))
                                   (trigger-event state side :reveal newcharacter))
                                 (system-msg state side (str "does not find any Character to place from R&D")))
                               (shuffle! state :contestant :deck)
                               (effect-completed state side eid)))))}
   ...
   {:async true
    :effect (req (if (empty? (:hand challenger))
                   (do (gain state :contestant :click 2)
                       ...
                       (effect-completed state side eid))
                   (do (show-wait-prompt state :contestant "Challenger to decide whether or not to discard a card from their Grip")
                       (continue-ability state side
                         {:optional
                          {:prompt "Discard 1 random card from your Grip?"
                           :player :challenger
                           :yes-ability {:effect (effect (discard-cards :challenger (take 1 (shuffle (:hand challenger))))
                                                         (clear-wait-prompt :contestant))}
   ...
   {:additional-cost [:tag 1]
    ...
    :async true
    :effect (effect (continue-ability
                      {:prompt "Select an placed card to discard"
                       :choices {:req placed?}
                       :msg (msg "remove 1 Challenger tag and discard " (:title target))
                       :effect (effect (discard target))}
                     card nil))}

   "Oversight AI"
   {:implementation "Discarding Character is manual"
    :choices {:req #(and (character? %) (not (revealed? %)) (= (last (:zone %)) :characters))}
    :msg (msg "reveal " (:title target) " at no cost")
    :effect (effect (reveal target {:ignore-cost :all-costs})
                    (host (get-card state target) (assoc card :zone [:discard] :seen true :condition true)))}
   ...
   "Peak Efficiency"
   {:msg (msg "gain " (reduce (fn [c locale]
                                (+ c (count (filter (fn [character] (:revealed character)) (:characters locale)))))
   ...
   {:req (req (last-turn? state :challenger :made-run))
    :trace {:base 2
            :successful {:msg "discard 1 piece of hazard"
                         :async true
                         :effect (req (let [max-cost (- target (second targets))]
                                      ...
                                      (system-msg
                                        state :contestant
                                        (str "discards 1 piece of hazard with place cost less than or equal to "
                                             (- target (second targets)))))}}}

   "Power Shutdown"
   {:req (req (last-turn? state :challenger :made-run))
    :prompt "Discard how many cards from the top R&D?"
    :choices {:number (req (apply max (map :cost (filter #(or (= "Resource" (:type %)) (= "Hazard" (:type %))) (all-active-placed state :challenger)))))}
    :msg (msg "discard " target " cards from the top of R&D")
    :async true
    :effect (req (mill state :contestant target)
                 (let [n target]
                   (continue-ability state :challenger
                                     {:prompt "Select a Resource or piece of Hazard to discard"
   ...
   (letfn [(place-card [chosen]
            {:prompt "Select a party locale"
             :choices (req (conj (vec (get-party-names state)) "New party"))
             :async true
             :effect (effect (contestant-place (assoc chosen :advance-counter 3) target {:no-place-cost true}))})]
     {:async true
      :prompt "Choose a piece of Character in HQ to place"
      :choices {:req #(and (in-hand? %) (= (:side %) "Contestant") (character? %))}
      :msg "place an Character from HQ and place 3 advancements on it"
      :cancel-effect (req (effect-completed state side eid))
      :effect (effect (continue-ability (place-card target) card nil))})

   "Product Recall"
   {:async true
    :prompt "Select a revealed site or region to discard"
    :choices {:req #(and (revealed? %)
                         (or (is-type? % "Site")
                             (is-type? % "Region")))}
    :effect (req (let [tcost (modified-discard-cost state side target)]
                   (wait-for (discard state side target {:unpreventable true})
                             (do (gain-credits state :contestant tcost)
                                 (system-msg state side (str "uses Product Recall to discard " (card-str state target)
   ...
   {:req (req tagged)
    ...
    (letfn [(choose-card [state cards]
              (let [allowed-cards (filter #(some #{"New party"} (placeable-locales state %))
                                          cards)]
                {:prompt "Select an agenda, site, or region to place"
                 :choices (cons "None" allowed-cards)
                 :async true
                 :effect (req (if-not (or (= target "None") (character? target) (is-type? target "Operation"))
                                (continue-ability state side (place-card target) card nil)
                                (system-msg state side "does not place an site, agenda, or region"))
                              (effect-completed state side eid)
                              (clear-wait-prompt state :challenger))}))
            (place-card [chosen]
             {:prompt "Select a party locale"
              ...
              :effect (effect (clear-wait-prompt :challenger)
                              (contestant-place (move state side chosen :play-area) target))})]
   ...
   "Red Planet Couriers"
   {:async true
    :req (req (some #(can-be-advanced? %) (all-placed state :contestant)))
    :prompt "Select an placed card that can be advanced"
    :choices {:req can-be-advanced?}
    :effect (req (let [placed (get-all-placed state)
                       total-adv (reduce + (map #(get-counters % :advancement) placed))]
                   (doseq [c placed]
   ...
   "Replanting"
   (letfn [(replant [n]
             {:prompt "Select a card to place with Replanting"
              :async true
              :choices {:req #(and (= (:side %) "Contestant")
                                   (not (is-type? % "Operation"))
                                   (in-hand? %))}
              :effect (req (wait-for (contestant-place state side target nil {:no-place-cost true})
                                     (if (< n 2)
                                       (continue-ability state side (replant (inc n)) card nil)
                                       (effect-completed state side eid))))})]
     {:async true
      :prompt "Select an placed card to add to HQ"
      :choices {:req #(and (= (:side %) "Contestant")
                           (placed? %))}
      :msg (msg "add " (card-str state target) " to HQ, then place 2 cards ignoring all costs")
      :effect (req (move state side target :hand)
                   (resolve-ability state side (replant 1) card nil))})

   "Restore"
   {:async true
    :effect (effect (continue-ability {:prompt "Select a card in Archives to place & reveal with Restore"
                                       :priority -1
                                       :async true
                                       :show-discard true
                                       :choices {:req #(and (= (:side %) "Contestant")
   ...
   "Restoring Face"
   {:async true
    :prompt "Select a Sysop, Executive or Clone to discard"
    :msg (msg "discard " (card-str state target) " to remove 2 bad publicity")
    :choices {:req #(and (revealed? %)
   ...
   {:async true
    :effect (req (let [n (count (:hand contestant))]
                   (continue-ability state side
                     {:prompt (msg "Select up to " n " cards in HQ to discard with Reuse")
                      :choices {:max n
                                :req #(and (= (:side %) "Contestant")
                                           (in-hand? %))}
                      :msg (msg (let [m (count targets)]
                                  (str "discard " (quantify m "card")
    ...
    :effect (req (if (= target "Gain 2 [Credits]")
                   ...
                   (let [pre-purge-virus (number-of-virus-counters state)]
                     ...
                     (let [post-purge-virus (number-of-virus-counters state)
                           ...
                           num-to-discard (quot num-virus-purged 3)]
                       (mill state :contestant :challenger num-to-discard)
                       (system-msg state side (str "uses Reverse Infection to purge "
                                                   num-virus-purged (pluralize " virus counter" num-virus-purged)
                                                   " and discard "
                                                   num-to-discard (pluralize " card" num-to-discard)
   ...
   {:prompt "Name a Challenger card"
    ...
    :effect (req (system-msg state side
                             (str "uses Salem's Hospitality to reveal the Challenger's Grip ( "
                                  (join ", " (map :title (sort-by :title (:hand challenger))))
                                  " ) and discard any copies of " target))
                 (doseq [c (filter #(= target (:title %)) (:hand challenger))]
                   (discard state side c {:unpreventable true})))}

   "Scarcity of Radicles"
   {:msg "increase the place cost of radicles by 2"
    :events {:pre-place {:req (req (and (is-type? target "Radicle")
                                          (not (second targets)))) ; not facedown
                           :effect (effect (place-cost-bonus [:credit 2]))}}}
   ...
   {:req (req tagged)
    :prompt "Select two placed Challenger cards"
    :choices {:req #(and (placed? %)
                         (= "Challenger" (:side %)))
              :max 2}
   ...
   "Shipment from MirrorMontestanth"
   (let [shelper (fn sh [n] {:prompt "Select a card to place with Shipment from MirrorMontestanth"
                             :async true
                             :choices {:req #(and (= (:side %) "Contestant")
                                                  (not (is-type? % "Operation"))
                                                  (in-hand? %))}
   ...
   {:choices {:req #(and (character? %) (not (revealed? %)))
              :max (req (min (:tag challenger)
                             (reduce (fn [c locale]
                                       (+ c (count (filter #(not (:revealed %)) (:characters locale)))))
                                     0 (flatten (seq (:locales contestant))))))}
    :req (req tagged)
    :effect (req (doseq [t targets] (reveal state side t {:ignore-cost :all-costs}))
                 (effect-completed state side eid))}

   "Snatch and Grab"
   {:trace {:base 3
            :successful
            {:msg "discard a connection"
             ...
             :effect (req (let [c target]
                            ...
                            (continue-ability
                              state side
                              {:player :challenger
                               :prompt (msg "Take 1 tag to prevent " (:title c) " from being discarded?")
                               :choices ["Yes" "No"]
                               :async true
                               :effect (effect (clear-wait-prompt :contestant)
                                               (continue-ability
                                                 (if (= target "Yes")
                                                   {:msg (msg "take 1 tag to prevent " (:title c)
                                                    ...
                                                    :async true
                                                    :effect (effect (gain-tags :challenger eid 1 {:unpreventable true}))}
                                                   {:async true
                                                    :effect (effect (discard :contestant eid c nil))
   ...
   (let [gaincr {:req (req (= (:title target) (:marketing-target card)))
     ...
     {:prompt "Name a Challenger card"
      ...
      :events {:challenger-place gaincr
               :play-event gaincr}})

   "The All-Seeing I"
   (let [discard-all-radicles {:player :challenger
                              :effect (req (discard-cards state side (filter #(is-type? % "Radicle") (all-active-placed state :challenger))))
                              :msg (msg "discard all radicles")}]
       {:req (req tagged)
        :async true
        :effect (effect
                 (continue-ability
                   (if-not (zero? (:bad-publicity contestant)) ;; If contestant's bad-pub is 0
                     {:optional {:player :challenger
                                 :prompt "Remove 1 bad publicity from the contestant to prevent all radicles from being discarded?"
                                 :yes-ability {:effect (effect (lose :contestant :bad-publicity 1))
                                               :player :contestant
                                               :msg (msg "lose 1 bad publicity, preventing all radicles from being discarded")}
                                 :no-ability discard-all-radicles}}
                    discard-all-radicles)
                  card targets))})

   "Threat Assessment"
   {:req (req (last-turn? state :challenger :discarded-card))
    :prompt "Select an placed Challenger card"
   ...
   {:trace {:base 1
            ...
            {:label "Give the Challenger X tags"
             ...
             :effect (req (let [tags (-> @state :challenger :tag)]
                            (if (pos? tags)
                              (do (gain-tags state :contestant eid tags)
                                  (system-msg
                                    state side
                                    (str "uses Threat Level Alpha to give the Challenger " tags " tags")))
                              (do (gain-tags state :contestant eid 1)
                                  (system-msg
                                    state side
   ...
   {:choices {:req #(and (is-type? % "Agenda")
                         (placed? %)
                         (not (faceup? %)))}
    :effect (effect (update! (assoc target :seen true :revealed true
   ...
   {:req (req (:accessed-cards challenger-reg))
    :trace {:base 4
            :label "Trace 4 - Discard a resource"
            :successful {:async true
                         :effect (req (let [exceed (- target (second targets))]
                                        (continue-ability
                                          state side
                                          {:async true
                                           :prompt (str "Select a resource with an place cost of no more than "
                                                        exceed "[Credits]")
                                           :choices {:req #(and (is-type? % "Resource")
                                                                (placed? %)
                                                                (>= exceed (:cost %)))}
                                           :msg (msg "discard " (card-str state target))
                                           :effect (effect (discard eid target nil))}
                                          card nil)))}}}
                 ...
                 (wait-for (draw state side 4 nil)
                           (continue-ability state side
                                             {:prompt "Choose a card in HQ to place"
                                              ...
                                              :msg "gain 10 [Credits], draw 4 cards, and place 1 card from HQ"
                                              :cancel-effect (req (effect-completed state side eid))
                                              :effect (effect (contestant-place target nil))}
                                             card nil)))}

   "Under the Bus"
   {:req (req (and (last-turn? state :challenger :accessed-cards)
                   (not-empty (filter
                                #(and (is-type? % "Radicle")
                                      (has-subtype? % "Connection"))
                                (all-active-placed state :challenger)))))
    :prompt "Choose a connection to discard"
    :choices {:req #(and (= (:side %) "Challenger")
                         (is-type? % "Radicle")
                         (has-subtype? % "Connection")
                         (placed? %))}
    ...
    :async true
    :effect (req (wait-for (discard state side target nil)
                           (gain-bad-publicity state :contestant eid 1)))}
   ...
   "Wake Up Call"
   {:req (req (last-turn? state :challenger :discarded-card))
    :prompt "Select a piece of hazard or non-virtual radicle"
    :choices {:req #(or (hazard? %)
                        (and (radicle? %) (not (has-subtype? % "Virtual"))))}
    :async true
    :effect (req (let [chosen target
                       wake card]
                   (show-wait-prompt state side "Challenger to resolve Wake Up Call")
                   (continue-ability state :challenger
                                     {:prompt (str "Discard " (:title chosen) " or suffer 4 meat damage?")
                                      :choices [(str "Discard " (:title chosen))
                                                "4 meat damage"]
                                      :async true
                                      :effect (req (clear-wait-prompt state :contestant)
                                                   (move state :contestant (last (:discard contestant)) :rfg)
                                                   (if (.startsWith target "Discard")
                                                     (do (system-msg state side (str "chooses to discard " (:title chosen)))
                                                         (discard state side eid chosen nil))
