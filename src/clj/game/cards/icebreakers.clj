...
(def cards-icebreakers
  {"Abagnale"
   ...
   "Adept"
   {:abilities [{:cost [:credit 2] :req (req (or (has-subtype? current-character "Barrier")
   ...
   (auto-icebreaker ["Sentry"]
                    {:abilities [(break-sub 0 1 "Sentry" (effect (update! (assoc card :faerie-used true))))
                                 (strength-pump 1 1)]
                     :events {:pass-character {:req (req (:faerie-used card))
   ...
   "Savant"
   {:abilities [{:cost [:credit 2] :req (req (has-subtype? current-character "Sentry"))
