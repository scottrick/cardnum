  ...
  {"Abagnale"
   ...
   {:abilities [{:cost [:credit 2] :req (req (or (has-subtype? current-ice "Barrier")
    ...
    :effect (req (add-watch state (keyword (str "adept" (:cid card)))
                            (fn [k ref old new]
                              (when (not= (get-in old [:challenger :memory]) (get-in new [:challenger :memory]))
    ...
    :strength-bonus (req (:memory challenger))}
   ...
   "Berserker"
   {:abilities [(break-sub 2 2 "Barrier")]
    :implementation "Number of subroutines on encountered ICE has to be entered by challenger when Contestant chooses 'No More Action'"
                    ...
                    {:flags {:challenger-phase-12 (req true)}
                     :abilities [(strength-pump 2 1)
                                 ...
                                 {:label "Take 1 tag to place 2 virus counters (start of turn)"
                                  :once :per-turn
                                  :effect (req (when-completed (tag-challenger state :challenger 1)
   ...
   "Overmind"
   (auto-icebreaker ["All"]
                    {:effect (effect (add-counter card :power (:memory challenger)))
                     :abilities [{:counter-cost [:power 1]
                                  :msg "break 1 subroutine"}
                                 (strength-pump 1 1)]})
   "Paperclip"
   ...
   (auto-icebreaker ["Sentry"]
                    {:implementation "Requires challenger to input the number of subroutines allowed to resolve"
                     :abilities [(break-sub 2 1 "Sentry")
                                 (strength-pump 1 1)]
                     :events {:pass-ice {:req (req (and (has-subtype? target "Sentry") (rezzed? target)) (pos? (count (:deck challenger))))
                                         :delayed-completion true
                                         :optional {:prompt (msg "Use Persephone's ability??")
                                                    :yes-ability {:prompt "How many subroutines resolved on the passed ICE?"
                                                                  ...
                                                                  :effect (effect (mill :challenger 1)
