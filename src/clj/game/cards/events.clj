...
(defn- cutlery
  [subtype]
  ;; Subtype does nothing currently, but might be used if discarding is properly implemented
  {:implementation "Ice discard is manual, always enables Reprisals"
   :prompt "Choose a locale"
   :choices (req runnable-locales)
   :effect (req (run state :challenger target nil card)
...
(def card-definitions
   ...
   {:req (req hq-runnable)
    ...
    :effect (effect (run :hq {:req (req (= target :hq))
                              ...
                              {:msg (msg "force the Contestant to lose " (min 5 (:credit contestant))
                               ...
                               :async true
                               :effect (req (wait-for (gain-tags state :challenger 2)
  ...
  {:events {:challenger-place {:silent (req true) ;; there are no current interactions where we'd want Grant to not be last, and this fixes a bug with Hayley
   ...
   {:req (req (some #{:hq :rd :archives} (:successful-run challenger-reg)))
    ...
    :choices {:req #(and (or (is-type? % "Hazard")
                         ...
                         (in-hand? %))}
    :async true
    :effect (req (place-cost-bonus state :challenger [:credit -8])
                 (wait-for (challenger-place state :challenger target nil)
                           (gain-tags state eid :challenger 1)))}
  ...
  (let [contestant-choose {:show-discard true
   ...
   "Guinea Pig"
   {:msg "discard all cards in the grip and gain 10 [credits]"
    :effect (req (doseq [c (:hand challenger)]
                   (discard state :challenger c {:unpreventable true}))
                 (gain-credits state :challenger 10))}
    ...
    :effect (effect (run :hq {:req (req (= target :hq))
                              :successful-run {:async true
                                               ...
                                               :effect (req (wait-for (gain-tags state :challenger 1)
                                                                      (gain-credits state :challenger 9)
   ...
   {:async true
    :effect (req
             (let [from (take 4 (:deck contestant))]
               (when (pos? (count from))
                 (do (show-wait-prompt state :challenger
                                       (str "Contestant to rearrange the top " (count from) " cards of R&D"))
                     (wait-for
                      (resolve-ability state :contestant (reorder-choice :contestant from) card targets)
                      (do (clear-wait-prompt state :challenger)
                          (system-msg state :challenger (str " reveals (top:) " ; here (get-in @state ...) is needed to refresh ref. to deck after reordering
                                                         (join ", " (map :title (take 4 (get-in @state [:contestant :deck])))) " from the top of R&D"))
                          (effect-completed state side eid)))))))}
   "Interdiction"
   ...
   {:req (req (some #(is-type? % "Radicle") (all-active-placed state :challenger)))
    ...
    :abilities [{:label "[Discard]: Avoid 3 tags"
                 ...
                 :effect (effect (tag-prevent :challenger 3)
   ...
   {:msg "change identities"
    ...
    :choices (req (let [is-swappable (fn [c] (and (= "Identity" (:type c))
     ...
     :effect (req
               ...
               (let [new-id (-> target :title locale-card make-card (assoc :zone [:identity]))]
   ...
   "Reboot"
   (letfn [(place-cards [state side eid card to-place titles]
             (if-let [f (first to-place)]
               (wait-for (challenger-place state :challenger f {:facedown true :no-msg true})
                         (place-cards state side eid card (rest to-place) titles))
               (do
                 (move state side (find-latest state card) :rfg)
                 (system-msg state :challenger (str "uses Reboot to place " (join ", " titles) " facedown"))
                 (effect-completed state side eid))))]
     {:req (req archives-runnable)
      :makes-run true
      :effect (effect
                (run :archives
                     {:req (req (= target :archives))
                      :replace-access
                      {:prompt "Choose up to five cards to place"
                       :choices {:max 5
                                 :req #(and (in-discard? %) (= (:side %) "Challenger") (not= (:cid %) (:cid card)))}
                       :mandatory true
                       :async true
                       :cancel-effect (req (move state side (find-latest state card) :rfg)
                                           (effect-completed state side eid))
                       :effect (req (place-cards state side eid card targets (map :title targets)))}}
                     card))})
  ...
  {:effect (effect (register-events (:events (card-def card)) (assoc card :zone '(:discard))))
   ...
   "Trade-In"
   ;; Basically a hack. Ideally the additional cost cause the cost discard to be passed in as targets
   (letfn [(discarded-hw [state] (last (get-in @state [:challenger :discard])))]
     {:additional-cost [:hazard 1]
      :msg (msg (let [{:keys [title cost]} (discarded-hw state)]
                  (str "discard " title " and gain " (quot cost 2) " [Credits]")))
      :effect (req (let [{:keys [cost]} (discarded-hw state)]
                     (gain-credits state :challenger (quot cost 2))
                     (continue-ability state :challenger
                                       {:prompt "Choose a Hazard to add to your Grip from your Stack"
