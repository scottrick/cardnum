...
(def corp-rez-toast
  "Effect to be placed with `:hazPlayer-turn-ends` to remind players of 'when turn begins'
  triggers"
  {:effect (req (toast state :resPlayer "Reminder: You have unrezzed cards with \"when turn begins\" abilities." "info"))})
...
(defn reorder-choice
  ...
  reorder-side is the side to be reordered, i.e. :resPlayer for Indexing and Precognition.
  wait-side is the side that has a wait prompt while ordering is in progress, i.e. :resPlayer for Indexing and Spy Camera.
  ...
  {:prompt (str "Select a card to move next "
                ...
                (if (= reorder-side :resPlayer) "R&D" "your Stack"))
   :choices remaining
...
(defn- reorder-final
  ...
  ([reorder-side wait-side chosen original dest]
   {:prompt (if (= dest "bottom")
              (str "The bottom cards of " (if (= reorder-side :resPlayer) "R&D" "your Stack")
                   " will be " (join  ", " (map :title (reverse chosen))) ".")
              (str "The top cards of " (if (= reorder-side :resPlayer) "R&D" "your Stack")
...
(defn swap-ice
  ...
  (let [a-index (ice-index state a)
    ...
    (swap! state update-in (cons :resPlayer (:zone a)) #(assoc % a-index b-new))
    (swap! state update-in (cons :resPlayer (:zone b)) #(assoc % b-index a-new))
...
(defn card-index
  ...
  [state card]
  (first (keep-indexed #(when (= (:cid %2) (:cid card)) %1) (get-in @state (cons :resPlayer (:zone card))))))
