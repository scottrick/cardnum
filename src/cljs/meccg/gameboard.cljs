...
(defn show-sites [event owner ref region]
        ...
        [:div.blue-shade.deck
         ...
         (when (= (:side @game-state) side)
           [:div.panel.blue-shade.menu {:ref map-menu-ref}
            [:div {:on-click #(show-sites % owner reg-ref nil)} "Regions"]
            [:div {:on-click #(show-sites % owner map-ref nil)} "Sites"]]
           )
         (when (= (:side @game-state) side)
           [:div.panel.blue-shade.popup {:ref reg-content-ref}
            [:div
             (general-map owner reg-ref)
             [:a {:on-click #(close-popup % owner reg-content-ref "stops looking at the map" false true)}
              "Close"]]
            ])
         (when (= (:side @game-state) side)
           [:div.panel.blue-shade.popup {:ref map-content-ref}
            [:div
             (general-map owner site-ref)
         ...
         (when (= (:side @game-state) side)
           [:div.panel.blue-shade.popup {:ref site-content-ref}
            [:div
             [:a {:on-click #(close-popup % owner site-content-ref "stops at a region" false true)}
              "Close"]]
            (om/build-all card-view sites {:key :cid})])
         ]))))

(defn sites-viewB [{:keys [identity sites] :as cursor} owner]
  (om/component
    (sab/html
      (let [is-challenger (= "Challenger" (:side identity))
            side (if is-challenger :challenger :contestant)
