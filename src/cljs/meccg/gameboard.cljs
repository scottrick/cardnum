(ns meccg.gameboard
  ...
  (:require [cljs.core.async :refer [chan put! <!] :as async]
            ...
            [meccg.utils :refer [influence-dot map-longest toastr-options]]
            [meccg.ws :as ws]
            [reagent.core :as r]
            [cardnum.utils :as utils]))
...
(defn send-command
  ...
  ([command {:keys [no-lock] :as args}]
   (when (or (not @lock) no-lock)
     ...
     (ws/ws-send! [:meccg/action {:gameid-str (:gameid @game-state)
                                      :command command
                                      :args args}]))))
...
(defn send-msg [s]
  (let [input (:msg-input @s)
        text (:msg @s)]
    (when-not (empty? text)
      (ws/ws-send! [:meccg/say {:gameid-str (:gameid @game-state)
                                    :msg text}])
...
(defn send-typing [s]
  ...
  (let [input (:msg-input @s)
        text (:msg @s)]
    (if (empty? text)
      (ws/ws-send! [:meccg/typing {:gameid-str (:gameid @game-state)
                                       :typing false}])
...
(defn mute-spectators [mute-state]
  (ws/ws-send! [:meccg/mute-spectators {:gameid-str (:gameid @game-state)
      ...
      (cond
        ...
        (case (first zone)
          ...
          ("locales") (when (and (= type "Character") (:revealed card))
                        ...
                        (if (:challenger-abilities @c-state)
                          (swap! c-state dissoc :challenger-abilities)
                          (swap! c-state assoc :challenger-abilities true)))
          nil)
        ...
        (case (first zone)
          "hand" (case type
                   ("Region" "Character") (if root
                                       ...
                                       (if (:locales @c-state)
                                         (swap! c-state dissoc :locales)
                                         (swap! c-state assoc :locales true)))
                   ("Agenda" "Site") (if (< (count (get-in @game-state [:contestant :locales])) 4)
                                        ...
                                        (if (:locales @c-state)
                                          ...
                                          (swap! c-state assoc :locales true)))
                   (send-command "play" {:card card}))
          ("locales" "scored" "current" "onhost") (handle-abilities card c-state)
          "rig" (when (:contestant-abilities card)
                  (if (:contestant-abilities @c-state)
                    (swap! c-state dissoc :contestant-abilities)
                    (swap! c-state assoc :contestant-abilities true)))
          nil)))))
...
(defn challenger-abs [card c-state challenger-abilities subroutines title]
  [:div.panel.blue-shade.challenger-abilities {:style (when (:challenger-abilities @c-state) {:display "inline"})}
   (map-indexed
     (fn [i ab]
       [:div {:key i
              :on-click #(do (send-command "challenger-ability" {:card card
                                                             :ability i}))
...
(defn contestant-abs [card c-state contestant-abilities]
    [:div.panel.blue-shade.contestant-abilities {:style (when (:contestant-abilities @c-state) {:display "inline"})}
     (map-indexed
       (fn [i ab]
         [:div {:on-click #(do (send-command "contestant-ability" {:card card
                                                             :ability i}))
                :dangerouslySetInnerHTML #js {:__html (add-symbols (str (ability-costs ab) (:label ab)))}}])
       contestant-abilities)])

(defn locale-menu [card c-state parties type zone]
  (let [centrals ["Archives" "R&D" "HQ"]
        parties (concat (party-list parties) ["New party"])
        locales (case type
                  ("Region" "Character") (concat centrals parties)
                  ("Agenda" "Site") parties)]
   [:div.panel.blue-shade.locales-menu {:style (when (:locales @c-state) {:display "inline"})}
     (map-indexed
       (fn [i label]
         [:div {:key i
                :on-click #(do (send-command "play" {:card card :locale label})
                               (swap! c-state dissoc :locales))}
          label])
       locales)]))
    ...
    [:div.card-frame
     [:div.blue-shade.card {:class (str (when selected "selected") (when new " new"))
      ...
      (when (and (= zone ["hand"]) (#{"Agenda" "Site" "Character" "Region"} type))
        [locale-menu card c-state parties type zone])

      (when (pos? (+ (count challenger-abilities) (count subroutines)))
        [challenger-abs card c-state challenger-abilities subroutines title])

      (when (pos? (count contestant-abilities))
        [contestant-abs card c-state contestant-abilities])
      ...
      (when (#{"locales" "onhost"} (first zone))
        (cond
          ...
          [:div.panel.blue-shade.menu.abilities
           [:div {:on-click #(send-command "advance" {:card card})} "Advance"]
           [:div {:on-click #(send-command "reveal" {:card card})} "Reveal"]]))]
...
(defn build-hand-card-view
  [user hand prompt parties wrapper-class]
  (let [size (count @hand)]
    [:div
     (doall (map-indexed
              (fn [i card]
                [:div {:key (:cid card)
                 ...
                 (if (or (this-user? @user)
                         ...
                         (spectator-view-hidden?))
                   [card-view (assoc card :parties @parties)]
                   [facedown-card (:side card)])])
              @hand))]))

(defn hand-view [user name hand prompt parties popup popup-direction]
  (let [s (r/atom {})]
    (fn [user name hand prompt parties popup popup-direction]
      (let [size (count @hand)]
        [:div.hand-container
         [:div.hand-controls
          [:div.panel.blue-shade.hand
           (drop-area name {:class (when (> size 6) "squeeze")})
           [build-hand-card-view user hand prompt parties "card-wrapper"]
         ...
         (when popup
           [:div.panel.blue-shade.popup {:ref #(swap! s assoc :hand-popup %) :class popup-direction}
            [:div
             ...
             [build-hand-card-view user hand prompt parties "card-popup-wrapper"]]])]))))
...
(defn deck-view [render-side player-side identity deck]
   (let [is-challenger (= :challenger render-side)
...
(defn discard-view-challenger [player-side discard]
  (let [s (r/atom {})]
    (fn [player-side discard]
      [:div.blue-shade.discard
       ...
       [:div.panel.blue-shade.popup {:ref #(swap! s assoc :popup %)
                                     :class (if (= player-side :challenger) "me" "opponent")}
        [:div
...
(defn discard-view-contestant [player-side discard]
  (let [s (r/atom {})]
    (fn [player-side discard]
      (let [faceup? #(or (:seen %) (:revealed %))
            draw-card #(if (faceup? %)
                         [card-view %]
                         (if (or (= player-side :contestant)
                                 (spectator-view-hidden?))
        ...
        [:div.blue-shade.discard
         ...
         [:div.panel.blue-shade.popup {:ref #(swap! s assoc :popup %)
                                       :class (if (= (:side @game-state) :challenger) "opponent" "me")}
          [:div
           [:a {:on-click #(close-popup % (:popup @s) nil false false)} "Close"]
...
(defmethod stats-view "Challenger" [challenger]
  (let [me? (= (:side @game-state) :challenger)]
    (fn [challenger]
      (let [{:keys [user click credit run-credit memory link tag
                    brain-damage agenda-point tagged hand-size active]} @challenger]
...
(defmethod stats-view "Contestant" [contestant]
  (let [me? (= (:side @game-state) :contestant)]
    (fn [contestant]
      (let [{:keys [user click credit agenda-point bad-publicity has-bad-pub hand-size active]} @contestant]
        [:div.panel.blue-shade.stats {:class (when active "active-player")}
         [:h4.ellipsis [avatar user {:opts {:size 22}}] (:username user)]
         [:div (str click " Click" (if (not= click 1) "s" ""))
          (when me? (controls :click))]
...
(defn locale-view [{:keys [locale central-view run]} opts]
  (let [content (:content locale)
        characters (:characters locale)
    ...
    [:div.locale
     [:div.characters {:style {:width (when (pos? max-hosted)
      ...
      (for [character (reverse characters)]
        [:div.character {:key (:cid character)
                   :class (when (not-empty (:hosted character)) "host")}
         (let [flipped (not (:revealed character))]
           [card-view character flipped])
      ...
      (when (not-empty content)
          (for [card content]
                 (let [is-first (= card (first content))
                       flipped (not (:revealed card))]
                   [:div.locale-card {:key (:cid card)
...
(defn board-view-contestant [player-side identity deck discard locales run]
  (let [rs (:locale @run)
        locale-type (first rs)]
    [:div.contestant-board {:class (if (= player-side :challenger) "opponent" "me")}
     (doall
     ...
     [locale-view {:key "rd"
                   :locale (:rd @locales)
                   :central-view [deck-view :contestant player-side identity deck]
                   :run (when (= locale-type "rd") @run)}]
     [locale-view {:key "archives"
                   :locale (:archives @locales)
                   :central-view [discard-view-contestant player-side discard]
                   :run (when (= locale-type "archives") @run)}]]))

(defn board-view-challenger [player-side identity deck discard rig run]
  (let [is-me (= player-side :challenger)]
    (let [centrals [:div.challenger-centrals
                    [discard-view-challenger player-side discard]
                    [deck-view :challenger player-side identity deck]
                    [identity-view identity]]]
      [:div.challenger-board {:class (if is-me "me" "opponent")}
       (when-not is-me centrals)
       (doall (for [zone [:resource :hazard :radicle :facedown]]
                ^{:key zone}
                [:div
    ...
    [:div
     [:table.win.table
      [:tbody
       ...
       (doall (map-indexed
          (fn [i [contestant challenger]]
            [:tr {:key i}
             [:td (first contestant)] [:td (show-stat contestant)]
        ...
        [:div.win.centered.blue-shade
         [:div
          ...
          (cond
            ...
            (str ") wins due to the Contestant being decked on turn " @turn)
...
(defn button-pane [{:keys [side active-player run end-turn challenger-phase-12 contestant-phase-12 contestant challenger me opponent sfx] :as cursor}]
  (let [s (r/atom {})
    ...
    :reagent-render
    (fn [{:keys [side active-player run end-turn challenger-phase-12 contestant-phase-12 contestant challenger me opponent sfx] :as cursor}]
               ...
               (when-let [base (:base prompt)]
                 ...
                 (if (nil? (:strength prompt))
                   (if (= "contestant" (:player prompt))
                     ...
                     [:div.info "Challenger: " (:link prompt) [:span {:class "anr-icon link"}]
              ...
              [:div
               ...
               [:div.run-button
                [cond-button "Run" (and (pos? (:click @me))
                 ...
                 #(-> (:locales @s) js/$ .toggle)]
                [:div.panel.blue-shade.locales-menu {:ref #(swap! s assoc :locales %)}
                 (map-indexed (fn [i label]
                                [:div {:key i
                                       :on-click #(do (send-command "run" {:locale label})
                                                      (-> (:locales @s) js/$ .fadeOut))}
                                 label])
...
(defn gameboard []
  (let [run (r/cursor game-state [:run])
        ...
        contestant-phase-12 (r/cursor game-state [:contestant-phase-12])
        challenger-phase-12 (r/cursor game-state [:challenger-phase-12])
        contestant (r/cursor game-state [:contestant])

        challenger (r/cursor game-state [:challenger])
        active-player (r/cursor game-state [:active-player])
        render-board? (r/track (fn [] (and contestant challenger side)))
  ...
  (go (while true
    ...
    (r/create-class
      {:display-name "gameboard"
       ...
       (fn []
         (when @@render-board?
           (let [me-side (if (= :spectator @side) :contestant @side)
                 ...
                 contestant-locales (r/cursor game-state [:contestant :locales])
                 contestant-parties (r/track (fn [] (get-parties (get-in @game-state [:contestant :locales]))))
                 challenger-rig (r/cursor game-state [:challenger :rig])
             ...
             [:div.gameboard
              ...
              [:div.centralpane
               (if (= op-side :contestant)
                 [board-view-contestant me-side op-ident op-deck op-discard contestant-locales run]
                 [board-view-challenger me-side op-ident op-deck op-discard challenger-rig run])
               (if (= me-side :contestant)
                 [board-view-contestant me-side me-ident me-deck me-discard contestant-locales run]
                 [board-view-challenger me-side me-ident me-deck me-discard challenger-rig run])]

              [:div.leftpane
               [:div.opponent
                [hand-view op-user (if (= :contestant op-side) "HQ" "Grip") op-hand op-prompt contestant-parties
                ...
                [:div.right-inner-leftpane
                 ...
                 (when-not (= @side :spectator)
                   [button-pane {:side me-side :active-player active-player :run run :end-turn end-turn
                                 :challenger-phase-12 challenger-phase-12 :contestant-phase-12 contestant-phase-12
                                 :contestant contestant :challenger challenger :me me :opponent opponent
                                 :sfx sfx}])]]

               [:div.me
                [hand-view me-user (if (= :contestant me-side) "HQ" "Grip") me-hand me-prompt
                 contestant-parties true "me"]]]])))})))
