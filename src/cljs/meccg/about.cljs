  ...
  (let [donators (r/cursor about-state [:donators])]
   ...
   [:div.about.panel.content-page.blue-shade
    ...
    [:p "This website was founded by " [:a {:href "http://twitter.com/mtgred" :target "_blank"} "@mtgred"]
     ", an avid MECCG player from Belgium. The goal is to provide a great way to create and test MECCG decks online."]
    ...
    [:ul.list.compact
     ...
     [:li "JoelCFC25, domtancredi, Zaroth, queueseven, erbridge, and "
      [:a {:href "https://github.com/mtgred/meccg/graphs/contributors" :target "_blank"} "many more"]
      ": Past contributors."]
     [:li "0thmxma, Sanjay, quarg, znsolomon, hbarsquared, yankeeflatline, rumirumirumirumi: Contestant and Challenger quotes for start-of-game splash screen."]]

    [:h4 "Tech Stack"]
    [:ul.list.compact
     [:li [:b "Game engine:"] " Clojure. Card data from " [:a {:href "https://meccgdb.com/" :target "_blank"} "MECCGDB"] " API."]
    ...
    [:p "The code is open source and available on "
     [:a {:href "https://github.com/mtgred/meccg" :target "_blank"} "Github"] "."]

    [:p "Bug reports and feature suggestions can be submitted "
     [:a {:href "https://github.com/mtgred/meccg/issues" :target "_blank"} "here"] "."]
