...
(deftest baba-yaga
  ...
  (do-game
    ...
    (let [baba (get-resource state 0)
      ...
      (click-card state :challenger (find-card "Faerie" (:hand (get-challenger))))
      ...
      (click-card state :challenger (find-card "Yog.0" (:hand (get-challenger))))
      ...
      (click-card state :challenger (find-card "Sharpshooter" (:resource (:rig (get-challenger)))))
  ...
  (testing "with Clone Chip"
    (do-game
      ...
      (let [chip (get-hazard state 0)]
        ...
        (click-card state :challenger (find-card "Chameleon" (:discard (get-challenger))))
  ...
  (testing "Returns to hand after hosting. #977"
    (do-game
      ...
      (let [scheherazade (get-resource state 1)]
        ...
        (click-card state :challenger (find-card "Chameleon" (:resource (:rig (get-challenger)))))
...
(deftest cradle
  ...
  (do-game
    (new-game (default-contestant ["Ice Wall"])
              (default-challenger ["Cradle" (qty "Cache" 100)]))
    (starting-hand state :challenger ["Cradle"])
    (play-from-hand state :contestant "Ice Wall" "HQ")
    (take-credits state :contestant)
    (core/gain state :challenger :credit 100 :click 100)
    (play-from-hand state :challenger "Cradle")
    (run-on state "HQ")
    (let [cradle (get-resource state 0)
          strength (:strength (refresh cradle))]
      (dotimes [n 5]
        (when (pos? n)
          (core/draw state :challenger n))
...
(deftest faust
  ...
  (testing "Pump does not trigger discard prevention. #760"
    (do-game
      ...
      (let [faust (get-resource state 0)]
        (card-ability state :challenger faust 1)
        (click-card state :challenger (find-card "Astrolabe" (:hand (get-challenger))))
        ...
        (card-ability state :challenger faust 1)
        (click-card state :challenger (find-card "Armitage Codebusting" (:hand (get-challenger))))
...
(deftest ika
  ;; Ika
  (testing "Can be hosted on both revealed/unrevealed character, respects no-host, is blanked by Magnet"
    (do-game
      (new-game (default-contestant ["Tithonium" "Enigma" "Magnet"])
                (default-challenger ["Ika"]))
      (play-from-hand state :contestant "Enigma" "HQ")
      (play-from-hand state :contestant "Tithonium" "Archives")
      (play-from-hand state :contestant "Magnet" "R&D")
      (take-credits state :contestant)

      (play-from-hand state :challenger "Ika")
      (core/gain state :challenger :credit 100)
      (core/gain state :contestant :credit 100)
      (let [ika (get-resource state 0)
            enigma (get-character state :hq 0)
            tithonium (get-character state :archives 0)
            magnet (get-character state :rd 0)]
        (let [creds (:credit (get-challenger))]
          (card-ability state :challenger ika 2) ; host on a piece of character
          (click-card state :challenger tithonium)
          (is (game.utils/same-card? ika (first (:hosted (refresh tithonium)))) "Ika was rehosted")
          (is (= (- creds 2) (:credit (get-challenger))) "Rehosting from rig cost 2 creds"))
        (run-on state :archives)
        (let [creds (:credit (get-challenger))
              ika (first (:hosted (refresh tithonium)))]
          (card-ability state :challenger ika 2)
          (click-card state :challenger enigma)
          (is (game.utils/same-card? ika (first (:hosted (refresh enigma)))) "Ika was rehosted")
          (is (= (- creds 2) (:credit (get-challenger))) "Rehosting from character during run cost 2 creds"))
        (core/reveal state :contestant tithonium)
        (let [creds (:credit (get-challenger))
              ika (first (:hosted (refresh enigma)))]
          (card-ability state :challenger ika 2)
          (click-card state :challenger tithonium)
          (is (= 0 (count (:hosted (refresh tithonium)))) "Ika was not hosted on Tithonium")
          (is (= creds (:credit (get-challenger))) "Clicking invalid targets is free")
          (click-prompt state :challenger "Done")
          (core/reveal state :contestant magnet)
          (click-card state :contestant ika)
...
(deftest musaazi
  ...
  (do-game
    ...
    (let [lancelot (get-character state :hq 0)
      ...
      (click-card state :challenger musaazi)
      (click-card state :challenger imp)
      (click-prompt state :challenger "Done")
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [nanotk (get-resource state 0)
        ...
        (click-card state :contestant (find-card "Eli 1.0" (:hand (get-contestant))))
...
(deftest paperclip
  ...
  (testing "Basic test"
    (do-game
      ...
      (run-on state "Archives")
      (core/reveal state :contestant (get-character state :archives 0))
      (click-prompt state :challenger "Yes") ; place paperclip
      (run-continue state)
...
(deftest tycoon
  ...
  (do-game
    (new-game (default-contestant ["Ice Wall"])
              (default-challenger ["Tycoon"]))
    (play-from-hand state :contestant "Ice Wall" "HQ")
    (core/reveal state state :contestant (get-character state :hq 0))
    (take-credits state :contestant)
    (play-from-hand state :challenger "Tycoon")
    (let [tycoon (get-resource state 0)
          credits (:credit (get-contestant))]
      (run-on state "HQ")
      (card-ability state :challenger tycoon 0)
      (is (= credits (:credit (get-contestant))) "Contestant doesn't gain credits until encounter is over")
      (run-continue state)
      (is (= (+ credits 2) (:credit (get-contestant))) "Contestant gains 2 credits from Tycoon being used"))))
...
(deftest yusuf
  ...
  (do-game
    ...
    (let [fire-wall (get-character state :hq 0)
      ...
      (click-card state :challenger yusuf)
