...
(deftest adjusted-chronotype
  ...
  (testing "Ensure adjusted chronotype gains 2 clicks when 2 clicks are lost and GCS is placed"
    (do-game
      (new-game (default-contestant)
                (default-challenger ["Adjusted Chronotype"
                                 (qty "Beach Party" 3)
...
(deftest aesop's-pawnshop
  ...
  (do-game
    ...
    (let [orig-credits (:credit (get-challenger))
      ...
      (card-ability state :challenger ap 0)
      (click-card state :challenger ap)
      (let [ap (get-radicle state 0)
        ...
        (is (not= ap nil) "Aesops should still be placed")))))
...
(deftest bank-job
  ...
  (testing "Manhunt trace happens first"
    (do-game
      ...
      (click-prompt state :contestant "2") ; Manhunt trace active
  ...
  (testing "Choose which to use when 2+ copies are placed"
    (do-game
      ...
      (let [bj1 (get-radicle state 0)
        ...
        (run-empty-locale state "Locale 1")
        (click-prompt state :challenger "Replacement effect")
        (click-card state :challenger bj2)
        (click-prompt state :challenger "6")
...
(deftest bazaar
  ;; Bazaar - Only triggers when placing from Grip
  (do-game
    (new-game (default-contestant)
    ...
    (let [peddler (get-radicle state 1)]
      ...
      (click-prompt state :challenger (first (:hosted peddler)))
      (is (empty? (:prompt (get-challenger))) "No Bazaar prompt from place off Peddler"))))
...
(deftest chrome-parlor
  ;; Chrome Parlor - Prevent all meat/brain dmg when placing cybernetics
  (do-game
...
(deftest citadel-sanctuary
  (testing "Interaction with Contestantorate Grant and Thunder Art Gallery"
    (do-game
      (new-game (default-contestant)
                (default-challenger ["Citadel Sanctuary" "Thunder Art Gallery" "Corroder" "Contestantorate \"Grant\""]))
      (take-credits state :contestant)
      (core/gain state :challenger :credit 5)
      (play-from-hand state :challenger "Citadel Sanctuary")
      (play-from-hand state :challenger "Thunder Art Gallery")
      (play-from-hand state :challenger "Contestantorate \"Grant\"")
      (take-credits state :challenger)
      (take-credits state :contestant)
      (core/gain-tags state :challenger 1)
      (core/lose state :challenger :click 3)
      (core/end-turn state :challenger nil)
      (is (= 11 (:credit (get-contestant))) "Contestant has 11 credits before Contestantorate Grant")
      (click-prompt state :contestant "0")
      (click-prompt state :challenger "1")
      (is (not (:end-turn @state)) "Challenger turn has not yet ended")
      (click-card state :challenger (find-card "Corroder" (:hand (get-challenger))))
      (is (:end-turn @state) "Challenger turn has now ended")
      (is (= 10 (:credit (get-contestant))) "Contestant lost 1 credit to Contestantorate Grant"))))
...
(deftest councilman
  ;; Councilman reverses the revealz and prevents re-revealz
  (do-game
    ...
    (let [jesus (get-content state :party1 0)
      ...
      (click-card state :challenger jesus)
...
(deftest-pending councilman-zone-change
  ;; Revealz no longer prevented when card changes zone (issues #1571)
  (do-game
    (new-game (default-contestant ["Jackson Howard"])
              (default-challenger ["Councilman"]))
    (play-from-hand state :contestant "Jackson Howard" "New party")
    ...
    (play-from-hand state :contestant "Jackson Howard" "New party")
    (let [jesus (get-content state :party2 0)]
      (core/reveal state :contestant jesus)
...
(deftest ddos
  ...
  (do-game
    ...
    (let [ddos (get-radicle state 0)
      ...
      (run-on state "HQ")
      (core/reveal state :contestant iwall)
      (is (:revealed (refresh iwall))))))

(deftest dean-lister
  ;; Basic test
  (do-game
    (new-game (default-contestant)
              (default-challenger ["Dean Lister" "Faust" (qty "Sure Gamble" 3)]))
    (take-credits state :contestant)
    (play-from-hand state :challenger "Dean Lister")
    (play-from-hand state :challenger "Faust")
    (run-on state :archives)
    (let [faust (get-resource state 0)
          dean (get-radicle state 0)]
      (is (= 2 (:current-strength faust)) "Faust at 2 strength")
      (is (zero? (-> (get-challenger) :discard count)) "Dean Lister not discarded yet")
      (card-ability state :challenger dean 0)
      (click-card state :challenger faust)
      (is (= 1 (-> (get-challenger) :discard count)) "Dean Lister discarded to use its abilitiy")
      (is (= 5 (:current-strength (refresh faust))) "Faust at 5 strength (2 base + 3 from Dean)")
      (card-ability state :challenger faust 1) ;boost by 2
...
(deftest district-99
  ;; District 99 - Gains power counters on hazard/resource discards, can spend 3 power counters to recur a card matching identity
  (testing "Discards by both sides and manual triggers"
    (do-game
      (new-game (default-contestant ["Bio-Ethics Association"])
                (default-challenger ["District 99" (qty "Spy Camera" 2) "Faerie"]))
      (play-from-hand state :contestant "Bio-Ethics Association" "New party")
      (take-credits state :contestant)
      (play-from-hand state :challenger "District 99")
      (let [d99 (get-radicle state 0)
            bea (get-content state :party1 0)]
        (card-ability state :challenger (refresh d99) 1) ; manually add power counter
        (is (= 1 (get-counters (refresh d99) :power)) "1 power counter was added manually")
        (card-ability state :challenger (refresh d99) 1) ; try to manually add power counter twcharacter
        (is (= 1 (get-counters (refresh d99) :power)) "Manual power counter addition is only possible once per turn")
        (play-from-hand state :challenger "Spy Camera")
        (card-ability state :challenger (get-hazard state 0) 1) ; pop spy camera
        ...
        (play-from-hand state :challenger "Spy Camera")
        (is (= 1 (count (:hand (get-challenger)))) "Faerie in hand")
        (is (= "Faerie" (:title (first (:hand (get-challenger))))))
        (core/reveal state :contestant bea)
        (take-credits state :challenger)
        (is (= 0 (count (:hand (get-challenger)))) "Faerie was discarded")
        (is (= 2 (get-counters (refresh d99) :power)) "Discarding Faerie from grip placed a counter")
        (card-ability state :challenger (get-hazard state 0) 1) ; pop spy camera
        (click-prompt state :challenger "OK")
        (is (= 2 (get-counters (refresh d99) :power)) "Discarding Spy Camera after Faerie did not place a counter"))))
  (testing "Rebirth interaction, basic functionality"
    (do-game
      (new-game (default-contestant ["Grim"])
                (make-deck "Armand \"Geist\" Walker: Tech Lord"
                           ["District 99" (qty "Spy Camera" 3) "Faerie" "Rebirth" "Sure Gamble"]))
      (play-from-hand state :contestant "Grim" "HQ")
      (take-credits state :contestant)
      ...
      (let [khan "Khan: Savvy Skiptracer"]
        (click-prompt state :challenger khan)
        (is (= khan (-> (get-challenger) :identity :title)) "Rebirthed into Khan"))
      (play-from-hand state :challenger "Spy Camera")
      (play-from-hand state :challenger "Faerie")
      (let [d99 (get-radicle state 0)
            faerie (get-resource state 0)
            spycam (get-hazard state 0)
            grim (get-character state :hq 0)]
        (run-on state :hq)
        (core/reveal state :contestant grim)
        (card-subroutine state :contestant (refresh grim) 0)
        (is (= 0 (get-counters (refresh d99) :power)) "No power counters before Faerie is discarded")
        (click-card state :contestant faerie)
        (is (= 1 (get-counters (refresh d99) :power)) "1 power counter was added for Faerie being discarded")
        (card-ability state :challenger spycam 1) ; pop spycam
        (click-prompt state :challenger "OK")
        (is (= 1 (get-counters (refresh d99) :power)) "Discarding Spy Camera after Faerie did not add a second power counter")
        (card-ability state :challenger (refresh d99) 2) ; manually add counter
        (is (= 1 (get-counters (refresh d99) :power)) "Can't manually add power counter after one has already been added")
        (run-jack-out state)
        (play-from-hand state :challenger "Spy Camera")
        (take-credits state :challenger)
        (card-ability state :challenger (get-hazard state 0) 1) ; pop spycam
        (click-prompt state :challenger "OK")
        (is (= 2 (get-counters (refresh d99) :power)) "Discarding Spy Camera on Contestant turn added a second power counter")
        (take-credits state :contestant)
        (play-from-hand state :challenger "Spy Camera")
        (take-credits state :challenger)
        (card-ability state :challenger (get-hazard state 0) 1) ; pop spycam
        (click-prompt state :challenger "OK")
        (take-credits state :contestant)
        (is (= 3 (get-counters (refresh d99) :power)) "Discarding Spy Camera on Challenger turn added a third power counter")
        (let [faerie (first (filter #(= (:title %) "Faerie") (:discard (get-challenger))))]
          (doseq [c ["Sure Gamble" "Faerie" "Spy Camera"]]
            (is (some? (filter #(= (:title %) c) (:hand (get-challenger)))) (str c " is in the discard")))
          (is (zero? (count (:hand (get-challenger)))) "Faerie is not in hand")
          (card-ability state :challenger (refresh d99) 0)  ; Retrieve card from Archives
          (is (= 2 (count (:choices (prompt-map :challenger)))) "Challenger can choose between Spy Camera and Faerie only")
          (click-prompt state :challenger faerie)
          (is (= 1 (count (:hand (get-challenger)))) "1 card added to hand")
          (is (= "Faerie" (-> (get-challenger) :hand first :title)) "Faerie added to hand")
  ...
  (deftest dj-fenris
    ...
    (testing "Hosting Chaos Theory"
      ...
      (do-game
        ...
        ;; Re-play DJ Fenris
        (play-from-hand state :challenger "DJ Fenris")
        (is (not (some #(= chaos (:title %)) (:choices (prompt-map :challenger)))) "Chaos Theory isn't available anymore")
    ...
    (testing "Hosting Geist"
      ...
      (do-game
        ...
        (is (not-any? #(some #{%} (prompt-titles :challenger))
                      [professor whizzard jamie kate kit]))
        (click-prompt state :challenger geist)
    ...
    (testing "Geist does not trigger Laguna Velasco"
      ...
      (do-game
        (new-game (default-contestant)
                  ...
                  {:start-as :challenger})
        (starting-hand state :challenger ["DJ Fenris" "Laguna Velasco District" "All-nighter"])
        (core/gain state :challenger :credit 10)
        (play-from-hand state :challenger "All-nighter")
        (play-from-hand state :challenger "Laguna Velasco District")
        (play-from-hand state :challenger "DJ Fenris")
        (is (= (first (prompt-titles :challenger)) geist) "List is sorted")
        (is (every? #(some #{%} (prompt-titles :challenger))
        ...
        (click-prompt state :challenger geist)
        (is (= geist (get-in (get-radicle state 2) [:hosted 0 :title])) "Geist hosted on DJ Fenris")
        (is (= sunny (:title (:identity (get-challenger)))) "Still Hayley, id not changed")
        (let [hand-count (count (:hand (get-challenger)))]
          ;; Use All-nighter to trigger Geist
          (card-ability state :challenger (get-radicle state 0) 0)
          (is (= (+ 1 hand-count) (count (:hand (get-challenger))))
              "Drew one card with Geist when using All-nighter discard ability, not two (from Laguna Velasco District)"))))))
...
(deftest dummy-box
  ;; Dummy Box - discard a card from hand to prevent contestant discarding placed card
  (testing "Basic test"
    (do-game
      (new-game (default-contestant)
      ...
      (click-card state :challenger (find-card "Clot" (:hand (get-challenger))))
      ...
      (is (= 1 (count (get-resource state))) "Cache still placed")
      (is (= 1 (count (get-radicle state))) "Dummy Box still placed")))
...
(deftest eden-shard
  ;; Eden Shard - Place from Grip in lieu of accessing R&D; discard to make Contestant draw 2
  (testing "Basic test"
    (do-game
      (new-game (default-contestant)
  ...
  (testing "Do not place when accessing cards"
    (do-game
      ...
      (play-from-hand state :challenger "Eden Shard")
      (is (not (get-radicle state 0)) "Eden Shard not placed")
      (is (= 1 (count (:hand (get-challenger)))) "Eden Shard not placed"))))
...
(deftest fan-site
  ...
  (testing "Don't trigger after swap with Exchange of Information. Issue #1824"
    (do-game
      ...
      (click-card state :contestant (find-card "Fan Site" (:scored (get-challenger))))
      (click-card state :contestant (find-card "Hostile Takeover" (:scored (get-contestant))))
  ...
  (testing "Do not take a net damage when a hosted agenda is discarded due to film critic discard #2382"
    (do-game
      ...
      (let [fc (first (get-radicle state))]
        ...
        (click-card state :contestant fc)
...
(deftest find-the-truth
  ...
  (testing "Equivocation & FTT - should get order of choice"
    (do-game
      ...
      (play-from-hand state :challenger "Find the Truth")
      (run-empty-locale state :rd)
      (click-prompt state :challenger "Find the Truth")
      (is (= "Use Find the Truth to look at the top card of R&D?" (-> @state :challenger :prompt first :msg)) "FTT prompt")
      (click-prompt state :challenger "Yes")
      (is (= "The top card of R&D is Restructure" (-> @state :challenger :prompt first :msg)) "FTT shows card")
      (click-prompt state :challenger "OK") ; Equivocation prompt
...
(deftest gang-sign
  ...
  (testing "accessing from HQ, not including root. Issue #2113"
    (do-game
      ...
      (play-from-hand state :contestant "Hostile Takeover" "New party")
      (score-agenda state :contestant (get-content state :party1 0))
      (click-prompt state :challenger "Gang Sign") ; simultaneous effect resolution
...
(deftest guru-davinder
  ...
  (testing "cannot steal Obokata while placed"
    (do-game
...
(deftest investigative-journalism
  ...
  (do-game
    ...
    (is (empty? (get-radicle state)) "Contestant has no bad pub, couldn't place")
  ...
  (testing "should fire when discarding Chairman Hiro"
    (do-game
      ...
      (let [jak (get-radicle state 0)]
        ...
        (is (= 1 (get-counters (refresh jak) :credit)) "Jackpot! gains 1 credit per turn")
        (run-empty-locale state "Locale 1")
        (click-prompt state :challenger "Pay 6 [Credits] to discard") ;discard CH
        (click-prompt state :challenger "Yes") ;discard Jackpot!
        (click-prompt state :challenger "1")
...
(deftest john-masanori
  ...
  (do-game
    ...
    (run-empty-locale state "HQ")
    (click-prompt state :challenger "Pay 5 [Credits] to discard") ; discard crisium #2433
    (run-empty-locale state "Archives")
...
(deftest london-library
  ;; Place non-virus resources on London library. Includes #325/409
  (do-game
    (new-game (default-contestant)
    ...
    (let [lib (get-radicle state 0)]
      ...
      (card-ability state :challenger lib 0) ; Place a non-virus resource on London Library
      (click-card state :challenger (find-card "Femme Fatale" (:hand (get-challenger))))
      (click-prompt state :challenger "Done") ; Cancel out of Femme's bypass
      (is (= 1 (count (:hosted (refresh lib)))) "1 resource hosted")
      (card-ability state :challenger lib 0)
      (click-card state :challenger (find-card "Study Guide" (:hand (get-challenger))))
      ...
      (card-ability state :challenger lib 0)
      (click-card state :challenger (find-card "Chameleon" (:hand (get-challenger))))
      (click-prompt state :challenger "Sentry")
      (is (= 3 (count (:hosted (refresh lib)))) "3 resources hosted")
      (is (= 2 (:click (get-challenger))) "At 2 clicks")
      (card-ability state :challenger lib 0)
      (click-card state :challenger (find-card "Darwin" (:hand (get-challenger)))) ; Darwin is a virus
      ...
      (click-prompt state :challenger "Done")
      (click-card state :challenger (find-card "Study Guide" (:hosted (refresh lib))))
...
(deftest miss-bones
  ;; Miss Bones - credits for discarding placed cards, discard when empty
  (do-game
    (new-game
      (default-contestant)
      (default-challenger ["Miss Bones"]))
    (take-credits state :contestant)
    (play-from-hand state :challenger "Miss Bones")
    (let [mb (get-radicle state 0)]
      (is (= 12 (get-counters (refresh mb) :credit)) "Miss Bones starts with 12 credits")
      (is (= 3 (:credit (get-challenger))) "Challenger starts with 3 credits")
      (card-ability state :challenger mb 0)
      (is (= 11 (get-counters (refresh mb) :credit)) "Miss Bones loses a credit")
      (is (= 4 (:credit (get-challenger))) "Challenger gains a credit")
      (dotimes [_ 11]
        (card-ability state :challenger mb 0))
      (is (= 1 (count (:discard (get-challenger)))) "Miss Bones in discard pile")
      (is (empty? (get-radicle state)) "Miss Bones not placed")
      (is (= 15 (:credit (get-challenger))) "Challenger gained all 12 credits from Miss Bones"))))
    ...
    (do-game
      ...
      (let [toll (get-character state :hq 0)
        ...
        (let [muer (get-radicle state 0)]
          ...
          (click-card state :contestant toll)
...
(deftest net-mercur
  ...
  (do-game
    ...
    (let [sil (get-hazard state 0)
      ...
      (click-prompt state :challenger "Place 1 [Credits]")
...
(deftest network-exchange
  ;; Character place costs 1 more except for inner most
  (testing "Basic test"
    (do-game
      ...
      (is (= 8 (:credit (get-contestant))) "Paid 0 to place Paper Wall")
      (play-from-hand state :contestant "Paper Wall" "HQ")
      (is (= 6 (:credit (get-contestant))) "Paid 1 extra  to place Paper Wall")
      (play-from-hand state :contestant "Paper Wall" "HQ")
      (is (= 3 (:credit (get-contestant))) "Paid 1 extra  to place Paper Wall")))
  (testing "Architect 1st sub should ignore additional place cost"
    (do-game
      ...
      (let [architect (get-character state :hq 0)]
        ...
        (click-prompt state :contestant (find-card "Architect" (:deck (get-contestant))))
...
(deftest neutralize-all-threats
  ...
  (do-game
    ...
    (run-empty-locale state "HQ")
    (click-prompt state :challenger "Card from hand")
    (click-prompt state :challenger "No action") ; access first Hedge Fund
    (click-prompt state :challenger "Card from hand")
    (click-prompt state :challenger "No action") ; access second Hedge Fund
    (run-empty-locale state "Locale 1")
  ...
  (testing "ability shows a simultaneous resolution prompt when appropriate"
    (do-game
      ...
      (let [oca (get-radicle state 0)]
        ...
        (click-card state :challenger (find-card "Underworld Contact" (:hand (get-challenger))))
        (is (= 2 (count (:hand (get-challenger)))) "Drew a card from OCA")
        (card-ability state :challenger oca 0)
        (click-card state :challenger (find-card "Street Peddler" (:hand (get-challenger))))
        ...
        (click-prompt state :challenger "Off-Campus Apartment")
        (is (= 2 (count (:hand (get-challenger)))) "Drew a card from OCA"))))
  (testing "second ability does not break cards that are hosting others, e.g., Street Peddler"
    (do-game
      ...
      (let [oca (get-radicle state 0)]
        (card-ability state :challenger oca 0)
        (click-card state :challenger (find-card "Street Peddler" (:hand (get-challenger))))
        (click-prompt state :challenger "Street Peddler")
        (let [ped1 (first (:hosted (refresh oca)))]
          (card-ability state :challenger ped1 0)
          (click-prompt state :challenger (-> (get-challenger) :prompt first :choices second)) ; choose Street Peddler
          ...
          (let [ped2 (first (:hosted (refresh oca)))]
            ...
            (click-prompt state :challenger (-> (get-challenger) :prompt first :choices first)) ; choose Spy Camera
            ;; the fact that we got this far means the bug is fixed
            (is (= 1 (count (get-hazard state))) "Spy Camera placed")))))))
...
(deftest pad-tap
  ...
  (do-game
    ...
    (let [mel (get-content state :party1 0)
      ...
      (let [credits (:credit (get-challenger))]
        (core/reveal state :contestant mel)
...
(deftest paige-piper
  ...
  (testing "interaction with Frantic Coding. Issue #2190"
    (do-game
      ...
      (is (= 1 (count (get-resource state))) "Placed Gordian Blade")
      (click-prompt state :challenger "Yes")
      (click-prompt state :challenger "0")
      (is (= 1 (count (:discard (get-challenger)))) "Paige Piper intervention stopped Frantic Coding from discarding 9 cards")
      (is (= 5 (:credit (get-challenger))) "No charge to place Gordian")
      ;; a second Frantic Coding will not trigger Paige (once per turn)
      (play-from-hand state :challenger "Frantic Coding")
      (click-prompt state :challenger "OK")
      (click-prompt state :challenger (find-card "Ninja" (:deck (get-challenger))))
      (is (= 2 (count (get-resource state))) "Placed Ninja")
      (is (= 11 (count (:discard (get-challenger)))) "11 cards in heap")
      (is (= 2 (:credit (get-challenger))) "No charge to place Ninja"))))
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [p (get-radicle state 0)]
        ...
        (click-prompt state :challenger "Locale 1")
        (run-empty-locale state "Archives")
        (is (= 5 (count (:hand (get-challenger)))) "Did not draw cards when running other locale"))))
...
(deftest power-tap
  ...
  (do-game
    (new-game (default-contestant ["Restructured Datapool"])
              (default-challenger ["Power Tap"]))
    (play-and-score state "Restructured Datapool")
    (let [agenda (get-scored state :contestant 0)
          tags (:tag (get-challenger))
          credits (:credit (get-challenger))]
      (card-ability state :contestant agenda 0)
      (is (= credits (:credit (get-challenger))) "Challenger shouldn't gain any credits from trace")
      (click-prompt state :contestant "0")
      (click-prompt state :challenger "0")
      (is (= (+ tags 1) (:tag (get-challenger))) "Challenger should gain 1 tag from losing trace"))
    (take-credits state :contestant)
    (play-from-hand state :challenger "Power Tap")
    (take-credits state :challenger)
    (let [agenda (get-scored state :contestant 0)
          tags (:tag (get-challenger))
          credits (:credit (get-challenger))]
      (card-ability state :contestant agenda 0)
      (is (= (+ credits 1) (:credit (get-challenger))) "Challenger should gain 1 credit from trace initiation")
      (click-prompt state :contestant "0")
      (click-prompt state :challenger "0")
      (is (= (+ tags 1) (:tag (get-challenger))) "Challenger should gain 1 tag from losing trace"))))
...
(deftest psych-mike
  ...
  (testing "Basic test"
    (do-game
      (new-game (default-contestant [(qty "Ice Wall" 100)])
                (default-challenger ["Psych Mike" "Deep Data Mining"]))
      (take-credits state :contestant)
      (play-from-hand state :challenger "Psych Mike")
      (let [credits (:credit (get-challenger))]
        (run-empty-locale state "R&D")
        (click-prompt state :challenger "No action")
        (is (= (inc credits) (:credit (get-challenger))) "Psych Mike should give 1 credit for accessing 1 card"))
      (let [credits (:credit (get-challenger))]
        (run-empty-locale state "R&D")
        (click-prompt state :challenger "No action")
        (is (= credits (:credit (get-challenger))) "Psych Mike should give 0 credits for second run of the turn"))
      (take-credits state :challenger)
      (take-credits state :contestant)
      (play-from-hand state :challenger "Deep Data Mining")
      (let [credits (:credit (get-challenger))]
        (run-successful state)
        (dotimes [_ 5]
          (click-prompt state :challenger "Card from deck")
          (click-prompt state :challenger "No action"))
        (is (= (+ credits 5) (:credit (get-challenger))) "Psych Mike should give 5 credits for DDM accesses"))))
  (testing "vs regions"
    (do-game
      (new-game (default-contestant ["Bryan Stinson" (qty "Ice Wall" 100)])
                (default-challenger ["Psych Mike"]))
      (starting-hand state :contestant ["Bryan Stinson"])
      (play-from-hand state :contestant "Bryan Stinson" "R&D")
      (take-credits state :contestant)
      (play-from-hand state :challenger "Psych Mike")
      (let [credits (:credit (get-challenger))]
        (run-empty-locale state "R&D")
...
(deftest reclaim
  ;; Reclaim - discard Reclaim, discard card from grip, place resource, hazard, or virtual radicle from heap
  (testing "Basic behavior"
    (do-game
      ...
      (is (empty? (get-resource state)) "No resources placed")
      (is (= 5 (:credit (get-challenger))) "Challenger starts with 5c.")
      (card-ability state :challenger (get-radicle state 0) 0)
      (click-prompt state :challenger (find-card "Clone Chip" (:hand (get-challenger))))
      (click-prompt state :challenger (find-card "Mimic" (:discard (get-challenger))))
      (is (= 1 (count (get-resource state))) "1 Resource placed")
      (is (= 2 (:credit (get-challenger))) "Challenger paid place cost")))
  ...
  (testing "Can place discarded card"
    (do-game
      (new-game (default-contestant)
      ...
      (is (= 5 (:credit (get-challenger))) "Challenger starts with 5c.")
      (card-ability state :challenger (get-radicle state 0) 0)
      (click-prompt state :challenger (find-card "Mimic" (:hand (get-challenger))))
      (click-prompt state :challenger (find-card "Mimic" (:discard (get-challenger))))
      (is (= 1 (count (get-resource state))) "1 Resource placed")
      (is (= 2 (:credit (get-challenger))) "Challenger paid place cost")))
  (testing "Can't afford to place card"
    (do-game
      (new-game (default-contestant)
      ...
      (is (= 5 (:credit (get-challenger))) "Challenger starts with 5c.")
      (card-ability state :challenger (get-radicle state 0) 0)
      (click-prompt state :challenger (find-card "Alpha" (:hand (get-challenger))))
      (is (empty? (get-resource state)) "Did not place resource")
...
(deftest rosetta-2.0
  ;; Rosetta 2.0 remove an placed resource from the game and place one from the heap lower place cost
  (do-game
    (new-game (default-contestant)
    ...
    (is (= 1 (count (get-resource state))) "One resource placed")
    (is (= "Gordian Blade" (:title (get-resource state 0))) "Gordian placed")))

(deftest sacrificial-construct
  ;; Sacrificial Construct - Discard to prevent discard of placed resource or hazard
  (do-game
    (new-game (default-contestant)
    ...
    (is (= 1 (count (get-hazard state))) "Astrolabe still placed")))
...
(deftest scrubber
  ...
  (testing "when under discard cost but can up with recurring credits"
    (do-game
      ...
      (is (= 2 (-> (get-challenger) :prompt first :choices count)) "Challenger can use Scrubber credits to discard")
      (click-prompt state :challenger "Pay 6 [Credits] to discard") ;; Whoops, challenger forgot to actually get the credits from Scrubber
      ...
      (let [scrubber (get-radicle state 0)]
        (card-ability state :challenger scrubber 0)
        (card-ability state :challenger scrubber 0))
      (click-prompt state :challenger "Pay 6 [Credits] to discard") ;; Now the challenger has actually gained the Scrubber credits
      (is (= 2 (:agenda-point (get-challenger))) "Challenger should discard The Board and gain 2 agenda points"))))
...
(deftest salsette-slums
  ...
  (do-game
    ...
    (let [ts1 (get-content state :party1 0)
          hostile2 (get-content state :party2 0)
          th3 (get-content state :party3 0)
      ...
      (click-card state :challenger (find-card "Thomas Haas" (:discard (get-contestant))))
    ...
    (let [salsette1 (get-radicle state 0)
          hostile2 (get-content state :party2 0)]
      (card-ability state :challenger salsette1 1)
      (click-card state :challenger (find-card "Hostile Infrastructure" (:discard (get-contestant))))
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [st (get-radicle state 0)]
        ...
        (click-prompt state :challenger "Locale 1")
        (run-empty-locale state "Archives")
        (is (= 12 (:credit (get-challenger))) "Did not gain credits when running other locale"))))
...
(deftest street-peddler
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [sp (get-radicle state 0)]
        ...
        (click-prompt state :challenger (find-card "Gordian Blade" (:hosted sp))) ; choose to place Gordian
        (is (= "Gordian Blade" (:title (get-resource state 0)))
            "Gordian Blade was placed")
        (is (= 3 (core/available-mu state)) "Gordian cost 1 mu"))))
  (testing "Can't afford place"
    (do-game
      (new-game (default-contestant)
      ...
      (let [sp (get-radicle state 0)]
        ...
        (is (zero? (count (get-resource state)))
            "Gordian Blade was not placed")
        (is (and (:placed (refresh sp)) (= 3 (count (:hosted (refresh sp))))
                 "Street Peddler still placed with 3 hosted cards")))))
  (testing "Interaction with Kate discount"
    (do-game
      ...
      (let [sp (get-radicle state 0)]
        ...
        (card-ability state :challenger sp 0)
        (is (= 2 (count (:choices (first (:prompt (get-challenger))))))
            "Only 1 choice (plus Cancel) to place off Peddler")
        (click-prompt state :challenger (find-card "Gordian Blade" (:hosted sp))) ; choose to place Gordian
        (is (= "Gordian Blade" (:title (get-resource state 0)))
            "Gordian Blade was placed")
        (is (= 3 (core/available-mu state)) "Gordian cost 1 mu"))))
  (testing "Resources should cost memory. Issue #708"
    (do-game
      ...
      (let [sp (get-radicle state 0)]
        ...
        (card-ability state :challenger sp 0)
        (click-prompt state :challenger (first (:hosted sp))) ; choose to place Gordian
        (is (= "Corroder" (:title (get-resource state 0)))
            "Corroder was placed")
        (is (= 3 (core/available-mu state)) "Corroder cost 1 mu"))))
  (testing "Muertos/Brain Chip unplace effect not fired when removed off peddler/hosting Issue #2294, #2358"
    (do-game
      (new-game (default-contestant ["Jackson Howard"])
      ...
      (let [jh (get-content state :party1 0)
            sp (get-radicle state 0)]
        (core/reveal state :contestant jh)
        (card-ability state :challenger sp 0)
        (click-prompt state :challenger (find-card "Street Peddler" (:hosted sp))) ; choose to another Peddler
  ...
  (testing "Discarding hazard should not reduce :in-play values"
    (do-game
      ...
      (let [sp (get-radicle state 0)]
        (card-ability state :challenger sp 0)
        (click-prompt state :challenger (first (:hosted sp))) ; choose to place HQ Interface
        (is (= 2 (:hq-access (get-challenger)))
            "HQ Access increased by 1 from placed HQI and not reduced by the 2 discarded ones"))))
  (testing "Placing Parasite with only 1cr. Issue #491."
    (do-game
      ...
      (let [sp (get-radicle state 0)
        ...
        (card-ability state :challenger sp 0)
        (click-prompt state :challenger (first (:hosted sp))) ; choose to place Parasite
        (is (= "Parasite" (:title (:card (first (get-in @state [:challenger :prompt])))))
            "Parasite target prompt")
        (click-card state :challenger pu)
  ...
  (testing "Tech Trader place"
    (do-game
      ...
      (let [sp (get-radicle state 0)]
        ...
        (card-ability state :challenger sp 0)
        (click-prompt state :challenger (find-card "Tech Trader" (:hosted sp))) ; choose to place Tech Trader
        (is (= "Tech Trader" (:title (get-radicle state 0)))
            "Tech Trader was placed")
...
(deftest symmetrical-visage
  ...
  (testing "Gain 1 credit the first and second time you click to draw each turn when GCS is placed"
    (do-game
      (new-game (default-contestant)
                (default-challenger [(qty "Symmetrical Visage" 3)
...
(deftest synthetic-blood
  ...
  (testing "The first and second time you take damage each turn (with GCS placed), draw one card"
    (do-game
      (new-game (default-contestant [(qty "Data Mine" 3) (qty "Hedge Fund" 3)])
                (default-challenger [(qty "Synthetic Blood" 3)
                                 "Sure Gamble"
...
(deftest technical-writer
  ;; Technical Writer - Gain 1c per resource/hazard place; click/discard to take all credits
  (do-game
    (new-game (default-contestant)
    ...
    (let [tw (get-radicle state 0)]
      ...
      (is (= 3 (get-counters (refresh tw) :credit)) "No credit gained for radicle place")
...
(deftest the-helpful-ai
  ...
  (do-game
    ...
    (let [corr (get-resource state 0)]
      (card-ability state :challenger (get-radicle state 0) 0)
      (click-card state :challenger corr)
...
(deftest the-source
  ...
  (do-game
    ...
    (click-prompt state :challenger "Pay 3 [Credits] to steal") ; pay 3c extra to steal
    (is (= 4 (:credit (get-challenger))) "Paid 3c to steal")
    (is (= 2 (count (:discard (get-challenger)))) "The Source is discarded")
    (play-from-hand state :challenger "The Source")
    (take-credits state :challenger)
    (play-from-hand state :contestant "Hostile Takeover" "New party")
    (let [ht (get-content state :party2 0)]
...
(deftest the-supplier
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [ts (get-radicle state 0)]
        ...
        (click-card state :challenger (find-card "Plascrete Carapace" (:hand (get-challenger))))
        (card-ability state :challenger ts 0)
        (is (= 1 (count (-> @state :challenger :prompt first :choices))))
        (click-card state :challenger (find-card "Utopia Shard" (:hand (get-challenger))))
        ...
        (click-card state :challenger (find-card "Plascrete Carapace" (:hosted (refresh ts))))
        (is (= 2 (:credit (get-challenger)))
            "Challenger charged 1 credit to place Plascrete off The Supplier")
        (take-credits state :challenger)
  ...
  (testing "Interaction with Kate discount. Issue #578."
    (do-game
      ...
      (let [ts (get-radicle state 0)]
        ...
        (is (= 1 (count (get-radicle state))) "Plascrete placed"))))
  (testing "Brain chip mem is deducted when it is hosted and Supplier is discarded. Issue #2358"
    (do-game
      ...
      (let [ts (get-radicle state 0)]
        (card-ability state :challenger ts 0)
        (click-card state :challenger (find-card "Brain Chip" (:hand (get-challenger))))
        (is (= 4 (core/available-mu state)) "Challenger has 4 MU")
        (run-empty-locale state "Locale 1")
        (click-prompt state :challenger "Steal")
        (take-credits state :challenger)
...
(deftest thunder-art-gallery
  ...
  (testing "Works when removing/avoiding tags"
    (do-game
      (new-game (default-contestant)
                (default-challenger ["Thunder Art Gallery" "New Angeles City Hall" "Corroder"]))
      (take-credits state :contestant)
      (play-from-hand state :challenger "Thunder Art Gallery")
      (core/gain-credits state :challenger 1)
      (core/gain-tags state :contestant 1)
      (core/remove-tag state :challenger nil)
      (click-card state :challenger "New Angeles City Hall")
      (is (= 1 (:credit (get-challenger))) "Challenger paid one less to place (but 2 to remove tag)")
      (is (= "New Angeles City Hall" (:title (get-radicle state 1))) "NACH is placed")
      (take-credits state :challenger)
      (is (= 3 (:credit (get-challenger))) "Challenger is now at 3 credits")
      (core/gain-tags state :contestant 1)
      (card-ability state :challenger (get-radicle state 1) 0)
      (click-prompt state :challenger "Done")
      (click-card state :challenger "Corroder")
      (is (= 0 (:credit (get-challenger))) "Challenger paid one less to place")
      (is (= "Corroder" (:title (get-resource state 0))) "Corroder is placed"))))
...
(deftest the-black-file
  ...
  (testing "Basic test"
    (do-game
      ...
      (take-credits state :challenger)
      (play-from-hand state :contestant "Vanity Project" "New party")
      (score-agenda state :contestant (get-content state :party1 0))
  ...
  (testing "Spending counters shouldn't increase accesses when running a non-R&D/HQ locale"
    (do-game
      ...
      (let [ttw (get-radicle state 0)]
        ...
        (is (= 1 (-> @state :run :access-bonus)) "Challenger should access 1 additional card")
        (run-successful state)
        (is (zero? (-> (get-challenger) :register :last-run :access-bonus)) "Access bonuses are zeroed out when attacked locale isn't R&D or HQ")))))
...
(deftest theophilius-bagbiter
  ...
  (do-game
    ...
    (is (zero? (:credit (get-challenger))) "Challenger loses all credits on place")
    (is (= 1 (count (get-radicle state))) "Theophilius Bagbiter placed")
    ...
    (is (empty? (get-radicle state)) "No radicles placed")
  ...
  (testing "Move counters"
    (do-game
      ...
      (let [hive (get-resource state 0)
        ...
        (is (= 1 (get-counters (refresh vbg) :virus)) "Virus Breeding Ground gains 1 counter per turn")
        (card-ability state :challenger vbg 0)
        (click-card state :challenger hive)
  ...
  (testing "Move counters to a non-virus radicle"
    (do-game
      ...
      (let [vbg (get-radicle state 0)
        ...
        (click-card state :challenger (refresh crypt))
        (click-prompt state :challenger "Done")
...
(deftest wasteland
  ;; Wasteland - Gain 1c the first time you discard an placed card of yours each turn
  (do-game
    (new-game (default-contestant ["PAD Campaign"])
    ...
    (is (= 4 (:credit (get-challenger))) "Challenger has 4 credits")
    (run-empty-locale state "Locale 1")
    (click-prompt state :challenger "Pay 4 [Credits] to discard") ; Discard PAD campaign
    ...
    (let [faust (get-resource state 0)]
      ...
      (click-card state :challenger (first (:hand (get-challenger))))) ;discards a card
    ...
    (card-ability state :challenger (get-radicle state 1) 1)
    (is (= 4 (count (:discard (get-challenger)))) "Fall Guy discarded")
