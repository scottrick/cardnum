...
(deftest acme-consulting:-the-truth-you-need
  (testing "Tag gain when revealing outermost character"
    (do-game
      (new-game
      ...
      (is (not (core/is-tagged? @state)) "Challenger does not encounter an unrevealed character")
      (core/reveal state :contestant (get-character state :archives 0))
      (is (core/is-tagged? @state) "Challenger is tagged when encountering outermost character")
      (run-continue state)
      (is (not (core/is-tagged? @state)) "Challenger no longer encountering outermost character")))
  (testing "Interaction with Data Ward"
    (do-game
      ...
      (play-from-hand state :contestant "Data Ward" "Archives")
      (take-credits state :contestant)
      (run-on state :archives)
      (is (not (core/is-tagged? @state)) "Challenger does not encounter an unrevealed character")
      (core/reveal state :contestant (get-character state :archives 0))
      (is (core/is-tagged? @state) "Challenger is tagged when encountering outermost character")
      (card-subroutine state :contestant (get-character state :archives 0) 0)
  ...
  (testing "Tag gain when starting run"
    (do-game
      ...
      (run-continue state)
      (is (not (core/is-tagged? @state)) "Challenger no longer encountering outermost character")))
  (testing "Tag loss when hidezing character"
    (do-game
      (new-game
        (make-deck "Acme Consulting: The Truth You Need" ["Vanilla" (qty "Hedge Fund" 5)])
        (default-challenger))
      ...
      (run-on state :archives)
      (is (core/is-tagged? @state) "Challenger is tagged when encountering outermost character")
      (core/hide state :contestant (get-character state :archives 0))
      (is (not (core/is-tagged? @state)) "Challenger no longer encountering the hidden character")))
  (testing "No tag on empty locale"
    (do-game
      (new-game
        (make-deck "Acme Consulting: The Truth You Need" ["Vanilla" (qty "Hedge Fund" 5)])
        (default-challenger))
      (take-credits state :contestant)
      (run-on state :archives)
      (is (not (core/is-tagged? @state)) "No character to encounter")))
  (testing "No tag when encountering second character"
    (do-game
      (new-game
      ...
      (is (core/is-tagged? @state) "Challenger is tagged when encountering outermost character")
      (run-continue state)
      (is (not (core/is-tagged? @state)) "Challenger is not tagged when encountering second character"))))
...
(deftest adam:-compulsive-hacker
  ...
  (testing "Allow challenger to choose directives"
    (do-game
      ...
      (click-card state :challenger (find-card "Neutralize All Threats" (get-in @state [:challenger :play-area])))
      (click-card state :challenger (find-card "Safety First" (get-in @state [:challenger :play-area])))
      (click-card state :challenger (find-card "Always Be Running" (get-in @state [:challenger :play-area])))
  ...
  (testing "Directives should not grant Pālanā credits"
    (do-game
      ...
      (click-card state :challenger (find-card "Safety First" (get-in @state [:challenger :play-area])))
      (click-card state :challenger (find-card "Always Be Running" (get-in @state [:challenger :play-area])))
      (click-prompt state :contestant "Keep")
      (click-prompt state :challenger "Keep")
...
(deftest akiko-nisei:-head-case
  ...
  (testing "Basic test"
    (do-game
      ...
      (is (= 2 (core/access-count state :challenger :rd-access)) "Should access additional card from ability")
      (take-credits state :challenger)
      (take-credits state :contestant)
      (run-on state :rd)
      (run-successful state)
      ...
      (click-prompt state :challenger "0 [Credits]")
      (is (= 1 (core/access-count state :challenger :rd-access)) "Should only access 1 from missed psi game")))
  (testing "Shiro interaction: second sub should give Akiko 2 accesses"
    (do-game
      (new-game (default-contestant [(qty "Hedge Fund" 10) "Shiro"])
                (make-deck "Akiko Nisei: Head Case" [(qty "Sure Gamble" 3)]))
      (starting-hand state :contestant ["Shiro"])
      (play-from-hand state :contestant "Shiro" "New party")
      (let [shiro (get-character state :party1 0)]
...
(deftest alcharacter-merchant:-clan-agitator
  ;; Alcharacter Merchant
  (do-game
    (new-game (default-contestant)
              (make-deck "Alcharacter Merchant: Clan Agitator"
                         ["Security Testing"]))
    ...
    (click-prompt state :challenger "Alcharacter Merchant: Clan Agitator")
    (click-prompt state :contestant (find-card "Hedge Fund" (:hand (get-contestant))))
    (is (= 1 (-> (get-contestant) :discard count)) "Alcharacter ability should discard 1 card from HQ")
    (is (-> (get-contestant) :discard first :seen not) "Discarded card should be facedown when access is replaced")))
...
(deftest apex:-invasive-predator
  ...
  (do-game
    ...
    (click-prompt state :challenger "Done") ; no facedown place on turn 1
...
(deftest asa-group:-security-through-vigilance
  (testing "Asa Group should not allow placing operations"
    (do-game
      ...
      (click-card state :contestant (find-card "BOOM!" (:hand (get-contestant))))
      (is (empty? (get-content state :party1)) "Asa Group placed an event in a locale")
      (click-card state :contestant (find-card "Urban Renewal" (:hand (get-contestant))))
  ...
  (testing "Asa Group ordering correct when playing Mirrormontestanth"
    (do-game
      ...
      (let  [marilyn (find-card "Marilyn Campaign" (:hand (get-contestant)))
        ...
        (click-card state :contestant marilyn)
        (click-prompt state :contestant "New party")
        (is (= (:cid marilyn) (:cid (get-content state :party1 0))) "Marilyn is placed as first card")
        (click-card state :contestant herrings) ;; This should be the Asa prompt, should be automatically placed in party1
        (is (= (:cid herrings) (:cid (get-content state :party1 1))) "Red Herrings is placed in Locale 1")
        (click-card state :contestant vitruvius)
        (click-prompt state :contestant "New party")
        (click-card state :contestant pup)
        (click-prompt state :contestant "New party")
...
(deftest ^{:card-title "ayla-\"bios\"-rahim:-simulant-specialist"}
  ...
  (do-game
    ...
    (is (zero? (count (get-in @state [:challenger :hand]))))
    (click-card state :challenger (find-card "Sure Gamble" (get-in @state [:challenger :play-area])))
    (click-card state :challenger (find-card "Desperado" (get-in @state [:challenger :play-area])))
    (click-card state :challenger (find-card "Bank Job" (get-in @state [:challenger :play-area])))
    (click-card state :challenger (find-card "Eater" (get-in @state [:challenger :play-area])))
    ...
    (click-prompt state :challenger (find-card "Bank Job" (:hosted (:identity (get-challenger)))))
...
(deftest chronos-protocol:-selective-mind-mapping
    ...
    (do-game
      ...
      (let [pup (get-character state :hq 0)]
        ...
        (let [imp (find-card "Imp" (:hand (get-challenger)))]
          (click-prompt state :contestant imp)
  ...
  (testing "with Obokata: Pay 4 net damage to steal.  Only 3 damage left after Chronos.  No trigger of damage prevent."
    (do-game
      ...
      (click-prompt state :contestant "Yes")
      (click-prompt state :contestant (find-card "Inti" (:hand (get-challenger))))
...
(deftest exile:-streethawk
  ...
  (testing "Simultaneous-resolution prompt shown for interaction with Customized Secretary"
    (do-game
      ...
      (click-card state :challenger (find-card "Customized Secretary" (:discard (get-challenger))))
      ...
      (click-prompt state :challenger "Exile: Streethawk")
  ...
  (testing "Only works with Sites, Character, Operations, and Regions"
    (letfn [(fk-test [card]
              (do-game
                ...
                (run-empty-locale state "HQ")
                (click-prompt state :challenger "[Freedom]: Discard card")
  ...
  (testing "Can use viruses on hosted cards"
    (do-game
      ...
      (let [iw (get-character state :rd 0)]
        ...
        (run-empty-locale state "HQ")
        (click-prompt state :challenger "[Freedom]: Discard card")
        (click-card state :challenger (-> (refresh iw) :hosted first)))
...
(deftest gagarin-deep-space:-expanding-the-horizon
  ...
  (do-game
    ...
    (run-empty-locale state :party1)
    (click-card state :challenger (get-content state :party1 0))
    (click-prompt state :challenger "OK") ; Could not afford message dismissed
...
(deftest jemison-astronautics:-sacrifcharacter.-audacity.-success.
    ...
    (do-game
      ...
      (let [enf (get-character state :hq 0)
        ...
        (let [dd (get-radicle state 0)]
          ...
          (click-card state :challenger (get-in (get-challenger) [:scored 0]))
  ...
  (testing "24/7 - Armed Intimidation combination"
    ...
    (do-game
      ...
      (click-card state :contestant (get-scored state :contestant 0)) ; select HT to forfeit
      (let [character-wall (get-character state :hq 0)]
        (click-card state :contestant character-wall) ; The Jemison forfeit triggers
        (is (= 2 (get-counters (refresh character-wall) :advancement)) "Ice Wall has 2 advancement counters from HT forfeit"))
      (click-card state :contestant (get-scored state :contestant 0)) ; select AI to trigger
      (click-prompt state :challenger "Take 2 tags") ; First challenger has prompt
...
(deftest cardnum-biotech:-life-imagined
  ...
  (testing "Brewery net damage"
    (do-game
      ...
      (click-prompt state :contestant "The Brewery")
      (core/start-turn state :contestant nil)
  ...
  (testing "Greenhouse four advancement tokens"
    (do-game
      (new-game
        ...
        (default-challenger)
        {:dont-start-turn true})
      (click-prompt state :contestant "The Greenhouse")
      (core/start-turn state :contestant nil)
      ...
      (let [bt (get-content state :party1 0)]
        ...
        (card-ability state :contestant (:identity (get-contestant)) 1)
        (click-card state :contestant (refresh bt))
  ...
  (testing "Tank shuffle Archives into R&D"
    (do-game
      (new-game
        ...
        (default-challenger)
        {:dont-start-turn true})
      (click-prompt state :contestant "The Tank")
      (core/start-turn state :contestant nil)
      (play-from-hand state :contestant "Hedge Fund")
      (play-from-hand state :contestant "Hedge Fund")
      (play-from-hand state :contestant "Hedge Fund")
...
(deftest leela-patel:-trained-pragmatist
  ...
  (testing "complicated interaction with mutiple Gang Sign"
    (do-game
      ...
      (take-credits state :challenger)
      (score-agenda state :contestant (get-content state :party1 0))
      (click-prompt state :challenger "Leela Patel: Trained Pragmatist")
      (click-card state :challenger (get-content state :party2 0))
      (is (find-card "Hostile Takeover" (:hand (get-contestant))) "Hostile Takeover returned to hand")
      (click-prompt state :challenger "Gang Sign")
      (click-prompt state :challenger "Card from hand")
      (click-prompt state :challenger "Steal")
      (is (find-card "Hostile Takeover" (:scored (get-challenger))) "Hostile Takeover stolen with Gang Sign")
      (click-card state :challenger (get-content state :party3 0))
  ...
  (testing "regions returned to hand in the middle of a run do not break the run. Issue #2008"
    (do-game
      ...
      (click-prompt state :challenger "Card from hand")
      (click-prompt state :challenger "Steal")
      (click-card state :challenger (get-content state :hq 0))
      ...
      (click-prompt state :challenger "Steal")
      (click-card state :challenger (get-content state :rd 0))
      (is (not (get-content state :rd 0)) "Region returned to hand")
      (is (not (:run @state)) "Run ended, no more accesses")
      (run-empty-locale state "Archives")
      (click-prompt state :challenger "Shock!")
      (click-prompt state :challenger "Project Atlas")
      (click-prompt state :challenger "Steal")
...
(deftest liza-talking-thunder:-prominent-legislator
  ...
  (do-game
    ...
    (take-credits state :contestant)
    (run-empty-locale state "R&D")
    (is (= 7 (count (:hand (get-challenger)))) "Drew 2 cards from successful run on Archives")
    (is (= 1 (:tag (get-challenger))) "Took 1 tag from successful run on Archives")))
...
(deftest mti-mwekundu:-life-improved
  ...
  (testing "No character"
    (do-game
      ...
      (click-card state :contestant (find-card "Enigma" (:hand (get-contestant))))
...
(deftest ^{:card-title "nathaniel-\"gnat\"-hall:-one-of-a-kind"} gnat
  ...
  (do-game
    ...
    (take-credits state :contestant)
    (is (= 5 (:credit (get-challenger))) "Did not gain a credit when Gnat is on 3 cards")
    (play-from-hand state :challenger "Sure Gamble")
    (take-credits state :challenger)
    (let [challenger-credits (:credit (get-challenger))]
      (take-credits state :contestant)
      (is (= (inc challenger-credits) (:credit (get-challenger)))) "Gained 1 credits when on 2 cards")))
...
(deftest new-angeles-sol:-your-news
  ...
  (do-game
    ...
    (click-card state :contestant (find-card "Paywall Implementation" (:hand (get-contestant))))
    (is (not (:run @state)) "Run ended")
...
(deftest next-design:-guarding-the-net
  ...
  (do-game
    (new-game
      ...
      (default-challenger)
      {:dont-start-turn true})
    (click-card state :contestant (find-card "Snowflake" (:hand (get-contestant))))
...
(deftest noise:-hacker-extraordinaire
  ...
  (do-game
    ...
    (let [chip (get-hazard state 0)]
      ...
      (click-card state :challenger (find-card "Cache" (:discard (get-challenger))))
    ...
    (let [chip-2 (get-hazard state 0)]
      ...
      (click-card state :challenger (find-card "Sharpshooter" (:discard (get-challenger))))
  ...
  (testing "does not affect next character when current is discarded. Issue #1788."
    (do-game
      ...
      (let [null (get-in @state [:challenger :identity])
        ...
        (click-card state :challenger (refresh spider))
        (run-on state "HQ")
        (run-continue state)
        (card-ability state :challenger null 0)
        (click-card state :challenger (first (:hand (get-challenger))))
...
(deftest saraswati-mnemonics:-endless-exploration
  ...
  (do-game
    (new-game
      (make-deck "Saraswati Mnemonics: Endless Exploration" ["Gene Splcharacterr" "House of Knives"])
      (default-challenger))
    (card-ability state :contestant (get-in @state [:contestant :identity]) 0)
    (click-card state :contestant (find-card "Gene Splcharacterr" (:hand (get-contestant))))
    (click-prompt state :contestant "New party")
    (let [splcharacterr (get-content state :party1 0)]
      (is (= 1 (get-counters (refresh splcharacterr) :advancement)) "1 advancements placed on Gene Splcharacterr")
      (core/reveal state :contestant (refresh splcharacterr))
      (is (not (:revealed (refresh splcharacterr))) "Gene Splcharacterr did not reveal")
      (take-credits state :contestant)
      (take-credits state :challenger)
      (core/reveal state :contestant (refresh splcharacterr))
      (is (:revealed (refresh splcharacterr)) "Gene Splcharacterr now revealed")
      (card-ability state :contestant (get-in @state [:contestant :identity]) 0)
      (click-card state :contestant (find-card "House of Knives" (:hand (get-contestant))))
      (click-prompt state :contestant "New party")
      (let [house (get-content state :party2 0)]
        (advance state house)
        (advance state house)
        (core/score state :contestant (refresh house))
        (is (empty? (:scored (get-contestant))) "House of Knives not scored")
        (is (zero? (:agenda-point (get-contestant))))
        (take-credits state :contestant)
        (take-credits state :challenger)
        (core/score state :contestant (refresh house))
        (is (= 1 (:agenda-point (get-contestant))) "House of Knives was able to be scored")))))
...
(deftest skontestantios-defense-systems:-persuasive-power
  ...
  (do-game
    ...
    (is (not (:run @state)))
    (card-ability state :contestant (get-in @state [:contestant :identity]) 0)
    (click-prompt state :contestant (find-card "The Maker's Eye" (:discard (get-challenger))))
...
(deftest silhouette:-stealth-operative
  ...
  (testing "Expose trigger ability resolves completely before access. Issue #2173"
    (do-game
      ...
      (let [psychic (get-content state :party1 0)
        ...
        (click-card state :challenger psychic)
  ...
  (testing "with Temüjin; broken interaction with other successful-run triggers. Issue #1968"
    (do-game
      ...
      (take-credits state :contestant)
      (run-empty-locale state :hq)
      (click-prompt state :challenger "Temüjin Contract")
      (click-card state :challenger (get-content state :party1 0))
      (click-prompt state :challenger "No action")
...
(deftest sportsmetal:-go-big-or-go-home
  ...
  (testing "Gain 2 credits on score"
    (do-game
      ...
      (click-prompt state :contestant "2 credits")
      (is (= 7 (:credit (get-contestant))) "Contestant gains 2 credits")))
  (testing "Gain 2 credits on steal"
    (do-game
      ...
      (click-prompt state :challenger "Steal")
      (click-prompt state :contestant "2 credits")
      (is (= 9 (:credit (get-contestant))) "Contestant gains 2 credits")))
  (testing "Gain 2 cards on score"
    (do-game
      (new-game
        (make-deck "Sportsmetal: Go Big or Go Home" ["Merger" (qty "Hedge Fund" 2)])
        (default-challenger))
      (starting-hand state :contestant ["Merger"])
      (play-from-hand state :contestant "Merger" "New party")
      (score-agenda state :contestant (get-content state :party1 0))
      (is (empty? (:hand (get-contestant))) "Contestant starts with no cards")
      (click-prompt state :contestant "2 cards")
      (is (= 2 (count (:hand (get-contestant)))) "Contestant draws 2 cards")))
...
(deftest sso-industries:-fueling-innovation
  ...
  (do-game
    ...
    (let [h0 (get-character state :hq 0)
      ...
      (take-credits state :contestant)
      (click-prompt state :contestant "Yes")
      (click-card state :contestant (refresh h0))
      ...
      (take-credits state :contestant)
      (click-prompt state :contestant "Yes")
      (click-card state :contestant (refresh h1))
...
(deftest the-foundry:-refining-the-process
  ...
  (testing "interaction with Accelerated Beta Test"
    (do-game
      ...
      (score-agenda state :contestant (get-content state :party1 0))
      (click-prompt state :contestant "Yes")
      (click-card state :contestant (find-card "Eli 1.0" (:play-area (get-contestant))))
      (click-prompt state :contestant "Archives")
      (click-prompt state :contestant "Yes")
      (is (empty? (:play-area (get-contestant))) "Play area shuffled into R&D")
      (is (= 1 (count (:hand (get-contestant)))) "Added Eli 1.0 to HQ"))))
...
(deftest the-outfit:-family-owned-and-operated
  ...
  (testing "basic test"
    (do-game
      ...
      (score-agenda state :contestant (get-content state :party2 0))
      (click-prompt state :contestant "3")  ;; Take 3 bad publicity from Profiteering, gain 15
  ...
  (testing "vs Valencia - 1 bad pub at start means 5 credits to start with (does not _gain_ BP)"
    (do-game
      ...
      (is (= 5 (:credit (get-contestant))) "The Outfit starts with 8 credits")
      (play-from-hand state :contestant "Hostile Takeover" "New party")
      (score-agenda state :contestant (get-content state :party1 0))
      (is (= 2 (:bad-publicity (get-contestant))) "Take 1 bad publicity")
      (is (= (+ 5 7 3) (:credit (get-contestant))) "Gain 7 from Hostile Takeover + 3 from The Outfit"))))
    ...
    (do-game
      ...
      (let [cst (get-content state :party1 0)
        ...
        (let [scored (get-scored state :contestant 0)]
          ...
          (click-card state :contestant (refresh scored))
...
(deftest wyvern:-chemically-enhanced
  ...
  (do-game
    ...
    (run-empty-locale state "Locale 1")
    (click-prompt state :challenger "Pay 2 [Credits] to discard")  ;; discard Launch Campaign, should trigger wyvern
