...
(deftest algernon
  ;; Algernon - pay 2 credits to gain a click, discard if no successful run
  (testing "Use, successful run"
    (do-game
      (new-game (default-contestant)
                (default-challenger ["Algernon"]))
      (take-credits state :contestant)
      (play-from-hand state :challenger "Algernon")
      (take-credits state :challenger)
      (take-credits state :contestant)
      (is (= 8 (:credit (get-challenger))) "Challenger starts with 8 credits")
      (is (= 4 (:click (get-challenger))) "Challenger starts with 4 clicks")
      (click-prompt state :challenger "Yes")
      (is (= 6 (:credit (get-challenger))) "Challenger pays 2 credits")
      (is (= 5 (:click (get-challenger))) "Challenger gains 1 click")
      (run-on state "Archives")
      (run-successful state)
      (take-credits state :challenger)
      (is (empty? (:discard (get-challenger))) "No cards discarded")
      (is (= "Algernon" (:title (get-resource state 0))) "Algernon still placed")))
  (testing "Use, no successful run"
    (do-game
      ...
      (take-credits state :challenger)
      (is (= 1 (count (:discard (get-challenger)))) "Algernon discarded")
  ...
  (testing "Not used, no successful run"
    (do-game
      ...
      (is (= 8 (:credit (get-challenger))) "No credits spent")
      (is (= 4 (:click (get-challenger))) "No clicks gained")
      (run-on state "Archives")
      (core/jack-out state :challenger nil)
      (take-credits state :challenger)
      (is (empty? (:discard (get-challenger))) "No cards discarded")
...
(deftest bankroll
  ...
  (do-game
    (new-game (default-contestant)
              (default-challenger ["Bankroll"]))
    (take-credits state :contestant)
    (play-from-hand state :challenger "Bankroll")
    (is (= 3 (core/available-mu state)) "Bankroll uses up 1 MU")
    (is (= 4 (:credit (get-challenger))) "Bankroll cost 1 to place")
    (let [bankroll (get-resource state 0)
          hosted-credits #(get-counters (refresh bankroll) :credit)]
      (is (= 0 (hosted-credits)) "No counters on Bankroll on place")
      ...
      (card-ability state :challenger bankroll 0)
      (is (= (+ 4 3) (:credit (get-challenger))) "Gained 3 credits when discarding Bankroll")
      (is (= 1 (-> (get-challenger) :discard count)) "Bankroll was discarded"))))
  ...
  (testing "does not affect next character when current is discarded. Issue #1788"
    (do-game
      ...
      (let [sucker (get-resource state 0)
        ...
        (click-card state :challenger "Spiderweb")
        (run-on state "HQ")
        (run-continue state)
  ...
  (testing "Hosted Chakana does not disable advancing agendas. Issue #750"
    (do-game
      ...
      (let [djinn (get-resource state 0)
        ...
        (click-card state :challenger (find-card "Chakana" (:hand (get-challenger))))
  ...
  (testing "Tutor a virus resource"
    (do-game
      ...
      (let [djinn (get-resource state 0)]
        ...
        (click-prompt state :challenger (find-card "Parasite" (:deck (get-challenger))))
...
(deftest equivocation
  ...
  (do-game
    ...
    (click-prompt state :challenger "Laramy Fisk: Savvy Investor")
    (click-prompt state :challenger "Yes")
    (is (= 2 (count (:hand (get-contestant)))) "Contestant forced to draw by Fisk")
    (click-prompt state :challenger "Yes") ; Equivocation prompt
    (click-prompt state :challenger "Yes") ; force the draw
...
(deftest false-echo
  ...
  (do-game
    ...
    (let [echo1 (get-resource state 0)
      ...
      (click-prompt state :contestant "Add to HQ")
      ...
      (click-prompt state :contestant "Reveal")
  ...
  (testing "triggering a Dhegdeered Hyperdriver should not grant +3 MU"
    (do-game
      ...
      (let [dheg (get-resource state 0)]
        ...
        (click-card state :challenger (find-card "Hyperdriver" (:hand (get-challenger))))
...
(deftest incubator
  ...
  (do-game
    ...
    (let [ds (get-resource state 0)
      ...
      (click-card state :challenger ds)
...
(deftest ixodidae
  ...
  (do-game
    ...
    (let [s (get-character state :hq 0)]
      ...
      (is (prompt-is-card? state :contestant s) "Contestant prompt is on Snowflake")
      (is (prompt-is-card? state :challenger s) "Challenger prompt is on Snowflake")
...
(deftest kyuban
  ;; Kyuban
  (testing "Gain creds when passing a piece of character, both when revealed and when unrevealed."
    (do-game
      (new-game (default-contestant [(qty "Lockdown" 3)])
                (default-challenger [(qty "Kyuban" 1)]))
      (play-from-hand state :contestant "Lockdown" "HQ")
      (play-from-hand state :contestant "Lockdown" "Archives")
      (let [ld1 (get-character state :archives 0)
            ld2 (get-character state :hq 0)]
        (take-credits state :contestant)
        (play-from-hand state :challenger "Kyuban")
        (click-card state :challenger ld1)
        (let [starting-creds (:credit (get-challenger))]
          (run-on state "HQ")
          (core/no-action state :contestant nil)
          (run-continue state)
          (is (= starting-creds (:credit (get-challenger))) "Gained no money for passing other character")
          (core/jack-out state :challenger nil)
          (run-on state "Archives")
          (core/no-action state :contestant nil)
          (run-continue state)
          (is (= (+ starting-creds 2) (:credit (get-challenger)))
              "Gained 2 creds for passing unrevealed host character"))
        (let [starting-creds-2 (:credit (get-challenger))]
          (core/jack-out state :challenger nil)
          (run-on state "Archives")
          (core/reveal state :contestant ld1)
          (core/no-action state :contestant nil)
          (run-continue state)
          (is (= (+ starting-creds-2 2) (:credit (get-challenger)))
              "Gained 2 creds for passing revealed host character"))))))
...
(deftest paintbrush
  ...
  (do-game
    ...
    (let [iwall (get-character state :hq 0)
      ...
      (card-ability state :challenger pb 0)
      (click-card state :challenger iwall)
      (click-prompt state :challenger "Code Gate")
...
(deftest parasite
  ...
  (testing "Placed facedown w/ Apex"
    (do-game
      ...
      (take-credits state :contestant)
      (core/end-phase-12 state :challenger nil)
      (click-card state :challenger (find-card "Parasite" (:hand (get-challenger))))
  ...
  (testing "Placed on undiscardable Architect should keep gaining counters past 3 and make strength go negative"
    (do-game
      ...
      (let [arch (get-character state :hq 0)]
        ...
        (play-from-hand state :challenger "Parasite")
        (click-card state :challenger arch)
  ...
  (testing "Should stay on hosted card moved by Builder"
    (do-game
      ...
      (let [builder (get-character state :archives 0)]
        ...
        (take-credits state :contestant)
        (play-from-hand state :challenger "Parasite")
        (click-card state :challenger builder)
    ...
    (do-game
      ...
      (let [enig (get-character state :hq 0)]
        ...
        (let [hive (get-resource state 0)]
          ...
          (play-from-hand state :challenger "Parasite")
          (click-card state :challenger enig)
...
(deftest reaver
  ...
  (testing "with Freelance Coding Construct - should not draw when discard from hand #2671"
    (do-game
      ...
      (click-card state :challenger "Snitch")
...
(deftest scheherazade
  ...
  (do-game
    ...
    (let [sch (get-resource state 0)]
      ...
      (card-ability state :challenger sch 0)
      (click-card state :challenger (find-card "Cache" (:hand (get-challenger))))
...
(deftest self-modifying-code
  ...
  (do-game
    ...
    (let [smc1 (get-resource state 0)
      ...
      (click-prompt state :challenger (find-card "Reaver" (:deck (get-challenger))))
...
(deftest snitch
  ...
  (do-game
    ...
    (let [snitch (get-resource state 0)]
      ...
      (is (prompt-is-card? state :challenger snitch) "Option to jack out")
      (click-prompt state :challenger "Yes")
...
(deftest takobi
  ...
  (do-game
    ...
    (let [tako (get-resource state 0)
      ...
      (card-ability state :challenger tako 1)
      (click-card state :challenger (refresh faus))
      (is (not-empty (:prompt (get-challenger))) "Can't select AI breakers")
      (click-card state :challenger (refresh corr))
  ...
  (testing "Hivemind and Architect interactions"
    (do-game
      ...
      (let [architect-revealed (get-character state :hq 0)
        ...
        (click-card state :challenger (game.core/get-card state architect-revealed))
        (play-from-hand state :challenger "Trypano")
        (click-card state :challenger architect-unrevealed)
    ...
    (do-game
      ...
      (let [architect-unrevealed (get-character state :rd 0)]
        ...
        (let [hive (get-resource state 0)]
          ...
          (click-card state :challenger (refresh hive))
...
(deftest wari
  (do-game
    ...
    (run-empty-locale state "HQ")
    (click-prompt state :challenger "Yes")
    (click-prompt state :challenger "Barrier")
