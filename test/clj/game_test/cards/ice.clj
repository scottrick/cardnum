...
(deftest blockchain
  (do-game
    (new-game (default-contestant ["Blockchain" (qty "Beanstalk Royalties" 5)])
              (default-challenger))
    (core/gain state :contestant :credit 2 :click 5)
    (play-from-hand state :contestant "Blockchain" "HQ")
    (let [bc (get-character state :hq 0)]
      (core/reveal state :contestant bc)
      (card-ability state :contestant bc 0)
      ...
      (is (= 12 (:credit (get-contestant))) "Contestant has 12 credits from four Beanstalks")
      (card-subroutine state :contestant bc 0)
      (is (= 13 (:credit (get-contestant))) "Contestant gained 1 credit from Blockchain")
      (is (= 4 (:credit (get-challenger))) "Challenger lost 1 credit from Blockchain"))))
...
(deftest crick
  ...
  (do-game
    ...
    (let [cr1 (get-character state :hq 0)
      ...
      (click-card state :contestant (find-card "Ice Wall" (:discard (get-contestant))))
...
(deftest data-hound
  ...
  (do-game
    ...
    (let [dh (get-character state :hq 0)]
      ...
      (click-prompt state :contestant (find-card "Desperado" (:deck (get-challenger))))
      (is (= 1 (count (:discard (get-challenger)))))
      (click-prompt state :contestant (find-card "Sure Gamble" (:deck (get-challenger))))
      (click-prompt state :contestant (find-card "Corroder" (:deck (get-challenger))))
      (click-prompt state :contestant (find-card "Patron" (:deck (get-challenger))))
      ;; try starting over
      (click-prompt state :contestant "Start over")
      (click-prompt state :contestant (find-card "Patron" (:deck (get-challenger))))
      (click-prompt state :contestant (find-card "Corroder" (:deck (get-challenger))))
      (click-prompt state :contestant (find-card "Sure Gamble" (:deck (get-challenger)))) ;this is the top card on stack
...
(deftest formicary
  ;; Formicary - when approaching locale, may reveal and move to innermost
  (testing "Verifies basic functionality and that First Responders may trigger"
    (do-game
     (new-game (default-contestant [(qty "Ice Wall" 2) (qty "Formicary" 3)])
               (default-challenger [(qty "First Responders" 6)]))
     (play-from-hand state :contestant "Ice Wall" "HQ")
     (play-from-hand state :contestant "Formicary" "Archives")
     (play-from-hand state :contestant "Formicary" "R&D")
     (take-credits state :contestant)
     (play-from-hand state :challenger "First Responders")
     (let [iw (get-character state :hq 0)
           form1 (get-character state :rd 0)
           form2 (get-character state :archives 0)
           responders (get-radicle state 0)]
       (run-on state "HQ")
       (run-continue state)             ; pass the first character
       (is (= 0 (get-in @state [:run :position])) "Now approaching locale")
       (core/reveal state :contestant form1)
       (click-prompt state :contestant "Yes")      ; Move Formicary
       ...
       (click-prompt state :challenger "Yes")      ; take 2 net
       (is (= 2 (count (:discard (get-challenger)))) "Did 2 net damage")
       (run-jack-out state)
       (let [cards-in-hand (count (:hand (get-challenger)))]
         (card-ability state :challenger responders 0)
         (is (= (inc cards-in-hand) (count (:hand (get-challenger)))) "First Responders was able to trigger"))
       (run-on state "Archives")
       (run-continue state)
       (core/reveal state :contestant form2)
       (click-prompt state :contestant "Yes")      ; Move Formicary
       (is (= 1 (get-in @state [:run :position])) "Now approaching Formicary")
       (card-subroutine state :contestant (refresh form2) 0)
       (click-prompt state :challenger "No")      ; ETR
       (is (not (get-in @state [:run])) "Formicary ended the run"))))
  (testing "Verifies that Formicary can be moved to the innermost positon of its own locale"
    (do-game
     (new-game (default-contestant ["Ice Wall" "Formicary"])
               (default-challenger))
     (play-from-hand state :contestant "Ice Wall" "HQ")
     (play-from-hand state :contestant "Formicary" "HQ")
     (take-credits state :contestant)
     (let [form (get-character state :hq 1)]
       (run-on state "HQ")
       (run-continue state)             ; pass the first character
       (run-continue state)             ; pass the second character
       (is (= 0 (get-in @state [:run :position])) "Now approaching locale")
       (core/reveal state :contestant form)
       (is (= "Ice Wall" (:title (get-character state :hq 0))) "Ice Wall is the innermost piece of character before swap")
       (is (= "Formicary" (:title (get-character state :hq 1))) "Formicary is the outermost piece of character before swap")
       (click-prompt state :contestant "Yes")      ; Move Formicary
       (is (= 1 (get-in @state [:run :position])) "Now approaching the innermost piece of character")
       (is (= "Formicary" (:title (get-character state :hq 0))) "Formicary is the innermost piece of character after swap")
       (is (= "Ice Wall" (:title (get-character state :hq 1))) "Ice Wall is the outermost piece of character after swap")))))
...
(deftest gatekeeper
  ;; Gatekeeper
  (do-game
    (new-game (default-contestant ["Gatekeeper" "Posted Bounty"
                             (qty "Hostile Takeover" 2) (qty "Ice Wall" 10)])
              (default-challenger))
    ;; Set up
    (starting-hand state :contestant ["Gatekeeper" "Ice Wall" "Ice Wall"
    ...
    ;; Actual test
    (play-from-hand state :contestant "Gatekeeper" "New party")
    (take-credits state :contestant)
    (let [gate (get-character state :party1 0)
          hand (-> (get-contestant) :hand count)
          deck (-> (get-contestant) :deck count)
          num-shuffles (count (core/turn-events state :contestant :contestant-shuffle-deck))
          hostile (find-card "Hostile Takeover" (:hand (get-contestant)))]
      (run-on state "Locale 1")
      (core/reveal state :contestant gate)
      (is (= 6 (:current-strength (refresh gate))))
      (card-subroutine state :contestant gate 0)
      (click-prompt state :contestant "3")
      (is (= (+ 3 hand) (-> (get-contestant) :hand count)) "Contestant should draw 3 cards")
      (click-card state :contestant hostile)
      (click-card state :contestant (find-card "Hostile Takeover" (:discard (get-contestant))))
      (click-card state :contestant (find-card "Posted Bounty" (:hand (get-contestant))))
      (is (= deck (-> (get-contestant) :deck count)) "R&D should have same number of cards as start")
      (is (= (inc num-shuffles) (count (core/turn-events state :contestant :contestant-shuffle-deck)))
          "Contestant should shuffle R&D")
      (is (core/in-deck? (core/find-latest state hostile)) "Hostile Takeover should be in deck now")
      (card-subroutine state :contestant gate 1)
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [gem (get-character state :hq 0)]
        ...
        (click-prompt state :contestant "3") ; boost to trace strength 5
        (click-prompt state :challenger "0")
        (is (= 2 (count (:discard (get-challenger)))) "Did 2 net damage")
        (card-subroutine state :contestant gem 0)
        (click-prompt state :contestant "3") ; boost to trace strength 5
        (click-prompt state :challenger "5") ; match trace
...
(deftest holmegaard
  ...
  (do-game
    ...
    (let [holm (get-character state :hq 0)]
      ...
      (click-card state :contestant "Cache")
...
(deftest hydra
  ;; Hydra - do an effect Challenger is tagged, otherwise give Challenger 1 tag
  (do-game
    (new-game (default-contestant ["Hydra"])
              (default-challenger))
    (play-from-hand state :contestant "Hydra" "HQ")
    (take-credits state :contestant)
    (core/gain-credits state :contestant 10)
    (run-on state :hq)
    (let [hydra (get-character state :hq 0)
          contestant-creds (:credit (get-contestant))]
      (core/reveal state :contestant hydra)
      (is (= (- contestant-creds 10) (:credit (get-contestant))) "Cost 10 credits to reveal Hydra")
      (is (not (core/is-tagged? @state)) "Challenger is not tagged approaching Hydra")

      (testing "Hydra subroutines give tags if Challenger is not tagged"
        (doseq [n (range 3)]
          (card-subroutine state :contestant hydra n)
          (is (= 1 (:tag (get-challenger))) (str "Hydra sub " (inc n) " gave Challenger 1 tag"))
          (core/lose state :challenger :tag 1)))

      (testing "Hydra subroutines do their effect if the Challenger is tagged"
        ;; Gain 1 tag to turn on main effect of subroutines
        (core/gain state :challenger :tag 1)
        (is (core/is-tagged? @state) "Challenger is tagged")

        (is (= 3 (count (:hand (get-challenger)))) "3 cards in Challenger grip before Hydra damage")
        (card-subroutine state :contestant hydra 0)
        (is (= 0 (count (:hand (get-challenger)))) "Hydra sub 1 did 3 damage when Challenger is tagged")

        (card-subroutine state :contestant hydra 1)
        (is (= (- contestant-creds 5) (:credit (get-contestant))) "Hydra sub 2 gave 5 credits to Contestant when Challenger is tagged")

        (is (:run @state) "Still a run going on before resolving last subroutine")
        (card-subroutine state :contestant hydra 2)
        (is (not (:run @state)) "Hydra sub 3 ended the run when Challenger is tagged")))))
...
(deftest ^{:card-title "it's-a-trap!"}
  ...
  (do-game
    ...
    (let [iat (get-character state :archives 0)]
      ...
      (click-card state :challenger iat)
  ...
  (testing "Subroutine effect - Select 2 challenger cards, challenger moves one to the stack"
    (do-game
      ...
      (let [jua (get-character state :hq 0)]
        ...
        (click-prompt state :challenger (get-resource state 0))
 ...
 (testing "Should only lock placing for Challenger, not for both sides"
    (do-game
      ...
      (let [mti (get-in @state [:contestant :identity])
        ...
        (click-card state :contestant (find-card "Kakugo" (:hand (get-contestant))))
...
(deftest kamali-1.0
  ...
  (do-game
   ...
   (let [kamali (get-character state :hq 0)]
     ...
     (click-prompt state :challenger "Discard an placed piece of hazard")
     ...
     (click-prompt state :challenger "Discard an placed resource")
     (click-card state :challenger (get-resource state 0))
...
(deftest lotus-field
  ...
  (do-game
    ...
    (let [lotus (get-character state :archives 0)]
      ...
      (click-card state :challenger lotus)
  ...
  (testing "Faceup character"
    (do-game
      ...
      (let [m (get-character state :hq 0)
        ...
        (play-from-hand state :challenger "Parasite")
        (click-card state :challenger (refresh e))
        (is (= 1 (count (:hosted (refresh e)))) "Parasite hosted on Enigma")
        (run-on state "HQ")
        (core/reveal state :contestant (get-character state :hq 0))
        (click-card state :contestant (first (:hosted (get-character state :rd 0))))
...
(deftest meridian
  (testing "ETR"
    (do-game
      (new-game (default-contestant ["Meridian"])
                (default-challenger))
      (play-from-hand state :contestant "Meridian" "HQ")
      (take-credits state :contestant)
      (let [mer (get-character state :hq 0)]
        (core/reveal state :contestant (refresh mer))
        (run-on state :hq)
        (card-subroutine state :contestant (refresh mer) 0)
        (click-prompt state :challenger "End the run")
        (is (not (:run @state)) "Run is ended")
        (is (empty? (:scored (get-challenger))) "Not in challenger score area")
        (is (= 1 (count (get-character state :hq))) "Character still placed"))))
  (testing "Score as -1 point agenda"
    (do-game
      ...
      (let [mer (get-character state :hq 0)]
        ...
        (run-on state :hq)
        (card-subroutine state :contestant (refresh mer) 0)
        (click-prompt state :challenger "Add Meridian to score area")
        (is (:run @state) "Run is still live")
        (is (= 1 (count (:scored (get-challenger)))) "In challenger score area")
        (is (= -1 (:agenda-point (get-challenger))) "Worth -1 agenda points")
        (is (empty? (get-character state :hq)) "Character unplaced")))))
...
(deftest minelayer
  ...
  (do-game
    ...
    (card-subroutine state :contestant (get-character state :hq 0) 0)
    (click-card state :contestant (find-card "Fire Wall" (:hand (get-contestant))))
    (is (= 2 (count (get-in @state [:contestant :locales :hq :characters]))) "2 Character protecting HQ")
...
(deftest mlinzi
  ...
  (do-game
    ...
    (let [ml (get-character state :hq 0)]
      ...
      (click-prompt state :challenger "Take 1 net damage")
      (is (= 1 (count (:discard (get-challenger)))) "Challenger discarded 1 card")
      ...
      (click-prompt state :challenger "Discard the top 2 cards of the stack")
...
(deftest next-sapphire
  ...
  (testing "Basic test"
    (do-game
      (new-game (default-contestant ["NEXT Bronze" "NEXT Sapphire" (qty "Ice Wall" 100)])
                (default-challenger))
      (starting-hand state :contestant ["NEXT Bronze" "NEXT Sapphire" "Ice Wall" "Ice Wall"])
      (dotimes [_ 5]
        (core/move state :contestant (find-card "Ice Wall" (:deck (get-contestant))) :discard))
      (core/gain state :contestant :credit 100)
      (play-from-hand state :contestant "NEXT Bronze" "HQ")
      (play-from-hand state :contestant "NEXT Sapphire" "R&D")
      (let [bronze (get-character state :hq 0)
            sapphire (get-character state :rd 0)]
        (core/reveal state :contestant sapphire)
        (take-credits state :contestant)
        (run-on state "R&D")
        (let [hand (count (:hand (get-contestant)))
              deck (count (:deck (get-contestant)))]
          (card-subroutine state :contestant sapphire 0)
          (is (= 1 (-> (get-contestant) :prompt first :choices :number)))
          (click-prompt state :contestant "1")
          (is (= (inc hand) (count (:hand (get-contestant)))) "Contestant should draw 1 card from R&D")
          (is (= (dec deck) (count (:deck (get-contestant)))) "R&D should lose 1 card"))
        (let [hand (count (:hand (get-contestant)))
              discard (count (:discard (get-contestant)))]
          (card-subroutine state :contestant sapphire 1)
          (click-card state :contestant (find-card "Ice Wall" (:discard (get-contestant))))
          (is (= (inc hand) (count (:hand (get-contestant)))) "Contestant should draw 1 card from Archives")
          (is (= (dec discard) (count (:discard (get-contestant)))) "Archives should lose 1 card"))
        (let [hand (count (:hand (get-contestant)))
              deck (count (:deck (get-contestant)))
              num-shuffles (count (core/turn-events state :contestant :contestant-shuffle-deck))]
          (card-subroutine state :contestant sapphire 2)
          (click-card state :contestant (find-card "Ice Wall" (:hand (get-contestant))))
          (is (= (dec hand) (count (:hand (get-contestant)))) "Contestant should add 1 card from HQ to R&D")
          (is (= (inc deck) (count (:deck (get-contestant)))) "R&D should gain 1 card")
          (is (= (inc num-shuffles) (count (core/turn-events state :contestant :contestant-shuffle-deck))) "Contestant should shuffle"))
        (core/reveal state :contestant bronze)
        (card-subroutine state :contestant sapphire 0)
        (is (= 2 (-> (get-contestant) :prompt first :choices :number)) "2 revealed NEXT character increases choice total"))))
  (testing "Should shuffle even when choosing 0"
    (do-game
      (new-game (default-contestant ["NEXT Sapphire" (qty "Ice Wall" 100)])
                (default-challenger))
      (starting-hand state :contestant ["NEXT Sapphire" "Ice Wall"])
      (play-from-hand state :contestant "NEXT Sapphire" "HQ")
      (take-credits state :contestant)
      (run-on state "HQ")
      (let [sapphire (get-character state :hq 0)
        ...
        (card-subroutine state :contestant sapphire 2)
        (click-prompt state :contestant "Done")
        (is (= hand (count (:hand (get-contestant)))) "Nothing selected so HQ shouldn't change")
        (is (= deck (count (:deck (get-contestant)))) "Nothing selected so R&D shouldn't change")
        (is (= (inc num-shuffles) (count (core/turn-events state :contestant :contestant-shuffle-deck)))
            "Contestant should shuffle even when selecting nothing")))))
...
(deftest oduduwa
  ...
  (do-game
    ...
    (let [odu (get-character state :hq 0)
      ...
      (click-card state :contestant (refresh eni))
...
(deftest otoroshi
  ;; Otoroshi
  (do-game
    (new-game (default-contestant ["Otoroshi" "Project Junebug" (qty "Ice Wall" 100)])
              (default-challenger))
    (starting-hand state :contestant ["Otoroshi" "Project Junebug"])
    (play-from-hand state :contestant "Otoroshi" "New party")
    (play-from-hand state :contestant "Project Junebug" "New party")
    (take-credits state :contestant)
    (run-on state :party1)
    (let [otoroshi (get-character state :party1 0)
          junebug (get-content state :party2 0)
      ...
      (core/reveal state :contestant otoroshi)
      (card-subroutine state :contestant otoroshi 0)
      (click-card state :contestant junebug)
      (is (= 3 (get-counters (refresh junebug) :advancement)) "Project Junebug should have 3 advancement tokens from Otoroshi subroutine")
      (is (= (list "Access card" "Pay 3 [Credits]") (-> (get-challenger) :prompt first :choices)) "Challenger should have 2 options")
      (click-prompt state :challenger "Pay 3 [Credits]")
      (is (= (- credits 3) (:credit (get-challenger))) "Challenger should pay 3 credits to Otoroshi subroutine")
      (run-jack-out state)
      (run-on state :party1)
      (card-subroutine state :contestant otoroshi 0)
      (click-card state :contestant otoroshi)
      (is (= 3 (get-counters (refresh otoroshi) :advancement)) "Otoroshi should have 3 advancement tokens from Otoroshi subroutine")
      (is (= (list "Access card") (-> (get-challenger) :prompt first :choices)) "Challenger should have 1 option")
      (click-prompt state :challenger "Access card")
      (is (= "You accessed Otoroshi." (-> (get-challenger) :prompt first :msg)) "Challenger should access Otoroshi even tho it's an character.")
      (click-prompt state :challenger "No action"))))

(deftest peeping-tom
  ;;Peeping Tom - Counts # of chosen card type in Challenger grip
  (do-game
    (new-game (default-contestant ["Peeping Tom"])
              (default-challenger [(qty "Sure Gamble" 5)]))
    (play-from-hand state :contestant "Peeping Tom" "HQ")
    (take-credits state :contestant)
    (run-on state "HQ")
    (let [tom (get-character state :hq 0)]
      (core/reveal state :contestant (refresh tom))
      (card-ability state :contestant tom 0)
      (click-prompt state :contestant "Hazard")
      (is (last-log-contains? state "Sure Gamble, Sure Gamble, Sure Gamble, Sure Gamble, Sure Gamble")
          "Revealed Challenger grip")
      (is (last-log-contains? state "0") "Correctly counted Hazard in Challenger grip")
      (card-ability state :contestant tom 0)
      (click-prompt state :contestant "Event")
      (is (last-log-contains? state "5") "Correctly counted Events in Challenger grip")
      (card-side-ability state :challenger tom 1)
      (card-side-ability state :challenger tom 1)
      (card-side-ability state :challenger tom 1)
      (card-side-ability state :challenger tom 1)
      (is (= 4 (:tag (get-challenger))) "Tag ability sucessful")
      (card-side-ability state :challenger tom 0)
      (is (not (:run @state)) "Run ended"))))
...
(deftest resistor
  ...
  (do-game
    ...
    (let [resistor (get-character state :hq 0)]
      ...
      (core/gain-tags state :challenger 2)
  ...
  (testing "Sub 1 - Look at the top 3 cards of R&D, arrange those or shuffle R&D. You may draw 1 card"
    (do-game
      ...
      (let [sadaka (get-character state :archives 0)]
        ...
        (click-prompt state :contestant "Shuffle R&D")
        (click-prompt state :contestant "Yes")
  ...
  (testing "Sub 2 - You may discard 1 card in HQ. If you do, discard 1 radicle. Discard Sadaka."
    (do-game
      ...
      (let [sadaka (get-character state :archives 0)
        ...
        (click-prompt state :contestant (find-card "Enigma" (:hand (get-contestant))))
...
(deftest sandman
  ...
  (do-game
    ...
    (let [sand (get-character state :hq 0)]
      ...
      (click-card state :contestant (find-card "Inti" (get-in (get-challenger) [:rig :resource])))
      (is (= 1 (count (:hand (get-challenger)))) "Challenger has 1 card in hand")
      (card-subroutine state :contestant (refresh sand) 0)
      (click-card state :contestant (find-card "Scrubber" (get-in (get-challenger) [:rig :radicle])))
...
(deftest self-adapting-code-wall
  ...
  (do-game
    ...
    (let [sacw (get-character state :archives 0)]
      ...
      (run-jack-out state)
      (play-from-hand state :challenger "Parasite")
      (click-card state :challenger sacw)
      ...
      (let [shiro (get-character state :rd 0)
        ...
        (let [credits (:credit (get-contestant))]
          ...
          (dotimes [_ 5]
            ...
            (click-prompt state :challenger "No action"))
          (run-jack-out state)
          (is (= (+ credits 10) (:credit (get-contestant))) "Contestant should only gain money once"))))))
...
(deftest thimblerig
  (testing "Thimblerig does not flag phase 1.2 if it's the only piece of character"
    (do-game
      (new-game (default-contestant ["Thimblerig" "Guard"])
                (default-challenger))
      (play-from-hand state :contestant "Thimblerig" "HQ")
      (core/reveal state :contestant (get-character state :hq 0))
      (take-credits state :contestant)
      (take-credits state :challenger)
      (is (not (:contestant-phase-12 @state)) "Contestant not in phase 1.2 when Thimblerig is the only piece of character")
      (play-from-hand state :contestant "Guard" "New party")
      (take-credits state :contestant)
      (take-credits state :challenger)
      (is (:contestant-phase-12 @state) "Contestant in phase 1.2 when there are 2 pieces of character")))
  (testing "Basic of swap ability - usable both during and outside runs"
    (do-game
      (new-game (default-contestant ["Vanilla" "Pup" "Thimblerig"])
                (default-challenger))
      (play-from-hand state :contestant "Thimblerig" "HQ")
      (play-from-hand state :contestant "Pup" "HQ")
      (play-from-hand state :contestant "Vanilla" "New party")
      (let [thimble (get-character state :hq 0)
            pup (get-character state :hq 1)]
        (core/reveal state :contestant thimble)
        (core/reveal state :contestant pup)
        (is (= "Thimblerig" (:title (get-character state :hq 0))) "Thimblerig innermost character on HQ")
        (is (= "Pup" (:title (get-character state :hq 1))) "Pup outermost character on HQ")
        (card-ability state :contestant (refresh thimble) 0)
        (click-card state :contestant (refresh pup))
        (is (= "Pup" (:title (get-character state :hq 0))) "Pup innermost character on HQ after swap")
        (is (= "Thimblerig" (:title (get-character state :hq 1))) "Thimblerig outermost character on HQ after swap"))
      (let [thimble (get-character state :hq 1)
            vanilla (get-character state :party1 0)]
        (run-on state "Locale 1")
        (is (= "Thimblerig" (:title (get-character state :hq 1))) "Thimblerig outermost character on HQ")
        (is (= "Vanilla" (:title (get-character state :party1 0))) "Vanilla character on party")
        (card-ability state :contestant thimble 0)
        (click-card state :contestant vanilla)
        (is (= "Vanilla" (:title (get-character state :hq 1))) "Vanilla outermost character on HQ after swap during run")
        (is (= "Thimblerig" (:title (get-character state :party1 0))) "Thimblerig character on party after swap during run")))))
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [ht (get-content state :party1 0)
        ...
        (click-prompt state :contestant "No") ; don't use alternative cost
        (is (= 3 (:credit (get-contestant))) "Spent 9 to Reveal")
        (core/hide state :contestant (refresh ti))
        (core/reveal state :contestant ti)
        (click-prompt state :contestant "Yes") ; use alternative cost
        ...
        (click-card state :contestant (refresh ti))
  ...
  (testing "Do not prompt for alt cost #2734"
    (do-game
      ...
      (let [ht (get-content state :party1 0)
        ...
        (click-card state :contestant ti)
  ...
  (do-game
    ...
    (let [sc (get-content state :party1 0)]
      ...
      (let [wend (get-character state :hq 0)]
        ...
        (click-prompt state :contestant "1")
        (click-card state :contestant wend)
